...
Mit der opsi-client-agent Version (>=4.1.1.1) steht der Kiosk als eigenständiges Produkt zu Verfügung und wird als solches bei der Installation des opsi-client-agents mit installiert (opsi-client-kiosk). Dabei wird versucht die vorhandene Kiosk Einstellungen zu migrieren +
...
CAUTION: Bei der Installation des opsi-client-agent (>=4.1.1.1) wird der alte Kiosk im opsi-client-agent Verzeichnis der Clients deaktiviert und es wird das neue opsi-client-kiosk Paket auf dem Depot installiert sowie für Clients bei denen _software-on-demand.active_ = _true_ ist und das neue opsi-client-kiosk Produkt nicht installiert ist das neue opsi-client-kiosk Produkt installiert.
