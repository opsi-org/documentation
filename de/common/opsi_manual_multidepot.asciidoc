...
== opsi-server mit mehreren Depots
...
image::../images/multidepot-without.png["opsi-manual-multidepot-concept-single",width=400]

.Schema: Kommunikation zwischen opsi-client und opsi-servern (mehrere Standorte)
image::../images/multidepot-with.png["opsi-manual-multidepot-concept-multi",width=400]

[[opsi-manual-multidepot-installation]]
=== Erstellung und Konfiguration eines (slave) depot-servers

Zur Erstellung eines externen '{opsi-depotserver}s' wird zunächst ein normaler '{opsi-server}' aufgesetzt. Dann wird auf diesem neuen '{opsi-server}' der Befehl `opsi-setup --register-depot` ausgeführt, um ihn zum externen '{opsi-depotserver}' zu konfigurieren. Da hierbei nicht nur der '{opsi-depotserver}' konfiguriert wird, sondern dieser auch noch per Webservice dem zentralen '{opsi-configserver}' bekannt gemacht wird, müssen username und password eines Mitgliedes der Gruppe 'opsiadmin' eingegeben werden.
