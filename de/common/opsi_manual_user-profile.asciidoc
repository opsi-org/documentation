...
Der 'opsi-script' verfügt über eine Reihe von speziellen Befehlen um Modifikationen in Profilen vorzunehmen. Diese Arbeiten aber auf den lokalen Profilen und sind beim Einsatz von 'Roaming Profiles' ('Servergespeicherte Profile') weitgehend nutzlos.
...
Dies geschieht in dem beim User Login der 'opsi-script' gestartet wird um spezielle 'userLoginScripte' auszuführen.
...
Der Aufrufparameter `/silent` sorgt dafür, das während der Scriptabarbeitung das Fenster des 'opsi-script' nicht angezeigt wird.
