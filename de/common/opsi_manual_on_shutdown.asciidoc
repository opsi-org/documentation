...
* WAN/VPN-Erweiterung: das On-Shutdown-Modul ist derzeit nur auf Clients anwendbar, die nicht mit der WAN/VPN-Erweiterung betrieben werden.

* Group Policies: da ein Teil des Mechanismusses für die Installation beim Shutdown auf Shutdown-Scripten per Local Group Policy beruht, werden diese Einstellungen eventuell durch übergeordnete Group Policies überschrieben. In diesem Fall sollten die notwendigen Einstellungen in die übergeordneten Group-Policies aufgenommen werden. Siehe hierzu Kapitel <<opsi-on-shutdown-concept-shutdownscript>>.

* Windows Home-Edition: Windows Home enthält nicht die erforderlichen Windows Shutdown-Script-Mechanismen. Von daher kann die On-Shutdown-Installation auf Windows-Home-Edition nicht eingesetzt werden. 
...
* Windows unterscheidet beim Herunterfahren systemtechnisch nicht zwischen einem Shutdown und einem Reboot. Die Installation beim Shutdown wird also sowohl beim Shutdown, als auch beim Reboot ausgeführt und es ist nicht möglich, diese beiden Fälle bei der Ausführung zu unterscheiden. 

[[opsi-on-shutdown-activate]]
=== Inbetriebnahme der Installation beim Shutdown

Da die Grundfunktionalität für die Installation beim Shutdown bereits im aktuellen `opsi-client-agent`-Paket enthalten ist, muss zur Freischaltung und Inbetriebnahme lediglich eine gültige `modules`-Datei erworben und auf den Server kopiert werden. 

Dann kann für geeignete Clients (siehe <<opsi-on-shutdown-prerequires>>) der neue Produktschalter `on_shutdown_install` des Paketes `opsi-client-agent` auf `on` und der `opsi-client-agent` für diese Clients auf setup gesetzt werden.
...
Per Shutdown-Skript wird ein opsi-Task ausgeführt, der über den opsi Systemdienst '{opsiclientd}' eine Installation anstösst und auf deren Beendigung wartet. Erst dann wird das System ganz runter gefahren. Systemtechnisch wird hier nicht zwischen einem Shutdown und einem Reboot unterschieden, so dass die Installation auch bei einem Reboot ausgelöst wird. 

Der opsi-Client-Systemdienst '{opsiclientd}' ist für die neue Aktionsart `on_shutdown` konfiguriert, die die Installation handhabt. Falls für die Installation Reboots benötigt werden, ist die neu eingeführte precondition `installation_pending` für die korrekte Steuerung des Ablaufs zuständig.
...
Über entsprechende Registry-Einträge wird per Local Group Policy beim Herunterfahren des Systems ein Shutdown-Skript ausgeführt, das die Installation anstösst. Die Registry-Einträge entsprechen den Einstellungen, wie sie auch mit dem Group Policy-Editor `gpedit.msc` erzeugt werden können.
...
Die Precondition `installation_pending` zeigt an, dass noch eine Installation im Gange ist. Falls bei Beendigung des Skriptes immer noch der Zustand `installation_pending` gesetzt ist, bedeutet dies, dass die aktuelle Installation einen Reboot benötigt und noch nicht abgeschlossen ist. Im Normalbetrieb mit Installation im `gui_startup` sind die Sektionen für die neue Aktion `event_on_shutdown` deaktiviert.  
...
Im Normalfall ist zur Inbetriebnahme nur, wie in <<opsi-on-shutdown-activate>> beschrieben, eine für dieses Modul gültige modules-Datei notwendig. Dann kann für geeignete Clients mit dem `opsi-client-agent`-Produktschalter `on_shutdown_install` die Installation beim Shutdown aktiviert werden. 
...
ist zu empfehlen das folgende '{opsi-config-object}' anzulegen:

* +opsiclientd.event_gui_startup.active+ (boolean, default: +true+)

Über dieses '{opsi-config-object}' kann dann das gui_startup-Event Client-spezifisch aktiviert bzw. deaktiviert werden.
...
Die Standard-Wert true entspricht hierbei dem Wert in der mitgelieferten `opsiclientd.conf`.

Wenn für einen Install_on_shutdown-Client die Installation im Startup deaktiviert werden soll, wird das entsprechende '{opsi-config-object}' wie folgt konfiguriert:

* +opsiclientd.event_gui_startup.active+: +false+

Dies sollte aber nur in begründeten Ausnahmefällen geschehen. Die Einstellungen mit der Zusatzbedingung 'installation_pending' sollten unter keinen Umständen geändert, es bleiben hier die default-Werte erhalten. +
...
Modules file signature verified (customer: uib gmbh)
Check completed.
Event fired
Task completed.
----
