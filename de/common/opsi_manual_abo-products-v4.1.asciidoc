...
Zur Bereitstellung der benötigten Pakete können Sie diese manuell herunterladen oder aber alle Pakete nach Anpassung der '/etc/opsi/opsi-package-updater.conf' mittels `opsi-package-updater -vv install` in einem Rutsch installieren.

Ab opsi Version 4.0.5 können Sie nach Ergänzung der Datei
'/etc/opsi/opsi-package-updater.conf' (s.u.)

auch z.B. mittels

[source,prompt]
----
opsi-package-updater install mshotfix mshotfix-win7-x86-glb mshotfix-win7-win2008r2-x64-glb
----

die mshotfix-Pakete für Windows 7 erstmalig herunterladen.

[[opsi-manual-abo-products-introduction-third]]
==== Aktualisierung der Abo-Pakete / Konfiguration des `opsi-package-updater`
...
Mittels dem Aufruf `opsi-package-updater -vv update` können dann z.B. per Cronjob die installierten Pakete aktualisiert werden.

CAUTION: Bei Bedarf muß der Proxy in jeder Sektion hinzugefügt werden! +
[source,prompt]
----
...
Eine gleichwertige Variante ist für jedes Repository z.B. eine eigene `/etc/opsi/opsi-package-updater.conf.abo.standard` zu erstellen, die dann mittels `opsi-package-updater -c /etc/opsi/psi-product-updater.conf.abo.standard` angewendet werden kann.
...
----
dotnetfx                  4.7-3                   .NET Framework
----

Paket zur Installation der Dotnet Framework Versionen 4.5 und höher insbesondere auf Windows 7 /2008 R2 / 8.1 / 10 / 2012 R2 
Auf neuen Windows Versionen (> Windows 7) können auch ältere, nicht in der Betriebssysteminstallation enthaltene, Dotnet Versionen installiert werden.  
...
values: ["3.5", "4.0", "4.5", "4.5.1", "4.5.2", "4.6", "4.6.1", "4.6.2", "4.7", "all"]
default: ["all"]
...
auto=win10 or opsi-local-image-win10 (defaullt); other ProductID for netboot-product
values: ["auto", ]
default: ["auto"]
