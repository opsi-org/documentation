...
Es bietet sich anschließend an gezielt nur die Pakete aus den neuen Repositories zu installieren:
[source,prompt]
----
# Pakete fuer mshotfix installieren
opsi-package-updater --repo uib_abo_mshotfix install
# Pakete fuer msoffice installieren
opsi-package-updater --repo uib_abo_msoffice install
# Pakete fuer standard produkte installieren
opsi-package-updater --repo uib_abo_standard install
----

Es ist möglich gezielt benötigte Pakete herunterzuladen:
[source,prompt]
----
# Gezielt die Pakete für das Update von Windows 7 installieren
