...
Der Platzhalter ist indiesem Fall der relative Pfad zu einem ausführbarem Programm, dessen Ausgabe den Platzhalter ersetzt.
...
"i" bedeutet: Dieses Attribut kann bei Geräten dieses Typs unterschiedliche
Werte haben.
...
** "Condition": "<condition>" +
dabei ist <condition> eine Bedingung die erfüllt sein muss, damit der 'Value' ermittelt wird. So legt z.B. die <condition> "vendor=[dD]ell*" fest, das der schon erhobene Wert von "vendor" 'Dell' oder 'dell' sein muss.
...
Das Opsi-Attribut "systemType" der Klasse COMPUTER_SYSTEM wird übersetzt in
"Typ".
