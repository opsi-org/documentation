...
Die Linux bezogenen Localboot und Netbootproukte können über den opsi-package-updater geladen werden. +
Dazu muss sichergestellt sein, dass in der `/etc/opsi/opsi-package-updater.conf` auch die entsprechenden Verzeichnisse eingetragen sind. Am einfachsten ist es, wenn sich folgende Sektion in der Konfigurationsdatei findet: +
[source,ini]
----
