...
Welches Protokoll verwendet wird, kann über das '{opsi-config-object}' +clientconfig.depot.sync_protocol+ Client-spezifisch konfiguriert werden.
Die möglichen Werte sind +cifs+ und +webdav+.
Existiert dieses '{opsi-config-object}' nicht, wird der Wert aus +clientconfig.depot.protocol+ verwendet.
