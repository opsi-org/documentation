...
Hierbei werden bevorzugt '{opsi-products}n' gelöscht, die seit längerem nicht mehr benötigten (synchronisiert) wurden.
...
* Der Zustand +products_cached+ den Wert +true+ annimmt und dieser auch über einen Neustart hinweg erhalten bleibt (siehe: <<opsi-manual-clientagent-configuration-events>>).
...
Kommt es bei der Rückübertragung der Modifikationen zu Konflikten greifen folgende Regeln:
  ...
  - Bei '{action-requests}' gilt der Wert des '{opsi-configserver}s'
  - Im Fall von '{installation-status}' und '{action-result}' wir der Client-Wert bevorzugt.
  - Die verwendeten Software-Lizenzen werden vom Client vorgegeben.
Bei der Replikation reservierte, ungenutzte Lizenzen werden wieder freigegeben.
  - Der '{opsi-configserver}' behält beim Zustand von '{opsi-config-objects}n' und '{product-properties}' recht.
...
* Der Zustand +config_cached+ den Wert +true+ annimmt und dieser auch über einen Neustart hinweg erhalten bleibt (siehe: <<opsi-manual-clientagent-configuration-events>>).
* Ein Event vom Typ +sync completed+ ausgelöst wird.
...
Da die Konfiguration des '{opsi-client-agent}s' auch zentral über den Webservice erfolgen kann (siehe: <<opsi-manual-clientagent-configuration-webservice>>),
...
image::../images/ospclientd-infopage-wan-cached.png["ospclientd-infopage-wan-cached",width=400]
