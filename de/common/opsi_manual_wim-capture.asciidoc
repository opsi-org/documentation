...
Start des Produktes opsi-wim-cature. +
...
Nach der Windows installation, kann nun weitere Software, Hotfixes und Konfigurationen per opsi oder Händig auf den Rechner aufgespielt werden.
...
[NOTE]
===============================
Für opsi-clonezilla wird das runcommand: `ocs-sr -q2 --batch -j2 -rm-win-swap-hib -i 2000 -p true savedisk imagefile sda` gesetzt. Innerhalb diese Kommandos wird `imagefile` abhängig von dem Wert des Properties 'clonezilla_imagefile' gesetzt: Steht diese Property auf 'auto' (default), so wird der Wert für 'imagefile' automatisch erstellt. Dies geschieht unter Verwendung von Propertywerten und dem Clientnamen nach dem Muster: +
`<FQDN des Clients>_<target_product>_<imagename>` +
Bei einem anderen Wert wie 'auto' wird der angegeben Wert als 'imagefile' verwendet. Weiterhin wird Produkt opsi-clonzilla auf setup gesetzt. Damit das Produkt opsi-clonzilla startet ist nun ein Reboot nötig.
...
[NOTE]
===============================
Die productOnClient objekte für all Localboot Produkte werdn nach `c:\opsi.org\tmp\productonclients.json` geschrieben.
===============================
...
Damit das Image welches erstellt werden soll sich wie ein Standard Windows setup auf einem beliebigen Rechner ausrollen laßt muß es depersonalisiert werden. Dies wird mit dem Winows Werzeug `sysprep` erledigt.

TIP: Installierte Software wird nicht depersonalisiert.
Es ist durchaus möglich das installierte Software sich in Ihrer Konfiguration merkt auf welchem Rechner sie installiert wurde. Eine solche Konfiguration wird dann wahrscheinlich Probleme machen, wenn das Image auf einen anderen Rechner ausgerollt wird. Von daher ist es nicht die ideale Idee möglichst viele Software in einem Image unterzubringen.
...
* Es wird geprüft ob das über das Property `target_product` angegeben Produkt auf dem share 'opsi_depot_rw' existiert und eine `install.wim` Datei an der richtigen Stelle besitzt.

* Prüfen und Erstellen einer Lock Datei im `target_product`. Wenn diese Datei bereits existiert, so wird hier abgebrochen um zu vermeiden das mehrere capture Vorgänge gleichzeitig in die selbe WIM-Datei schreiben.
...
CAUTION: Imagename merken !
Der Name des erstellten Images wird momentan noch nicht automatisch in die Liste der Möglichen Imagenamen aufgenommen. Sie müssen sich den Namen merken und beim ausrollen angeben !
...
Diese Property ist eine Liste und kann auch mehre ProduktIds aufnehmen.


TIP: opsi-clonzilla auf setup stellen lassen !
Der Rechner ist nach dem Capture Vorgang depersonalisert und damit weitgehend unbrauchbar. Das Produkt opsi-clonzilla ist so vorbereitet das ein weiter oben gemachtes Backup automatisch wieder hergestellt wird, wenn es hier auf setup gestellt wird.
...
War das Produkt `opsi-clonezilla` hier auf 'setup' gestellt worden ist, so wird nun automatisch ein Restore der Platte durch geführt.
...
Die Target Produkte dienen dazu die gecapturten Images aufzunehem.

Warum Target-Produkte ?

Die Target-Produkte unterscheiden sich nicht von den Standard opsi Windows-Install Produkten. Technisch kann also z.B. ein normals `win7-x64` als TargetProdukt dienen. +
...
Dabei dürfen die Verzeichnisse wie `winpe` oder z.B. `drivers/drivers/additional/byAudit` durchaus Symbolisch Links auf die entsprechenden Verzeichniss aus dem passenden Nicht-Target-Produkt sein. Achtung: das `installfiles` Verzeichnis muß tatsächlich mit bem Inhalt der Windows DVD befüllt werden und darf kein symbolischer Link sein.
...
*Wiederherstellung der opsi Daten zu installierten Produkten*

Wenn Sie ein Windows mit opsi neu installieren z.B. aus `win7-x64` dann werden alle Localboot-Produkte welche bei diesem Rechner vorher auf installed standen automatisch auf setup gestellt und erneut installiert. 

Dies ist beim ausrollen eines 'gecapturten`Images anders. +
Beim ausrollen eines 'gecapturten' Images werden nach der Installation des Images nur die Produkte automatisch Installiert, welche schon vor dem beginn der Installation des Betriebssystems auf setup standen. Dies kann durch Ihren Eingriff oder das Property 'setup_after_install' erfolgt sein.

Warum dieses Verhalten ? +
Im Image befindet sich das Backup der opsi-Daten das dort während des capture Vorgangs abgelegt wurde. Dieses wird bei der Installation des opsi-client-agent entdeckt und wieder in den opsi-server eingespielt. Damit stehen die Produkte die in den 'gecapturten' Image installiert waren jetzt für den frisch installierten Rechner auf 'installed'.
...
* UEFI +
opsi-wim-capture funktioniert nicht mit UEFI-Rechnern. Dies liegt im wesentlichen daran, das die derzeitige opsi-clonezilla Version noch nicht UEFI tauglich ist. Wir werden die UEFI tauglichkeit von opsi-wim-capture herstellen sobald eine entsprechende opsi-clonezilla Version vorliegt.

* RAID-Controller non /dev/sda +
Das vom opsi-wim-capture angestossene Platten Backup funktioniert derzeit nicht auf Rechnern deren erste Platte unter Linux (und damit in opsi-clonezilla) nicht als `/dev/sda` gesehen wird. Dies wird zu einem späteren Zeitpunkt gebaut wrden.
