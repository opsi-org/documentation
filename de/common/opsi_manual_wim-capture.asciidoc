...
. Installation von Windows mit PreservePE
. opsi-clonezilla Backup der Platte (System+PE)
. Backup opsi Metadaten
. PE bootfähig machen und PE script erstellen
. Sysprep (Depersonalisierung)
. PE-Boot Capture und schreiben ins Zielprodukt
. opsi-clonezilla Restore der Platte (System+PE)
