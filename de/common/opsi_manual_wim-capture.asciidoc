...
Bei einem anderen Wert als 'auto' wird der angegebene Wert als 'Imagefile' verwendet. Weiterhin wird das Produkt opsi-clonzilla auf setup gesetzt. Damit das Produkt opsi-clonzilla startet ist nun ein Reboot nötig.
...
Die Maschine wir rebootet, dabei bleibt das Produkt 'opsi-wim-capture' auf 'setup' stehen. Nun startet das Netboot Produkt opsi-clonzilla und führt das Backup aus.
...
Es ist durchaus möglich, dass installierte Software sich in Ihrer Konfiguration merkt, auf welchem Rechner sie installiert wurde. Eine solche Konfiguration wird dann wahrscheinlich Probleme machen, wenn das Image auf einen anderen Rechner ausgerollt wird. Von daher ist es nicht die ideale Idee, möglichst viel Software in einem Image unterzubringen.
...
Der Rechner ist nach dem Capture Vorgang depersonalisert und damit weitgehend unbrauchbar. Das Produkt opsi-clonzilla ist so vorbereitet, dass ein weiter oben gemachtes Backup automatisch wieder hergestellt wird, wenn es hier auf setup gestellt wird.
...
Zum Ausrollen des erstellten Images dient das bei der Erstellung per `target_product` angegebene 'Zielprodukt'. In diesem wählen Sie bei `imagename` das erstellte Image aus, und können dieses jetzt ganz normal wie eine andere Windowsinstallation installieren. D.h. sämtliche Standards wie Treiberintegration, Installation des opsi-client-agent finden genauso statt, wie wenn Sie ein Original Microsoft Image ausrollen.
