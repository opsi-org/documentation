...
[[opsi-manual-wimcap]]
=== opsi Erweiterung 'opsi WIM Capture'
...
CAUTION: Für das Produk `opsi-wim-capture` muß der share `opsi_depot_rw` für 'pcpatch' beschreibbar sein. Prüfen Sie Ihre Samba Konfiguration.
...
Mit dem `media creation tool` von 'Mircrosoft' erstellte ISO's enthalten statt einer `install.wim` eine `install.esd`.
Dies ist ein etwas anderes Format was bisher noch nicht unterstützt wird.
