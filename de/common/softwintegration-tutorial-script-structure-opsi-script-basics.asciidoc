...
ShellInAnIcon::
Der Inhalt dieser Sektion wird der Betriebssystemtypischen _shell_ zur Ausführung übergeben. Diese _shell_ist bei Windows die `cmd.exe`, bei Linux und bei MacOS die `bash`. Hier können also normale Batch-Skripte abgelegt werden. +
...
Dieses Dialog-Fenster wird (so es auftaucht) mit Hilfe von _AutoIt_ geschlossen.
