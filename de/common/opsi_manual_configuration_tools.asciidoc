...
image::opsi-configed-mode-depotselection.png["{opsi-configed}: Depotauswahl", width="52mm"]
...
image::opsi-configed-treeview1.png["{opsi-configed}: Treeview", width="66mm"]
...
Seit opsi 4.0.4 können, falls die opsi-Clients z.B. über über einen UCS-Service angelegt werden sollen, die Optionen zum Anlegen und
...
Per default wird der opsi-atftpd so installiert, daß er nicht ständig läuft sondern nur bei Bedarf durch den inetd gestartet wird.

Um den opsi-atftpd als Daemon laufen zu lassen und ihn nicht über den inetd zu starten, müssen Sie folgende Änderungen vor nehmen:

* In det /etc/inetd.conf die Zeile zum Protokoll 'tftp' auskommentieren. Danach den inetd reloaden. Der Befehl hierzu ist abhängig vom verwendet Linux:
