...
usage: opsi-package-updater [-h] [--version] [--config CONFIGFILE]
                            ...
                            [--force-checksum-calculation | --use-existing-checksum]
                            [--repo repository_name]
                            {install,update,download,repo} ...
...
optional arguments:
  ...
  --use-existing-checksum
                        Use existing checksums from the .md5-file of a package
                        if possible. (Default)
  --repo repository_name
...
# opsi-package-updater repo --help
usage: opsi-package-updater repo [-h] [--list-active] [--list-products]
                                 [--list-product-and-installationstatus]
                                 [--list-updatable]

optional arguments:
  -h, --help            show this help message and exit
  --list-active         Lists all active repositories
  --list-products       Lists the repositories and the products they provide.
  --list-product-and-installationstatus
                        Lists the repositories, the products they provide and
                        information about the local installation status.
  --list-updatable      Lists packages that have updates in the remote
                        repositories.
----

Es gibt eine Reihe von allgemeinen Optionen.

- `--verbose` erhöht die Menge an sichtbaren Ausgaben. Dies kann mehrfach angegeben werden, um mehr Ausgabe zu erhalten.
- `--repo <Name eines Repositories>` schränkt die Aktionen auf das angegebene Repository ein. Die Namen der verfügbaren Repositories können über `repo --list-active` ermittelt werden.

Die verschiedenen Modi haben unterschiedliche Auswirkungen auf das Verhalten.
Die Modi `install`, `update` und `download` laden Pakete aus einem Repository, wohingegen mit `repo` Vorranging Informationen über die in den Repositories vorhandenen Pakete angezeigt werden können.

Der Modus `install` installiert neue Pakete.
Der Modus `update` aktualisiert vorhandene Produkte.
...
Mittels `repo --list-active` werden die aktivierten Repositories angezeigt.
...
Über `repo --list-products` wird angezeigt welche Produkte im Repository vorhanden sind.

Zum Anzeigen der verfügbaren Aktualisierungen kann `repo --list-updatable` verwendet werden.
...
[source,prompt]
----
opsi-package-updater repo --list-updatable
...
Ein Repository hat den Typ 'opsi-server', wenn in der `/etc/opsi/opsi-package-updater.conf`
...
Damit zieht der Depotserver seine Pakete per Aufruf des 'opsi-package-updater'
