...
In Version 4.2 wurden die _interface page_ und die _info page_ in der _admin page_ zusammengefasst. Die Seite wird über die URL 'https://<opsi-server>:4447/admin' aufgerufen. Um die Seite zu öffnen müssen Sie sich als Mitglied der Gruppe 'opsiadmin' authentifizieren.
...
image::opsi-webinterface-blocked-clients.png["opsiconfd: Gesperrte Clients", width=400]
...
.'opsiconfd': RPC Liste
image::opsi-webinterface-rpc-list.png["opsiconfd: RPC Liste", width=400]
...
Auf der Adminseite (https://<opsi-server>:4447/admin) gibt es einen Tab, der zum grafana Dashboard des opsiconfds weiterleitet. Dort erhalten Sie grafisch
aufbereitete Informationen über den Lastverlauf des 'opsiconfd', sowie weitere Informationen.

.opsiconfd info: 'opsiconfd'-Werte der letzten 3 Stunden
image::opsiconfd-grafana-hour.png["opsiconfd grafana: opsiconfd-Werte der letzten 3 Stunden", width="120mm"]

.opsiconfd info: 'opsiconfd'-Werte des letzten Tages
image::opsiconfd-grafana-day.png["opsiconfd grafana: opsiconfd-Werte des letzten Tages", width="120mm"]
