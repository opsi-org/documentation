...
IMPORTANT:  Der '{opsi-configed}' in der Version 4.0.5 setzt Java in der Version 7 voraus und benötigt einen laufenden 'opsiconfd' auf der Serverseite.
...
Da es im Normalfall kaum einen Unterschied macht, bei WAN Verbindungen jedoch meist notwendig, ist die Gzip-Option per Default eingeschaltet.

Mittels des Features der Client-Erreichbarkeit lässt sich prüfen, welche Clients verbunden sind. Es lässt per Aufrufparameter aktivieren oder deaktivieren. In der Anmeldemaske ist ein Test-Intervall von 0 Minuten (= ausgeschaltet) als Default eingetragen. 
...
[NOTE]
.Beispiele für Suchpattern:
=========================================
Alle PCs, in deren Namen oder Beschreibung der Name Meyer vorkommt, werden mit dem Pattern

_.*eyer.*_ 

gefunden. Dabei steht in ".*" der Punkt für ein "beliebiges Zeichen" und das Sternchen für "beliebig oft". Um sicherzugehen, dass nicht auch Beyer gefunden wird, sollte das Suchpattern genauer heißen

_.*[Mm]eyer.*_

Die eckigen Klammern, die eine Aufzählung von Zeichen enthalten, bedeuten, dass eines der aufgeführten Zeichen vorhanden sein muss.

Bei der Produktsuche funktioniert es analog:

_0.-opsi.*standard_ sucht alle Produkte, deren Namen mit "0" beginnt, worauf _ein_ beliebiges Zeichen folgt, weiter geht es mit _-opsi_, dann irgendwelchen _beliebig vielen_ Zeichen; und am Schluss steht _standard_

Um sicherzugehen, dass das zweite Zeichen eine Ziffer ist, könnte man schreiben

_0[0123456789]-opsi.*standard_

Als Abkürzung für _[0123456789]_ ist _[0-9]_ erlaubt. 
=========================================
...
image:../images/opsi-configed-client-reachable.png["Client reachable",width=20] : Client erreichbar +
image:../images/opsi-configed-client-unreachable.png["Client unreachable",width=20] : Client nicht erreichbar
...
[[opsi-manual-configuration-tools-opsi-configed-module-uefi-wan-installbyShutdown]]
