...
Hier müssen zwei Gruppen angelegt sein: 'pcpatch' und 'opsiadmin'.
In der Gruppe 'pcpatch' sollten alle Benutzer sein, die mit Paketverwaltung zu tun haben.
In der Gruppe 'opsiadmin' müssen alle user sein, die den opsiconfd-Webservice verwenden wollen z.B. über den {opsi-Configed}.
...
Siehe <<opsi-manual-configuration-tools-opsi-package-updater>>
...
Hier werden '{opsi-product-packages}' gespeichert, welche über den Aufruf des `opsi-package-updater` auf den Server geladen werden.
...
* [Package]-'Version' +
ist die Version des Paketes für die Produktversion. Die dient dazu, Pakete mit gleicher Produktversion aber z. B. korrigiertem '{opsi-winst}'-Skript zu unterscheiden.
