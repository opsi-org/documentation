...
Sollte der der Start auf diese Weise nicht funktionieren und das Starmenü nicht reagieren (beobachtet unter Linux / KDE), so probieren sie es von der Kommandozeile unter Angabe irgend eines Pfades und bestätigen die Fehlermeldung das der Pfad nicht gefunden wurde: 
...
Dann öffnet sich das Produkt-fenster mit verschiedenen Reitern. Der default Reiter ist 'Paket'.
...
Mit dem Button könnee Sie die Datei in dem in der Konfiguration angegebenen Scripteditor aufrufen und das Script modifizieren. Unter Windows ist das der Scripteditor des {opsi-package-builder}.
...
image::opb_tab_property.jpg[Reiter Produktvariablen (Properies),width=200]
...
Dieser Button startet eine SSH-Verbindung vom Server und ruft dort den Installations auf um das Produkt auf dem Server zu installieren. +
Sie können das selbe auch in einem Terminal selber machen wie in 
