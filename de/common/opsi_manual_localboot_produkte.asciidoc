...
Analog verhält es sich mit dem Property 'disable_customer_experience'. Dies deaktiviert das Sammlen von Daten im Bezug auf Daten zur Anwendungsverwendung. +
Das Entfernen von gefährlicher Software, kurz MRT (kurz für Malware Removal Tools), wird mit dem Property 'disable_mrt' deaktiviert. Dieser Dienst scannt in regelmäßigen Abständen vorhandene Dateien auf der Festplatte des Rechners und vergleicht diese mit einer Liste von potentiell gefährlicher Software. +
...
Das Property 'config-updates' erlaubt es zu wählen wie die Maschine die Updates erhält. Sei es direkt von den Microsoft Servern, einem lokalen Peer-to-Peer Netzwerk oder oder einem Peer-to-Peer Netz aus dem Internet. +
In Windows 10 werden unter anderem auch Daten über den aktuell eingeloggten Microsoft Konto Benutzer gesammelt. Dies kann mit dem Property 'disable_mac' deaktiviert werden. +
Ein weiteres Property ist 'disable_advertising_id'. Die Advertiding Id speichert Daten über den Browserverlauf um benutzerspezifische Werbung einzublenden. +
...
Es kommt auch vor, das ein Dokument eine Schriftart nutzt, welcher nicht auf dem eigenen System installiert ist. Hierfür hat Microsoft das 'Font Streaming' eingeführt. Die fehlende Schriftart wird aus dem Internet geladen und bereitgehalten. Dieses Verhalten kann mit dem Property 'disable_font_streaming' gesteuert werden. +
