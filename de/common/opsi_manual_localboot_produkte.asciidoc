...
* *config-updates* erlaubt es die Quelle der Updates zu Ändern. Die Updates werden dann entweder direct von Microsoft Servern, einem lokalen Peer-To-Peer Netz oder einem Perr-To-Peer Netz aus dem Internet geladen. Die Option 'disable' ist derweil in ein eigenes Property namens 'disable_updates' 
...
* *disable_font_streaming* sorgt dafür, das nicht auf dem System installierte Fonts über das Internet nachgeladen werden.
