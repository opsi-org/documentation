...
== opsi Erweiterung 'opsi-clonezilla'
...
* Stellen Sie das Property `runcommand` auf `/opt/drbl/sbin/ocs-live`. Dies ist der Interaktive Modus von clonezilla.
...
In der Abfrage `Expert` oder `Beginner` bestimmen Sie ob Sie im folgenden die Defaultparameter ungefragt übernehmen (`Beginner`) oder diese abändern können (`Expert`). Die Wahl sollte hier auf `Expert` fallen um in Hinblick auf die Automatisierung z.B. Rückfragen ausschalten zu können. 
...
Hier beispielhaft anhand der savedisk Opertation die zusätzlichen Masken des Expertmodes.
