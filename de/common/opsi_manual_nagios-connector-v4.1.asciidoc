...
Die Installation eines opsi-Pakets kann manchmal eine ungewöhnliche lange Zeit dauern, ohne das ein Problem besteht.
Falls die Sperre eine lange Zeit besteht, dann kann dies ein Indikator für Installationsprobleme sein.
...
Es gibt diverse Plugins, die es schon vorgefertigt im Internet runter zu laden gibt. Eine mögliche Anlaufstelle ist link:http://exchange.nagios.org/[Nagios Exchange].
