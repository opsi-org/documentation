...
Dies ist wichtig um zu unterbinden, dass ein Vireninfizierter Rechner evtl. während der Installation über opsi den Share infiziert und dann der Virus über opsi weiterverbreitet wird.

Seit opsi 4.0.1 gibt es den Share 'opsi_depot' welcher read-only ist.
Um diesen Share zu verwenden führen Sie bitte folgendes aus:
...
Dieser Befehl erzeugt einen neuen Share 'opsi_depot', welcher read-only ist.
Dieser Share verweist auf das Verzeichnis `/var/lib/opsi/depot`.
Je nach Distribution des Servers ist dieser Pfad ein symbolischer Link auf `/opt/pcbin/install`.
...
Die Prüfung betrifft die Kommunikation mit dem '{opsi-configserver}'.
Wird als +clientconfig.depot.protocol+ +webdav+ verwendet, so wird auch der '{opsi-depotserver}' entsprechend geprüft. <<opsi-manual-wansupport-depot-protocol>>
...
----
Mit dem folgenden Befehl kann dies für die gesamte opsi-Umgebung aktiviert werden:
