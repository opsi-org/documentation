...
Die Eigenschaften eines Depots werden zum Teil abgefragt, wenn ein opsi-server über den Befehl `opsi-setup --register-depot` als Depot registriert wird (siehe <<opsi-manual-multidepot-installation>>).
...
image::configed-masterbuttons.png["Aufruf der Depoteigenschaften",width=200]
...
image::configed-depotsettings.png["Depoteigenschaften im opsi-configed",width=400]
...
In den Repository-Konfigurations-Dateien des `opsi-package-updater` (Dateien mit der Dateiendung `.repo` unter `/etc/opsi/package-updater.repos.d/`) ist hierzu in den Repository-Sektionen '[repository_...]' der Wert von 'active' auf 'false' zu setzen.
Anschließend kann in `/etc/opsi/package-updater.repos.d/master-depot.repo` in der Sektion '[repository_master]' der Wert von 'active' auf 'true' gesetzt werden.
Zur Auswahl des Depots muss dort noch bei 'opsiDepotId' die ID des Depots (FQDN) eingetragen werden, von welchem synchronisiert werden soll.

Der `opsi-package-updater` synchronisiert dann gegen die Pakete, die auf dem angegebenen Depot im Verzeichnis `/var/lib/opsi/repository` liegen.
