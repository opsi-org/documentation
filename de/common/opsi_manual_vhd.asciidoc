...
Es ist gebundelt mit der Erweiterung 'opsi-local-image' (siehe <<opsi-manual-localimage>>) - das heißt: die Freischaltung für 'opsi-local-image' gilt auch für 'opsi-vhd-reset'. +
...
Da dises Produkt nicht nur im Kontext von 'opsi-vhd-reset' eingesetzt werden kann, ist es im Kapitel 'opsi Standardprodukte' / 'opsi-auto-update' beschrieben: +
