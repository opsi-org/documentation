...
NOTE: Für das 'working_window' Feature wird der '{opsi-client-agent}' >= Version 4.1.0.0-37 benötigt.
...
Beispiel 2: Clientspezifisches Einstellen des 'working_window' für die Zeit zwischen 07:00 Uhr und 19:00 Uhr für das Event 'event_gui_startup'.
[source,prompt]
opsi-admin -d method configState_create opsiclientd.event_gui_startup.working_window "client.domain.de" "07:00-19:00"

Ist die Startzeit größer ist als die Endzeit gilt das 'working_window' über den nächtlichen Tageswechsel. +
...
Für das Beispiel "working_window=20:00-07:00" würde das Log des opsiclientd ('clientconnect') beispielsweise so aussehen:
...
Ein Hilfsprogramm zur Auswahl von Farben und deren richtigen Schreibweise ist der link:http://download.uib.de/opsi4.0/helper/opsi_color_chooser.exe[opsi color chooser].
...
Ein Hilfsprogramm mit dem Sie schnell prüfen können ob eine gegeben Bitmap Grafik korrekt angezeigt werden wird, ist der link:http://download.uib.de/opsi4.0/helper/opsi_bitmap_viewer.exe[opsi bitmap viewer].
