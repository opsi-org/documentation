...
Bei einem Einsatz mit aktiviertem Bitlocker, kann ein Reboot, der während der Installation oder durch opsi auf dem Client ausgelöst wird, die unbeaufsichtigte Installation unterbrochen werden.

Genau wie der '{opsi-winst}' ist es nun auch möglich Reboots, die von Events des {opsiclientd} ausgelöst werden, ebenfalls die Passwort Eingabe beim Booten des verhindern.

CAUTION: Dieses Feature ist zwangsläufig mit einem Sicherheitsverlusst verbunden. Bei diesem Vorgang wird der Key als Klartext auf die Festplatte geschrieben und ist damit auch potenziell eine Schwachstelle.
...
INFO: Diese Option kann ebenfalls auf Clients aktiviert werden, die keine Bitlocker-Verschlüsselung aktiviert haben und sollte den Betrieb des {opsiclientd} nicht stören.
