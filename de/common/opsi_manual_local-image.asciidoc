...
Zur Installation der Produkte aktivieren stellen Sie bitte in der Datei `/etc/opsi/opsi-package-updater.conf` das Attribut *active* des Repositories *uib_local_image* auf 'True'.
Anschließend können Sie über einen Aufruf von `opsi-package-updater -i -vv` die neuen Produkte installieren lassen.
