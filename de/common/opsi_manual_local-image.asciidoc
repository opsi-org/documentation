...
CAUTION: Für das Produk `opsi-local-image-capture` muß der share `opsi_depot_rw` für 'pcpatch' beschreibbar sein. Prüfen Sie Ihre Samba Konfiguration.
...
. das angegebene Image (inklusive seiner opsi Meta-Daten) restored,
. anhand der restorten Meta-Daten festgestellt welche Produkt per opsi Upgedatet werden können
. die Updates werden durchgeführt
. Von der aktualisierten Systempartition wird wieder durch 'backup' ein Image erzeugt.
...
Zur Installation der Produkte stellen Sie bitte in der Datei `/etc/opsi/opsi-product-updater.conf` das Attribut *active* des Repositories *uib_local_image* auf 'True'.
Anschließend können Sie über einen Aufruf von `opsi-product-updater -i -vv` die neuen Produkte installieren lassen.
...
IMPORTANT: Ändern Sie die Propertyeinstellungen von opsi-local-image-prepare nicht mehr nach dem sie die Maschine damit präpariert haben, da die nachfolgenden Produkte auf diese Werte zugreifen. 
...
. Restore des Images
. Update des restoreten Betriebssystems (Alle nicht aktuellen Produkte werden aktualisiert)
...
Bei Systemen mit mehr als einer Platte wird die Systemfesplatte bestimmt durch das opsi-local-image-prepare Property multi_disk_mode. +
...
Die Abläufe und Einstellungen beim Produkt `opsi-local-image-wim-capture` entsprechen denen des Produktes `opsi-wim-capture` welches hier: <<opsi-manual-wimcap>> beschrieben ist. Die Properies von `opsi-wim-capture` sind hier: <<opsi-manual-wimcap-products-main>> beschrieben.
...
IMPORTANT: `opsi-local-image-wim-capture` schlägt fehl, wenn Sie Ihr System mit einer Datenpartition angelegt haben. Installieren Sie in diesem Fall den Rechner neu mit dem opsi-local-image-prepare Property data_partition_size=0.
