...
Der Schulungsrechner wird mit einer (fast) statischen Partitionstabelle verwendet:

* Partition 1 (System) +
Hier findet sich das aktuell verwendete Betriebssystem (Windows / Linux) +
Die Größe dieser Partition wird zentral über den Hostparameter (config_state) `opsi-local-image.system_partition_size` gesteuert. So wird sicher gestellt, daß alle Netbootprodukte die selbe Größe verwenden und es zu keinen Konflikten kommt.
...
Diese Partition wird im Moment nicht verwendet.
...
Die Netbootprodukte zur Betriebssystem Installation verwenden die ersten beiden Partitionen (XP nur die erste) und lassen die Partitionen 3 und 4 unberuhrt. Somit bleiben die auf der Partition 4 liegenden Images auch bei der Installation eines neuen Betriebsystems erhalten.


[[opsi-manual-localimage-components]]
=== Komponenten
 
==== Netboot Produkte zur Installation der Betriebssysteme
...
Das installierte System kennt 2 user: `root` und `user`. Für beide wird das im Produktproperty `root_password` festgelegte Passwort gesetzt (Default: `linux123`).
Da bisher es noch keinen opsi-client-agent für Linux gibt, enthält diese Installation eine statische über die Produktproperties gesteuerte Softwareausstattung. Die wesentlichen Produktproperties sind dabei: 
** desktop +
...
==== Netboot Produkte zum Backup und Retore
...
[[opsi-manual-localimage-configuration]]
=== Konfiguration

Um die 'opsi local image' Erweiterung zu nutzen muss 

Einrichtung des zentralen configs 'opsi-local-image.system_partition_size' mit einem default von 30 GB:

[source, prompt]
----
opsi-admin -d method config_createUnicode opsi-local-image.system_partition_size "system partition size" '["20G","30G","40G","50G","60G","70G"]' '["30G"]'
----


Einrichtung des zentralen configs 'opsi-local-image.delete_partition_table' mit einem default 'if_needed':

[source, prompt]
----
opsi-admin -d method config_createUnicode opsi-local-image.delete_partition_table "Darf die Festplatte komplett neu formatiert werden (Verlust aller images) ?" '["always","if_needed","never"]' '["if_needed"]'
----
