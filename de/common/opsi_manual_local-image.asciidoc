...
Scheiter der Aufruf des Restore mit der Methode `rsync-partclone-image`, so wird automatisch der Restore mit der Methode `partclone-image-restore` weitergeführt.
** `update_and_backup` +
...
[[opsi-manual-localimage-releasenotes]]
=== opsi-local-image Releasenotes

[[opsi-manual-localimage-releasenotes4041]]
==== opsi-local-image Releasenotes 4.0.4.1
...
* 'setup_after_install' Properties bei den OS-Install Produkten +
Hier können ein oder mehrere Produkte angegeben werden, welche nach Abschluß der Betriebssysteminstallation auf 'setup' gestellt werden. Dabei werden auch die Abhängigkeiten diese Produkte mit aufgelöst.

* Neue Produkte: `opsi-local-image-win81` und `opsi-local-image-win81-x64`
