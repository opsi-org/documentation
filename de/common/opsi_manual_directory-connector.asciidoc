...
Das Ziel-Opsi-System sollte opsi 4.0.7 oder neuer verwenden.
...
Bitte verwenden Sie den Paket-Manager ihres Betriebssystems um das Paket `opsi-directory-connector` zu installieren.
...
Der Connector macht aktuell bei der Ausführungs einen Synchronisationslauf,
aber die Chancen stehtn gut, dass eine ständige Synchronsiation gewünscht ist.

Es ist einfache den Ausführung wiederkehrender Läufe zu automatisieren.
