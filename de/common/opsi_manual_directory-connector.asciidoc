...
Mit `ski_adding_clients` kann das anlegen von Clients in opsi komplett übersprungen werden. Diese Option kann zum Beispiel in Verbindung mit `prefer_location_from_directory` verwendet werden, sodass nur bestehende Clients verschoben werden.
