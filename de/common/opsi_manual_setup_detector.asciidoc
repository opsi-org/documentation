...
* Sie sollten idealerweise den opsi-Package-Builder auf Ihrem Rechner installiert haben
...
Im Falle von `... und baue opsi Paket` wird der opb im Hintergrund aufgerufen, um das vom opsi-setup-detector erstellte Verzeichnis mit den opsi Dateien zu einem Paket zu bauen. Dabei kann über die Checkboxen 'Bau Modus' ausgewählt werden, ob der Bau 'still' sein soll, ob das Paket gebaut werden soll und ob das gebaute Paket installiert werden soll.
...
Der opsi-setup-detector kann gestartet werden aus dem Prorgramm Menü und findet sich dort unter `opsi.org`. Der opsi-setup-detector wird unter Windows auch in das Kontextmenü des Explorers eingebunden, um so per Rechte Maustaste Setup-Programm direkt zur Analyse aufrufen zu können.
...
Verläuft analog zu dem obigen Punkt 1 nur, dass nach der Analyse des Setup-Programms abgebrochen wird.

. `Eine opsi Paketvorlage (Template) erzeugen` +
Dieser Punkt fragt nicht nach einer Setup-Datei, sondern erstellt ein Template analog dem opsi-Produkt `opsi-template` nur, dass hier die Angaben aus der Produktkonfiguration bereits übernommen werden.
...
* `Erstelle opsi Produkt Dateien` erzeugt falls noch nicht vorhanden, den Verzeichnisbaum für das neue opsi Paket auf der gewählten `opsi-Workbench`. Die für das Paket benötigten Dateien werden erzeugt bzw. kopiert.
