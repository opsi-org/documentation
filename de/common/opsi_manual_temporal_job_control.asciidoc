...
Um die Steuerung per cron job möglichst einfach und effektiv zu machen, bringt `opsi` das `opsi-wakeup-clients` (basierend auf dem Skript wake_clients_for_setup.py von opsi 4.1) mit. Dieses Kommando hat folgende Aufgaben:
...
35 5 * * * /usr/bin/opsi-wakeup-clients --log-level=5 --event=gui_startup --product-group-id=nightly-cron-produkte --host-group-id=nightly-jpr-gruppe-530 --wol-timeout=120 --event-timeout=120
----
