...
* Für die Installation über PXE-boot wird ein UEFI-taugliches
winpe_uefi (analog dem winpe für die legacy-Installation) benötigt. Oftmals enthält ein existentes winpe bereits UEFI- Unterstützung (Ordner 'EFI' und Datei 'bootmgr.efi' im winpe-Ordner des opsi-Netboot-Produkts.
