...
** Schalten Sie das BIOS in den UEFI-Mode. Wenn Sie die Wahl haben zwischen 'UEFI only','Legacy only', 'Both' dann nehmen Sie 'UEFI only'. Wenn der Rechner fest auf 'Both' steht ist dies schlecht, kann aber evtl. trotzdem funktionieren. Wenn es 'Legacy Support' gibt, so diesen disbalen. 'CSM Support' in Verbindung mit 'UEFI only' kann enabled bleiben., ansonsten eher disablen. 'UEFI Network Boot' muss enabled werden. Der entsprechende Eintrag kann auch 'Network Stack' heissen und unterhalb von 'UEFI' liegen. Wenn dies noch gesondert für 'IPv4' und 'IPv6' passieren kann ist hier 'IPv4' die richtige Wahl.
