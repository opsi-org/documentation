...
Falls Sie diese Alternative `elilo.efi` verwenden wollen, so ist sie unter https://download.uib.de/opsi4.1/misc/uefi/elilo_with_timeout/ zu finden und kann dann im tftpboot-Verzeichnis plaziert werden (usually in `/tftpboot/linux/pxelinux.cfg/`), um das Original zu ersetzen.
