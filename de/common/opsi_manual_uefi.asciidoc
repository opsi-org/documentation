...
* Für die Installation über PXE-boot wird ein spezielles 
winpe_uefi (analog dem winpe für die legacy-Installation) benötigt. Dieses sollte auf Basis von Windows 8.1 mittels dism erstellt werden und die vorhandene Datei `startnet.cmd` durch eine `startnet.cmd` mit einer Zeile `c:\opsi\startnet.cmd` als Inhalt ersetzt werden.


* Einen externen DHCP-Server müssen Sie so konfigurieren, dass er ein PXE-Boot über den {opsi-Server} ermöglicht. Als Bootfile ist einzutragen: 'linux/pxelinux.cfg/elilo32.efi'
...
* Das UEFI BIOS ist in 32 oder 6
...
----			
* In den Bereichsoptionen gibt es dann zwei Einträge für die Startdatei, in einem Fall jedoch an eine Richtlinie gekoppelt:
----
