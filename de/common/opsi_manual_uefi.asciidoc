...
Damit Uefi-Clients auch per PXE-Boot betankt werden können, benötigen sie einen anderen Bootfile Eintrag, als den herkömmlichen für Bios-Clients. Als Bootfile ist einzutragen: 'linux/pxelinux.cfg/elilo-x86.efi'.
...
==== Alternative elilo.efi

Die standardmäßig verteilte `elilo.efi` ist so konfiguriert, dass unendlich lange auf Informationen für einen Netboot gewartet wird.
In den meisten Szenarien ist das kein Problem, da nach einer erfolgreichen Installation die Bootreihenfolge auf einen Boot von Festplatte angepasst wird.
Es gibt allerdings einige Geräte, bei denen diese Anpassung nicht automatisiert möglich ist, obwohl dies in der UEFI-Spezifikation vorgeschrieben ist.

Es gibt eine alternative `elilo.efi`, welche nur eine kurze Zeit wartet, bevor ein Boot von Festplatte gemacht wird.
Leider funktioniert dies nicht mit allen Geräten, inbesondere bei der Verwendung von NVME-Festplatten, und ist daher nicht der Default.
