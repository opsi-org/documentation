...
Das Objekt vom Typ 'Product', welches das opsi-Paket 'javavm' beschreibt sieht z.B. so aus:
...
* 'createObjects' (zum Erzeugen vieler Objekte, aktualisiert vorhandene Objekte)
...
* 'insertObject' (zum Erzeugen eines neuen Objektes, aktualisiert ein vorhandenes Objekt)
...
So liefert z.B die Methode 'product_getObjects', parametrisiert mit 'attributes:["name"]' für das Paket ('Product') 'javavm':
...
Mit 'filter' kann eingeschränkt werden, zu welchen Objekten Informationen geholt werden sollen, ähnlich einer SQL-where-Bedingung.
So schränkt für 'product_getObjects' der Filter '{"id":"javavm"}' die Rückgabe auf das Object 'javavm' ein.
