...
Als Beispiel dient hier das Objekt 'product'.
...
Über den 'returnType' kann die Datenstruktur der Elemente im Ergebnis gewählt werden.
...
TIP: Zum Aktualisieren eines Objekts bietet sich in der Regel die Verwendung der Methode 'updateObjects' an.
...
Der Parameter 'attributes' dient dazu, nur bestimmte Attribute des Objektes abzufragen, was Geschwindigkeitsvorteile bringen kann.
Wird eine Liste von Attributen angegeben, werden nur diese aus dem Backend gelesen.
Die restlichen Attribute werden mit dem Wert 'null' zurückgegeben.
Attribute die das Objekt identifizieren ('type'/'id'/'ident') werden immer befüllt.
Wenn alle Attribute ausgelesen werden sollen (default), kann 'null' oder '[]' als 'attributes' übergeben werden.
...
Mehrere übergebene Filter-Attribute werden hierbei 'AND'-verknüpft.
Wird für ein Attribut eine Liste von Werten übergeben, resultiert hieraus eine 'OR'-Verknüpfung.
...
Falls sich aktuell ein anderes Event in einer unterbrechbaren Wartephase befindet, wird dieses unterbrochen und stattdessen das neue Event gestartet.
