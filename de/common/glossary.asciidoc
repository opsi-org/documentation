...
Backend::
  opsi unterstützt unterschiedliche Arten der Datenhaltung, bspw. Dateibasiert oder in einer Datenbank. Diese werden als Backends bezeichnet und über den -> BackendManager konfiguriert.


BackendManager::
  Programm / Konfigurationsdatei in der festgelegt wird, welche Daten wie und wo gespeichert werden.


clientId::
  Eindeutige Bezeichnung eines Clients durch Verwendung des 'fully qualified hostname' (FQDN).
...
hostId::
  Eindeutige Bezeichnung eines Hosts (umfasst sowohl Client als auch Server) durch Verwendung des 'fully qualified hostname' (FQDN).
...
Produkt-ID::
  Eindeutiger Bezeichner eines opsi-Produkts. Dieser darf keine Leerzeichen oder Sonderzeichen (außer Bindestrich enthalten).
...
tftp (Trivial File Transfer Protocol)::
  Ein tftp-Dienst ('tftpd') erlaubt das Übertragen von Dateien von und zu anderen Rechnern ohne Login-Prozedur, allerdings mit restriktiven Zugangsbeschränkungen.
