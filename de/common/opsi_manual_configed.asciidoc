...
Das Terminal bietet außerdem die Möglichkeit Datenquellen anzugeben, die vor der Ausführung durch konkrete Daten ersetzt werden. (Mehr zu dieser Funktionalität: <<opsi-manual-configed-serverkonsole-commandcontrol>> - Punkt: Datenquellen)
.'{opsi-configed}': Menü: Server-Konsole
image::configed_serverkonsole.png["{opsi-configed}: Server-Konsole", width=300]
...
image::configed_serverkonsole-defcommands.png["{opsi-configed}: Befehle definieren", width=300]
...
Zusätzlich können Methoden als Datenquelle hinterlegt werden, d.h. vor Ausführung des Befehls werden Parameter mit dem Ergebnis einer Methode überschrieben. folgende Methoden sind möglich:
    ...
    Hinweis: Außer für die "Interaktive Eingabe" kann die Rückgabe der Methoden formatiert werden und beispielsweise in eine kommaseparierte Liste. 
...
image::configed_serverkonsole-exec.png["{opsi-configed}: Befehl ausführen - Parameterabfrage", width=300]
