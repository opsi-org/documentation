...
IMPORTANT:  Der '{opsi-configed}' in der Version 4.0.6 setzt Java mindestens in der Version 7 voraus. 
...
Der Aufruf über Webstart (via den Menüeintrag bei der ersten Adresse) ist, wenn der Webstart-Mechanismus installiert ist, gegenüber dem als Applet zu bevorzugen.
Der Grund ist, dass dabei die Einbindung des Anwendungsfensters in das Browserfenster entfällt (das ist auch der einziger Unterschied zwischen 
Applet und der per Webstart laufenden Anwendung).

Außerdem kann der '{opsi-configed}' als lokale Anwendung - und daher einfacher zu administrieren -  über das entsprechende
opsi-configed-Paket, das auf download.uib.de bereitsteht, installiert werden. Dabei wird ein entsprechender Startmenü-Eintrag angelegt.

Auf dem Server wird '{opsi-configed}' als Debianpaket (opsi-configed.xxxxx.deb) bzw. entsprechendes Paket einer anderen Distribution installiert.
Sofern es sich um einen Server mit graphischer Oberfläche handelt, ist das Programm über einen Menüeintrag im Desktopmenü sowie über `/usr/bin/opsi-configed` startbar. 
Auf Non-Gui-Servern kann lediglich die Kommandozeilen-Option _querysavedsearch_ des '{opsi-configed}' verwendet werden. 
...
Beim Login versucht sich der  '{opsi-configed}' per 'https' auf den Port 4447 des 'opsiconfd' - bzw. einen anderen angegebenen Port - zu verbinden.
...
Da im LAN die Kompression im Normalfall kaum einen Unterschied macht, bei WAN Verbindungen jedoch meist notwendig ist, 
...
Per default ist das Depot auf dem '{opsi-configserver}' markiert und die zu diesem Depot gehörigen 
...
d.h. beim Auswählen eines Depots werden seine Daten sofort geladen. Dazu existieren folgende Buttons:
...
Der '{opsi-configed}' speichert ab Version 4.0.4 lokal (auf dem PC, auf dem er läuft) userbezogen die zuletzt getätigte Gruppenauswahl 
...
* ob der Suchstring auch mitten im Text als Teilstring auftauchen darf (Suchmodus "Volltext", das ist der Default),

* ob der Feldtext mit dem Suchstring beginnen muss (Suchmodus "Anfangstext"),

* ob der Suchstring als regulärer Ausdruck zum Pattern-Matching dienen soll (Suchmodus "Reguläre Ausdruck", eher etwas für Experten, im Zweifel ist die Dokumentation des 
Java-Pattern-Matching heranzuziehen).
...
Im Gegensatz zu einem 'Directory', in dem jeder Client einmal vorkommen darf.
...
Der Bereich 'Directory' im Treeview ist ähnlich der von 'Gruppen' nur das unterhalb von 'Directory' jeder Client exakt an einer Stelle in der Hierarchie auftaucht. Clients die in keiner Gruppe des Directory existieren, verwenden die vorgegebene Gruppe 'NICHT_ZUGEWIESEN'.
...
Seit der Version 4.0.5.8.1 gibt es die Möchlichkeit verschiedene standard Konfigurationen für einen Client direkt in der 
Client-Information zu setzten, die sich rechts von der Client-Liste befinden.


* Install By Shutdown: +
Der herkömmliche Weg Installationen beim Herunterfahren durchzuführen, befindet sich im <<opsi-on-shutdown>>. 
Dies wird nun vereinfacht durch die Buttons "An" und "Aus", welche beim Betätigen, den opsi-client-agent mit dem entsprechenden
...
* WAN-Konfiguration: +
Bei dieser Konfiguration handelt es sich um die in <<opsi-manual-wansupport-recommended-config>> beschriebenen, 
von uib GmbH empfohlenen standard WAN-Konfiguration. Die darin befindlichen Schritte werden beim akt- bzw. deaktieren 
entsprechend durchgeführt und beim Auswählen eines Clients ausgelesen. Eine eigene Konfiguration für diese Zustände ist nicht
implementiert.
...
übertragen werden. 
Die zwei angebotenen byAudit-Treiberpfade setzten sich zusammen aus dem Hersteller und dem Produkt bzw. dem Modell, welche jeweils aus dem Computer und der Hauptplanine ausgelesen werden. Betätigt man den rechten Button zum Treiberupload, so erscheint ein neues Fenster um weitere Einstellungen zu tätigen. 
 ...
 * 'byAudit': In dem Verzeichnis `./drivers/drivers/additional/byAudit/<Vendor>/<Model>` hinterlegte Treiber, werden bei einer Windows-Installation nach einem Verzeichnisnamen durchsucht, der dem bei der Hardwareinventarisierung gefundenen 'Vendor' entspricht. In diesem 'Vendor' Verzeichnis wird nun nach einem Verzeichninamen gesucht, das dem bei der Hardwareinventarisierung gefundenen 'Model' entspricht. Wird ein solche Verzeichnis gefunden, so wird diese Verzeichnis genauso behandelt, als wären sie über das Produktproperty 'additional_drivers' manuell zugewiesen.
...
[[opsi-manual-configed-serverkonsole]]
=== Server-Konsole
Mit der Version 4.0.8 wird der Configed um einen neuen Hauptmenüeintrag erweitert, die "Server-Konsole". Damit wird erstmalig die Möglichkeit geboten vom Configed aus über eine SSH-Verbindung Aktionen auf dem opsi-Server auszulösen. Zum einen kann ein Terminal auf dem opsi-Server gestartet werden, zum anderen existieren Menüpunkte für definierte Kommandozeilenbefehle. Wenn nicht anderes angegeben, wird für die Verbindung der im Configed angemeldete User und der verbundene Configserver verwendet. Diese Einstellungen lassen sich unter dem Menüpunkt  "Verbindungsdaten" ändern.  +
Um diverse Funktionen verwenden zu können müssen entsprechende Server-Configs aktiviert werden.+

* configed.ssh_config_active: +
De-/aktiviert die Verbindungseinstellungen. 

* configed.ssh_control_active: +
De-/aktiviert die Befehlsbearbeitung. 

* configed.ssh_menu_active: +
De-/aktiviert das gesamte Menü "Server-Konsole". 

* configed.ssh_shell_active: +
De-/aktiviert das Terminal. 


.'{opsi-configed}': Menü: Server-Konsole
image::configed_serverkonsole.png["{opsi-configed}: Server-Konsole", width=400]
...
Neben der Möglichkeit die Eingabe durch Sternchen (*) zu ersetzen, was für Passwort-Eingaben empfohlen wird, kann auch ein Prozess mit Hilfe des Buttons "Prozess/Verbindung beenden" unterbrochen werden (Unix-Terminal: "Strg+C"). +
Außerdem können Datenquellen angegeben werden, die vor der Ausführung durch konkrete Daten ersetzt werden. (Mehr zu dieser Funktionalität: <<opsi-manual-configed-serverkonsole-commandcontrol>> - Punkt: Datenquellen)
...
* Erstellung opsi-Paket ... +
Vorraussetzung für diesen Befehl ist ein opsi-utils-Paket mit der Version >=4.0.8. Über diesen Menüpunkt kann dann, mit Angabe eines Server-Verzeichnis, aus diesem ein opsi-Paket erstellt werden. Außerdem können die gefundenen Versionen (Paket- und Produkt-Version) über einen Button angezeigt und überschrieben werden. Zusätzlich kann eine md5-Summe und/oder eine zsync-Datei erstellt werden.
...
Möchte man existierende Rechner zu opsi hinzufügen, muss der opsi-client-agent auf dem Zielrechner installiert werden. Wählt man die betreffenden Clients im configed aus und ruft diesen Befehl auf, werden die Client-Namen in das entsprechende Feld übernommen. Wenn der Befehl für mehrere Clients mit einem Aufruf ausgeführt werden soll, müssen die Login-Daten bei den beteiligten Rechnern gleich sein. Der Button rechts des Passwort-Fensters zeigt das Passwort in Klartext bzw. ersetzt durch Sternchen(*) an. +
Genauere Informationen finden Sie im Handbuch 'opsi-getting-started' im Kapitel 'Erste Schritte'.

[[opsi-manual-configed-serverkonsole-commandcontrol]]
==== Befehle definieren
Zusätzlich zu den vordefinierten Befehlen lassen sich eigene Befehle erstellen bzw. verändern oder entfernen, die über Menüpunkte aufgerufen werden können. Dabei ist zu beachten, dass unterschiedliche Linux-Systeme unter Umständen nicht die gleichen Kommandozeilenbefehle ausführen können. Deswegen muss der Administrator sicher stellen, dass die Befehle entsprechend dem adressierten Linux-System funktionieren. 
Voraussetzung dafür ist, dass das python-opsi-Paket mindestens in der Version 4.0.8 vorliegt, da erst ab dieser Version entsprechende Methoden bereitgestellt werden (Siehe <<opsi-manual-configuration-tools-opsi-admin>>).  +
...
TIP: -- Unter Linux lassen sich Befehle mit Hilfe von zwei kaufmännischen Unds ("&&") kombinieren. Dabei muss jedoch darauf geachtet werden, dass zum zweiten Befehl, falls benötigt ein sudo ergänzt wird, da dies nicht automatisch geschieht. Beispiel: Benötigt root-Rechte:"aktiviert" , Befehlsliste: "apt-get update --yes && sudo apt-get upgrade --yes". + +
 ...
 -- An den Linux-Befehl kann "" angehängt werden, damit wird erreicht, dass der Prozess weiter ausgeführt wird, selbst wenn der opsi-configed geschlossen wird.
