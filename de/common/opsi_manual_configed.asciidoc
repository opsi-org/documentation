...
Aktiviert die Verbindungseinstellungen. (Default: false)
...
Deaktiviert den Hauptmenüeintrag "Server-Konsole". (Default: true)
...
Vorraussetzung für diesen Befehl ist ein opsi-utils-Paket mit der Version >=4.0.8. Über diesen Menüpunkt kann dann, mit Angabe eines Server-Verzeichnis, aus diesem ein opsi-Paket erstellt werden. Außerdem können die in der Control-Datei gefundenen Versionen (Paket- und Produkt-Version) über einen Button angezeigt und überschrieben werden. Zusätzlich kann eine md5-Summe und/oder eine zsync-Datei erstellt werden.
