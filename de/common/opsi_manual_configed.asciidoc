...
IMPORTANT:  Der '{opsi-configed}' in der Version 4.0.7 benötigt eine Java-Laufzeitumgebung mindestens in der Version 7 voraus. 

Er arbeitet mit den Daten eines auf dem Server laufenden 'opsiconfd' mindestens der Version 4.0.6.

Der '{opsi-configed}' steht auf download.uib.de in der jeweils aktuellen Version als opsi-Paket zur lokalen Installation zur Verfügung.
Auf Nicht-opsi-Systemen kann der '{opsi-configed}' auch durch Kopieren der zum Paket gehörenden Dateien bereitgestellt werden.

Die jeweils stabile Version des '{opsi-configed}' ist auch Element der opsi-Server-Installation und wird über die opsi-Distributionsrepositories aktualisert.
Das Programm ist code-identisch mit lokalen Version, wird aber als Java-Webstart-Anwendung zur Verfügung gestellt. Der Aufruf erfolgt

* über den Link auf der Server-Übersichtsseite https://<servername>:4447 
...
Seit  '{opsi-configed}' Version 4.0.7.5 bietet der '{opsi-configed}' die Userrollen-Funktion an. 

In der Oberfläche zeigt die Existenz der Kategorie _user_ in der Übersicht der Server-Hostparameter, dass diese Funktion verfügbar ist. 
...
Des Weiteren finden sich hier die Defaultwerte für die userspezifische Konfiguration der Server-Konsole (vgl. [[opsi-manual-configed-serverkonsole]]).
...
Die dabei für die Rechteverwaltung verwendeten Defaulteinstellungen entsprechend den "klassischen" Vorgaben für einen Adminitrator, 
d.h. zunächst werden dem User keine Einschränkungen auferlegt. Z.B. für einen User admindepot1 werden die Einträge

[source,prompt]
----
user.{admindepot1}.privilege.host.all.registered_readonly 	[false]
user.{adminuser}.privilege.host.depotaccess.configured  	[false]
user.{adminuser}.privilege.host.depotaccess.depots  		[]
user.{adminuser}.privilege.host.opsi.opsiserver write 		[true]
...
- _admindepot1_ hat _nicht_ lediglich _readonly_-Zugriff auf den Server (wie es möglicherweise für einen Helpdesk-Mitarbeiter angezeigt wäre);
- bis jetzt existieren _keine_ zu berücksichtigenden Depotrestriktionen;
- entsprechend kann die Liste der zugänglichen Depots leer bleiben (aber auch wenn hier etwas stünde, hätte es keine Wirkung);
- der User darf configserver-Einstellungen aller Art bearbeiten.

Wenn nun der _admindepot1_ tatsächlich nur noch Zugriff auf die Rechner im Depotserver _depot1_ haben soll, ist folgendes einzustellen:
...
Nach einem Datenreload sind für den User _admindepot1_ keine Clients aus anderen Depots mehr sichtbar. 
Das kann er allerdings selbst ändern, solange er über das Privileg _host.opsiserver.write_ verfügt. 
Um die Abriegelung komplett zu machen, ist daher noch
...
Bei anderen Zugriffsmethoden auf die json-RPC des opsi-servers, werden sie nicht wirksam.
...
Wer jetzt befugt ist, diese Einstellung zu bearbeiten, kann dies in  
...
=== Server-Konsole
CAUTION: Eine der nachfolgend beschriebenen Configed-Funktionen wird mit der kommenden python-opsi Version 4.0.7.40 (?)  freigeschaltet. Dazu zählt vorallem die Befehlsbearbeitung (<<opsi-manual-configed-serverkonsole-commandcontrol>>) und die Ausführung der darüber hinterlegten Befehle im Configed.

Mit der Version 4.0.7.5 wird der Configed um einen neuen Hauptmenüeintrag erweitert, die "Server-Konsole". Damit wird erstmalig die Möglichkeit geboten vom Configed aus über eine SSH-Verbindung Aktionen auf dem opsi-Server auszulösen. Zum einen kann ein Terminal auf dem opsi-Server gestartet werden, zum anderen existieren Menüpunkte für definierte Kommandozeilenbefehle. 
...
Wenn nicht anderes angegeben, wird für die Verbindung der im Configed angemeldete User und der verbundene Configserver verwendet. 
...
Dafür wurden zwei neue Parameter hinzugefügt: 

* --ssh-key PATH:  z.B. --ssh-key /home/user/.ssh/id_rsa
* --ssh-passphrase PASSPHRASE: z.B. --ssh-passphrase meinpassword

Diese Einstellungen lassen sich unter dem Menüpunkt  "Verbindungsdaten" ändern. +

Um diverse Funktionen verwenden zu können, müssen entsprechende Server-Configs aktiviert werden. In Verbindung mit dem Userroles-Feature können diese Konfigurationen auch User-Spezifisch gesetzt werden.  +

* user.{}.ssh.menu_config.active: +
De-/aktiviert die Verbindungseinstellungen. 

* user.{}.ssh.menu_control.active: +
De-/aktiviert die Befehlsbearbeitung. 

* user.{}.ssh.menu_main.active: +
De-/aktiviert das gesamte Menü "Server-Konsole". 

* user.{}.ssh.menu_terminal.active: +
De-/aktiviert das Terminal. 

* user.{}.ssh.menus_commands.active: +
De-/aktiviert alle Menüeinträge, die hinterlegte Befehle darstellen. 
...
Wie aus der Kommandozeile gewohnt, kann die "TAB"-Taste Befehle vervollständigen. Achtung: Pfade werden nicht vervollständigt - ledigich Linux-Systembefehle. +
