...
=== Server-Konsole
CAUTION: Eine der nachfolgend beschriebenen Configed-Funktionen wird mit der kommenden python-opsi Version 4.0.7.38 freigeschaltet. Dazu zählt vor allem die Befehlsbearbeitung (<<opsi-manual-configed-serverkonsole-commandcontrol>>) und die Ausführung der darüber hinterlegten Befehle im Configed.
...
Um diverse Funktionen verwenden zu können, müssen entsprechende Server-Configs aktiviert werden. In Verbindung mit dem Userroles-Modul können diese Konfigurationen auch User-Spezifisch gesetzt werden.  
...
Aktiviert den Hauptmenüeintrag "Server-Konsole". (Default: true)
...
Vorraussetzung für diesen Befehl ist ein opsi-utils-Paket mit der Version >=4.0.7.7. Über diesen Menüpunkt kann dann, mit Angabe eines Server-Verzeichnis, aus diesem ein opsi-Paket erstellt werden. Außerdem können die in der Control-Datei gefundenen Versionen (Paket- und Produkt-Version) über einen Button angezeigt und überschrieben werden, sowie eine md5-Summe und/oder eine zsync-Datei erstellt werden kann. Beim Ausführen wird eine .opsi-Datei erstellt. Zur Installation auf dem Depot leitet der Button "Das neu erstellte Paket installieren" zum opsi-package-manager weiter.
...
Voraussetzung dafür ist, dass das python-opsi-Paket mindestens in der Version 4.0.7.38 vorliegt, da erst ab dieser Version entsprechende Methoden bereitgestellt werden (Siehe <<opsi-manual-configuration-tools-opsi-admin>>).
