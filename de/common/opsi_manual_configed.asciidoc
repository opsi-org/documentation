...
Das Programm ist code-identisch mitder lokalen Version, wird aber als Java-Webstart-Anwendung zur Verfügung gestellt. Der Aufruf erfolgt entweder 
im Browser über 
...
z.B. mit einem entsprechenden Eintrag im Startmenü (wie er z.B. durch das opsi-configed-Produkt erzeugt wird).
...
das vermehrte Logging eingeschaltet werden.
...
Die erzeugten Logdateien liegen per Default im Heimatverzeichnis des Users im Unterverzeichnis ".configed"; 
die letzte Logdatei heißt configed.log, die bis zu drei vorangegangen configed_0.log, configed_1.log, configed_2.log

Das Heimatverzeichnis des Users wird in den neueren Versionen von Windows interpretiert als 

c:\users\[USERNAME]\appdata\local

Zu beachten ist, dass im Windows-Explorer c:\users als c:\benutzer dargestellt wird. Das Unterverzeichnis AppData 
wird standardmäßig nicht gelistet, erscheint aber, wenn "c:\benutzer\[USERNAME]\" um den Text "appdata" ergänzt wird

Per Kommandozeilenparameter -d kann das Logging-Verzeichnis beliebig geändert werden. Im opsi-Produkt opsi-configed 
wird mit diesem Mechanismus das Logging-Verzeichnis auf 

c:\opsi.org\log

eingestellt.

Weil in dieses Verzeichnis aber nur Administratoren schreiben können, loggt der  '{opsi-configed}' bei anderen Rechten 
doch wieder in 
