...
IMPORTANT:  Der '{opsi-configed}' in der Version 4.0.6 benötigt eine Java-Laufzeitumgebung mindestens in der Version 7 voraus. 
...
* über die allgemene Server-Seite https://<servername>:4447
...
Auf jedem System kann, wenn die benötigten jar-Archive verfügbar sind, der  '{opsi-configed}'  `java -jar configed.jar` erfolgen.
...
<<<<<<< HEAD
Wenn die jnlp-Version z.B. in einer externen Webseite eingebunden wird, in dem Parameter für den '{opsi-configed}' generiert werden, 
können diese in einer speziellen Form auch an den jnlp-Aufruf übergeben werden.

=======
==== Benutzerdefinierte Startparameter mit Java Web Start

Es ist möglich beim Start mittels Java Web Start Parameter mit Werten vorzubelegen.
Dazu muss die Adresse zur JNLP-Datei mit `javaws` direkt aufgerufen und die gewünschten Parameter übergeben werden.
...
>>>>>>> b408be0732c4e2ee399220cd21ec16999476333d
...
seinen Defaultport 4447 verwendet, muss dieser nicht angegeben werden. Der User müssen der Unix-Gruppe 'opsiadmin' angehören. 

Der '{opsi-configed}' speichert im lokalen Benutzerprofil einige Information über die jeweilige Session, damit beim erneuten Login die jeweilige Arbeitsumgebung wiederhergestellt werden kann, 
insbesondere eine ausgewählte Client-Gruppe. Entsprechend schlägt er auch, wenn mit mehreren opsi-servern gearbeitet wird, in der Auswahlliste an oberster Stelle den zuletzt verbundenen vor. 
