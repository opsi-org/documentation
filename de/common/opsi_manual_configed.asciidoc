...
Des Weiteren finden sich hier die Defaultwerte für die userspezifische Konfiguration der Server-Konsole.
...
user.{adminuser}.privilege.host.opsiserver.write 		[true]
----
...
Das kann _admindepot1_ allerdings selbst ändern, solange er über das Privileg _host.opsiserver.write_ verfügt. 
...
=== Server-Konsole
CAUTION: Eine der nachfolgend beschriebenen Configed-Funktionen wird mit der kommenden python-opsi Version 4.0.7.40 (?)  freigeschaltet. Dazu zählt vor allem die Befehlsbearbeitung (<<opsi-manual-configed-serverkonsole-commandcontrol>>) und die Ausführung der darüber hinterlegten Befehle im Configed.
