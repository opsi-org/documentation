...
* `askbeforeinst`: +
Soll das Starten der Installation am Client bestätigt werden müssen? 

* `architecture`: +
Architektur Auswahl, beeinflusst die Auswahl des bootimages und die Installationsarchitektur.

* `system_partition_size`: +
Größe der Systempartition. Die Größe kann in Prozent der Festplattengröße oder als absoluter Wert (G=Gigabyte) angegeben werden. Wenn Sie einen kleineren Wert als 100% angeben, wird der verbleibende Rest als Datenpartition verwendet (wenn das Property data_partion_create = true).

* `swap_partition_size`: +
Größe der Swappartition

* `data_partition_create`: +
Verwende freien Plattenplatz zur Erstellung einer Datenpartition. (true/false)
...
*never* = Die gesamte Partitionstabelle wird immer neu geschrieben.

* `language`: +
Welche Sprache / locale soll installiert werden.

* `console_keymap`: +
Zu installierendes Tastaturlayout
...
* `install_opsi_server`: +
Installiere die opsi-server Pakete.


* `online_repository`: +
Repository der Distribution für die Installation. (Nicht bei SLES)

* `opsi_online_repository`: +
Repository der opsi-server Pakete.

* `proxy`: +
Proxystring (wenn benötigt) in der Form: `http://<ip>:<port>`

* `additional_packages`: +
Welche zusätzlichen Pakete sollen installiert werden? Angabe der 

* `wget_and_execute`: +
Url (http) einer Datei welche am ende der Installation geholt und ausgeführt wird.

* `install_opsi-client-agent`: +
Installiere den Linux opsi-client-agent (Kofinanzierungsprojekt: Sie benötigen eine Aktivierung durch die /etc/opsi/modules)

* `release`: +
(nur Debian und Ubuntu) +
Welches Release der Distribution soll installiert werden

* `setup_after_install`: +
Welche opsi Produkte sollen zum Abschluss der Betriebssysteminstallation auf setup gestellt werden.
