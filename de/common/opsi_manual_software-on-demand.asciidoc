...
Ab opsi-client-agent Version 4.1.1.6-5 steht der Kiosk als eigenständiges Paket (opsi-client-kiosk) zur Verfügung wird aber bei der Installation des opsi-client-agent gegenenfalls mit installiert falls es noch nicht vorhanden ist. 
...
|==========================
**wird bei der Installation des opsi-client-agents (>=4.1.1.6-5) automatisch mit installiert falls nicht vorhanden 

[[software-on-demand_kiosk_migration]]
===== Automatische Migration des Kiosk aus dem opsi-client-agent (>=4.1.1.6.5)

Damit möglichst wenig Änderungen manuell durchgeführt werden müssen wird versucht die Einstellungen des Kiosk automatisch aus dem opsi-client-agent in das opsi-client-kiosk Paket zu migrieren und dieses dann auch gegebenenfalls mit auf dem Depot bzw. den entsprechenden Clients zu installieren. In der Regel geschieht das weitestgehend automatisch und Sie müssen nichts weiter tun. 
...
* das opsi-client-kiosk Paket ist noch NICHT auf dem Depot installiert
* der opsi-client-agent (>=4.1.1.6-5) wird über einen alten opsi-client-agent installiert der den alten Kiosk noch enthält (d.h. enthält das Verzeichnis opsiclientkiosk, bzw. kleiner/gleich Version 4.1.0.0-*)

[underline]#Installation opsi-client-agent Version (>=4.1.1.18-1) auf einem Client#

Bei der Installation des opsi-client-agent (>=4.1.1.18-1) auf dem Client wird der opsi-client-kiosk unter bestimmten Voraussetzungen auf setup gestellt. Dies passiert wenn folgende Bedingungen erfüllt sind:

* das opsi-client-kiosk Paket ist noch NICHT auf dem Client installiert
* der config/Host-Patrameter software-on-demand.active steht auf true für diesen Client

Somit wird das opsi-client-kiosk Paket auf dem Client in den meisten Fällen im Anschluss der opsi-client-agent mit installiert, da der Client-PC in der Regel rebooted wird und das opsi-clientd Event gui_startup eintritt welches meist konfiguriert sein sollte.
...
CAUTION: Bei der Installation des opsi-client-agent (>=4.1.1.6-5) wird der alte Kiosk im opsi-client-agent Verzeichnis der Clients deaktiviert und es wird das neue opsi-client-kiosk Paket auf dem Depot installiert sowie für Clients bei denen _software-on-demand.active_ = _true_ ist und das neue opsi-client-kiosk Produkt nicht installiert ist das neue opsi-client-kiosk Produkt installiert.
...
Das verwendete Desktop/Startmenü-Icon kann durch Ablegen einer `kiosk.ico` Datei im ock_custom Verzeichnis verändert werden (ab opsi-client-kiosk Version 4.1.1.7-2).
