...
|==========================
**ab opsi-client-agent Version >=4.1.1.6-5 wird der Kiosk als eigenständiges Produkt (opsi-client-kiosk) geführt. Er wird aber bei der Installation des opsi-client-agents gegebenenfalls automatisch mit installiert falls er nicht vorhanden ist. Für mehr Details siehe <<software-on-demand_opsi-client-kiosk, Neue opsi-client-kiosk Anwendung>>.
...
Kiosk als eigenständiges Produkt (opsi-client-kiosk): `/var/lib/opsi/depot/opsi-client-kiosk/files/app/ock_custom` +
Kiosk noch im opsi-client-agent Produkt integriert: 
...
*Erstinstallation / Installation über ein schon vorhandenen opsi-client-kiosk*

Installieren sie das aktuelle opsi-client-kiosk Paket (zu finden auf download.uib.de) auf dem gewünschten Depot und aktualisieren Sie gegebnenfalls auch das opsi-client-agent Paket.
...
*Installation über einen alten opsi-client-agent der den alten Kiosk noch enthält (d.h. enthält das Verzeichnis opsiclientkiosk, bzw. \<=4.1.0.0)*
...
Das Programm loggt nach `C:\opsi.org\applog`.
