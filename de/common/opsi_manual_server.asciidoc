...
* +[global] max log size+: +
Über diese Option kann die Größe der Logdateien beschränkt werden. Aus historischen Gründen ist dies auf 5MB pro Logdatei beschränkt. Seit opsi 4.0.6 ist es möglichen diesen Wert an eigene Bedürfnisse anzupassen. Um die Größenbeschränkung zu deaktivieren, kann der Wert auf 0 gesetzt werden.

Ergänzend zu dieser Einstellmöglichkeit ist es möglich mittels 'logrotate' die Logdateien auf einem Server automatisch zu komprimieren und rotieren zu lassen. Bitte entnehmen Sie dem zugehörigen Handbuch die Konfigurationsmöglichkeiten.
...
Dieser System-User pcpatch hat üblicherweise die user-ID 992. Dieser User hat per Voreinstellung das Heimatverzeichnis `/var/lib/opsi`.Setzen und Ändern Sie das Passwort mit `opsi-admin -d task setPcpatchPassword`.
...
In der Voreinstellung liegt dieses Verzeichniss auf dem opsi-server unter `/opt/pcbin`.
