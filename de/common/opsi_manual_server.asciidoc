...
Seit Version 4.2 wird der In-Memory Speicher Redis eingesetzt (https://redis.io/). Der '{opsi-server}' speichert folgende Daten in Redis: 

* Log-Ausgaben werden in einen Redis-Stream geschrieben. Dieser Stream wird dann zB. vom Logviewer gelesen. 

* Session Daten werden in Redis als MessagePack abgelegt (https://msgpack.org/).

* Statistiken wie zB. CPU Auslastung werden als Time Series in Reids gespeichert. Hierzu wird das Modul RedisTimeSeries verwendet (https://github.com/RedisTimeSeries/RedisTimeSeries).

Die Statistiken werden von Grafana (https://grafana.com/) ausgelesen und angezeigt. Das Grafana Dashboard ist über die URL https://grafana:3000 erreichbar.
...
Die einzelnen Konfigurations-Optionen können über den Befehl `opsiconfd --help` abgefragt werden. Um Optionen bei jedem Start des 'opsiconfds' zu verwenden, können die Option aus dem Hilfetext ohne das `--` in die Konfigurationsdatei eingetragen werden.
...
'userlogin'.
Es gibt außerdem einige Client-unabhängige Logs: +
'opsi-backup.log' und 'opsi-package-updater.log'.
...
Ab Version 4.2 ist im 'opsiconfd' und 'opsipxeconfd' ein eigenes Logrotate eingebaut, welche über die Optionen 'max-log-size' und 'keep-rotated-logs' gesteuert wird.
