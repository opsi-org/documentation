...
* opsi Erweiterung: Silent-Install
...
** Freigabe von opsi-Feature: Dynamisches Depot
** opsi Erweiterung: Install-on-shutdown
** opsi Erweiterung: Silent-Install
** Verbessertes Customizing
** Anpassungen für Windows 8 Support
** Überarbeitetes Sessionhandling: Bessere Lokalisierung der Benutzerkonten.
...
** Funktion um Datenpartition bei zu behalten.
** UEFI Support
...
fernzuhalten. Diese werden dort durch '***(secret)***' ersetzt. +
Wir der Loglevel auf '9' gesetzt, so werden die 'secrets' im Klartext gelogt. +
...
comment "should be in the log file: This is a ***(secret)*** string"
----
...
opsi-winst (4.11.3.5) stable; urgency=low
  ...
  * new command: 'setSecret <secret string>' ; adds the given string to a list of strings which are replaced with *** while logging 
