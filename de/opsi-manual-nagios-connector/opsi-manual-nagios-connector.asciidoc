...
Schritt 2: Legen Sie die Datei `check_opsiwebservice.php` unterhalb von `/usr/share/pnp4nagios/templates` ab. Diese Datei können Sie über svn.opsi.org auschecken.
...
Wichtig bei diesen Anpassungen ist, dass die Namen der Dateien genauso heißen wie das check_command in der command-Definition, welches im Nagios für diesen Check konfiguriert wurde. Stimmen die Bezeichnungen nicht überein, wird statt dieses Template ein Standardtemplate von pnp4nagios verwendet.
...
Bitte auch hier beachten, dass nur die Füllstände der Pfade, die für opsi Relevant beim Check berücksichtigt werden. Dieser Check soll keinen allgemeinen DiskUsage-Check ersetzen.
...
Bei einer Multidepot-Umgebung wird der obige Befehl aber nicht die komplette Umgebung nach diesem Produkt überwachen, sondern nur den Configserver. Wenn man mehrer Depotserver hat, kann man das Depot auch direkt mit angeben:
