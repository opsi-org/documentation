...
Sollten Sie opsi auf einer Betriebssystemsversion einsetzen welche oben nich aufgeführt ist, so empfehlen wir Ihnen eine Betriebsystem Update bevor Sie opsi v 4.0.5 einspielen.
...
Eine ander Möglichkeit ist es in die `/etc/opsi/opsi-product-updater.conf`
