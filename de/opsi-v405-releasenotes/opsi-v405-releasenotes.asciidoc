...
IMPORTANT:  Für folgende Distributionen sind in der ersten Testing Teillieferung die opsi 4.0.5 opsi server Pakete noch nicht fertig: +
Ubuntu: 14.04 (Trusty): 
Alle Pakete ausser der opsi-atftpd sind fertig. Für Tests kann der opsi-atftpd aus 12.04 (Precise) verwendet werden. +
UCS: 3.0, 3.1, 3.2 +
SLES: 11SP1, 11SP3 +
Fedora: 20 +
RHEL: 6 +
CentOS: 6.5 +
Wir bemühen uns diese Pakete möglichs bald bereit zustellen.

Sollten Sie opsi auf einer Betriebssystemsversion einsetzen welches oben nicht aufgeführt ist, so empfehlen wir Ihnen ein Betriebsystem Update bevor Sie opsi v 4.0.5 einspielen.

KNOWN BUGS:
Im testing-Release ist das configed-Applet noch nicht funktionsfähig.

KNOWN PROBLEMS:
Auf XP-Rechnern ist hoher Wahrscheinlichkeit die WebStart-Version nicht funktionsfähig, 
da sie einen Maximalspeicherbedarf von 1000 MB anmeldet, den XP-Rechner, 
selbst wenn der Speicher vorhanden ist, regelmäßig nicht anbieten. Wir empfehlen das opsi-Localboot-Produkt 
opsi-configed zu verwenden.
...
* Neues opsi-bootimage basierend auf Ubuntu Precise / Kernel 3.15.1 mit Unterstützung für aktuelle Hardware (wie z.B Tablets), UEFI Support und Unterstützung für Linux Fesplattenverschlüsselung.
...
Eine andere Möglichkeit ist es in die `/etc/opsi/opsi-product-updater.conf`
...
Der Test auf Client-Erreichbarkeit wurde erweitert, sodass er im Hintergrund ablaufen kann  
...
Wählt man ein Produkt aus, so werden, wie aus der Client-Produktkonfiguration gewohnt, rechts oben die allgemeinen Informationen zum Produkt-Paket angezeigt. Darunter befindet sich 
die Auflistung aller Depots, die das ausgewählte Produkt besitzen. 
