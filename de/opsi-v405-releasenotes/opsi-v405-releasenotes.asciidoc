...
SLES: 11SP1 +
CentOS 7 +
Wir bemühen uns diese Pakete möglichst bald bereit zustellen.

Sollten Sie opsi auf einer Betriebssystemversion einsetzen, welche oben nicht aufgeführt ist, 
so empfehlen wir Ihnen ein Betriebsystem-Update bevor Sie opsi v 4.0.5 einspielen.
...
* opsi-clonezilla: +
Es können Fehler auftreten wenn das Property `drbl_ocs_conf` leer ist. +
Das Handling von Linux Systemen (ext filesystem) funktioniert noch nicht.

* opsiconfd: +
Beim Durchschnittswert der RPC Statistiken ist die Erfolgsrate entweder 100%, wenn alle Anfragen erfolgreich waren, oder 0%, falls einer oder mehrere Fehler auftraten.
...
Version 1.7. Auf dem Server ist für opsi Java allerdings nur erforderlich, wenn Sie den '{opsi-configed}' tatsächlich auf dem Server 
ausführen, was im Normalfall heißt, dass der Server auch über eine graphische Benutzungsoberfläche verfügen muss.
...
Durch den schnelleren Wechsel zwischen der depot-server, entfällt die Notwendigkeit einen kompletten Daten-Reaload durchzuführen, 
