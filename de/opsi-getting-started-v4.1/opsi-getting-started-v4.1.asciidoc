...
Beachten Sie ebenfalls die <<opsi-getting-started-supportmatrix,unterstützten Betriebssysteme>>!
...
In diesem Abschnitt wird davon ausgegangen, dass Sie mit dem Debian-Paketsystem vertraut sind (Informationen zu diesem Thema finden Sie in den einschlägigen Büchern, in den man-pages oder unter  https://www.debiananwenderhandbuch.de/).
...
Machen Sie mit dem Punkt <<opsi-getting-started-installation-config-backend>> weiter.
...
Um vorhandene Windows-Clients in opsi aufzunehmen, muss auf diesen der {opsi-client-agent} installiert werden. Dies kann auf mehrere Arten durchgeführt werden. Nachdem Sie, wie im Folgenden beschrieben, den {opsi-client-agent} installiert haben, erscheint der Client auch in der Client-Liste des {opsi-configed}.
...
Zu den Standard-Produkten gehört das Produkt `opsi-configed` welches das opsi Managementinterface als Anwendung auf einem Rechner installiert.
...
Bei +hwaudit+ und +swaudit+ wird Hard- bzw Softwareinformationen erhoben und zum {opsi-Server} übermittelt.
...
Booten Sie dann den Client (neu). Er sollte jetzt per PXE über das Netz ein Linux-Image ziehen, das die Hardware des PCs scannt und dann den Rechner rebootet (wenn der Rechner nicht ansonsten schon eingerichtet war, kommt im Anschluss korrekterweise die Meldung, dass auf der Platte kein Betriebssystem installiert ist).
...
Das opsi-Handbuch (https://download.uib.de/opsi_stable/doc/opsi-handbuch-stable-de.pdf) enthält eine Fülle von weiteren Informationen, die für den produktiven Betrieb wichtig sind.
...
https://uib.de/de/support-schulung/support/
