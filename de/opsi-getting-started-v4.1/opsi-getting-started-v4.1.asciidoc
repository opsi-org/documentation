...
Als Client-PC eignen sich reale oder virtuelle Rechner mit mindestens 1536 MB RAM, die über eine Netzwerkkarte mit Netzwerkboot-Unterstützung verfügen:
...
Auf der Downloadseite von uib finden Sie unter http://download.uib.de/opsi4.1/ verschiedene iso-Images der opsi-client-boot-cd. Laden Sie sich das neueste herunter und brennen es auf eine CD. Booten Sie den Rechner von der CD. Sie sollten dann folgendes Bild sehen:
...
* Den Inhalt von `C:\winpe\media`  nach `/var/lib/opsi/depot/win81/winpe` bzw. `/var/lib/opsi/depot/win10/winpe` kopieren. +
Rechte anpassen (z.B.):
[source,prompt]
----
opsi-setup --set-rights /var/lib/opsi/depot/win10/winpe
----
