...
* Sie können den {opsi-configed} über das bereitgestellte opsi-Produkt 'opsi-configed' auf ihrem von opsi verwalteten Rechner lokal installieren. +
Das erforderliche Java RE ist in dem Produkt bereits integriert.

* Unter der Adresse `https://<opsidepotserver>:4447` finden Sie den Link zu einer Programmdatei mit welcher sie den 'opsi-configed' lokal installieren können. +
Auch hier ist das erforderliche Java RE bereits integriert.

Loggen Sie sich als ein User ein, der Mitglied der Gruppe {opsiadmin} ist (in der opsi-VM als root, solange Sie keinen anderen User eingerichtet haben).
