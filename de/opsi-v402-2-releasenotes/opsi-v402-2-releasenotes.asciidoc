...
Folgendes einfaches Beispiel soll das Verhalten verdeutlichen: Wenn Sie ein RedHat System haben und bei der Anmeldung am '{opsi-configed}' eine PAM-Fehlermeldung bekommen, obwohl mit den selben Benutzerdaten eine SSH Verbindung zum Server geöffnet werden kann, kann man die Datei `/etc/pam.d/opsi-auth` mit folgendem Inhalt erstellen:
