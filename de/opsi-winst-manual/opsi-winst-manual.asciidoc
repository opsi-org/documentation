...
`encoding=`<encoding> +
Dieser Befehl kann an einer beliebigen Stelle in der Scripdatei stehen.
...
* `RestoreWinst` +
setzt das '{opsi-winst}/opsi-script' Fenster auf 'maximierten' Modus.
...
Seit 4.11.4.2 gibt es für einfache Befehle und besonders (aber nicht nur) für den Einsatz unter Linux folgende 'Abkürzung':
...
Bricht die Scriptabarbeitung ab ohne das ein Erfolg oder Fehler gemeldet werden. Das Produkt bleibt auf Setup.
...
*  `Delete` [-sfd[n]] <Pfad> [W/L] 

oder

*  `Delete` [-sfd[n]] <Datei(maske)> [W/L] 
+
