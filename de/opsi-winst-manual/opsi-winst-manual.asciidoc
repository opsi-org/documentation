...
opsi-script Handbuch (4.12.4.25)
================================
...
Es existieren eine Vielzahl von Optionen welche teilweise nur in Kombination und dann in der richtigen Reihenfolge verwendet werden können (im folgendenfolgende Syntax-Varianten des Aufrufs:
...
Einen String als abfragbaren Parameter übergeben: +
...
Dabei muß auch der Parameter `/productid` angegeben sein.

* `/logproductid` <productId> +
Bei der Erstellung der Logdatei soll <productId> als Quelle des Logs angegeben werden. +
Wird z.B. verwendet bei Scripten welche im Kontext eines Produktes welches mit einem temporären user arbeitet als teil dieses Produktes ausgeführt werden
...
Bearbeite die angegebene `/productlist` so, als würden für diese Produkte die Aktionsanforedung auf dem opsi-server auf 'setup' stehen. +
...
{opsi-script} kann die Protokoll-Informationen über den opsi-web-service an den opsi-server senden.
...
Seit Version 4.11.3.5 sucht der 'opsi-script' nach dem zu verwendenden Skin Verzeichnis in folgender Reihenfolge, wobei das erste Verzeichnis, welches eine skin.ini enthält verwendet wird:
