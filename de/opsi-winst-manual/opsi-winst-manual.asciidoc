...
Nachdem sich Eigenschaften im Feld 'Selected WMI Properties'  befinden kann durch anglicken der Schaltfläche 'Execute' die Anfrage and den WMI Service gestellt werden. In dem Feld 'Condition' kann die Abfrage noch mittels SQL Anweisungen verfeinert werden, z.B. `where` <property> = <value>.
