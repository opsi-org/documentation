...
Achtung: Dadurch können evtl. bestehende Abhänigkeiten nicht aufgelöst werden.
...
Für den Test von WMI Abfragen gibt es eine kleine Hilfsanwendung welche genauso wie diese Funktion arbeitet: +
...
Sucht in der übergebenen <xml2stringlist> das letzte Vorkommen eines node mit dem Namen <node name str> und liefert diesen node (mit allen child nodes) als xml2stringlist zurück. +
...
[source,winst]
----
set $list$= powersshellCall_list('Get-Process -ProcessName "opsi*"')
----
Ist eine Abkürzung für den Ausdruck:
[source,winst]
----
