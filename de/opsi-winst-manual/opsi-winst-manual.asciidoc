...
opsi-script Handbuch (4.12.4.17)
================================
...
[[opsi-script-linux]]
== Betrieb unter Linux oder MacOS
...
`opsi-script` <scriptfile>
      [`/logfile` <logfile> ] +
      [`/batch` | `/histolist` <opsi-scriptconfigfilepath>] +
      [`/productid` <productid> ] +
...
(4) Abarbeitung des Software-Profils über den opsi Service mit der entsprechenden Umsetzung (seit '{opsi-script}' 4.11.2)
