...
opsi-winst / opsi-script Handbuch (4.12.3)
==========================================
...
Pfad (ohne schließenden Backslash), in dem der aktive '{opsi-winst}/opsi-script' liegt. +
Identisch mit veraltet `%WinstDir%`

`%OpsiscriptVersion%`  (since 4.12.3.6) +
Versionsstring des laufenden '{opsi-winst}/opsi-script'. +
...
Erläuterung: +
Der '{opsi-winst}/opsi-script' wartet auf das Ende eines per `dosbatch` gestarteten Prozesses bevor mit der nächsten Zeile des Scriptes forgefahren wird:
...
* Neustart des Client, damit die Einstellungen für den Auto Logon in Kraft treten.
...
** Während des Auto Logons erfolgt die Installation über den lokal installierten opsi-winst.
...
* False (Default)
** Vor der Installation wird nicht geprüft ob die Software auf dem Client bereits vorhanden ist, und diese ggf. deinstalliert.
* True
** Vor der Installation wird geprüft ob die Software auf dem Client bereits vorhanden ist. Falls ja wird diese Version vor der Installation deinstalliert.

===== Aufbau des Produkts
Das Produkt gliedert sich in ein Hauptscript, dass den Auto Logon und die Installation vorbereitet und das eigentliche Installationsscript, dass während des Auto Logons des lokalen Benutzers ausgeführt wird.
...
====== Anpassen der Datei delsub-local.opsiscript
