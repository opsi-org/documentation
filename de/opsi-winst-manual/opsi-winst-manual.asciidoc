...
liefert den String <str> mit dem encoding <to> zurück wobei davon ausgegangen wird das <str> gemäß <from> encoded war. <from> und <to> sind dabei encodings wie sie im Kapitel <<opsi-script-encoding, '{opsi-script}' encoding>> aufgelistet sind. +
...
siehe auch : <<loadUnicodeTextFile>> +
siehe auch : <<loadTextFileWithEncoding>> +
siehe auch : <<opsi-script-encoding, '{opsi-script}' encoding>> +
...
liefert die Stringliste <list> mit dem encoding <to> zurück wobei davon ausgegangen wird das <list> gemäß <from> encoded war. <from> und <to> sind dabei encodings wie sie im Kapitel <<opsi-script-encoding, '{opsi-script}' encoding>> aufgelistet sind.
...
siehe: <<opsi-script-encoding,'{opsi-script}' encoding>>
...
for `DosInAnIcon` (`ShellInAnIcon`),`ExecWith` and `ExecPython` calls. Siehe auch <<getOutStreamFromSection>>

* `getreturnListFromSection(<section with params>)` +
for `XMLPatch` sections and `opsiServiceCall` sections. Siehe auch <<getReturnListFromSection>>
