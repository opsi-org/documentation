...
`<requestedExecutionLevel level="highestAvailable" />`. Dies bedeutet, das unter NT6 als Administrator aufgerufen versucht wird als 'elevated' Prozess zu arbeiten. Wird der '{opsi-winst}' mit User Rechten aufgerufen so läuft er unter den Rechten dieses Users.
...
* Die Option `/allloginscripts` schaltet das Verhalten auf das Abarbeiten von 'userLoginScripts' um. +
Ssiehe hierzu im opsi-manual das Kapitel 'User Profile Management'.
...
*  `getLastExitCode` +
Die String-Funktion getLastExitCode gibt den ExitCode des letzten Prozessaufrufs der vorausgehenden WinBatch / DosBatch Sektion aus.
