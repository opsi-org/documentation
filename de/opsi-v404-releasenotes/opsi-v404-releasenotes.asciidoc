...
Da auch die oben genannten Repositories geschlossen werden, sind die Pakete mit 4.0.4 nicht mehr verfügbar. Für den Fall, dass die letzten freigegebenen Pakete für diese Distributionen noch einmal benötigen werden, haben wir ein Archiv dieser Repositories abgelegt, dieses befindet sich unter:
...
kann aber nicht nur als root sondern auch per sudo von Mitgliedern der Gruppe pcpatch (bzw. opsi-file-admins) aufgerufen werden. +
Beispiel: +
[source,prompt]
----
...
----
* Neue opsi shares: +
`opsi_depot_rw` zeigt (wie opsi_depot) nach `/var/lib/opsi/depot` ist aber beschreibbar. Der share `opt_pcbin` ist damit abgekündigt.`opt_pcbin` wird bei existierenden Installationen nicht geöscht, bei Neuinstallationen jedoch nicht mehr angelegt. +
...
** new command: isSuccess : Terminates Script execution without error
...
Mit diesem Servicerelease wird offiziell die Unterstützung für Python 2.5 eingestellt. Dies betrifft Python 2.5 Distributionen: Debian Lenny und UCS 2.x. Die betroffenen Repositories wurden am 31.12.2013 geschlossen. Die Paketstände zu diesem Termin, werden auf download.uib.de im Archiv veröffentlicht. Für Betroffene opsi-Systeme empfehlen wir den Upgrade auf die Neuen Distributionsversionen, die noch unterstützt werden.
...
Insgesamt befinden sich damit dort jetzt sechs Schaltflächen:
...
image::configed-masterbuttons2.png["{opsi-configed}: Buttonleiste mit den neuen Buttons für Gruppenaktionen und Produktaktionen", width=400]


[[opsi-configed-groupactions]]
=== Gruppenaktionen

Das Fenster "Gruppenaktionen" ist als Startpunkt für gruppenbezogene Funktionen vorgesehen. Im Moment wird nur eine Funktion angeboten, die für die opsi-local-image-Option interessant ist: 
...
image::configed_package-actions.png["{opsi-configed}: Paketaktionen", width=400]
