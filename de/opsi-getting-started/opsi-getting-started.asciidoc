...
= opsi Getting Started opsi-Version 4.0.5
...
CAUTION: Ubuntu 14.04, Codename Trusty Tahr, wird momentan noch nicht offiziell unterstützt, obwohl es dafür bereits Repositories gibt. Es kann hierbei noch zu Problemen kommen, weshalb wir vom Einsatz in einer produktiven Umgegbung abraten.
...
Wenn Sie das opsi Managementinterface '{opsi-configed}' direkt auf dem Server ausführen möchten, so benötigen Sie eine Java Laufzeitumgebung. Die Laufzeitumgebung von Sun/Oracle ist die am besten getestete. Allerdings ist diese unter Linux nur noch eingeschränkt zu beziehen. Als Alternative bietet sich das OpenJDK an. Unseren Erfahrungen nach funktioniert hier zumindest der Start über Webstart und als Applikation.
...
opsi 4.0 ist getestet und freigegeben für UCS UCS 3.0, 3.1 und 3.2

CAUTION: UCS 3.0, UCS 3.1 und UCS 3.2 werden momentan noch nicht offiziell unterstützt, obwohl es dafür bereits Repositories gibt. Es kann hierbei noch zu Problemen kommen, weshalb wir vom Einsatz in einer produktiven Umgegbung abraten.
...
* Die Unterstützung für openSUSE 11.3, 12.1, 12.2 ist abgekündigt.
...
[source,prompt]
----
zypper ar --refresh http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/openSUSE_12.3/home:uibmz:opsi:opsi40.repo
...
[source,prompt]
----
zypper ar --refresh http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/openSUSE_13.1/home:uibmz:opsi:opsi40.repo
zypper mr -p 50 home_uibmz_opsi_opsi40
----
...
* opsi 4.0 ist getestet und freigegeben für SLES11SP1, SLES11SP3.

CAUTION: SLES wird momentan noch nicht offiziell unterstützt, obwohl es dafür bereits Repositories gibt. Es kann hierbei noch zu Problemen kommen, weshalb wir vom Einsatz in einer produktiven Umgegbung abraten.
...
* `python-pyasn1` muss installiert sein
...
[source,prompt]
----
zypper ar --refresh http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/SUSE_SLE-11_SP3_standard/home:uibmz:opsi:opsi40.repo
zypper mr -p 50 home_uibmz_opsi_opsi40
----
...
* Die RHEL Pakete für opsi 4.0 sind gebaut für RHEL 6.
...
* Die CentOS Pakete für opsi 4.0 sind gebaut für CentOS 6.
...
----
[opsi40]
name=opsi4.0 for Fedora $releasever - $basearch
type=rpm-md
baseurl=http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/Fedora_20/
gpgcheck=1
gpgkey=http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/Fedora_20/repodata/repomd.xml.key
enabled=1
----
...
Mit dem `opsi-deploy-client-agent` Skript kann auch eine ganze List von Clients bearbeitet werden. Das Skript findet sich unter '/var/lib/opsi/depot/opsi-client-agent' Führen Sie das Script mit root Rechten aus.
...
`/var/lib/opsi/depot/win2003`
bzw.
`/var/lib/opsi/depot/winxppro`
...
Statt vom {opsi-Server} aus unter Linux können Sie die Dateien auch über das Windows-Netzwerk kopieren. Dafür müssen Sie sich mit der Freigabe 'opsi_depot_rw' auf dem {opsi-Server} als Mitglied der Gruppe 'pcpatch' verbinden. Das entsprechende Verzeichnis befindet sich auf der Freigabe in `winxppro` bzw. `win2003`.
...
Die Treiber-Integration verläuft analog zu dem Verfahren bei NT5: Die Treiber werden unter `/var/lib/opsi/depot/win7/drivers/drivers` abgelegt.
