...
= opsi Getting Started opsi-Version 4.0.6
...
Sofern Sie bereits über einen Gastrechner verfügen, auf dem die VMware-Vollversion oder ein VMware-Player installiert ist, erledigen Sie die Grundinstallation des 	 mit wenigen Mausklicks:
...
Öffnen Sie "Open a Virtual Maschine", suchen Sie in dessen Dateiauswahldialog das Verzeichnis `opsidemo` und darin die Datei `opsidemo.ovf`. Gegebenenfalls müssen Sie ganz unten noch die anzuzeigenden Dateitypen auf ovf ändern.
...
Installieren Sie die minimalen opsi-Produkte durch Doppleklick auf das Icon 'opsi-product-update FirstRun'. Bitte geben Sie das Passwort für den adminuser ein.
Hierdurch werden automatisiert die aktuellen opsi-Pakete von http://download.uib.de/opsi4.0/products/ aus den Verzeichnissen netboot/ und localboot/ geholt und auf dem Server installiert.
...
NOTE:: Unter Ubuntu 10.04 (Lucid) muß statt `cifs-utils` das Paket `smbfs` installiert sein.
...
Wenn Sie das opsi Managementinterface '{opsi-configed}' direkt auf dem Server ausführen möchten, so benötigen Sie eine Java 7 Laufzeitumgebung.
...
Es sieht nach bisherigen Tests so aus, das diese Probleme nicht
auftauchen, wenn auf dem Server der winbindd nicht läuft.

Winbindd start disablen mit:
[source,prompt]
----
...
Die Installation von opsi mit dem file oder mysql Backend ist möglich auf den Rollen Master, Backup, Slave, nicht aber auf Member.
...
univention-install install mysql-server
----
...
[source,prompt]
----
zypper ar --refresh http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/SLE_11_SP3/home:uibmz:opsi:opsi40.repo
zypper mr -p 50 home_uibmz_opsi_opsi40
----
...
* Die CentOS Pakete für opsi 4.0 sind gebaut für CentOS 6 & 7.
...
== Integration vorhandener Windows-Clients in opsi. (Installation des opsi-client-agent)
...
Für einige Distributionen können Sie unter http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40-testing/ eine aktuellere Version finden.
Mehr Informationen entnehmen Sie bitte der Projekteseite unter
...
Das Kopieren der Installationsdateien kann auf zwei Wege geschehen.
...
optional arguments:
  ...
  --use-hostname        Use hostname to connect to client. (Default)
...
==== Verwendung von opsi Standard Produkten
...
==== Verwendung von opsi Produkten von freien opsi Repositories

===== Herunterladen und auf dem Server installieren
...
Installieren Sie das ADK (nach Möglichkeit auf einer 64Bit- Maschine) in den vorgeschlagenen Pfad. wählen Sie nur die "Windows-Vorinstallationsumgebung (Windows PE)" aus; Abhängigkeiten werden automatisch ausgewählt.
...
Version vom 06.08.2009 in deutscher Sprache. Sie können das ISO brennen oder z.B. mit Deamon Tools mounten und dann installieren.


[[opsi-getting-started-firststeps-osinstall-fill-base-packages-nt6-pe]]
==== Erstellen eines PE

Die einfachste Methode setzt voraus, dass Sie bereits einen Windows - Computer haben, auf dem sowohl der opsi-client-agent installiert ist, als auch das Windows ADK (Win8.1,Win10) . Der manuelle Weg wird anschließend beschrieben.
...
* Führen Sie nun zuletzt den Kommandozeilen - Befehl  auf dem neuen opsi-Server aus (identisch zur manuellen Erstellung). Fertig.
[source,prompt]
----
opsi-setup --set-rights
----

Nun folgt die Beschreibung der manuellen Anpassung (zB auch für ein Win7 PE). Die Befehle für 32- und 64-Bit sind nahezu identisch, nur müssen die Einträge *<ARCH>* mit entweder *x86* , *amd64* oder *ia64* ersetzt werden.
...
Führen Sie  > Start > Programme > Windows Kits > Windows ADK > "Umgebung für Bereitstellungs- und Imageerstellungstools" aus; eine Eingabe-Aufforderung erscheint (jedoch sind in der Folge benötigte Umgebungsvariablen gesetzt)
...
werden gegenüber den Treibern in `./drivers/drivers/` bevorzugt anhand der PCI-Kennungen (bzw. USB- oder HD_Audio-Kennung) in der Beschreibungsdatei des Treibers als zur Harware passend erkannt und in das Windows Setup mit eingebunden. +
...
CAUTION: Unter SLES ist dieses Verzeichnis unter `/var/lib/opsi/workbench`.
