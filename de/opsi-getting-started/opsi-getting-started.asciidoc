...
* Samba muss konfiguriert sein. Für den Einsatz auf einem Server in der Rolle Member muss *univention-samba* (und nicht *univention-samba4*) zum Einsatz kommen.
...
* Soll die Installation auf der Rolle Member durchgeführt werden, so sollte der Befehl `ucr get ldap/server/type` als Ergebnis `memberserver` ausgeben, wenn der Join automatisch stattfinden soll.
Es ist möglich dies mit folgendem Befehl zu setzen: `ucr set ldap/server/type='memberserver'`
...
===== Manuelle opsi-Installation auf einem UCS-Server in der Rolle Member

Die Installation von opsi auf einem Server in der Rolle Member kann aktuell nicht über das Univention App Center vorgenommen werden.

Nach einer Installation sollte sicher gestellt werden, dass der für den Depotzugriff verwendete Benutzer mitsamt Domäne aufgeführt ist.
...
WARNING: Durch die eingeschränkten AD-Zugriffsrechte des Member-Servers muss die Synchronisation des Kennworts auf allen von opsi verwendeten Servern manuell vorgenommen werden.

Außerdem muss sichergestellt sein, dass das Kennwort für den User pcpatch auf allen Servern, auf welchen der Benutzer existiert, gleich gesetzt ist.
