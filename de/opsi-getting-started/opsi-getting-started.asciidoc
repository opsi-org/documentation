...
* Der 'opsi-package-updater' wird aufgerufen zum Einspielen der minimalen opsi-Produkte. Nach dem Download werden die Pakete auf dem Server installiert.
...
'pcpatch' angelegt. +
Da dieser Umstand nicht nur im Installationshandling ein Problem darstellt, sondern auch Probleme bei möglichen Migrationen (besonders von Samba3 auf Samba4) verursachen kann, wird mit seit opsi 4.0.5 die Gruppe 'pcpatch' als 'opsifileadmins' angelegt.
Dabei ist es egal welche Kombination von UCS 3.2/4.0 und Samba3/Samba4 bei der Installation vorgefunden wird.
...
Sie können das interaktiv für jedes einzelne Paket tun. Wir empfehlen Ihnen dies automatisiert zu tun. Hierzu gibt es das Werkzeug `opsi-package-updater`, welches wie in '/etc/opsi/opsi-package-updater.conf' konfiguriert, automatisch die aktuellen Pakete vom opsi Repository holt und auf dem Server installiert.

[source,prompt]
----
opsi-package-updater -i -vv
----

Sollte der `opsi-package-updater` Befehl scheitern, so muss evtl. ein Proxy in der Konfigurationsdatei eingetragen werden unter
[source,configfile]
----
[repository_uib]
proxy =
...
* (sofern das opsi-Produkt `opsi-winpe` nicht vorhanden ist, installieren Sie es auf dem opsi - Server mittels `opsi-package-updater -i -p opsi-winpe -vv` )
...
Usage: opsi-makepackage [-h] [-v|-q] [-F format] [-l log-level] [-i|-c custom name] [-I required version] [-t temp dir] [source directory]
...
Eine solche Datei wird mit dem Schalter `--md5` erstellt.
...
Eine solche Datei wird mit dem Schalter `--zsync` erstellt.
