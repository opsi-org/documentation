...
Das Setzen des Kennworts für den Benutzer `pcpatch` über `opsi-admin` scheitert auf Grund der fehlenden Schreibberechtigungen eines Member-Servers.
Daher muss die Änderung des Kennworts des technischen Benutzers `pcpatch` auf einem Server mit Schreibrechten im AD vorgenommen werden - ein Master, Backup oder Slave.
