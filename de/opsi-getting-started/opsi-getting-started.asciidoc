...
Diese beinhalten unter anderem den Agent, welche für ein Management auf Clients installiert werden muss.
...
Muss für den Zugriff auf das Internet die Verbindung über einen Proxy geleitet werden, so muss dieser in den `.repo`-Konfigurationsdatein unter `/etc/opsi/package-updater.repos.d/` als Wert für `proxy` eingetragen werden.
...
Die Pakete finden Sie unter http://download.uib.de/opsi4.0/products/ in den Verzeichnissen `netboot/`, `localboot/`, für Linux-Clients zusätzlich unter `opsi-linux/`.

Wir empfehlen die .opsi-Dateien unter `/var/lib/opsi/repository` zu speichern.
Zum Sicherstellen, dass opsiconfd auf die Dateien zugreifen kann, sollte `opsi-setup --set-rights /var/lib/opsi/repository` ausgeführt werden.
...
* (sofern das opsi-Produkt `opsi-winpe` nicht vorhanden ist, installieren Sie es auf dem opsi - Server mittels `opsi-package-updater -vv install opsi-winpe` )
