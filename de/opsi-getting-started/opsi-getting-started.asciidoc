...
* Der Befehl `hostname -f` muss einen fully qualified domainname zurück liefern, in dem mindestens zwei Punkte vorkommen, z.B. 'opsiserver.domain.local'.
...
TIP: Wenn die `servervm` verwendet wurde und der {opsi-configed} über den Browser auf dieser VM aufrufbar sein soll, muss man noch zusätzlich das java-plugin installieren: `aptitude install sun-java6-plugin`.
