...
Um diese Betriebssysteme installieren zu können, muss einmalig ein sogenanntes WinPE als "DOS-Ersatz" erstellt werden.
(für 32-Bit Installationen ein 32-Bit PE, bzw. für 64-Bit ein 64-Bit PE) 
Wir empfehlen die Verwendung des PE entsprechend der Betriebssystem - Version (Win 7/8/10)
...
Hierzu benötigen Sie das Windows Automated Installation Kit (Windows AIK; bis Windows 7), bzw dessen Nachfolger "Assessment and Deployment Kit" (ADK), und ein, in der Liste für unterstützte Betriebssysteme aufgelistetes, Windows (die Architektur ist hierbei anscheinend nicht wichtig, beide PEs wurden unter einem 64-Bit Windows 7 erfolgreich erstellt): +
