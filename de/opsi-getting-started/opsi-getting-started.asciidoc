...
opsi4ucs, opsi-tftpd, p7zip-full, cabextract, mysql-server.
...
* Ist der opsi-tftpd so konfiguriert, dass er nicht wie sonst das Verzeichnis `/tftpboot` als Basisverzeichnis nutzt, sondern `/var/lib/univention-client-boot`. Alle wichtigen Dateien vom {opsi-linux-bootimage} werden deshalb vom /tftpboot in das Basisverzeichnis verlinkt. Dies hat den Nebeneffekt, dass die DHCP-Option: Filename `pxelinux.0` statt `linux/pxelinux.0` lauten muss.
...
Bei den Abfragen können außer beim 'Database Admin Password' alle Vorgaben mit Enter bestätigt werden. Das 'Database Admin Password' ist auf der opsi-VM 'linux123' ansonsten das, was Sie bei der Installation des Datenbankservers vergeben haben.
