...
Erlaubt sind z.B.: 'uib.local', 'uib.de', 'subdomain.domain.de'. +
...
Wenn Sie MySQL als Backend z.B. für die Inventarisierungsdaten und Lizenzmanagement verwenden möchten, benötigen Sie noch MySQL-Server:
...
Wenn Sie das opsi Managementinterface '{opsi-configed}' direkt auf dem Server ausführen möchten, so benötigen Sie die Java Laufzeitumgebung von Sun/Oracle. Um diese zu installieren gehen Sie bitte wie folgt vor:
...
.Ubuntu:
Fügen Sie in der `/etc/apt/sources.list` den Repository den branch +partner+ hinzu.
...
----
Wichtig: Setzten Sie 'lucid' ein so muss da natürlich statt 'maverick' 'lucid' stehen.
...
* Der Befehl `hostname -f` muss einen fully qualified domainname zurück liefern, in dem mindestens zwei Punkte vorkommen, z.B. 'opsiserver.uib.local'.

* Der Befehl `getent hosts $(hostname -f)` muss die IP-Adresse der Netzwerkschnittstelle zurück liefern, zu der sich die Clients verbinden sollen. Gibt der Befehl die Adresse '127.0.0.1' oder '127.0.0.2' aus, muss die Datei '/etc/hosts' korrigiert werden.
...
* ldap (Datenhaltung in der LDAP Datenbank)
...
Wenn Sie das {mysql-Backend} verwenden wollen (z.B. für die Inventarisierung), so muss dieses nun mit dem Befehl 
[source,prompt]
----
...
Zur Verwaltung des {opsi-Server}s und der angeschlossenen Clients dient das Programm {opsi-configed}. Dieses Programm ist in Java geschrieben und benötigt mindestens Java Version 6 bzw. entsprechend der alten Zählung Version 1.6. Verwenden Sie das original SUN Java, da bei anderen Java Implementationen Probleme beobachtet wurden.
...
(passen Sie gegebenfalls das angegebene Produkt an):
[source,prompt]
----
...
Auslesen und wegschreiben (passen Sie ggf. das angegebene Produkt an):
[source,prompt]
----
opsi-admin -d method productProperty_getObjects [] '{"productId":"winxppro"}' > /tmp/property_config.json
----

Editieren Sie die entstandene Datei `/tmp/property_config.json` und ändern dabei die Einträge für '"defaultValues"' und '"possibleValues"' der gewünschten Properties. Die veränderte Datei lesen Sie wieder ein mit dem Befehl:
...
Kontrollieren Sie das Ergebnis mit einem Aufruf von (passen Sie ggf. das angegebene Produkt an):
[source,prompt]
----
opsi-admin -d method getProductProperties_hash winxppro
----
...
Starten Sie `winst32.exe` per Doppelklick. Wenn der `opsi-client-agent` bereits auf Ihrem Rechner installiert ist, finden Sie `winst32.exe`
...
image::../images/winst-interaktiv.png[Screenshot: opsi-Winst im interaktiven Modus]
