...
** Produkte auf dieser Basis gibt es für: SLES12, Ubuntu14-04, Debian7, Debian8, CentOS70, Redhat70
...
* Security Upgrade zu Windows SSL Libraries 1.0.2c (libeay32.dll, ssleay32.dll) +
...
Im Rahmen der Verwendung von Samba4 kann es notwendig sein zum mounten des depotshares explizit anzugeben mit welche Domain / User Kombination dies erfolgen soll. Gibt es diesen config nicht, so wird der user `pcpatch` genommen.
Der config hat den Namen: `clientconfig.depot.user`
