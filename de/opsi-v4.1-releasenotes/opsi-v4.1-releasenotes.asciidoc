...
Das Ersetzen von `opsi-product-updater` durch `opsi-package-updater` wird begleited durch eine neue Konfigurationsdatei.
Die <<opsi-releasenotes-4.1-changes-opsi-package-updater-configfile,Übernahme der Konfiguration>> wird später in diesem Dokument beschrieben.
...
TIP: Das Vorhandensein von opsi 4.0- und opsi 4.1-Repositories auf der gleichen Maschine ist in der Regel kein Problem. Es ist dennoch trotzdem empfehlenswert gleich bei der Migration das alte Repository zu deaktivieren.
...
IMPORTANT: Falls Sie Anpassungen an der Konfigrationsdatei des `opsi-product-updater` gemacht haben, bspw. bei der Verwendung zusätzlicher Repositories oder bei der Synchronisation von anderen opsi-Servern, so sollten Sie die Einstellungen  <<opsi-releasenotes-4.1-changes-opsi-package-updater-configfile,migrieren>>, bevor Sie die Pakete aktualisieren.
...
* `/etc/opsi/opsi-product-updater.conf` - beachten Sie hierzu bitte die Hinweise zur <<opsi-releasenotes-4.1-changes-opsi-package-updater-configfile,Migration der Konfigurationsdatei>>, bevor Sie diese Entfernen.
...
Falls Sie Anpassungen in `/etc/opsi/opsi-product-updater.conf` vorgenommen haben, wird empfohlen die Änderungen in `/etc/opsi/opsi-package-updater.conf` zu übernehmen.
Für die Repository-Sektionen wird empfohlen diese aus der Konfigurationsdatei zu entfernen und in einzelne Konfigurationsdateien mit der Dateiendung `.repo` in `/etc/opsi/package-updater.repos.d` zu übernehmen.

Es gibt mit dem Werkzeug `opsi-migrate-product-updater-configuration` die
Möglichkeit automatisiert eine Konfigurationsdatei automatisiert in das
neue Format zu migrieren.
Der Aufruf von `opsi-migrate-product-updater-configuration --help` liefert weitere Informationen dazu.
Es kann sein, dass im Anschluss ein Repository in mehreren Dateien eingetragen ist (einmal als Resultat einer Migration und einmal durch das Paket `opsi-utils`), weshalb anschließend eine Kontrolle der Repository-Dateien erfolgen sollte.
...
CAUTION: Die alte Konfigurationsdatei enthält opsi 4.0-Repositories. Die Weiterverwendung dieser Konfiguration wird dazu führen, dass nicht die neuesten Pakete verwendet werden.
...
die Festplatte vorbereitet und ein Win-PE Partition erstellt. Diese wird gebootet um die eigentliche Windowdsinstalltion zu starten. +
