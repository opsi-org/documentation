...
----
Die Veränderungen an den Konfigurationsdateien `dispatch.conf`, `hostcontrol.conf` und `opsiconfd.conf` müssen Sie überprüfen.
Auf den geänderten Namen der ursprünglichen `opsi-product-updater.conf` wird beim Aufruf des Migrationssriptes eingegangen.
In der Regel sollten Sie die ursprüngliche dispatch.conf.default weiter verwenden, die Sie mit dem folgenden Aufruf zurück kopieren können:
[source,prompt]
----
cp /etc/opsi/backendManager/dispatch.conf.default.rpmsave /etc/opsi/backendManager/dispatch.conf.default
----
