...
* python-opsi 4.1.1.x
* opsi-linux-bootimage 20171018-11
* opsi-configed 4.1.x
...
Auf opsi 4.1 unterstützten Betriebsystemen ist es möglich eine bestehende opsi 4.0 Installation auf opsi 4.1 zu migrieren. Hierfür sind mehrere Schritte nötig.

Als erstes müssen die opsi 4.1 Repositories der Server-Pakete in den Paketquellen. Im opsi 4.1 Release wird der 'opsi-atftpd' durch den neuen 'opsi-tftpd-hpa' ersetzt. Hierbei ist zu beachten das eine bestehende Konfiguration der Datei '/etc/inetd.conf' bzw. '/etc/default/atftpd' rückgängig gemacht werden muss. Bitte löschen Sie in der entsprechenden Datei die Zeile beginnend mit 'tftp', welche die Parameter für den 'opsi-atftpd' bereitstellt. 

Auf Debian basierten Systemen aktualisieren Sie auf opsi 4.1 wie folgtt.:
[source,prompt]
----
...
RedHat und Centos Systeme sind wie folgt zu aktualisieren:
[source,prompt]
----
...
SUSE und OpenSUSE benötigen die folgenden Schritte für eine Migration
[source,prompt]
----
