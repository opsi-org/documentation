...
Für Debian-basierte Maschinen existiert eine Transition-Paket, welches allerdings durch die geänderten Abhängigkeiten im Rahmen eines `dist-upgrade` nicht automatisch aktualisiert wird und manuell aktualisiert werden muss.

Das ist möglich mit den folgenden Schritten:

[source,prompt]
----
apt-get install opsi-depotserver opsi-server
