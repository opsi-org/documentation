...
TIP: Entfernen Sie anschliessend ggfs noch vorhandene Verweise auf opsi 4.0-Repositories

----
Debian/Ubuntu/UCS : etwa in Datei /etc/apt/sources.list
----

oder je nach Distribution deaktivieren Sie vorhandene opsi 4.0 Repositories im Verzeichnis
...
* Unter _UCS_ wird die Datei `/etc/pam.d/opsi-auth` nicht aus dem vorhandenen Template erstellt. Ein Workaround ist diese manuell anzulegen.
