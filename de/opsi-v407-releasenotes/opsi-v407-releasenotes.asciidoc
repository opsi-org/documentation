...
=== opsi-package-updater Verhalten

Bisher wurden die Windows Netboot-Produkte nur aktualisiert, wenn man den opsi-package-updater mit der Option *-i* aufgerufen hat. Dieses Verhalten wurde nun geändert.
...
Version der Datei `/etc/opsi/opsi-package-updater.conf` eingespielt werden kann und der +
...
active = false
----
Danach rufen sie den opsi-package-updater auf.
...
Wir empfehlen in der Datei `/etc/opsi/opsi-package-updater.conf`
...
Danach die opsi-Produkte. Dies erledigt der opsi-package-updater:
