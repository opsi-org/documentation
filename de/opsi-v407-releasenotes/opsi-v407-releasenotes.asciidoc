...
** Bereitstellung von lokalen http Repositories für die Linuxinstallation (Verzeichnis `/var/www/html/opsi/`)
...
Bisher wurden die Windows Netboot-Produkte nur aktualisiert, wenn man den opsi-product-updater mit der Option -p aufgerufen hat. Dieses Verhalten wurde nun geändert. Die Windows Netboot-Produkte werden nicht mehr bei einem opsi Produktupdate ausgeschlossen. Diese Produkte werden wie die Localboot-Produkte upgedatet. Ein ensprechender Hinweis bei der Paketinstallation weist auf eine geänderte Konfigurationsdatei hin. Sie müssen danach ihre Zugangsdaten in der Konfogurationsdatei erneuet eintragen.
...
In dieses Verzeischnis kopieren Sie nun den Inhalt einer Ubuntu16.04 CD oder Debian8 DVD.
