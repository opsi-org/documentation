...
Das Anlegen eines neuen opsi Backups erfolgt mit dem Befehl `opsi-backup create`. Wird dieser Befehl ohne weitere Parameter angegeben erstellt das Programm ein Archiv mit allen Daten der Backends an. Der Dateiname wird dabei automatisch generiert.
[source, prompt]
----
opsi-backup create
----
...
* +--backends {file,mysql,all}+ +
Ermöglicht die Steuerung der Backends, die in dem Backup eingeschlossen werden sollen. Diese Option kann mehrfach angegeben werden, um mehrere Backends anzugeben. Die Option `--backends=all` steht für alle Backends und ist die Standardeinstellung.
Im Moment werden folgende Backends unterstützt: +mysql+, +file+
[source, prompt]
----
...
* +--configuration+ +
Schließt die <<opsi-backup_config-part,Opsi Konfiguration>> in das Backup mit ein. Es bietet sich an, diese Daten mit zu sichern, da das Zurückspielen eines Backends eine funktionierende Konfiguration vorraussetzt.
[source, prompt]
----
opsi-backup create --configuration
----

* +--dhcp+ +
Schließt die DHCP Konfiguration in das Backup mit ein. Diese Option ist sinnvoll, wenn der DHCP Dienst auf dem opsi-Server eingerichtet ist.
[source, prompt]
----
opsi-backup create --dhcp
----
...
[source, prompt]
----
opsi-backup create --dhcp --configuration --backends=all opsi_backup.tar.bz2
----
...
* +--dhcp+ +
Stellt die DHCP Konfiguration wieder her.
[source, prompt]
----
opsi-backup restore --dhcp opsi_backup.tar.bz2
----
