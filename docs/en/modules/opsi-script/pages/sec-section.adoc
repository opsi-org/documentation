...
** `c` -> continue +
If a system file is in use, then it can be deleted only after a reboot. The _{opsi-script}_ default behaviour is therefore that a file in use will be marked for delete after the next reboot, AND the _{opsi-script}_ reboot flag is set. Setting the copy option `-c` turns the automatic reboot off. Instead normal processing continues, the deleting will be completed only when a reboot is otherwise triggered.
