...
In order for opsi to be able to manage computers, these clients must be known to the system. In addition, an xref:clients:windows-client/windows-client-agent.adoc[client-agent] must be running on the computers so that communication between server and client is possible. Depending on how your environment is set up, there are different procedures for adding clients:
