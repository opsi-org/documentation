...
//Link anpassen, wenn 4.3 stable ist
Den Client-Agent für Windows gibt es ebenfalls als MSI-Paket (Microsoft Installer). Wir stellen es in unserem öffentlichen https://tools.43.opsi.org/{release}/opsi-client-agent.msi[Repository] zur Verfügung. Es bietet dieselben Parameter wie das Programm `oca-installation-helper[.exe]` 
(siehe Abschnitt xref:windows-client/windows-client-agent.adoc#opsi-manual-clientagent-subsequent-installation-oca-installation-helper[]), die Sie über das Property `INSTALL_PARAMS` übergeben:
