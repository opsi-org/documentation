...
With the updates for macOS officially released by Apple in December 2023, it is necessary to grant full disk access to the `opsiclientd` process for the normal operation of the `opsi-mac-client-agent`. This setting can be adjusted in the macOS System Preferences under _Privacy & Security_ or through an Apple MDM message of the type `PrivacyPreferencesPolicyControl`. This step is not necessary for operation in WAN mode.
