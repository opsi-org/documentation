...
CAUTION: The share must be mounted by the SMB protocol (cifs), as WebDav is sometimes buggy under macOS in connection with signed apps and seems to be no longer maintained.

[[opsi-macclient-hints-fulldiskaccess]]
== Granting Full Disk Access

Following the macOS updates officially released by Apple in December 2023, it has become necessary to grant full disk access to the opsiclientd process to facilitate normal functioning of the `opsi-mac-client-agent`. This adjustment can be made in the macOS system settings under _Privacy & Security_ or through an Apple MDM message using the type `PrivacyPreferencesPolicyControl`. 
