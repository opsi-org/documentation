...
With the updates officially released by Apple in December 2023 for MacOS, it is necessary for the normal operation of the `opsi-mac-client-agent`,
to allow full disk access for the `opsiclientd` process. This setting can be set in the MacOS settings under `Privacy and Security`
or through an apple-MDM message using a `PrivacyPreferencesPolicyControl`-Payload.
This step is not necessary for operation in WAN mode.
