...
Some distributions use selinux also on the client side.
For these, we have to install a specific selinux config to enable regular operations.
During `opsi-linux-client-agent` installation, we search for the `semodule` command and install this selinux config if present.
