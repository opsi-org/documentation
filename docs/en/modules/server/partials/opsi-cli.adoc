...
The `package` plugin for `opsi-cli` provides a set of commands to manage opsi packages. This includes creating, listing, extracting, installing, and uninstalling packages.
...
.*make*
   ...
   * *Options*:
      ...
      ** `--custom-name`: Custom name for directories.
      ** `--custom-only`: Use only custom directories.
      ** `--md5/--no-md5`: Create md5 checksum for the package.
      ** `--zsync/--no-zsync`: Create zsync file for the package.
   * *Example*:

[source,console]
----
opsi-cli package make --overwrite --md5 --zsync ./source_dir ./destination_dir
----

[[server-components-opsi-cli-commands-package-list]]
.*list*
   * *Description*: List opsi packages.
   * *Usage*: `opsi-cli package list [OPTIONS] [PRODUCT_IDS]`
   * *Options*:
      ** `--depots`: Depot IDs (comma-separated) or 'all'.
   * *Example*:

[source,console]
----
opsi-cli package list --depots all
----

[[server-components-opsi-cli-commands-package-control-to-toml]]
.*control_to_toml*
   * *Description*: Generate TOML from control file.
...
.*extract*
   ...
   * *Options*:
      ...
      ** `-o, --overwrite`: Overwrite destination directory if it exists.
   * *Example*:

[source,console]
----
opsi-cli package extract --overwrite ./package_archive.tar.gz ./destination_dir
----
...
.*install*
   ...
   * *Options*:
      ** `--depots`: Depot IDs (comma-separated) or 'all'. Default is configserver.
      ** `--update-properties`: Trigger an interactive prompt to update Product property default values.
      ** `--force`: Force installation.
   * *Example*:

[source,console]
----
opsi-cli package install --depots all --force package1 package2
----
...
.*uninstall*
   ...
   * *Options*:
      ...
      ** `--keep-files`: Keep files on uninstallation.
   * *Example*:

[source,console]
----
opsi-cli package uninstall --depots all --force product1 product2
----
