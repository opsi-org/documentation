...
To list the packages available in the repositories (this ignores configured includes/excludes):
...
To display the available updates (this takes into account configured includes/excludes):
...
* `excludes`: Regular expression specifying package names that should not be handled, for example, ^test.* will exclude anything that starts with "test".
...
image::server:opsi-package-updater.png["Install the standard products on your opsi-server.", width=800, pdfwidth=80%]
