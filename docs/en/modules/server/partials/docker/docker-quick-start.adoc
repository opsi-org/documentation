...
IMPORTANT: There is now a file `docker-compose.yml` in the `opsi-server` folder. Open this file with a text editor and look for the environment variable `OPSI_ADMIN_PASSWORD`. You will need the specified password for logging into the OPSI server as the `adminuser` user in subsequent steps.
