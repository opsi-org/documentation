...
However, if you encounter an error message similar to the one shown in <<oqiFailed>>, you can refer to the log files for details on potential issues. These logs are located in two files: `/var/log/opsi-quick-install-l-opsi-server.log` and `/tmp/opsi_quickinstall.log`.

[[oqiFailed]]
.If the installation fails, please check the log files for error messages.
image::oqiFailed.png["If the installation fails, please check the log files for error messages.", pdfwidth=80%]
...
However, if you encounter an error message (`failed`), you can refer to the log files for details on potential issues. These logs are located in two files: `/var/log/opsi-quick-install-l-opsi-server.log` and `/tmp/opsi_quickinstall.log`.

.The setup program provides information about errors and log files.
image::quickinstallNoGuiFailed.png["The setup program provides information about errors and log files.", width=800, pdfwidth=80%]
