...
The file redis-server.te is created for this purpose. This file describes the exceptions.
...
Next, a policy moudle file is created with the following commands:
...
The module can then be activated with `semodule -i redis-server.pp`.