...
In this case, the opsi-Depotserver acts as a boot server -- opsi therefore enables the execution of automated processes independently of an operating system installed on the client.
...
== DHCP on the opsi-Depotserver

An opsi-Depotserver can serve as a DHCP server.
...
opsi can configure the DHCP server automatically if it is installed on the opsi server. In this case, opsi also creates and updates client entries. All you have to do is enter the MAC address and, if necessary, the IP address, for example if you create a client via the management interface *opsi-configed* (see chapter xref:configed:configed.adoc[Management interface *opsi-configed*]).
...
* *Boot server (DHCP option 66)*: Enter the IP address of your opsi-Depotserver for the boot server.
