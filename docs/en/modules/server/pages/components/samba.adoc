...
You can configure this either via the `opsi-configed` management interface (see section xref:configed:configed.adoc#opsi-manual-configed-hostproperties[Host parameters: Client and server configuration]) or on the command line with the `opsi-admin` tool (see section xref:server:components/commandline.adoc#server-components-opsi-admin[*opsi-admin*]):
