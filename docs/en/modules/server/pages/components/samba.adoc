...
By default, the opsi clients use the user `pcpatch` to access the `opsi_depot` share (see chapter xref:server:components/authorization.adoc[Authorizations]). To mount the depot shares, enter the domain and username. To do this, use the host parameter `clientconfig.depot.user`:
