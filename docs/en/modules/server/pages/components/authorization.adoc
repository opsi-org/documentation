...
saml-idp-slo-url:: The URL of the single log-out (SLO) of the identity provider (IdP).
saml-sp-client-signature:: Activate signatures?
saml-encrypted-assertions:: Activate encrypted assertions?
...
saml-slo:: Activate single log-out (SLO)?

The easiest way to configure this is via the command `opsiconfd setup --configure-saml`.
The command guides you through the configuration and saves the settings in the file `/etc/opsi/opsiconfd.conf`.
...
and ensure that AES_256_CBC is set as the encryption algorithm and RSA1_5 (http://www.w3.org/2001/04/xmlenc#rsa-1_5) is set as the key transport algorithm
...
* Select _SAML_.
* Copy the URL that is displayed under `App Federation Metadata Url`.
* Open a console on the opsi config server.
* Execute the command `opsiconfd setup --configure-saml`.
* At the prompt _Enter SAML IdP XML metadata URL of filename:_ enter the previously copied URL.
* This is followed by the output of the opsiconfd service provider metadata.
* Save this XML data in a file.
* Click on _Upload metadata file_.
* Upload the previously saved file.
* In the following dialog, the _Sign on URL_ must be filled in manually. To do this, copy the value from _Reply URL_ and remove the suffix `/callback/login`.
* Save the SAML configuration.

**Verification certificates**

* Now edit the _Verification certificates_.
* Check the box next to _Require verification certificates_.
* Save the value between the tags _<ds:X509Certificate>_ and _</ds:X509Certificate>_ from the XML data above in a file with the extension `.cer`.
* Import this file using the _Upload certificate_ button.
...
Log in administratively to the Keycloak Admin Console.
Navigate to _Configure_ => _Realm Settings_.
Under _Endpoints_ you will find a link to _SAML 2.0 Identity Provider Metadata_.
Copy this URL.

Open a terminal on the opsi-Configserver.
Execute the command `opsiconfd setup --configure-saml`.

At the prompt _Enter SAML IdP XML metadata URL of filename:_ enter the URL from the Keycloak admin console.

The opsiconfd service provider metadata is then output.
Save this XML data in a file.

In the Keycloak admin console, navigate to _Manage_ => _Clients_ and click on _Import Client_.
In the following dialog, import the previously saved file as _Resource file_.
Click on _Save_ to save the client configuration.
