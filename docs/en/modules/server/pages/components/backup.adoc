...
Die Backup-Datei können Sie auch mit AES (Advanced Encryption Standard) verschlüsseln. Dazu verwenden Sie den Parameter `--parameter`. Optional schreiben Sie das Kennwort direkt dahinter; lassen Sie es weg, geben Sie es auf Aufforderung ein (und es erscheint anschließend nicht in der Bash-History).

.Ab opsi 4.3 können Sie opsi-Backups verschlüsseln und mit einem Passwort sichern.
image::opsi-backup-pw.png["Ab opsi 4.3 können Sie opsi-Backups verschlüsseln und mit einem Passwort sichern.", pdfwidth=80%]
