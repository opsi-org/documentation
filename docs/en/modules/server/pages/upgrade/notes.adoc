...
In opsi 4.3 it is no longer necessary to differentiate between UEFI and legacy, as the grub is used in each case. 
The UEFI hook is therefore not required for switching. After the first installation, the tick now indicates whether the system was booted via UEFI or Lagacy. The configuration value `clientconfig.uefibootlabel` is decisive for this. 
