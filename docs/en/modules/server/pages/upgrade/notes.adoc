...
The tools `opsi-configed`, `opsi-setup-detector-setup`, the WebGUI, and others are now accessible at https://tools.43.opsi.org/.


== *opsi-configed*

The actions available for products in the context menu (accessed by right-clicking on a product) have undergone revisions. The options _Save and execute_ and _Save and execute (selected products only)_ now take into account the configuration setting `opsiclientd.control_server.process_actions_event`.

.Context Menu of the right Mouse Button
image::server:opsi-configed-actions.png["Context Menu of the right Mouse Button"]
...
.UEFI hook
image::server:opsi-configed-uefi.png["UEFI hook"]