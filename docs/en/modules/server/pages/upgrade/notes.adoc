...
The actions in the product right-click menu have been revised. 
"Save and Execute" and "Save and Execute (selected products only)" now respect the configuration `opsiclientd.control_server.process_actions_even`.

image::opsi-configed-actions.png[opsi-configed actions]

The configuration can have the following values:

- auto: Trigger `timer` event in WAN mode, otherwise `on_demand` (default)
