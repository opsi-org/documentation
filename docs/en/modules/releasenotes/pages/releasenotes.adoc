...
TIP: You can control this behavior via the `verify_server_cert` and `verify_server_cert_by_ca` options in the `opsiclientd.conf` file (see chapter xref:manual:security.adoc#opsi-manual-security-server2client[Security]).
