...
Additionally, you must have access to the opsi-Configserver or the `opsiconfd` service. An internet connection isn't strictly necessary for local operations. Your user account should also be a member of the `opsiadmin` group (refer to chapter xref:server:components/authorization.adoc[Authorizations]).
