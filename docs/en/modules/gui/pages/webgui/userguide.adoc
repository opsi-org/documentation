...
Additionally, you must have access to the OPSI config server or the `opsiconfd` service.
An internet connection isn't strictly necessary for local operations.
Your user account should also be a member of the `opsiadmin` group (refer to chapter xref:server:components/authorization.adoc[Authorizations]).
...
.WebGUI: Login dialog
image::gui:webgui/opsi-webgui_login.png["WebGUI: Login dialog", pdfwidth=80%]
