...
If you operate multiple OPSI depot servers, they appear in the list on the left side. Select for that the tab _Depots_. By default, the depot on the OPSI config server is highlighted, and `opsi-configed` displays the clients belonging to this depot.

.*opsi-configed*: Depot Selection
image::gui:opsi-configed-depotserver-selection.png["opsi-configed: Depot Selection", pdfwidth=80%]
...
You can use the context menu (right-click) to select all depots or depots with an identical product stock.
...
image::gui:opsi-configed-depotconfiguration.png["*opsi-configed*: Depot Configuration Tab", width=800, pdfwidth=80%]
...
image::gui:opsi-configed-product-defaultproperties.png["*opsi-configed*: Default Properties", width=800, pdfwidth=80%]
