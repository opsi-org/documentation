...
image::gui:opsi-configed-login.png["*opsi-configed*: Login Window", pdfwidth=80%]
...
image::gui:opsi-configed-ask-for-certificate.png["*opsi-configed*: Verifying the opsi CA Certificate", pdfwidth=80%]

The program stores certain session information in the local user profile to restore the working environment upon a new login. This primarily includes selected client groups. `opsi-configed` also uses this session information to create a list of recently connected opsi servers, such as production and test servers, with the most recently used server appearing at the top of the list.
