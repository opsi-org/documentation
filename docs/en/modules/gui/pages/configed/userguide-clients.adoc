...
image::gui:opsi-configed-clientselection.png["*opsi-configed*: Context Menu after Right-clicking a Client", width=800, pdfwidth=80%]

At the top, there's a search field where you can enter a string. Right next to it, you decide through a drop-down menu where `opsi-configed` should search: in all columns (meaning all columns that are displayed in the current configuration), by client name, description, status (on/off), date (last seen), and by all other active columns. There are two further options within the search field that influence the search:

* _Case sensitivity_ (Cc): If this button is active, the upper and lower case of the results must match the search string. Otherwise it does not matter.

* _Regular expression_ (.*): If this button is active, the search term is interpreted as a regular expression. A regular expression consists of patterns that are searched for. For details on these patterns, please refer to the Java documentation for `java.util.regex.Pattern`.

When you press the [Enter] key after entering your search expression, `opsi-configed` jumps to the next match (without match there is no selection). Right-click in the search box to display additional options.

.*opsi-configed*: Client Search
image::gui:opsi-configed-clientselection-search.png["*opsi-configed*: Client Search", width=800, pdfwidth=80%]
...
* _On_: The column shows which clients are currently connected. The values are updated automatically via the messagebus.

.*opsi-configed*: Client can be reached.
image::gui:opsi-configed-client-connected.svg["Client connected", width=20]
...
image::gui:opsi-configed-config-host-displayfields.png["*opsi-configed*: Configure the visible Columns in the Client List.", width=800, pdfwidth=80%]

In the column _Session information_, the account name of the currently active user session is displayed. This information can be retrieved for the selected clients via the context menu or via the _Client_ menu item by clicking on _Request session information_. The `opsiconfd` attempts to establish a connection to these clients and collect this information. If no client is selected, the information is retrieved for all clients. By selecting specific clients, you avoid unnecessary waiting for network timeouts if an attempt is made to establish a connection to computers that are switched off.
...
You can select one or several clients in the client list to edit them together. Use the funnel icon to limit the list to your selected clients.

.*opsi-configed*: Use this Icon to only show selected Clients.
image::gui:configed_icons_filter.png["*opsi-configed*: Use this Icon to only show selected Clients.", pdfwidth=80%]
...
image::gui:configed_clientselection.png["*opsi-configed*: Use the Free Client Search to create your own Search Criteria.", pdfwidth=80%]
...
image::gui:configed_clientselection2.png["*opsi-configed*: Saved Search Queries", pdfwidth=80%]
...
image::gui:opsi-configed-failedactions-today.png["*opsi-configed*: Search for Failed Actions.", pdfwidth=80%]
...
NOTE: When a client is selected, all groups to which it belongs are highlighted.

.*opsi-configed*: Tree View
image::gui:opsi-configed-treeview.png["*opsi-configed*: Tree View", pdfwidth=80%]
...
image::gui:opsi-configed-treeview-generate-group.png["*opsi-configed*: Create a new Group.", pdfwidth=80%]
...
image::gui:opsi-configed-treeview-groupname.png["*opsi-configed*: Enter Group Name and Description.", pdfwidth=80%]
...
* Drag clients from one group in the client tree, e.g., from the _CLIENT LIST_ node, to another group.
...
image::gui:opsi-configed-client-contextmenu.png["*opsi-configed*: The *Client* Menu", pdfwidth=80%]
...
The Linux client agent is a xref:opsi-modules:linux.adoc[paid extension]. This means that you need a license file to unlock it. You will receive this file after you have purchased the extension. For evaluation purposes, we're happy to provide you with a temporary license free of charge. Please contact us via mailto:info@uib.de[email].


NOTE: The WAN/VPN extension is a xref:opsi-modules:wan-support.adoc[paid extension]. This means that you need a license file to unlock it. You will receive this file after you have purchased the extension. For evaluation purposes, we're happy to provide you with a temporary license free of charge. Please contact us via mailto:info@uib.de[email].
...
image::gui:opsi-configed-clientinfos.png["*opsi-configed*: Extended Column View for opsi Clients", width=800, pdfwidth=80%]
...
It is also possible to trigger other events configured in the `opsiclientd.conf` file. The available options are determined by the server host parameter `configed.opsiclientd_events`.
...
image::gui:opsi-configed-client-message-editing.png["*opsi-configed*: Send a message to Client(s).", pdfwidth=80%]

Click on the "OK" button to send the message. The client will receive a pop-up window with the message you entered.

.*opsi-configed*: Client message window
image::gui:opsi-configed-client-message-reading.png["*opsi-configed*: Client message window", pdfwidth=80%, width=800]
...
image::gui:opsi-configed-client-remotecontrol-choice.png["*opsi-configed*: Starting Remote Control Software", pdfwidth=80%]
...
You can edit the commands in the server configuration in _configed_ / _remote_control_:

.*opsi-configed*: Configure the Remote Control Commands.
image::gui:opsi-configed-properties-remotecontrol.png["*opsi-configed*: Configure the Remote Control Commands.", pdfwidth=80%]

TIP: If a command has the `.editable` attribute is active, you can enter variations of the command in the displayed command line, including options or passwords.
...
image::gui:opsi-configed-client-new.png["*opsi-configed*: Adding a new Client", pdfwidth=80%]
...
You can also directly specify these options:

* Activation of Install on Shutdown and WAN configuration
* Assignment of the new client to specific groups
...
image::gui:opsi-configed-csv.png["*opsi-configed*: Creating multiple Clients", pdfwidth=80%]
...
image::gui:opsi-configed-client-change-depot.png["*opsi-configed*: Move Clients to another Depot", pdfwidth=80%]
...
image::gui:opsi-configed-localbootproducts.png["*opsi-configed*: List of Localboot Products with detailed View on the right", width=800, pdfwidth=80%]
...
image::gui:opsi-configed-editlist.png["*opsi-configed*: Property Table", pdfwidth=80%]

You can edit the values directly in the table. In this case, for example, you can select from a list of preconfigured values. Depending on the property (if it is editable), you can also enter new values.

Only if a list with multiple values is allowed an extra dialog will open. You can easily select multiple entries by holding down the [Ctrl] key while clicking.
...
the following provision is in place:
...
image::gui:opsi-configed-netbootproducts.png["*opsi-configed*: List of Netboot Products with detailed View on the right", width=800, pdfwidth=80%]
...
NOTE: Some manufacturers use names with special characters, such as `5000/6000/7000`. The following special characters are internally replaced by an underscore (`_`): `< > ? " : | \ / *`
...
image::gui:opsi-configed-logfiles.png["*opsi-configed*: Viewing the Log Files", width=800, pdfwidth=80%]
...
image::gui:opsi-configed-hostparameter_setting.png["*opsi-configed*: Host Parameters (Context Menu of a Client Entry)", width=800, pdfwidth=80%]
...
- There is a context menu for client properties with options to _Set the current default as specific value_ and _Remove the specific value_ (client value is always the current server default).
...
image::gui:opsi-configed-hostparameters.png["*opsi-configed*: Host Parameters in the Client Configuration", pdfwidth=80%]
