...
the so-called 'opsi-Loginblocker' is also installed, which only allows logon after the installation has been completed.
...
=== opsi-Loginblocker
The 'opsi-Loginblocker' is implemented as a 'credential provider filter' ('OpsiLoginBlocker.dll').
This 'credential provider filter' blocks all 'credential providers' until the opsiclientd reports, that all 'Produktaktionen' are finished or, if the opsiclientd is not reachable, until the connection timeout to the opsiclientd is reached (normally 120 seconds).
...
image::eventflowchart.png["simplified work flow of a 'standard event'", pdfwidth=80%, width=1000]
...
Configuration is done by adding the 'Hostparameter' 'working_window' to the event of your choice.
...
See <<opsi-manual-clientagent-configuration-webservice>> for how to add 'Hostparameter' using 'opsi-configed'.
...
WARNING: For Clients that work in WAN/VPN-mode you must set this options in sync-event and also in the cacheservice-section, because the cache service have no access to the configuration of main sync-event.
...
WARNING: This configuration file is UTF-8 encoded. +
...
WARNING: This feature is inevitably associated with a security issue. During this process, the password is written to the hard disk as plain text and is therefore a potential weak point.
...
To set the 'Hostparameter' via 'opsi-admin', the following command has to be executed on the 'opsi-Configserver'
...
To prevent a user from logging into the system before the installation is complete, the opsi-Loginblocker can be installed.
This will not allow access to the login until the installation process is complete.

Whether the opsi-Loginblocker is installed or activated during the opsi-client-agent installation,
...
* Via the new installer (opsi-client-agent-installer.exe, opsi-linux-client-agent-installer.sh, opsi-mac-client-agent-installer.sh)
...
The installer can be downloaded from an opsi 4.2 server without authentication via the following address (similar for opsi-linux-client-agent-installer.sh and opsi-mac-client-agent-installer.sh):
