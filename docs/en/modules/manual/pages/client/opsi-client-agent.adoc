...
For installation afterwards or for repair purposes, see chapter xref:client/opsi-client-agent#opsi-manual-clientagent-subsequent-installation[Subsequent installation of the opsi-client-agents].
...
proxy_url = <url> ::
  ...
  Example: `http://<proxy-address>:3128`
...
Colors can be configured in three different ways: as symbolic name (`clRed`), as hexadecimal value (`$FF00FF`) and as rgb value (`(255,0,0)`). There is a helper program set allows you simple to choose your colors and get the correct way to write the colors to the configuration file. The program is opsi color choose.
