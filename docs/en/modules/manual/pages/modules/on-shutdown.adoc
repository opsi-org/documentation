...
The opsi package of the _{opsi-client-agent}_ automatically migrates the states of the old product switch with the new {opsi-config-object}. For new clients only this Hostparameter has to be activated. A new installation of the _{opsi-client-agent}_ is no longer necessary, the Hostparameter should take effect immediately. The old on_shutdown scripts have also been removed, as they are no longer needed in the new module.
