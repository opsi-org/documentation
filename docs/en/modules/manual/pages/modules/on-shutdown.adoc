...
For older '{opsi-client-agent}' packages, i.e. 4.0.2.3-2 to 4.0.5.4-2, a corresponding modules file is required. Further details on cofunding projects can be found in xref:modules/modules.adoc#opsi-manual-modules[opsi Extensions] and at http://www.uib.de/www/kofinanziert/index.html[co-financed opsi extensions].
