...
TIP: A list of netboot products with UEFI support and Secureboot support can be found at the end of this chapter.
...
* UEFI is implemented in 32 or 64{nbsp}bit. This then also specifies the bitness of the operating system, i.e. a 32{nbsp}bit OS cannot be installed on a 64{nbsp}bit UEFI (and vice versa).

* xref:modules/secureboot#opsi-manual-secureboot[Secureboot] is a UEFI feature which ensures that booting Windows only works if certain firmware elements (e.g. the boot loader) have not been modified by third parties. Secureboot is enabled by default since Windows 8.
...
* Disable Secureboot: The setting is often found in the _Boot_ or _Startup_ section, sometimes under _Security_. For more information, see the xref:modules/secureboot#opsi-manual-secureboot[Secureboot Support] chapter.
...
* providing new (64{nbsp}bit) opsi Linux bootimages with tools for UEFI and GPT management
...
* _066 Host name of the start server_: IP address of the opsi server.

* _067 Name of the start file_: `linux/pxelinux.cfg/shimx64.efi.signed`.
