...
* xref:modules/secureboot#opsi-manual-secureboot[Secure boot] is a UEFI feature which ensures that booting Windows only works if certain firmware elements (e.g. the boot loader) have not been modified by third parties. Secure boot is enabled by default since Windows 8.
...
* Disable secure boot: The setting is often found in the _Boot_ or _Startup_ section, sometimes under _Security_. For more information, see the xref:modules/secureboot#opsi-manual-secureboot[Secure Boot Support] chapter.
