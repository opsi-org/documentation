...
Since the extension uses standard protocols for communication over the network, no any additional opsi or directory service-specific components are required.
...
TIP: Since version 14 it is possible to test the connection to directory via the parameter `--check-directory` without connecting to the {opsi-server}.
