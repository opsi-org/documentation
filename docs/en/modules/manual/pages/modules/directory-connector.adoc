...
An example configuration is provided as `/usr/share/opsi-directoy-connector/opsi-directory-connector.example.conf.conf`
This file can be used as a template for your own configuration.

[source,prompt]
----
cp `/usr/share/opsi-directoy-connector/opsi-directory-connector.example.conf.conf /etc/opsi/opsi-directory-connector-custom.conf
----
...
If the parameters are not specified, `start_tls` is activated, but the certificate is not checked. 
