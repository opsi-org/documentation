...
TIP: You can use the RPC interface on the admin page to call the {health-check} method.

=== opsiconfd health-check

On the opsi server console, you can run the command `opsiconfd health-check`. 
`opsiconfd health-check --help` prints the help text and  
`opsiconfd health-check --manual` describes all checks.

//TODO Link to admininterface
TIP: Via the admininterface or {opsi-cli} you can open a terminal on the server and execute the command (see xref:opsi-manual-configuration-tools-opsi-cli-commands-terminal[opsi-cli terminal]). 

=== opsi-cli

The {health-check} can also be invoked via the {opsi-cli}. 
See here for the section xref:opsi-manual-configuration-tools-opsi-cli-commands-support[] in the {opsi-cli} chapter.


=== Health Checks

This section describes what the {health check} checks and how it assesses the health of the system.

==== opsiconfd configuration

The {health check} examines the configuration of the opsiconfd service by checking the following values:
...
Currently only opsi 4.3 checks if `opsiclientd.global.verify_server_cert` is activated. 
...
Since opsi 4.3 the opsi repository must be located under /var/lib/opsi/repository. If this is not the case, an error will be reported.
...
The check issues a warning 90 days before the end of life of a distribution. After the end-of-life date, it issues an error.
