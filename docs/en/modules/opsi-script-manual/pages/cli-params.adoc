...
By default log files are written at Windows into the directory `c:\opsi.org\log` which _opsi-script_ tries to create. If _opsi-script_ has no access to this directory it uses the user-TEMP directory.
...
NOTE: After you checked a script with `testsyntax`, you may find additional runtime error messages in the log file. You should ignore these runtime errors. In many cases these runtime errors are side effects of the `testsyntax` run. The reason is, that while `testsyntax` many variables have empty (or other unexpected) values.


In order to run opsi-script in `testsyntax` mode, you have the following possibilities:

.opsi-script in interactive mode
image::opsi-script-interaktiv.png["Screenshot: opsi-script in interactive mode", pdfwidth=90%]
