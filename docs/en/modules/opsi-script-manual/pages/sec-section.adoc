...
+
CAUTION: Please respect the order of both the 'rwx' (read, write, execute) and the 'ugo' (user, group, others). +
...
CAUTION: If *umlauts in filenames* of the unzipped files are not displayed properly,
...
CAUTION: Use the third parameter for filename encoding only if necessary. Do not set 'cp437' or 'utf8' as a third parameter. These encodings are handled by default. 
...
* _{opsi-script}_ variables or constants in the ShellScript section are replaced with their content before execution and can thus be used easily within shell scripts.
...
CAUTION: _{opsi-script}_ waits for the ShellScript section to finish before the opsi script is processed further.
...
CAUTION: The environment variable gets a changed value after a reboot or after a call of `UpdateEnvironment` see: xref:prim-section.adoc#UpdateEnvironment[UpdateEnvironment]
