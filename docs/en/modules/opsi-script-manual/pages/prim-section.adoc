...
NOTE: We encourage you to use ModifyTOML2 instead. +
...
* `ModifyTOML2(`<TOMLcontents: String> , <command: String> , <tablePath: String> , <key: String> ,  <value: String>`) : String`   //since 4.12.8.0 +
This function, like the previous one ModifyTOML, allows to modify the <TOMLcontents> String given as input. +

- If the modifications are for the root table, <tablePath> must be an empty string "".
- If the modifications are meant for another table within the root table, <tablePath> must be the table's name.
- If the modifications are meant for a sub-table, <tablePath> must be the sequence of the tables' names delimited with dots. Example : "myTable.mySubTable".

4 commands are available: +

** 'ADD' : Adds the input <key>-<value> pair if the key does not exist. It returns a new modified TOMLcontens String. +
But, if the <key> already exists, this function does not do any modifications. The same input <TOMLcontents> String will be returned. +

** 'SET' : Sets a new <key>-<value> pair if the key does not exist, and even if it exists. The corresponding value will be changed with the new input <value>. +
The function returns the new modified TOMLcontens String. +

** 'CHANGE ' : Changes the corresponding value of the input <key> to the given <value>. It returns a new modified TOMLcontens String. +
But if the keyPath does not exist, nothing will be done. The same input <TOMLcontents> String will be returned. +

** 'DEL' : Deletes the <key> and its found value. It returns a new modified TOMLcontens String. +
But if the keyPath does not exist, nothing will be done. The same input <TOMLcontents> String will be returned. +
...
Adds a new empty table to the given <TOMLcontents> String. +
The <tablePath> can be a simple table name, which will be added to the root table, or a sequence tables' names delimited with dots. Example : "myTable.mySubTable".
