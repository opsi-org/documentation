...
* `GetValueFromInifile (` <FileName>, <section>, <key>, <default value>, OPTIONAL <encoding> `) : string` [W/L/M] +
The function tries to open the ini file path <FileName>, retrieve the requested <section> and find the value belonging to the specified <key> which the function will return. If any of these operations fail <default value> is returned. +
Starting from opsi-script 4.12.6, `GetValueFromInifile` supports a fifth optional parameter <encoding>, which helps reading the ini file in the correct encoding. We recommend using it in case of use of special encodings.
...
Make sure to insert in <Hashing Algorithm> one of these supported hashing algoriths :
...
NOTE: Starting from opsi-script v. 4.12.4.37, opsi-script allows TOML files handling. +
Although, TOML's syntax primarily consists of : key = "value" pairs, [section names] interpreted as tables and #comments, opsi-script does not conserve comments for the moment. +

.Encoding
TOML files are usually encoded in UTF-8. So the 'TOML' functions expect the given TOML files to be in UTF-8. +

A full opsi-script using all the functions is available below. +
...
- A succession of the tables' names containing the searched key-value pair. For example : "Table.subTable.key" +
...
This function converts the content of the given first <TOMLfilePath> to a JSON formatted content and saves it in the given <JSONfilePath>. +
If saving the file run without errors, "True" will be returned. +
If any errors occur, "False" will be returned. +
