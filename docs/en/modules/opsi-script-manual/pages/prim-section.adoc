...
assumes that <str> is encoded in <from> and returns the in <to> encoded version of <str>. <from> and <to> are encodings as listet in chapter <<opsi-script-encoding, _opsi-script_ encoding>>. +
...
The normal execution of a script breaks at the point of the call, and the _opsi-script_ stops running. This behaviour is needed if an automated user log in for some other user shall take place (cf. <<opsi-script-cookbook-local-admin>>).
