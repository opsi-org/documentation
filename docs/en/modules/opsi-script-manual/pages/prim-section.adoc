...
`SetDebug_Lib` = <boolean value>  //since 4.12.8.0 +
If false, then log messages from defined functions that are imported from external library files will be suppressed except for Warnings and Errors. +
The default value is false. +
see also : xref:cli-params.adoc#opsi-script-configs_debug_lib[opsi-script-configs_debug_lib]
...
* `opsi-wmi-test.exe`
You can test your WMI query with a small helper application (opsi-wmi-test.exe).
...
* Winbatch/ShellScript/ExecWith sections with `/RunAsLoggedOnUser` +
Even if opsi-script is started via the login event, it runs in the SYSTEM context and not in the context of the user who has just logged in. A Winbatch, ShellScript or ExecWith section with the option `/RunAsLoggedOnUser` can be used to start a process in the context of this user.
