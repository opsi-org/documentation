...
* Winbatch/ShellBatch/ExecWith sections with `/RunAsLoggedOnUser` +
Even if opsi-script is started via the login event, it runs in the SYSTEM context and not in the context of the user who has just logged in. A Winbatch, ShellBatch or ExecWith section with the option `/RunAsLoggedOnUser` can be used to start a process in the context of this user.
