...
The product properties for Windows `opsi-local-image-win*` are a subset of the standard windows netbooot product properties (see section xref:opsi-products:netboot-products.adoc#opsi-manual-netboot-nt6[Some hints to the Windows netboot products]). For notes about the hard disk drive properties, see the <<opsi-manual-localimage-components-part>> section. The missing properties for disks and partitions are taken from the product `opsi-local-image-prepare`.
