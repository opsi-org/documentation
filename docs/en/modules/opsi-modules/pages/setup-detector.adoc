...
image::opsi-modules:osd_config_dlg_en.png["Configuration dialog", pdfwidth=40%]

At the start page select the desire task and follow the dialogs (click `Next step`)

.opsi-setup-detector Start
image::opsi-modules:osd_page_start_en.png["Startpage", pdfwidth=90%]
...
* Creation of an OPSI package with the necessary scripts for installation and uninstallation.
...
image::opsi-modules:osd_page_create_modes_with_opb_en.png["Create and Build Modes", pdfwidth=90%]
...
image::opsi-modules:osd_page_setup2_en.png["Result of analysis 2", pdfwidth=90%]
...
Path to the OpsiPackageBuilder. OpsiPackageBuilder can be used to build the OPSI packages via ssh. +
see also: https://forum.opsi.org/viewtopic.php?f=22&t=7573 +
It is better to use the connection to the OPSI-Server to build the OPSI packages
...
Activates support for the extension xref:opsi-modules:background-install.adoc#opsi-manual-background-install[opsi-background-install]: +
If true, show `Background Info File` button on start tab.
Make `Product configuration 3` tab visible and enabled.
Write `opsi-meta-data.toml` file on product creation.
...
* *LastLanguage* : +
Default value is `auto`: select Language from the running OS. +
If you change the language, your selection will be stored.
