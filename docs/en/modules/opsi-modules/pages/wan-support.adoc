...
* When all the 'Produktaktionen' are completed, the log files are transferred to the opsi config server
...
* The replication only takes place, if any 'Action-Requests' are set on the opsi config server. The 'Produktaktion' +always+ does not count in this respect.
