...
* +pause_on_metered+ (boolean): If set to +true+, product caching pauses on metered or disconnected networks and resumes on unmetered connections. This only applies to Windows clients and uses the Windows Runtime API to monitor network status. A connection is considered metered if the network cost type is not "unrestricted", the connection is over or approaching the data limit, or is roaming.
...
[source,shell]
----
opsi-cli jsonrpc execute config_createUnicode opsiconfd.transfer.slots_opsiclientd_product_sync "Maximum number of simultaneous product synchronizations" 100 100 true false
----
