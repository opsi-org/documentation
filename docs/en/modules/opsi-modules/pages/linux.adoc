...
The opsi-linux-client-agent includes 15 free starts by which the agent can be used without any license.

In detail: After the initial Install from the opsi-linux-client-agent the opsi-script can be started 15 times in service context without the need of a license. +
...
For permanent maintenance of the installed Linux computer beyond these 15 free starts, you will need a license for this feature.
...
* `backend`: +
Which backend should be installed ? (mysql needs a valid license file). (Default='file') +
A license file (`.opsilic`) with the required licenses can be stored in the custom directory of the product.
If a license file is found there, it will be used.
