...
CAUTION: Using the file `shimx64.efi.signed` via the `opsi-admin command` only serves the purpose to enable the _UEFI-Boot_ option in opsi-configed. It does not affect the use of the DHCP boot file. In future releases, opsi-configed should accept the correct file and enable a secure boot checkbox.
