...
The properties of a depot are partly determined when an opsi-Server is registered as a depot with the command `opsiconfd setup --register-depot` (see xref:opsi-modules:multidepot.adoc#opsi-manual-multidepot-installation[Creating an depot server]).
