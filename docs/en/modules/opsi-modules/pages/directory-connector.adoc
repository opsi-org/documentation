...
CAUTION: Please be careful when assigning the names manually, as this may have undesired side effects. Therefore, use this mapping option only in exceptional cases!
...
CAUTION: Always use the parameter `--delete-clients` with caution! For safety, you can combine the switch with `--dry-run` and check beforehand whether the correct objects are found.
