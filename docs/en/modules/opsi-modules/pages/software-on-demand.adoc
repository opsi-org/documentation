...
There you have to change to the tab `Product groups` in the left pane and to "Localboot products" in the right pane.

TIP: Since version 4.0.1.6 of the `opsi-configed` you can change to `product configuration` without choosing a client. The number of currently selected clients is displayed in the bottom-left corner. The “Selected in table” value next to “Number of clients” indicates how many clients are selected.

.product-group menue
image::opsi-modules:configed_productgroup_en.png[pdfwidth=70%]

Product groups are added by right-clicking on “Groups” in the left pane.

Products can be added to a group via drag-and-drop. This is possible both from “All Products” in the lower part of the left pane and from the “Localboot Products” tab in the right pane when “All Products” is selected on the left side.

To remove a product from a group, right-click the product and select “Remove selection from group”.

The assignment of product groups to clients is configured using host parameters. To do this, first select one or more clients and then switch to the “Host Parameters” tab in the right pane. The corresponding configuration options are available under “software-on-demand”. See also the following sections.
...
image::opsi-modules:configed_hostparameters.png[pdfwidth=70%]
