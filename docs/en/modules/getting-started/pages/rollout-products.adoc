...
If the product 'hwinvent' is installed on your opsi server and you have added a xref:os-installation.adoc#opsi-getting-started-firststeps-osinstall-create-client[client] which is configured to boot over the network, you can do something else useful: Hardware inventory when there is no operating system installed.
