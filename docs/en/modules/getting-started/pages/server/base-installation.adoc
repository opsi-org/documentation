...
<<opsi-getting-started-installation-base-quickinstall>>, and the <<opsi-getting-started-installation-base-packages>> on Debian, Ubuntu, openSUSE/Suse Linux Enterprise Server (SLES), CentOS, Red Hat Enterprise Linux (RHEL), AlmaLinux, Rocky Linux, as well as the <<opsi-getting-started-installation-base-ucs>>.

[[opsi-getting-started-installation-base-vm]]
== Starting the preconfigured Virtual Machine

The opsi-Server can be installed and operated as a virtual machine without any problems. We offer a ready-to-use and preconfigured virtual machine for download on our link:https://uib.de/de/opsi/opsi-testen-download/[website].
...
After logging in, the Firefox browser starts and opens a page with links to the manual, to our forum (community support), to the opsi wiki and to the professional uib support.
...
opsi QuickInstall is a program to quickly and easily install an opsi server on a Linux system--via a graphical setup tool or on the command line. You can find the program on our servers at the following link: https://download.uib.de/opsi4.2/stable/quickinstall/

[[opsi-getting-started-installation-base-quickinstall-download]]
=== Download and unpack opsi QuickInstall
...
NOTE: The installer asks for so-called properties. For more information, please read the section xref:manual:modules/linux.adoc#opsi-manual-linux-localboot-l-opsi-server[The Product `l-opsi-server`]. Here you can also find the default values of the properties. Note that for opsi QuickInstall the default for `allow_reboot` is set to `false`.
...
image::oqi.png["Choose the language and a setup type.", pdfwidth=90%]
...
image::oqiInfo.png["Hover the mouse over the small icon to display more information.", pdfwidth=90%]

. In the last dialog window you fill in the fields opsi admin user and opsi admin password, among other things.
+
IMPORTANT: Be sure to choose a different name and a strong password--not the one shown in this example (<<Information>>).
...
image::quickinstall_l-opsi-server.png["The opsi server installation is running.", pdfwidth=90%]

The installation may take a few minutes. When it's finished, opsi QuickInstall will tell you if it was successful. If you see the message `success`, then your opsi server is configured and ready for operation.
...
image::oqiFailed.png["If the installation fails, please check the log files for error messages.", pdfwidth=90%]
...
image::quickinstallNoGuiFailed.png["The setup program provides information about errors and log files.", pdfwidth=90%]
...
NOTE: Please note that only WebDAV is used as protocol to communicate with the opsi depot; there is no Samba support in this variant (yet). Also, the file backend is not supported, you will need an opsi MySQL module license (see chapter xref:opsi-modules:modules[opsi Extensions]).
...
You can install the opsi server using the package management tool of your distribution. This usually includes the following steps:
...
[source,bash]
----
gpg --show-keys /usr/local/share/keyrings/opsi.gpg 2>/dev/null
----
...
Installing the opsi server on Red Hat Enterprise Linux (RHEL), AlmaLinux, and Rocky Linux is basically the same. There are only differences in the repository used.
...
The link:https://www.univention.com/products/univention-app-center/app-catalog/opsi/[Univention App Center Catalog] offers opsi for UCS 5.0. During the installation the package manager automatically installs other packages: `opsi-tftpd-hpa` and `univention-mariadb`.
...
WARNING: If you install the opsi server on a machine with a UCS system role other than Primary Directory Node, the UCS server must first join the domain.
...
Now proceed with the server update (section xref:getting-started:server/configuration.adoc#opsi-getting-started-installation-config-update-server[Updating the opsi Server]). After that, you can install the standard products (section xref:getting-started:server/configuration.adoc#opsi-getting-started-installation-config-update-opsi-product[Installing the Standard Products]).
