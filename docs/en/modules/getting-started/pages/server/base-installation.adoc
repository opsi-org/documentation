...
and the <<opsi-getting-started-installation-base-packages>> on Debian, Ubuntu, openSUSE/Suse Linux Enterprise Server (SLES), CentOS, Red Hat Enterprise Linux (RHEL), Alma Linux, Rocky Linux, and Univention Corporate Server (UCS).

TIP: To jump right in, you can use the opsi QuickInstall program to set up an opsi server on Linux via a graphical installer or the command line (see chapter xref:quickinstall[opsi QuickInstall]).

NOTE: If you use a proxy server to access the internet, remember to specify it on your opsi server as well. Use the environment variables `http_proxy` and `https_proxy` as described in chapter xref:server/requirements[prerequisites].
