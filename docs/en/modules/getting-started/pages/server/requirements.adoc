...
This chapter describes the requirements for the installation of the opsi server on Linux. A list of supported distributions can be found at the end of the chapter in the section <<opsi-getting-started-installation-linux-support>>.
...
You can install the opsi server on many different Linux distributions. The next table lists the supported systems.
