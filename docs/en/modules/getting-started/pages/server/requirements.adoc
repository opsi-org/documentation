...
Config-Server = central opsi-server which holds tha data of all clients. +
Depot-Server = opsi-server placed in the different locations. +
If there is only one opsi-server, this one has both roles.

* opsi-server web service: TCP 4447 +
Client to (Config / Depot)-Server, Depot-Server to Config-Server (bidirectional, connections via localhost).
...
Client to Depot-Server (bidirectional). +
Depends on the version of the client operating system.

* WINEXE: UDP 137 / UDP 138 (netbios) / TCP 139 / TCP 445 +
Depot-Server to client (bidirectional). +
Depends on the version of the client operating system.

* grafana web service (optional): TCP 3000 +
Connection from client to Config-Server. +
(used for accessing the managing web page of the Config-Server)

* SSH (optional): TCP 22 +
Management access to (Config / Depot)-Server

* DNS: TCP 53 +
Every Client / Server should be reach a DNS-Server.

* WakeOnLan (WOL): UDP 7 / UDP 9 / UDP 12287 +
Config-Server to Client. These ports are configurable.

* HTTP: TCP 80 +
(Config / Depot)-Server to the Internet. +
