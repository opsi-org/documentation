...
The program `oca-installation-helper[.exe]` requires root/administrator privileges to operate. It recognizes various parameters and options, which you can view by using the `--help` command.

.*oca-installation-helper* Online Help
image::common:oca_installer_help.png["The online help of *oca-installation-helper*", pdfwidth=80%]
...
For windows you can specify parameters for `oca-installation-helper` in the registry.
To do that, create a key "INSTALL_PARAMS" under HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\opsi-client-agent.
With that key you can set parameters like `--service-password \{crypt\}qwertzuiop --client-id client1.test.lan`.
...
image::common:opsiclientd-blocklogin-notifier.png["*opsiclientd*: Blocklogin Notifier", pdfwidth=15%]
...
image::common:opsiclientd-event-notifier.png["*opsiclientd*: Event Notifier", pdfwidth=30%]
...
image::common:opsiclientd-action-notifier.png["*opsiclientd*: Action Notifier", pdfwidth=30%]
...
image::common:opsiclientd-shutdown-notifier.png["*opsiclientd*: shutdown notifier (default)", pdfwidth=30%]
...
image::common:opsiclientd-shutdown-notifier_timepicker.png["*opsiclientd*: Shutdown Notifier with Time Selection (*timepicker*)", pdfwidth=30%]
...
image::common:eventflowchart.png["Overview of a Standard Event", pdfwidth=80%, width=1000]
...
image::common:opsiclientd-control-server-web-interface.png["One Authentication Method: the Client's local Administrator Account", pdfwidth=70%, width=800]
...
image::common:opsiclientd_infopage_event_on_demand.png["The Info Page presents a Timeline.", pdfwidth=70%, width=800]
...
The upcoming sections will explore various ways to configure the client agent. The central configuration file for this purpose is `opsiclientd.conf`, which can be found in different directories on the client computer, varying based on the operating system. You can refer to our link:https://github.com/opsi-org/opsiclientd/tree/v4.3/opsiclientd_data/[GitHub repository] for the default configuration files for Windows, Linux, and macOS, showcasing the standard values for each platform.

.Our GitHub repository contains a *opsiclientd.conf* file with default values.
image::common:github-client-agent.png["Our GitHub repository contains a *opsiclientd.conf* file with default values.", pdfwidth=80%, width=800]
...
To change host parameters, there are three available methods:

* <<opsi-manual-clientagent-configuration-webservice-opsi-configed>>
* <<opsi-manual-clientagent-configuration-webservice-webgui>>
...
image::common:opsiclientd-configuration-via-configed-serverdefault.png[".Host Parameters for all Clients are configured in the *Server configuration*", pdfwidth=70%, width=800]
...
image::common:opsiclientd-configuration-via-configed.png["Host Parameters for individual Clients are configured in the *Client configuration*.",pdfwidth=70%, width=800]

[[opsi-manual-clientagent-configuration-webservice-webgui]]
==== *OPSI-WebGUI*

In the `OPSI-WebGUI`, you can edit host parameters either per client via xref:gui:webgui/userguide-clients.adoc#opsi-manual-opsiwebgui-opsiclients-attributes-parameter[_Clients_ / _Configuration_] or globally for server defaults via xref:gui:webgui/userguide-server.adoc#opsi-manual-opsiwebgui-opsiservers-attributes-parameter[_Servers_ / _Configuration_].

This side-by-side layout is especially convenient when you switch between multiple clients.
In addition, the WebGUI collects changes locally first and shows them together in the _Unsaved Changes_ view before you explicitly save them back to the server.
...
image::common:opsi-script-configuration-skin.png["Here you can find the Background Image and the Configuration File for *opsi-script*.",pdfwidth=70%, width=800]
...
image::common:opsi-systray-message.png["The Systray Program of the Client Agent notifies Users about Upcoming Installations."]

.Use the right-click Context Menu to manage and initiate different Actions.
image::common:opsi-systray-menue.png["Use the right-click Context Menu to manage and initiate different Actions."]

.There is a Systray Program for macOS that you control with the right Mouse Button.
image::common:opsi-systray-macos.png["There is a Systray Program for macOS that you control with the right Mouse Button.", pdfwidth=70%, width=326]
...
image::common:opsi-systray-properties.png["To configure the Systray Program, change its Product Properties.", width=800, pdfwidth=70%]
...
image::common:opsi-systray-linux.png["The Systray Program on Linux (GNOME Shell)", pdfwidth=70%]
