...
The upcoming sections will explore various ways to configure the client agent. The central configuration file for this purpose is `opsiclientd.conf`, which can be found in different directories on the client computer, varying based on the operating system. You can refer to our link:https://github.com/opsi-org/opsiclientd/tree/v4.4/opsiclientd_data/[GitHub repository] for the default configuration files for Windows, Linux, and macOS, showcasing the standard values for each platform.

.Our link:https://github.com/opsi-org/opsiclientd/tree/v4.3/opsiclientd_data/[GitHub repository] contains a *opsiclientd.conf* file with default values.
