...
If the product 'hwinvent' is installed on your opsi server and you have added a client xref:clients:windows-clients/adding-clients.adoc#firststeps-osinstall-create-client[] which is configured to boot over the network, you can do something else useful: Hardware inventory when there is no operating system installed.
