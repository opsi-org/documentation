...
In this dialog you may choose to abort the create process. You may also choose to continue based on the pattern of a selectable installer type.

If we have a successful analyze in some cases you will get a window with additional information to the detected installer type. You will get this for example at Installshield, QT-Installer, Install Anywhere.

image::osd_installer_info_installshield_en.png["Additional Installer Info", pdfwidth=90%]

image::osd_installer_info_QT_en.png["Additional Installer Info", pdfwidth=90%]

image::osd_installer_info_anywhere_en.png["Additional Installer Info", pdfwidth=90%]

If we have a successful analyze, you will always see the result page.
...
* *Uninstall Command:* The determined command for a non-interactive uninstalling. The details are dependent from the value of the checkbox `*Prefer Silent Install:*`.

* *Uninstall Program:* The determined uninstall program. +
...
Since opsi 4.3 this text may be written in `Markdown`. In this case you have the edit area on the left and the preview of the interpretation on the right. 
...
** *Structured*: Empty in version 4.2.2 and higher  (fallback to default)
...
More details: xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-support_custom_directory[custom directory]
...
More details: xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-install_from_local_temp_dir[local temp dir]

* *Handle License Key* : Adds a property and code for the handling of a license key. +
More details: xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-handle_license_key[Handle License Key]

* *DesktopIcon* : Adds a property and code for the handling of desktop icons. +
More details: xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-desktopicon[DesktopIcon]
...
More details: xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-customize_profile[Customize Profile]

// This is jump target for the opsi-setup-detector online help
[[opsi-setup-detector-product-configuration-priority_dependency]]
=== opsi-setup-detector: Priority and dependencies

The tab _Product Configuration 2_ may be used to define priorities and dependencies:

.opsi-setup-detector Product configuration 2
image::osd_page_product2_en.png["Product configuration 2", pdfwidth=90%]

NOTE: For normal application software you don't have to do anything here,
...
_Priority_: affects the installation order. *Recommended for application software: 0* +
...
If the configuration contains the connection data for your opsi-server, the connection will here be started. If the configuration does not contain the password (for security reasons) you will be asked for the password (see also <<opsi-setup-detector-product-configuration-dependency-config>>).

image::osd_password_dlg_en.png["Password Dialog", pdfwidth=40%]

* _Properties_: Here you can define variable properties of the product(see also <<opsi-setup-detector-product-configuration-properties-config>>).
...
.Use this dialog to define dependencies.
image::osd_dependency_editor.png["Use this dialog to define dependencies", pdfwidth=40%]
...
Require Mode:: If you creat a _Meta Product_ this area is disabled to avoid senseless configurations. + 
You can either request the *Action* +setup+ or (see below) the *State* (_installed_).
...
NOTE: The actual installation order is determined by a combination of product dependencies and product prioritization. Details about this can be found in the opsi manual in the chapter 'Manipulating the installation sequence by priorities and dependencies' (see also: xref:opsi-products:localboot-products.adoc#opsi-manual-localboot-product-order[Dependencies and Sequence]).
