...
The following sections describe how to analyze a setup file and create an OPSI product from it. To do this, click the _Analyze File and Create OPSI Package_ button on the start page (<<osd_page_start>>). Then navigate to the desired installer file in the file selection dialog. The `opsi-setup-detector` starts the analysis immediately.
...
image::opsi-modules:osd_page_analyze_en.png["The *opsi-setup-detector* has successfully analyzed a File.", pdfwidth=80%, width=800]

If the analysis was not successful, you will see this dialog instead:

.The *Sorry Unknown Installer* Dialog
image::opsi-modules:osd_unknown_select_installer_en.png["The *Sorry Unknown Installer* Dialog", pdfwidth=30%]

No installer type could be detected in the selected file. +
Now, you have the option to either terminate the process by clicking on _Cancel_ or to proceed with creating the package by selecting an installer type from the drop-down menu.

Or you will see this dialog:


.The *Detected Installer not known* Dialog
image::opsi-modules:osd_detected_not_known_installer_en.png["The *Detected Installer not known* Dialog", pdfwidth=30%]
...
image::opsi-modules:osd_installer_info_installshield_en.png["Additional Info: InstallShield", pdfwidth=90%]

image::opsi-modules:osd_installer_info_QT_en.png["Additional Info: Qt Installer", pdfwidth=90%]

image::opsi-modules:osd_installer_info_anywhere_en.png["Additional Info: InstallAnywhere", pdfwidth=90%]
...
image::opsi-modules:osd_page_setup1_en.png["This Tab shows the Result of a successful Analysis.", pdfwidth=80%, width=800]
...
* _MsiUpgrade_: Upgrade code for MSI installers or installers that contain MSI
...
See also: xref:opsi-modules:setup-detector.adoc#osd_config_preferMsiUninstall[Configuration: preferMsiUninstall]
...
image::opsi-modules:osd_page_product1_en.png["Configure the OPSI Product.", pdfwidth=80%, width=800]
...
* _Uninstall_before_install_: This creates an additional boolean property (defaulted to `true`) to control if we should uninstall before we try to install. (see section xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-uninstall_before_install[Uninstall_before_install]).
...
image::opsi-modules:osd_page_product2_en.png["Configure Priority and Dependencies.", pdfwidth=80%, width=800]
...
image::opsi-modules:osd_password_dlg_en.png["Password Dialog", pdfwidth=40%]
...
image::opsi-modules:osd_dependency_editor.png["You can configure Dependencies in this Dialog.", pdfwidth=40%]
...
image::opsi-modules:osd_property-editor.png["You can configure Product Properties in this dialog.", pdfwidth=40%]
...
image::opsi-modules:osd_page_producticon_en.png["Select an Icon for your Product.", pdfwidth=80%, width=800]
...
image::opsi-modules:osd_page_create_en.png["Create the Product on the last Tab.", pdfwidth=80%, width=800]
...
image::opsi-modules:osd_overwrite_dlg_en.png["Backup Dialog", pdfwidth=40%]
