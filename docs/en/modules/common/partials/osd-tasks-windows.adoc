...
* _Create an opsi Package Template 'with user'_: Doesn't ask for a setup file, but creates an opsi template product for an installation with a logged in user and takes the information from the product configuration.

* _Create an opsi winget Package_: Doesn't ask for a setup file, but shows you the 'winget' input mask. There you have to fill the `wingetId` and `wingetSource` input fields. +
(see the section xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-create-winget[Create an opsi winget Package]).

* _Create Background info file_: +
(Only visible if `EnableBackgroundMetaData=true` is set in the configuration). +
Used to add the metadata for the background installation to an existing product.
(see section xref:opsi-modules:background-install.adoc#opsi-manual-background-install-old-products[Provide legacy products with background metadata]).