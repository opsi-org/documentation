...
Wenn z.B. `admin-networks` für den Zugriff in `/etc/opsi/opsiconfd.conf` definiert ist und ein Proxy verwendet wird, der nicht in diesen Adressbereich fällt, kann sich der Admin nicht am Server anmelden.
Dann kann der Parameter `trusted-proxies` in der gleichen Datei gesetzt werden, um die Anmeldung wieder zu ermöglichen. Für die Adressen in der Liste `trusted_proxies` wertet der Server den `x-forwarded-for` Header aus, um die ursprüngliche Client-Adresse zu erhalten.
