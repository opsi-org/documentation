...
NOTE: In den OpenSuse Distributionen muss gegebenenfalls der Parameter `NO_PROXY` in der Datei /etc/sysconfig/proxy angepasst werden. Hier dann entsprechend `NO_PROXY=127.0.0.1,localhost,company.tld` setzen.