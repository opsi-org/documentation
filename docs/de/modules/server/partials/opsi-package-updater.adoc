...
So listen Sie die in den Repositorys verfügbaren Pakete auf (dabei werden konfigurierte includes/excludes ignoriert):
...
So zeigen Sie die verfügbaren Aktualisierungen an (dabei werden konfigurierte includes/excludes berücksichtigt):
...
* `excludes`: Regulärer Ausdruck um Paketnamen anzugeben, die ignoriert werden, z.{nbsp}B. ^test.* schließt alle Pakete aus, die mit "test" beginnen.
* `proxy`: Proxyserver
...
image::server:opsi-package-updater.png["Installieren Sie die Standard-Produkte auf Ihrem opsi-Server.", width=800, pdfwidth=80%]
