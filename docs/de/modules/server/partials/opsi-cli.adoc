...
image::server:opsi-cli-general-options.png["Ausgabe: opsi-cli general options", width=800, pdfwidth=80%]
...
image::server:opsi-cli-io-options.png["Ausgabe: opsi-cli io options", width=800, pdfwidth=80%]
...
image::server:opsi-cli-table-format.png["Tabellenformat", pdfwidth=80%, width=400]
...
image::server:opsi-cli-service-options.png["Ausgabe: opsi-cli service options", width=800, pdfwidth=80%]
...
image::server:opsi-cli-config-list.png["Ausgabe: opsi-cli config list", width=800, pdfwidth=80%]
...
[[server-components-opsi-cli-commands-log]]
==== *log*
Das `log` Plugin ermöglicht das Anzeigen von Live-Logs vom opsi-Server und den Clients über die opsi-Messagebus Verbindung.
Wenn der angegebene Host nicht verbunden ist, wird ein Fehler ausgegeben.
Das Plugin bestimmt den entsprechenden Dienst oder Host, um die Logs basierend auf dem Log-Typ abzurufen, und fordert die Übertragung der Log-Datei an.

[[server-components-opsi-cli-commands-log-subcommands]]
===== Unterbefehle

[[server-components-opsi-cli-commands-log-view]]
.*view*
   * *Beschreibung*: Logs für einen angegebenen Host anzeigen.
   * *Verwendung*: `opsi-cli log view [OPTIONS] [HOST_ID]`

image::server:opsi-cli-log-view-help.png["Output: opsi-cli log view --help"]

   * *Optionen*:
      ** `--log-type` (Standard: opsiclientd): Den anzuzeigenden Log-Typ angeben (opsiconfd oder opsiclientd).
      ** `--live` (Flag): Live-Logdatei direkt vom Client anzeigen (nur verfügbar für opsiclientd); andernfalls die auf dem Server gespeicherten Client-Logs anzeigen.
      ** `--follow` (Flag): Die Logdatei kontinuierlich für Echtzeit-Updates verfolgen.
      ** `--log-level` (Standard: 6): Das Log-Level zum Filtern angeben (1 bis 8).
   * *Argumente*:
      ** `HOST_ID`: Die Host ID für den die Logs angezeigt werden sollen.
   * *Beispiel*:

image::server:opsi-cli-log-view-opsiconfd.png["Output: opsi-cli log view --log-type opsiconfd"]

TIP: Deaktivieren Sie die Farbformatierung mit der Option `--no-color` oder durch Setzen der Umgebungsvariable `NO_COLOR`. Verwendung: `opsi-cli --no-color log view [OPTIONS] HOST_ID`

[[server-components-opsi-cli-commands-package]]
==== *package*

Das `package` Plugin für `opsi-cli` bietet eine umfassende Reihe von Befehlen zur Verwaltung von opsi-Paketen.
Diese Befehle umfassen das Erstellen, Extrahieren, Auflisten, Installieren und Deinstallieren von Paketen und bieten eine einheitliche Lösung für das Paketmanagement.

image::server:opsi-cli-package-help.png["Output: opsi-cli package --help", width=800, pdfwidth=80%]

[[server-components-opsi-cli-commands-package-subcommands]]
===== Unterbefehle

[[server-components-opsi-cli-commands-package-make]]
.*make*
   * *Beschreibung*: Der `make` Befehl verpackt ein Quellverzeichnis und speichert es in ein Zielverzeichnis, wobei ein `.opsi` Paket zusammen mit `.md5` und `.zsync` Dateien erstellt wird.
   * *Verwendung*: `opsi-cli package make [OPTIONS] [SOURCE_DIR] [DESTINATION_DIR]`
   * *Optionen*:
      ** `-o, --overwrite` (Flag): Vorhandenes Paket überschreiben, falls es existiert.
      ** `--follow-symlinks` (Flag): Symlinks folgen.
      ** `--custom-name`: Verzeichnisse mit dem Custom Namen einbeziehen.
      ** `--custom-only` (Flag): Nur Custom Verzeichnisse einbeziehen.
      ** `--md5/--no-md5` (Flag, Standard: true): MD5-Prüfsumme für das Paket erstellen (mit `--no-md5` deaktivieren).
      ** `--zsync/--no-zsync` (Flag, Standard: true): Zsync-Datei für das Paket erstellen (mit `--no-zsync` deaktivieren).
   * *Argumente*:
      ** `SOURCE_DIR` (Standard: aktuelles Verzeichnis): Quellverzeichnis.
      ** `DESTINATION_DIR` (Standard: aktuelles Verzeichnis): Zielverzeichnis.
   * *Beispiel*:

image::server:opsi-cli-package-make.png["Output: opsi-cli package make"]

[[server-components-opsi-cli-commands-package-control-to-toml]]
.*control-to-toml*
   * *Beschreibung*: Der `control-to-toml` Befehl erzeugt eine `control.toml` Datei aus einer `control` Datei, die sich im angegebenen Quellverzeichnis befindet.
   * *Verwendung*: `opsi-cli package control-to-toml [SOURCE_DIR]`
   * *Argumente*:
      ** `SOURCE_DIR` (Standard: aktuelles Verzeichnis): Quellverzeichnis.

[[server-components-opsi-cli-commands-package-extract]]
.*extract*
   * *Beschreibung*: Der Befehl `extract` extrahiert ein opsi-Paket aus dem angegebenen Paketarchiv in das Zielverzeichnis.
   * *Verwendung*: `opsi-cli package extract [OPTIONS] [PACKAGE_ARCHIVE] [DESTINATION_DIR]`
   * *Optionen*:
      ** `--new-product-id`: Eine neue Produkt-ID, um die vorhandene in der control datei zu ersetzen.
      ** `-o, --overwrite` (Flag): Zielverzeichnis überschreiben, falls es existiert.
   * *Argumente*:
      ** `PACKAGE_ARCHIVE`: Pfad zum Paketarchiv.
      ** `DESTINATION_DIR` (Standard: aktuelles Verzeichnis): Zielverzeichnis.
   * *Beispiel*:

[source,console]
----
opsi-cli package extract <package_archive.opsi> <destination_dir>
----

[[server-components-opsi-cli-commands-package-list]]
.*list*
   * *Beschreibung*: Der Befehl `list` listet OPSI-Produkte auf, die auf einem OPSI-Depotserver installiert sind. Standardmäßig zeigt er Produkte auf allen Depots an.
   * *Verwendung*: `opsi-cli package list [OPTIONS] [PRODUCT_IDS]`
   * *Optionen*:
      ** `--depots` (Standard: alle): Depot-IDs (kommagetrennt) oder 'alle'.
      ** `--product-type`: Filtere die Ausgabe nach Produkttyp [localboot|netboot].
   * *Argumente*:
      ** `PRODUCT_IDS`: Liste der Produkt-IDs zur Filterung der Ausgabe. "*" kann als wildcard verwendet werden.
   * *Beispiel*:

image::server:opsi-cli-package-list.png["Output: opsi-cli package list"]

[[server-components-opsi-cli-commands-package-install]]
.*install*
   * *Beschreibung*: Der Befehl `install` installiert opsi-Pakete von lokalen Pfaden oder URLs. Standardmäßig wird das Paket auf dem OPSI-Configserver installiert.
   * *Verwendung*: `opsi-cli package install [OPTIONS] [PACKAGES]`
   * *Optionen*:
      ** `--depots` (Standard: OPSI-Configserver): Depot-IDs (kommagetrennt) oder 'alle'.
      ** `--update-properties` (Flag): Interaktives Prompt zur Aktualisierung der Standardwerte der Produkteigenschaften auslösen.
      ** `--force` (Flag): Installation erzwingen.
      ** `--setup-where-installed` (flag): Setup, wo das Paket installiert ist.
      ** `--setup-where-installed-with-dependencies` (flag): Setup, wo das Paket installiert ist, zusammen mit deren Abhängigkeiten.
      ** `--update-where-installed` (flag): Update, wo das Paket installiert ist.
   * *Argumente*:
      ** `PACKAGES`: Liste der zu installierenden Pakete, die als lokale Pfade oder URLs angegeben werden können.
   * *Beispiel*:

image::server:opsi-cli-package-install.png["Output: opsi-cli package install"]

[[server-components-opsi-cli-commands-package-uninstall]]
.*uninstall*
   * *Beschreibung*: Der Befehl `uninstall` deinstalliert OPSI-Produkte, die durch ihre Produkt-IDs angegeben sind. Standardmäßig wird von dem OPSI-Configserver deinstalliert.
   * *Verwendung*: `opsi-cli package uninstall [OPTIONS] [PRODUCT_IDS]`
   * *Optionen*:
      ** `--depots` (Standard: OPSI-Configserver): Depot-IDs (kommagetrennt) oder 'alle'.
      ** `--force` (Flag): Deinstallation erzwingen.
      ** `--keep-files` (Flag): Dateien bei der Deinstallation behalten.
   * *Argumente*:
      ** `PRODUCT_IDS`: Liste der zu deinstallierenden Produkt-IDs.
   * *Beispiel*:

[source,console]
----
opsi-cli package uninstall --depots <depot_id> --force <produkt1> <produkt2>
----

TIP: Verwenden Sie die Option `--quiet`, um die Fortschrittsanzeige und andere nicht wesentliche Ausgaben zu unterdrücken.
