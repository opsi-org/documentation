...
==== *log*
Das `log` Plugin bietet die Möglichkeit, Live-Logs vom opsi Server und von Clients über die `opsi-messagebus` Verbindung anzuzeigen.

image::server:opsi-cli-log-view-help.png["Output: opsi-cli log view --help", width=800, pdfwidth=80%]
...
.*view*
   ...
   * *Optionen*:
      ...
      ** `--color/--no-color` (Flag, Standard: true): Farbformatierung für die Log-output aktivieren oder deaktivieren.
   * *Argumente*:
...
Das `package` Plugin für `opsi-cli` bietet eine Reihe von Befehlen zur Verwaltung von opsi-Paketen. Dies umfasst das Erstellen, Extraktieren, Auflisten, Installieren und Deinstallieren von Paketen.
...
.*make*
   * *Beschreibung*: Ein opsi-Paket erstellen.
...
.*control-to-toml*
   * *Beschreibung*: control.toml aus der control datei generieren.
...
.*extract*
   * *Beschreibung*: Ein opsi-Paket extraktieren.
...
.*list*
   * *Beschreibung*: opsi-Pakete auflisten.
...
.*install*
   * *Beschreibung*: opsi-Pakete installieren.
   * *Verwendung*: `opsi-cli package install [OPTIONS] [PACKAGES]`
   * *Optionen*:
      ** `--depots` (Standard: OpsiConfigserver): Depot-IDs (kommagetrennt) oder 'alle'.
...
.*uninstall*
   * *Beschreibung*: opsi-Produkte deinstallieren.
