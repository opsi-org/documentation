...
Im Laufe der Jahre hat opsi einige Kommandozeilentools erhalten. Da die `opsi-utils`- und `opsi-python`-Skripte zu unterschiedlichen Zeiten entstanden sind, haben sie jeweils ihre eigene Struktur, eigene Kommandozeilenoptionen und Ausgabeformate. Viele der Werkzeuge nutzen das Standard RPC-Interface, andere umgehen den opsi-Webservice und arbeiten direkt auf dem Backend.

Das relativ neue Tool `opsi-cli` bietet Zugriff auf dieselben Funktionen (und mehr!). Darüber hinaus hat `opsi-cli` ein Plugin-System, dass es opsi-Anwendern ermöglicht, die Funktion durch eigene Plugins zu erweitern.  
