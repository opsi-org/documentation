...
* Definieren Sie eine Host-Gruppe, die Sie z.{nbsp}B. mit `opsi-configed` erstellt haben (siehe Abschnitt xref:gui:configed/userguide-clients.adoc#opsi-manual-configed-treeview[Baumansicht]): +
[source,console]
----
...
CAUTION: Die Namen von Gruppen in opsi müssen eindeutig sein, egal, ob es um eine Host-Gruppe (Bereich _Directory_ oder _Gruppen_ in `opsi-configed`) oder eine Produktgruppe geht. 
