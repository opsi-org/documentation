...
Zum Beispiel kann über *productOnClient_updateObjects* der Zustand eines Paketes auf einem Client aktualisiert werden.
...
Beschreibt die Eigenschaften eines *Product* wie sie bei der Erstellung des Paketes definiert wurden.
...
Die JSON-RPC-Requests werden per *POST* an den Endpoint */rpc* (`\https://<server-address>:4447/rpc`) des opsi-Configservers gesendet.
