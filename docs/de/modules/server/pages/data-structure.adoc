...
Als Beispiel dient hier das Objekt .
Das Objekt vom Typ , welches das {opsi-product}  beschreibt sieht z.B. so aus:
...
* : Liefert eine Liste von Objekten zurück, die auf den angegebenen Filter passen.
Wird eine Liste von Attributen angegeben, werden nur diese aus dem Backend befüllt.
* : Wie , nur dass ungeprüfte Rohdaten aus dem Backend direkt ausgeliefert werden.
Arbeitet performanter als , ist jedoch mit Vorsicht zu verwenden.
*  Liefert eine Liste von Objekt-IDs zurück, die auf den angegebenen Filter passen.
Über den  kann die Datenstruktur der Elemente im Ergebnis gewählt werden.
Mögliche Werte sind hierbei: ,  und .
* : Erzeugt ein neues Objekt. Existiert das Objekt bereits, wird es mit den neuen Werten komplett überschrieben.
Hierbei werden Attribute, die nicht (oder mit dem Wert ) übergeben werden, im Backend auf  gesetzt.
* : Aktualisiert ein Objekt. Attribute, die nicht (oder mit dem Wert ) übergeben werden,
...
* : Es kann ein Objekt oder eine Liste von Objekten übergeben werden. Jedes Objekt wird intern an  übergeben.
* : Es kann ein Objekt oder eine Liste von Objekten übergeben werden. Jedes Objekt wird intern an  übergeben falls es noch nicht existiert, andernfalls jedoch an .
* : Erzeugt ein neues Objekt und nimmt hierbei alle möglichen Attribute als einzelne Parameter entgegen.
Intern wird hierbei 'createObjects' verwendet. Existierende Objekte werden also komplett überschrieben.
* : Löscht eine Liste von Objekten. Es muss hierbei zwingend eine Liste übergeben werden. Es werden nur die das Objekt identifizierenden Attribute (//) zur Auswahl der zu löschenden Objekte verwendet.
* : Löscht das über die angegebenen Parameter identifizierte Objekt.

TIP: Zum Aktualisieren eines Objekts bietet sich in der Regel die Verwendung der Methode  an.
Zum Beispiel kann über updateObjects_ der Zustand eines Pakets auf einem Client aktualisiert werden.
...
Dadurch unterscheiden sie sich von den  Methoden aus opsi 3.x welche in der Regel mit ,  oder  anfangen.

Die -Methoden haben zwei optionale Parameter:
...
=== ProductDependency_(Paket-Abhängigkeiten)
...
Beschreibt die ermittelten Hardwaretypen (inclusive der Client-spezifischen Daten). Die Idee ist in diesem Objekt die Client-spezifischen Daten zu halten und in `auditHardware` nur die allgemeinen, so dass es dort z.B. nur einen Eintrag für eine Netzwerkkarte gibt, die in vielen Clients benutzt wird. +
...
Beschreibt die ermittelten Hardwaretypen (ohne die Client-spezifischen Daten). Die Idee ist in diesem Objekt nur die allgemeinen Daten eines Hardwaretyps zu halten, so dass es hier z.B. nur einen Eintrag für eine Netzwerkkarte gibt, die in vielen Clients benutzt wird. +
Leider funktioniert diese Idee in der Praxis nicht wirklich.
...
Beschreibt die ermittelten Softwaretypen (inclusive der Client-spezifischen Daten). Die Idee ist in diesem Objekt die Client-spezifischen Daten zu halten und in `auditSoftware` nur die allgemeinen, so dass es dort z.B. nur einen Eintrag für eine Office-Software gibt, die in vielen Clients benutzt wird.
...
Beschreibt die ermittelten Softwaretypen (ohne die Client-spezifischen Daten). Die Idee ist in diesem Objekt nur die allgemeinen Daten eines Softwaretyps zu halten, so dass es hier z.B. nur einen Eintrag für eine Office-Software gibt, die in vielen Clients benutzt wird.
...
Beschreibt einen {license-pool} und dessen Zuordnung zu Produkten.
...
IMPORTANT: Gruppen anhand ihrer ID identifiziert. Diese ID muss gruppenübergreifend einzigartig sein.
...
Löscht alle {product-properties} zum Produkt *productid*.
...
Liefert für die angegebenen Clients {installation-status} und {Action-requests} für alle Localboot-Produkte.
...
Liefert für die angegebenen Clients {installation-status} und {Action-request= für alle Netboot-Produkte.
...
Liefert die möglichen {installation-status} (*installed*, *not_installed*, ... ).
...
Liefert den {installation-status} zum angegebenen Client und Produkt.
...
Liefert den {installation-status} zum angegebenen Client.
...
Liefert {installation-status} und {Action-requests} der einzelnen Produkte (zu den agegebenen Clients).
