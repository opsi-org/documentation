...
NOTE: Achten Sie unbedingt darauf, dass Sie vor dem Upgrade die Pakete `opsi-script`, `opsi-client-agent`, `opsi-linux-client-agent` bzw. `opsi-mac-client-agent` auf allen Clients auf den aktuellen Stand bringen!
