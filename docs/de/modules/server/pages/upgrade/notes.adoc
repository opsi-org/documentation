...
Die Aktionen im Produkte-Rechtsklick-Menü wurden überarbeitet. 
"Speichern und Ausführen" und "Speichern und Ausführen (nur ausgewählte Produkte)" beachten jetzt die Konfiguration `opsiclientd.control_server.process_actions_even`.

image::opsi-configed-actions.png[opsi-configed actions]

Die Konfiguration kann folgende Werte annehmen:

- auto: `timer`-Event bei WAN-Modus auslösen, sonst `on_demand` (Standard)
