...
_Speichern und ausführen_ und _Speichern und ausführen (nur ausgewählte Produkte)_ beachten jetzt die Konfiguration `opsiclientd.control_server.process_actions_event`.

.Kontextmenü der rechten Maustaste
image::server:opsi-configed-actions.png["Kontextmenü der rechten Maustaste"]
...
.UEFI-Haken
image::server:opsi-configed-uefi.png["UEFI-Haken"]