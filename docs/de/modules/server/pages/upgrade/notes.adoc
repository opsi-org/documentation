...
TIP: In der control.toml können Sie auch Markdown verwenden. So können Sie z.B. Links in der Beschreibung verwenden, die im opsi-configed klickbar sind.
...
Wenn noch Pakete für opsi 4.1 gebaut werden sollen, dann kann die Kompression auf gz umgestellt werden. Hierfür wird die Datei `/etc/opsi/makepackagemarkeruse_gz` angelegt.
