...
image::redis-cli-save.png["Mit dem Befehl `redis-cli save` erzeugen Sie einen Dump der Redis-Datenbank.", pdfwidth=80%]
...
Alternativ zu den hier gezeigten Befehlen können Sie das Backup auch über die Admin-Seite des `opsiconfd` im Webbrowser erzeugen (siehe Abschnitt xref:server:components/opsiconfd.adoc#server-components-opsiconfd-admin-page-maintenance[*Maintenance*]). Über Checkboxen im Bereich _Create backup_ definieren Sie, ob die Konfigurationsdateien 
