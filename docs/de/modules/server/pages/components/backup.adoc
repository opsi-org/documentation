...
TIP: Weitere Details zum Thema finden Sie in der Redis-Dokumentation, Kapitel link:https://redis.io/docs/latest/operate/oss_and_stack/management/persistence/[Redis persistence,window=_blank].
...
image::server:redis-cli-save.png["Mit dem Befehl *redis-cli save* erzeugen Sie einen Dump der Redis-Datenbank.", width=800, pdfwidth=80%]
...
image::server:opsi-backup-pw.png["Ab OPSI 4.3 können Sie opsi-Backups verschlüsseln und mit einem Passwort sichern.", width=800, pdfwidth=80%]
...
image::server:opsi-webinterface-backup.png["Auf dem Reiter _Maintenance_ der Admin-Seite können Sie auch Backups erzeugen und wiederherstellen.", pdfwidth=80%, width=500]
