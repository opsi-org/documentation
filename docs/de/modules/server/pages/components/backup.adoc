...
== Backup des {opsi-server}s

Es sollte regelmäßig ein Backup des {opsi-server}s durchgeführt werden.
...
Beim Einspielen eines Backups kann die ID des {opsi-configserver}s geändert werden (`--server-id`).
