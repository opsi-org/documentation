...
NOTE: Sie können den `opsiconfd` über Add-ons erweitern. So ist beispielsweise das opsi-WebGUI ein solches Add-on. Mehr dazu lesen Sie in Abschnitt <<server-components-opsiconfd-admin-page-addons>>.
...
* `setup`: Startet ein <<server-configuration-opsiconfd-setup>>.
...
TIP: Alternativ können Sie den Health Check auch mit dem Kommandozeilentool `opsi-cli` aufrufen (siehe Abschnitt ##). Einen schnellen Zugriff auf ein Terminal auf dem opsi-Server bietet die Admin-Seite über den Reiter _Terminal_ (siehe Abschnitt <<server-components-opsiconfd-admin-page-terminal>>).
//TODO: Link zu Kommandozeilenkapitel setzen
