...
TIP: Alternativ können Sie den Health Check auch mit dem Kommandozeilentool `opsi-cli` aufrufen (siehe Abschnitt xref:server:components/commandline.adoc##server-components-opsi-cli-commands-support[*support*]). Einen schnellen Zugriff auf ein Terminal auf dem opsi-Server bietet die Admin-Seite über den Reiter _Terminal_ (siehe Abschnitt <<server-components-opsiconfd-admin-page-terminal>>).
