...
* `health-check`: Startet einen xref:server:components/monitoring.adoc#server-components-opsiconfd-health-check[Health-Check].
...
image::server:opsi-conf.png["Die Konfigurationsdatei */etc/opsi/opsi.conf* legt unter anderem die Server-Rolle fest.", pdfwidth=80%]
...
image::server:opsiconfd/opsiconfd-admin-info.png["Auf dem Reiter *Info* können Sie auch die *opsiconfd*-Konfiguration betrachten.", width=800, pdfwidth=80%]
