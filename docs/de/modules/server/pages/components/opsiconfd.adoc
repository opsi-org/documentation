...
Eine Möglichkeit, einen Health Check zu starten, ist über die <<server-components-opsiconfd-admin-page>>, Reiter _RPC-Interface_. Auf der Kommandozeile rufen Sie den Befehl `opsiconfd health-check` auf. Über den Parameter `--help` blenden Sie eine Onlinehilfe ein; `opsiconfd health-check --manual` gibt eine Beschreibung aller Checks aus. Ohne weitere Optionen läuft der Check einmal durch und schreibt seine Ergebnisse nach Stdout.
...
Alternativ können Sie die Logfiles auf dem Reiter _Log Viewer_ im Admin-Interface anschauen.
