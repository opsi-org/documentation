...
Eine Möglichkeit, einen Health Check zu starten, ist über die <<server-components-opsiconfd-admin-page>>, Reiter RPC-Interface. Auf der Kommandozeile rufen Sie den Befehl `opsiconfd health-check` auf. Über den Parameter `--help` blenden Sie eine Onlinehilfe ein. Ohne weitere Optionen läuft der Check einmal durch und schreibt seine Ergebnisse nach Stdout.
