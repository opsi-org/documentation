...
Der Opsiconfd-Daemon bringt ein Template Shell-Skript mit, das die Ausgabe des Health-Checks in das CheckMK-Format umwandelt. Dieses Skript ist in der Datei opsi_checkmk enthalten, die unter /usr/lib/opsiconfd zu finden ist. Die Datei kann als "local check" in CheckMK eingebunden werden, um die Health-Check-Daten regelmäßig zu erheben und auszuwerten.

Für die Integration kopiert man das Skript nach /usr/lib/check_mk_agent/local/<cache-time>, wobei <cache-time> für das Cache-Intervall steht, in dem die Ergebnisse aktualisiert werden. Nach der Kopie muss das Skript ausführbar gemacht werden, um korrekt zu arbeiten.
...
In diesem Beispiel wird der Health-Check alle 7200 Sekunden (2 Stunden) aktualisiert. Die Zeitspanne kann je nach Bedarf angepasst werden, um eine feinere oder gröbere Granularität bei der Datensammlung zu erreichen.
...
Mit der Funktionalität zur Downtime können Administratoren flexibel auf Wartungsfenster reagieren, ohne das Monitoring unnötig zu belasten oder mit falschen Alarmausgaben zu stören.
