...
Es gibt einige Möglichkeiten die Leistung von Redis in Verbindung mit opsi zu optimieren.

Standardmäßig verbindet dich der opsiconfd über den Port 6379 per TCP/IP mit dem Redis-Server.
Wenn der Redis-Server auf dem gleichen System wie der opsi-Server läuft, kann die Verbindung alternativ auch über einen Unix-Socket erfolgen. Laut link:https://redis.io/docs/management/optimization/benchmarks/[Redis-Dokumentation,window=_blank] erhält man bei Verwendung des Unix-Socket einen etwa 50 % höheren Durchsatz als mit der TCP/IP-Verbindung.

Um die Verbindung über den Unix-Socket zu aktivieren, muss der Parameter `unixsocket` in der Datei `/etc/redis/redis.conf` angepasst werden.
In der `/etc/opsi/opsiconfd.conf` muss der Parameter `redis-internal-url` auf `unix://<pfad-zu-redis-server.sock>` gesetzt werden.
Dabei ist sicherzustellen, dass der opsiconfd-User Zugriffsberechtigungen auf den Socket hat.
