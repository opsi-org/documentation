////
; Copyright (c) uib gmbh (www.uib.de)
; This documentation is owned by uib
; and published under the german creative commons by-sa license
; see:
; https://creativecommons.org/licenses/by-sa/3.0/de/
; https://creativecommons.org/licenses/by-sa/3.0/de/legalcode
; english:
; https://creativecommons.org/licenses/by-sa/3.0/
; https://creativecommons.org/licenses/by-sa/3.0/legalcode
;
; credits: https://www.opsi.org/credits/
////

:Author:    uib gmbh
:Email:     info@uib.de
:Date:      08.05.2023
:Revision:  4.3
:toclevels: 6
:doctype:   book
:icons:     font
:xrefstyle: full

include::common:partial$opsi_terms.adoc[]


[[server-components-opsi-package-manager]]
== opsi-package-manager (opsi-Pakete installieren)

Der _opsi-package-manager_ dient zur Installation und Deinstallation von {opsi-product-packages}n auf einem {opsi-depotserver}.

Beim Aufruf von _opsi-package-manager_ zur Installation muss das zu installierende Paket für den Benutzer _opsiconfd_ lesbar sein.
Es wird daher dringend empfohlen, {opsi-product-packages} einem Unterverzeichnis `/var/lib/opsi` zu installieren.

Es folgen einige Beiepiele für die Verwendung.

Ein opsi-Paket installieren:

[source,shell]
----
opsi-package-manager -i opsi-package_1.0-5.opsi
----

Ein opsi-Paket installieren und interaktives Setzen von Standardwerten für {product-properties}:

[source,shell]
----
opsi-package-manager -p ask -i opsi-package_1.0-5.opsi
----

Paket installieren und für alle Clients auf `setup` stellen, bei denen es installiert ist:

[source,shell]
----
opsi-package-manager -S -i opsi-package_1.0-5.opsi
----

Paket installieren und für alle Clients, inklusive abhängigen Paketen, auf `setup` stellen, bei denen es installiert ist:

[source,shell]
----
opsi-package-manager -s -i opsi-package_1.0-5.opsi
----

Ein opsi-Paket deinstallieren:

[source,shell]
----
opsi-package-manager -r opsi-package
----

Ein opsi-Paket entpacken und dabei umbenennen:

[source,shell]
----
opsi-package-manager -x opsi-template.opsi --new-product-id new-product
----

Eine Übersicht über alle Optionen liefert `opsi-package-manager --help`.
