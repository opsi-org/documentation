...
Statt mit lokalen Unix- und Samba-Accounts zu arbeiten, können Sie einen opsi-Server auch in eine Windows- bzw. Samba-4-Domäne einbinden.

kann ein opsi-Server auch in eine Windows- bzw. Samba4-Domäne eingebunden werden.
...
Setzen Sie dann dann das Passwort des Benutzers `pcpatch` neu, wie im Abschnitt xref:server:components/authorization.adoc#authorization-pcpatch-password[] beschrieben.
