...
Zur Konfiguration der Zwei-Faktor-Authentifizierung gibt es das Konfigurations-Attribut für den `opsiconfd` (siehe Abschnitt xref:server:components/opsiconfd.adoc#server-components-opsiconfd-config[Konfiguration]). Folgende Werte sind möglich:
