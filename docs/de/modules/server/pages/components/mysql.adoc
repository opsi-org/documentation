...
TIP: Alternativ bearbeiten Sie die MySQL-Konfiguration in der Datei `/etc/opsi/backends/mysql.conf` mit einem Texteditor. Nach dem Speichern Ihrer Änderungen führen Sie das Kommando `sudo opsiconfd setup` aus, um die opsi-Datenbank auf dem MySQL-Server zu erstellen bzw. zu aktualisieren.

.Die MySQL-Konfiguration können Sie auch mit einem Texteditor bearbeiten.
image::opsi-backends-mysql.png["Die MySQL-Konfiguration können Sie auch mit einem Texteditor bearbeiten.", pdfwidth=80%, width=600]
