...
Die Konfiguration nehmen Sie entweder über die Managementoberfläche `opsi-configed` vor (siehe Abschnitt xref:configed:configed.adoc#opsi-manual-configed-hostproperties[Host-Parameter: Client- und Serverkonfiguration]) oder auf der Kommandozeile mit dem Tool `opsi-admin` vor (siehe Abschnitt xref:server:components/commandline.adoc[Kommandozeilentools]):
