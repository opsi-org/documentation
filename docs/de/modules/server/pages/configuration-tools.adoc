...
Options:
   ...
   --patch-sudoers-file	        patching sudoers file for tasks in {opsi-admin} context.
----
...
(_{opsi-configserver}_) als Depotserver anzumelden. Details hierzu vgl. xref:opsi-modules:multidepot.adoc#opsi-manual-multidepot-installation[Erstellung und Konfiguration eines Depot-Servers].
...
Details vgl. xref:opsi-backends.adoc#opsi-manual-backends-mysql-initializing[Initialisierung des MySQL-Backends].
...
{opsi-products} aus einem oder mehreren Repositories zu laden und auf dem Server zu installieren.
