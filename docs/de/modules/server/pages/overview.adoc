...
In diesem Kapitel geht es um den opsi-Server, die zentrale Komponente des Client-Management-Systems. Das Kapitel xref:server:installation/installation.adoc[Installation] beschreibt verschiedene Möglichkeiten, einen opsi-Server zu installieren. In xref:server:components/components.adoc[Tools & Settings] geht es dann um den Betrieb (Konfiguration und Werkzeuge).
