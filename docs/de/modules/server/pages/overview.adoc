...
Seit Version 4.2 wird die In-Memory-Datenbank Redis eingesetzt (https://redis.io/). Der  speichert folgende Daten in Redis:
...
Die Installation und Inbetriebnahme eines  ist in dem gesonderten Handbuch:*opsi-getting-started*ausführlich erläutert.
...
Der *opsiconfd* wird über die Datei `/etc/opsi/opsiconfd.conf`, über Umgebungsvariablen oder über Kommandozeilen-Parameter konfiguriert.
...
* User  +
Dies ist der user, den der  verwendet um den  zu mounten und von diesem zu lesen.
...
Ehemals hieß diese Gruppe , seit opsi 4.2 wird standardmäßig  als Gruppenname verwendet. Wird eine bestehende opsi-Umgebung auf opsi 4.2 aktualisiert wird der verwendete Gruppenname jedoch beibehalten.

Bei Anbindung des {opsi-server}s an ein Active Directory muss in jedem Fall der Gruppenname  verwendet werden.
...
* Bereich:  mit Softwarepaketen (*opsi_depot*) +
