...
** *True*: Es wird ausschließlich der opsi-Messgebus verwendet. +
Wenn ein Client nicht mit dem opsi-Messagbus verbunden ist, gilt er als nicht erreichbar und das Kommando wird nicht ausgeführt.
Wenn alle Clients so konfiguriert sind, dass sie den opsi-Messagebus verwenden, ist das die bevorzugte Einstellung.
** *hybrid* (Standardwert): Der opsi-Messgebus wird verwendet, wenn der Client eine aktive Messagbus-Verbindung hat. +
...
* *maxConnections*: +
Die maximale Anzahl simultaner Verbindungen zu {opsi-client-agent}s.
