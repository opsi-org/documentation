...
Weitere Informationen hierzu finden Sie unter <<server-configuration-authorization-domain-join>> und <<server-configuration-authorization-ldap-authentication>>.
...
WARNING: Verwenden Sie in beiden Passwörtern kein `§`-Zeichen!
...
Setzen Sie dann dann das Passwort des Benutzers `pcpatch` neu, wie im Abschnitt <<server-configuration-authorization-pcpatch-password>> beschrieben.
