...
Weiterhin sind hier Backend-Erweiterungen untergebracht, die Konfigurationen des _opsipxeconfd_, des _opsi-package-updater_ mit seinen Repositories und auch die modules-Datei, die Ihnen Ihre kofinanzierten Module freischaltet.
...
WARNING: Ein Backup, dass mit `opsi-backup` erstellt wird, kann unter anderem Passwörter und PC-Keys enthalten, und sollte daher entsprechend sicher archiviert werden.
...
----
+
WARNING: Wenn Sie seit der Erstellung des Backups das Backend gewechselt haben, so wird die default Einstellung keine Daten zurück sichern.
