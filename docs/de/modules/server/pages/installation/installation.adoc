...
Danach können Sie die xref:getting-started:server/configuration.adoc#opsi-getting-started-config-update-opsi-product[opsi-Standard-Produkte einspielen].

////
* Lizenz einspielen
* Nehmen Sie die xref:server:configuration/configuration.adoc#configuration-base-configuration[Grundkonfiguration des {opsi-server}s] vor.
* xref:server:components/opsi-package-update.adoc#server-components-opsi-package-updater-install-default-products[opsi-Standard-Produkte einspielen]
* Die Management-Oberfläche *opsi-configed* starten Sie in der virtuellen Maschine über einen Doppelklick auf das Desktopsymbol . Weitere Informationen zu dieser grafischen Anwendung finden Sie in xref:configed:configed.adoc[Management-Oberfläche `opsi-configed`].
* Backup einrichten
