...
Führen Sie die Installation nach einer der folgenden Anleitungen durch:
...
* xref:server:installation/ucs.adoc[Installation auf Univention Corporate Server (UCS)]
...
== Nächste Schritte
// TODO
Nach der Installation ist der opsi-Server nun bereit für die nächsten Schritte.
Diese unterscheiden sich, je nachdem ob der opsi-Server die Rolle eines opsi-Configservers oder eines opsi-Depotservers übernehmen soll.

=== opsi-Configserver
* Sollten Sie kostenpflichtige xref:opsi-modules:modules.adoc[opsi-Erweiterungen] einsetzen wollen, xref:server:components/opsiconfd.adoc#server-components-opsiconfd-admin-page-licensing[spielen Sie jetzt Ihre Lizenz-Dateien ein].
...
=== opsi-Depotserver
Soll der Server die Rolle eines opsi-Depotservers übernehmen, dann muss der opsi-Depotserver nun am opsi-Configserver registriert werden.

NOTE: Beim Betrieb des opsi-Depotservers als Docker-Container wir dies über Umgebungsvariablen gesteuert und dieser Schritt entfällt.
