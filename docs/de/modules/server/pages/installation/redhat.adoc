...
Wenn SELinux aktiviert wird der Start von Redis verhindert. 
Um Redis ordnungsgemäß auszuführen muss eine Ausnahme für SELinux eingerichtet werden.
Hierfür wird die Datei redis-server.te angelegt. Diese Datei beschreibt die Ausnahmen.
...
Als nächstes wird mit den folgenden Befehlen eine Policy Moudle Datei erzeugt:
...
Anschließend kann das Modul mit `semodule -i redis-server.pp` aktiviert werden.