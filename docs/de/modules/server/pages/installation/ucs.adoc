...
Nach dem Hinzufügen der Repositories können Sie die Installation starten:
...
Benutzer müssen Mitglied der Gruppe `{opsi-admin}` sein, um den *opsi-configed* verwenden zu können. Während der Installation, wird der Benutzer *Administrator* automatisch zu dieser Gruppe hinzugefügt. Andere Accounts fügen Sie über das UMC-Modul *Benutzerverwaltung*, Reiter *Gruppen* hinzu.
