...
image::server:oqi.png["Wählen Sie die Sprache und die Art der Installation aus.", pdfwidth=80%]
...
image::server:oqiInfo.png["Fahren Sie mit der Maus über das kleine Symbol, um weitere Informationen einzublenden.", width=800, pdfwidth=80%]
...
image::server:quickinstall_l-opsi-server.png["Die opsi-Server-Installation läuft.", pdfwidth=80%]
...
image::server:oqiFailed.png["Falls die Installation fehlschlägt, finden Sie in den Logdateien mögliche Fehlerursachen.", pdfwidth=80%]
...
image::server:quickinstallNoGuiFailed.png["Das Setup-Programm gibt Hinweise auf Fehler und Logdateien.", width=800, pdfwidth=80%]
