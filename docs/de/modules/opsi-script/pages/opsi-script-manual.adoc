...
Die Details zu den genannten Befehlen der Library finden sich hier: <<Doc_fileuib_lin_install.opsiscript, Documentation of opsi library: `uib_lin_install.opsiscript`>>
...
Die Details zu den genannten Befehlen der Library finden sich hier: <<Doc_fileuib_macosinstalllib.opsiscript, Documentation of opsi library: `uib_macosinstalllib.opsiscript`>>
...
Eine String-Liste (oder ein String-Listenwert) ist eine Abfolge von String-Werten. Der Wert einer Stringliste umfasst also alle Strings dieser Liste. Für diese Werte gibt es die Variable der String-Listen. Sie sind wie folgt definiert
...
Im Gegensatz zu der Funktion `GetProductProperty` welche in diesem Fall die einzelnen Werte als einen kommaseparierten String zurückliefert. Dieses Vorgehen ist aber problematisch wenn Kommas auch in der Werten vorkommen. +
...
liefert die Stringliste <list> mit dem Encoding <to> zurück. Dabei wird davon ausgangen, dass <list> gemäß <from> encoded war. <from> und <to> sind dabei Encodings wie sie im Kapitel <<opsi-script-encoding, '{opsi-script}' encoding>> aufgelistet sind.
...
include::opsi-docs-en:common:partial$opsi-doc-generator.adoc[]
