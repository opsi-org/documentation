...
for `DosInAnIcon` (`ShellInAnIcon`),`ExecWith` and `ExecPython` calls. Siehe auch xref:prim-section#getOutStreamFromSection[getOutStreamFromSection]

* `getreturnListFromSection(<section with params>)` +
for `XMLPatch` sections and `opsiServiceCall` sections. Siehe auch xref:prim-section#getReturnListFromSection[getReturnListFromSection]
...
**  `c` ->  continue [W] +
Wenn eine Datei in Benutzung ist, kann sie im Rahmen eines Reboots gelöscht werden. Das _{opsi-script}_ default-Verhalten ist dabei, dass ein Datei in Benutzung zum Löschen beim nächsten Reboot markiert wird UND die _{opsi-script}_ Reboot Markierung gesetzt wird. Das Setzen der Option `-c` stellt den automatischen Reboot aus. Das Löschen wird in diesem Fall erst dann vervollständigt, wenn ein Reboot auf eine andere Weise ausgelöst wird.
...
see : xref:prim-section#opsi-script-xml2-functions[XML2 Funktionen] +
