...
** Beschreibung: (Blacklist) Welche OPSI-Produkte sollen vom Update ausgeschlossen werden? +
Liste von Produkten, die nicht installiert werden sollen, auch wenn ein Update verfügbar ist (z. B. windomain).

* Name:`products_to_exclude_by_regex`
** Beschreibung: (Blacklist) Welche OPSI-Produkte sollen anhand von Regular Expressions vom Update ausgeschlossen werden? +
Liste von Regular Expressions, die auf Produkte zutreffen, welche nicht installiert werden sollen, auch wenn ein Update verfügbar ist (z. B. windomain).

* Name:`products_to_include`
** Beschreibung: (Whitelist) Welche OPSI-Produkte sollen auf Updates geprüft werden? Ist die Liste leer, werden alle Produkte geprüft. +
Hier kann eine Liste von Produkten angegeben werden, auf die die Update-Prüfung beschränkt werden soll. Produkte, die nicht in dieser Liste enthalten sind, werden bei der Prüfung auf Updates nicht berücksichtigt. Ausnahme: Ist die Liste leer, werden alle Produkte geprüft.
Properties zur Behandlung von Ergänzungen

* Name:`products_to_run_always`
** Beschreibung: Welche OPSI-Produkte sollen bei jedem Update installiert werden? (Die Liste wird nach dem Lauf nicht geleert.) +
Liste von Produkten, die bei jedem Lauf von opsi-auto-update auf Setup gesetzt werden.

* Name:`setup_after_install`
** Beschreibung: Welche OPSI-Produkte sollen nach Abschluss des Updates auf Setup gesetzt werden? +
Liste von Produkten, die nach jedem Lauf von opsi-auto-update auf Setup gesetzt werden (z. B. shutdownwanted).

* Name:`failed_products_to_setup`
** Beschreibung: Wenn True, werden alle fehlgeschlagenen Produkte auf allen Clients erneut auf Setup gesetzt. +
Ist diese Property auf True gesetzt, werden alle Produkte mit dem Status failed auf allen Clients erneut auf Setup gesetzt.
Properties zur Ablaufsteuerung
* Name:shutdown_on_finish
** Beschreibung: Wenn True, wird der Computer nach Abschluss heruntergefahren. Bei False erfolgt weder ein Neustart noch ein Herunterfahren. default: False. +
Legt fest, ob der Computer nach Abschluss des Produkts heruntergefahren werden soll.
...
Liste von Produkten, die nach jedem Lauf von opsi-auto-update auf Setup gesetzt werden (z. B. shutdownwanted).
////
Spezielle Properties für local-image / vhd-reset
...
* Name:`do_cleanup`
** Beschreibung: Bei False wird das Restore vor dem Update übersprungen. +
Diese Property wird ignoriert, wenn es sich nicht um eine vhd- oder local-image-Installation handelt.
Bei einer vhd-Installation wird bei do_cleanup=True vor den Updates opsi-vhd-control ausgeführt. Dadurch werden alle Änderungen verworfen und der gespeicherte Zustand wiederhergestellt.
Bei einer local-image-Installation wird bei do_cleanup=True vor den Updates opsi-local-image-restore ausgeführt. Dadurch werden ebenfalls alle Änderungen verworfen und der gespeicherte Zustand wiederhergestellt.
In beiden Fällen werden dabei auch die Informationen über Actionrequests verworfen. Damit während eines Laufs von opsi-auto-update dennoch Produkte hinzugefügt oder entfernt werden können, stehen die folgenden beiden Properties zur Verfügung.

* Name:`products_to_install`
** Beschreibung: Welche OPSI-Produkte sollen während des Updates installiert werden? (Die Liste wird nach dem Lauf geleert.) +
Liste von Produkten, die während eines Laufs von opsi-auto-update auf Setup gesetzt werden. Wurden die Produkte erfolgreich verarbeitet, werden sie anschließend aus dieser Liste entfernt.

* Name:`products_to_uninstall`
** Beschreibung: Welche OPSI-Produkte sollen während des Updates deinstalliert werden? (Die Liste wird nach dem Lauf geleert.) +
Liste von Produkten, die während eines Laufs von opsi-auto-update auf Uninstall gesetzt werden. Wurden die Produkte erfolgreich verarbeitet, werden sie anschließend aus dieser Liste entfernt.
* Name:`do_merge`
** Beschreibung: Bei False wird das Backup nach dem Update übersprungen. +
Diese Property wird ignoriert, wenn es sich nicht um eine vhd- oder local-image-Installation handelt.
Bei einer vhd-Installation wird bei do_merge=True nach den Updates opsi-vhd-control mit upgrade=True aufgerufen. Dadurch werden alle Änderungen gespeichert.
Bei einer local-image-Installation wird bei do_merge=True nach den Updates opsi-local-image-backup aufgerufen. Dadurch werden ebenfalls alle Änderungen gespeichert.
...
Name: driver_path +
** Beschreibung: Pfad zum Treiberverzeichnis. +
auto = Verwendung des Treiberverzeichnisses aus dem Netboot-Produkt-Driver-Repository. +
default: auto
Name: netbootproduct +
** Beschreibung: Name des Netboot-Produkts, aus dem im Modus driver_path=auto die Treiber bezogen werden. +
auto = Es wird versucht, das verwendete Netboot-Produkt automatisch zu erkennen. +
default: auto
Name: force_import_cert_from_sys +
** Beschreibung: Wenn True, können auch nicht korrekt signierte Treiber installiert werden. Dazu werden die Zertifikate aus der .sys-Datei extrahiert und in den Zertifikatsspeicher importiert. +
default: False
Name: force_reinstallation +
** Beschreibung: Wenn True, wird versucht, den Treiber erneut zu installieren, auch wenn die im Repository enthaltene Version bereits installiert zu sein scheint. +
default: False
...
Produkt um bestimmte Windows 10 Releases durch die Installation eines Hotfixes auf auf eine höhere Version upzugraden, ohne ein komplettes Inplace-Upgrade durchführen zu müssen. +
Aktualisiert Windows 10 1903 auf 1909 oder Windows 10 2004, 20H1 und 21H1 auf Version 21H2

[[opsi-manual-localboot-windows11-enablement]]
=== windows11-enablement

Produkt um bestimmte Windows 11 Releases durch die Installation eines Hotfixes auf eine höhere Version upzugraden, ohne ein komplettes Inplace Upgrade durchführen zu müssen. +
Aktualisiert Windows 11 22H2 auf Version 23H2
...
Ist für ein Produt die Aktion `uninstall` gesetzt, wird hierbei die Priorität für dieses Produkt negiert.
