...
Die Abläufe und Einstellungen des Produktes `opsi-local-image-wim-capture` entsprechen denen von `opsi-wim-capture` (siehe xref:opsi-modules:wim-capture#opsi-manual-wimcap[Windows Imaging Format (opsi-wim-capture)]). Die Propertys von `opsi-wim-capture` sind in Abschnitt xref:opsi-modules:wim-capture#opsi-manual-wimcap-products-main[Hauptprodukt opsi-wim-capture] beschrieben.
