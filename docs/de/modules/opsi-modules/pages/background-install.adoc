...
Alle `\*.exe` und `*.bin` innerhalb des Installationsverzeichnis sind potentiell bedenkliche Prozesse.

* Über die Metadaten des Produktes können noch weitere Prozesse angegeben und Verzeichnisse angegeben werden.

Von der Liste der potentiell bedenklichen Prozesse, werden aber nur die als tatsächlich bedenkliche Prozesse angesehen, welche ein sichtbares Fenster haben, also Foregroundprozesse sind. +
Die Erfahrung hat gezeigt, das Dienstprozesse und Prozesse ohne sichtbares Fenster von den Installationsprogrammen korrekt und nicht interaktiv behandelt werden.
...
** Für den eingeloggten User werden die alle laufenden Prozesse mit einem sichtbaren Fenster ermittelt: +
`Die Liste der laufenden Vordergrundprozesse`.

** Aus der `Liste kritischer Prozesse` werden werden alle Einträge entfernt, welche sich nicht auch in der `Die Liste der laufenden Vordergrundprozesse` finden.
...
* Um Metadaten für eine Setup- / Installerdatei zu erzeugen, muß das Häkchen bei `Installer 1 Aktiv` gesetzt sein.
...
* Das Feld `Liste der zu prüfenden Prozesse` ist eine Liste welche pro Zeile einen Prozessnamen enthält
...
das bestehende Produkt zu ermitteln. Dafür wird nach einer `opsi-setup-detector Projektdatei`
...
Danach muß das Produkt neu gebaut werden (`opsi-make-package`) und installiert werden. +
...
Hierzu müssen die folgenden Änderungen an der Standardkonfiguration des Events vorgenommen werden:
...
*Vorteil*: Geeignet für `Roaming Profiles`. +
*Nachteil*: Neues Event muß konfiguriert werden.

Die Alternative 2: +
*Installation vom User gesteuert über den opsi-client-systray* +
Dazu muß der `opsi-client-agent` auf dem Rechnern mit dem Property `systray_install=true` installiert sein. Sinnvoll ist es auch den Wert des Property `systray_check_interval` auf einen kleineren Wert zu setzen. Dieses Property gibt an alle wieviel Minuten auf anstehende Installationen geprüft werden soll und informiert den User gegebenenfalls über anstehende Installationen. +
Der Anwender kann dann über das Kontextmenüs des `opsi-client-systray` die Installation selbst starten.

*Vorteil*: Geeignet für `Roaming Profiles`. +
*Nachteil*: Keine Installationen, wenn der User diese nicht startet.
...
Die folgenden Produkte sind mit Stand 23.02.2026 noch *nicht* für die Backgroundinstallation zugelassen:
