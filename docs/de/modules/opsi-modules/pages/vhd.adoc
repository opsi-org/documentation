...
`opsi-vhd-reset` kommt im Bundle mit der Erweiterung `opsi-local-image` (siehe xref:local-image#opsi-manual-localimage[Lokale Images (`opsi-local-image`)]), das heißt, dass die Lizenz von `opsi-local-image` automatisch auch für `opsi-vhd-reset` gilt.
...
. initiale Windows-11-Installation in einem VHD-Container
...
Installieren Sie mit dem Produkt `opsi-vhd-win11-x64` ein Windows-11-System in einem VHD-Container.

.Schema: Partitionieren und VHD erstellen (`opsi-vhd-win11-x64`)
image::opsi-modules:opsi-vhd-inst1.png["Schema: Partitionieren und VHD erstellen (`opsi-vhd-win11-x64`)", width=332]

.Schema: Windows installieren (`opsi-vhd-win11-x64`)
image::opsi-modules:opsi-vhd-inst2.png["Schema: Windows installieren (`opsi-vhd-win11-x64`)", width=332]

Anschließend können Sie auf diesem Windows-System die gewünschten Applikationen installieren.

.Schema: Anwendungssoftware installieren (`opsi-vhd-win11-x64`)
image::opsi-modules:opsi-vhd-inst3.png["Schema: Anwendungssoftware installieren (`opsi-vhd-win11-x64`)", width=332]
...
.Schema: Windows-PE-Partition aktivieren (`opsi-vhd-win11-x64`)
image::opsi-modules:opsi-vhd-inst4.png["Schema: Windows-PE-Partition aktivieren (`opsi-vhd-win11-x64`)", width=332]
...
image::opsi-modules:opsi-vhd-control-1stsnap.png["Schema: Initiale Installation versiegeln (`opsi-vhd-control`)", width=332]

Ab sofort landen alle Änderungen in der Child-VHD.

.Schema: Arbeiten mit dem versiegelten System
image::opsi-modules:opsi-vhd-control-work.png["Schema: Arbeiten mit dem versiegelten System", width=332]
...
image::opsi-modules:opsi-vhd-control-activatepe.png["Schema: Initiale Installation wiederherstellen, Teil 1 (`opsi-vhd-control`)", width=332]
...
image::opsi-modules:opsi-vhd-control-resnap.png["Schema: Initiale Installation wiederherstellen, Teil 2 (`opsi-vhd-control`)", width=332]
...
NOTE: Folgende Propertys fehlen dem Netboot-Produkt `opsi-vhd-win11-x64`:
