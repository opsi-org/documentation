...
* +pause_on_metered+ (boolean): Wenn auf +true+ gesetzt, pausiert das Produkt-Caching bei getakteten oder getrennten Netzwerken und startet wieder bei ungetakteten Verbindungen. Diese Option funktioniert nur auf Windows-Clients und nutzt die Windows Runtime API, um den Netzwerkstatus zu prüfen. Eine Verbindung gilt als getaktet, wenn der Netzwerkkostentyp nicht "unrestricted" ist, das Datenlimit erreicht oder fast erreicht ist oder Roaming aktiv ist.
...
[source,shell]
----
opsi-cli jsonrpc execute config_createUnicode opsiconfd.transfer.slots_opsiclientd_product_sync "Maximale Anzahl gleichzeitiger Produktsynchronisationen" 100 100 true false
----
...
[source,shell]
----
opsi-cli jsonrpc execute config_createBool opsiclientd.event_gui_startup.active "gui_startup active" true
opsi-cli jsonrpc execute config_createBool opsiclientd.event_gui_startup{user_logged_in}.active "gui_startup{user_logged_in} active" true
opsi-cli jsonrpc execute config_createBool opsiclientd.event_net_connection.active "event_net_connection active" false
opsi-cli jsonrpc execute config_createBool opsiclientd.event_timer.active "event_timer active" false
----
...
[source,shell]
----
opsi-cli jsonrpc execute configState_create opsiclientd.event_gui_startup.active vpnclient.domain.de false
opsi-cli jsonrpc execute configState_create opsiclientd.event_gui_startup{user_logged_in}.active vpnclient.domain.de false
opsi-cli jsonrpc execute configState_create opsiclientd.event_net_connection.active vpnclient.domain.de true
opsi-cli jsonrpc execute configState_create opsiclientd.event_timer.active vpnclient.domain.de true
----
...
image::opsi-modules:ospclientd-infopage-wan-cached.png["ospclientd-infopage-wan-cached",width=400]
