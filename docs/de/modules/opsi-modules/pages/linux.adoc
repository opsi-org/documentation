...
und einer xref:opsi-modules:modules.adoc[kostenpflichtigen Erweiterung] (dem Agent für die Clients).
...
Der opsi-linux-client-agent beinhaltet 15 Freistarts bei denen der Agent auch ohne Lizenz verwendet werden kann.

Genauer formuliert: Nach der initalen Installation des opsi-linux-client-agent kann der der opsi-script 15 mal im Servicekontext gestartet werden ohne eine Lizenz zu fordern. +
...
Für eine dauerhafte Pflege des installierten Linuxrechners über diese 15 Freistarts hinaus benötigen Sie aber eine Lizenz dieses Features.
...
* `backend`: +
Welches Backend soll installiert werden ? (Die Auswahl `mysql` benötigt die Hinterlegung einer gültigen Lizenz-Datei). (Default='file') +
Eine Lizenz-Datei (`.opsilic`) mit den benötigten Freischaltungen kann im custom Verzeichnis des Produktes abgelegt werden.
Wird dort eine Lizenz-Datei gefunden so wird diese verwendet.
...
In dem Verzeichnis `custom` des Produktes `l-opsi-server` kann eine Lizenz-Datei (`modules`) abgelegt werden, welche bei der Installation durch das Produkt `l-opsi-server` verwendet wird und beim Einspielen einer neuen Version des Produktes erhalten bleibt.
