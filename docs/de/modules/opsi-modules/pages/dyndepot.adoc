...
Die Eigenschaften eines Depots werden zum Teil abgefragt, wenn ein opsi-server über den Befehl `opsiconfd setup --register-depot` als Depot registriert wird (siehe xref:multidepot#opsi-manual-multidepot-installation[Erstellung und Konfiguration eines Depot-Servers]).
