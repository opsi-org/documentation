...
== Konfiguration des {opsi-server}s für Secure-Boot-Clients
...
In der Management-Oberfläche {opsi-configed} setzen Sie für UEFI-Clients ein Häkchen bei _UEFI-Boot_. Alternativ konfigurieren Sie den {opsi-config-object} `clientconfig.dhcpd.filename` für die Clients und tragen dort die Bootdatei ein:
...
WARNING: Die Angabe der Datei `elilo.efi` dient lediglich zum Aktivieren der Option _UEFI-Boot_ in {opsi-configed}. Sie hat keinen Einfluss auf die Verwendung der DHCP-Bootdatei. In zukünftigen Versionen soll {opsi-configed} die korrekte Datei akzeptieren und eine Secure-Boot-Checkbox aktivieren.
