...
image::opsi-modules:osd_config_dlg_de.png["Konfigurationsdialog", pdfwidth=40%]
...
image::opsi-modules:osd_page_start_de.png["Startpage", pdfwidth=90%]
...
Das kann dazu führen, dass Sie mit wenigen Klicks ein fertiges OPSI-Paket bekommen. Häufig bekommen Sie ein OPSI-Paket dem nach erstmaliger Installation der Programms (z.B. mit dem erstellten OPSI-Paket) noch weite Informationen hinzugefügt werden müssen.
...
image::opsi-modules:osd_page_create_modes_with_opb_de.png["Create and Build Modes", pdfwidth=70%]
...
Im Falle von `Erstelle OPSI Produkt Dateien und baue OPSI Paket` wird versucht den opsi-webservice aufzurufen um das vom opsi-setup-detector erstellte Verzeichnis mit den OPSI Dateien zu einem Paket zu bauen. Dabei kann über die Checkboxen 'Bau Modus' ausgewählt werden, ob das Paket gebaut werden soll und ob das gebaute Paket auch installiert werden soll. +
Sollte der Aufruf des opsi-webservice nicht möglich sein, so wird versucht den
`opsi-package-builder` im Hintergrund aufgerufen um den Paketbau gemäß den Checkboxen 'Bau Modus' auszulösen.

Im Falle von '... und starte interaktiven opsi-package-builder' wird versucht den `opsi-package-builder` interaktiv zu starten.
...
image::opsi-modules:osd_page_setup2_de.png["Ergebnis der Analyse 2", pdfwidth=90%]
...
// include docu: create-winget package
include::common:partial$osd-task-create-winget.adoc[]
...
Der opsi-package-builder kann verwendet werden um via ssh die OPSI Pakete auf dem opsi-server zu bauen. siehe auch:
https://forum.opsi.org/viewtopic.php?f=22&t=7573 +
Besser ist das Bauen über die Verbindung zum opsi-server.
...
Aktiviert die  Unterstützung für die Erweiterung xref:opsi-modules:background-install.adoc#opsi-manual-background-install[opsi-background-install]: +
...
* *LastLanguage* : +
Der Defaultwert ist `auto`: Die Sprache wir automatisch aus dem Betriebssystem ermittelt. +
Wenn Sie die Sprache ändern, wird dies hier gespeichert und wieder verwendet.

[[osd_config_preferMsiUninstall]]
...
https://explore.transifex.com/opsi-org/opsiorg/ +
