...
Die Logdatei wird sehr groß, weil sie die kompletten Daten der Analyse der Setupdatei(en) enthält. Zur Analyse der Logdatei empfehlen wir den `opsi-log-viewer` oder einen anderen Editor welcher die nicht benötigten Log-Level ausblenden kann, da die im Setup gefundenen Textmuster (auf Log-Level 8) in vielen Fällen nicht intressieren.
