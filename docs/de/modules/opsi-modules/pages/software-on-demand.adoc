...
*Konventionen:* Wird hier allgemein vom opsi-client-kiosk/agent gesprochen bezieht sich das auf alle OS Versionen (Windows/Linux/macOS) wenn nichts anderes erwähnt ist. Wird von Paketen bzw. Produkten gesprochen dann ist das opsi-client-kiosk/agent Paket/Produkt das Windows Paket bzw. Produkt. Das l-opsi-client-kiosk bzw. opsi-linux-client-agent Paket/Produkt das Linux Paket bzw. Produkt und das m-opsi-client-kiosk bzw. opsi-mac-client-agent Paket/Produkt das macOS Paket bzw. Produkt
...
opsi-client-agent und opsi-configed benötigt. Wir empfehlen OPSI 4.3 und die aktuellen OPSI 4.3 Pakete zu verwenden.
...
* software-on-demand.active (veraltet, hat für den opsi-client-kiosk ab Version >=4.1.1.6-5 keine Relevanz mehr und sollte gelöscht werden falls noch vorhanden)
...
Diese Config-Variablen werden beim Einspielen des opsi-server-Paketes bzw. opsi-client-kiosk Paketes angelegt. Sollten diese Einträge nicht oder falsche vorhanden sein können diese auch manuell serverseitig angelegt bzw. bearbeitet werden, z.B. über den opsi-configed bei Server-Konfiguration.
...
Gegebenenfalls muss hierzu der Config erstmal serverseitig angelegt werden, z.B. über den opsi-configed bei Server-Konfiguration.
Der Wert für den Config `opsiclientd.control_server.process_actions_event` kann dann über den opsi-configed gesetzt werden (z.B. auf `on_demand`).
...
Dafür nutzt man im linken Bereich den Reiter "Produkt Gruppen" und im rechten Bereich den Reiter "Localboot-Produkte".

TIP: Seit Version 4.0.1.6 des _opsi-configed_ kann man direkt zur
Produktkonfiguration wechseln, ohne vorher einen Client auszuwählen. Wieviele Clients gerade ausgewählt sind, sieht man auf der unteren Leiste links: hinter "Anzahl Clients" folgt "In Tabelle ausgewählt".

.Ausschnitt von der Produktgruppen-Leiste
image::opsi-modules:configed_productgroup.png[]

Produktgruppen werden durch rechtsklick auf "Gruppen" im linken Bereich hinzugefügt. +

Produkte können per Drag-and-Drop in eine Gruppe gezogen werden. Dies ist sowohl aus „Alle Produkte“ im unteren linken Bereich als auch aus dem rechten Bereich im Reiter „Localboot-Produkte“ möglich, wenn links „Alle Produkte“ ausgewählt ist. +

Um ein Produkt aus einer Gruppe zu entfernen, klicken Sie mit der rechten Maustaste auf das Produkt und wählen anschließend „Auswahl aus Gruppe entfernen“. +

Die Zuordnung von Produktgruppen zu Clients erfolgt über die Host-Parameter. Wählen Sie dazu zunächst einen oder mehrere Clients aus und wechseln Sie anschließend im rechten Bereich zum Reiter „Host-Parameter“. Unter „software-on-demand“ finden Sie die entsprechenden Konfigurationsmöglichkeiten. Siehe hierzu auch die folgenden Abschnitte.
...
|software-on-demand.active|Hat für den opsi-client-kiosk ab Version >=4.1.1.6-5 keine Relevanz mehr und sollte gelöscht werden. |true/false
...
|software-on-demand.disable-tilesview|Default-Wert ist false. Schaltet bei Bedarf die Kachelansicht ab (setze dafür den Wert auf true). Sinnvoll wenn mehrere hundert Softwareprodukte mit dem Kiosk verwaltet werden sollen.|true/false
|==========================
...
image::opsi-modules:configed_serverconfiguration.png[]

TIP: Natürlich ist eine Bearbeitung auch über die opsi-python-API oder über `opsi-admin` oder `opsi-cli` möglich.
...
image::opsi-modules:configed_hostparameter.png[]
...
== opsi-client-kiosk Anwendung

*Windows*

Mit der opsi-client-agent Version (>=4.1.1.6-5) steht der Kiosk als eigenständiges Produkt mit dem Namen `opsi-client-kiosk` für Windows zu Verfügung.
...
Der opsi-client-kiosk steht auch unter macOs zur Verfügung als `m-opsi-client-kiosk` Paket bzw. Produkt.
...
Kiosk als eigenständiges Produkt (opsi-client-kiosk bzw. l/m-opsi-client-kiosk): +
`/var/lib/opsi/depot/(l/m-)opsi-client-kiosk/files/app/ock_custom` +

*Erstinstallation / Installation über ein schon vorhandenes (l-)opsi-client-kiosk Produkt*

Installieren sie das aktuelle (l/m-)opsi-client-kiosk Paket über den opsi-package-updater oder opsi-package-manager auf dem gewünschten Depot und aktualisieren Sie gegebnenfalls auch das opsi-(linux/mac)-client-agent Paket.
Zu finden sind die Kiosk-Pakete unter https://opsipackages.43.opsi.org/stable/windows/localboot/ bzw. https://opsipackages.43.opsi.org/stable/linux/localboot/ oder https://opsipackages.43.opsi.org/stable/macos/localboot/
...
Die Installation lässt sich über Properties des Produktes (l/m-)opsi-client-kiosk modifizieren:

* `startmenue_entry` (Windows/Linux) +
Steuert den Namen des Startmenü Eintrags. +
Default=`software on demand`; Editierbar

* `startmenue_folder` (Windows) +
...
Legt fest ob eine Benutzerauthentifizierung beim Start des Kiosks erfolgen soll (nur Windows). Default-Wert ist false. In der Regel ist false die gewünschte Einstellung.
...
image::opsi-modules:opsi-client-kiosk_hauptfenster.png["Hauptfenster mit Produktkacheln", width=500]
...
.Suchfeld des Kiosks
image::opsi-modules:opsi-client-kiosk_suchfeld.png["Suchfeld des Kiosks", height=60]
...
image::opsi-modules:opsi-client-kiosk_produktansicht.png["Detaillierte Produktansicht", width=500]
...
image::opsi-modules:opsi-client-kiosk_dialog_bestaetigung.png["Dialog zur Bestätigung der gewünschten Aktion"]

[[ock_image_dialog_timechoice]]
.Kiosk - Dialog zur Auswahl des Zeitpunktes der Durchführung der gewünschten Aktion.
image::opsi-modules:opsi-client-kiosk_dialog_zeitpunkt.png["Dialog zur Bestätigung der gewünschten Aktion"]
...
image::opsi-modules:opsi-client-kiosk_hilfe.png["Hilfefenster"]
...
image::opsi-modules:opsi-client-kiosk_expertenmodus.png["Kiosk - Expertenmodus (Liszenansicht)", width=500]
...
image::opsi-modules:opsi-client-kiosk_aktionsfenster.png["enster zur sofortigen Aktionsausführung"]
...
image::opsi-modules:opsi-client-kiosk_adminmodus.png["Kiosk - Admin mode"]
