...
Ab Version 39 benutzt der Directory Connector den opsi-Logger (Log-Level 0 bis 9). In der Voreinstellung protokolliert die Anwendung nach `/var/log/opsi-directory-connector` bzw. schreibt Fehler nach `stderr`. Um den Log-Level für die beiden Protokolle zu beeinflussen, verwenden Sie die Parameter `--log-level-stderr` bzw. `--log-level-file`. Der Schalter `--log-file` definiert ein anderes Logfile.
