...
Um die Erweiterung installieren zu können, benötigen Sie die Pakete `opsi-clonezilla` und `win10-sysprep-app-update-blocker`
...
Ein WIM (*Windows Imaging Format*) Image ist kein Platten- oder Partitionsimage, sondern mehr ein Dateien und Metadaten Archiv. Eine WIM Datei kann mehrere Images enthalten. Die normale Installation eines NT6 Rechners basiert darauf, dass die setup.exe ein Image aus der Datei install.wim auspackt, und dieses danach konfiguriert und mit zusätzlichen Treibern versieht.
...
TIP: `opsi-wim-capture` unterstützt auch die Erweiterung `install.esd` (statt `install.wim`) als Format im Target.
...
image::opsi-modules:opsi-wim-cap-pre1.png["Schema: Installation des Orginal Windows auf der Systempartition", width=332]
...
image::opsi-modules:opsi-wim-cap-pre2.png["Schema: Installation von Produkten auf dem installierten System", width=332]
...
* Passen Sie die folgenden Properties an:
+
[source]
----
...
Der ganze Ablauf benötigt einige Zeit, läuft aber ohne Interaktion ab. Sie sollten mit mindestens einer Stunde rechnen.
...
image::opsi-modules:opsi-wim-cap-backup.png["Schema: Backup der Platte mit opsi-clonezilla", width=332]
...
TIP: Muss ein opsi-clonezilla Backup erstellt werden? +
Wenn der Rechner lediglich zum Erstellen eines WIM-Captures dient und danach neu Installiert wird, oder es sich um einen virtuellen Rechner handelt der sich aus einem Snapshot wieder herstellen lässt, kann auf die Erstellung eines Backups mit Clonezilla und den abschließenden Restore verzichtet werden. +
...
image::opsi-modules:opsi-wim-cap-backup2.png["Schema: Sicherung der opsi-meta-daten nach c:\opsi.org\tmp", width=332]
...
image::opsi-modules:opsi-wim-cap-deactivate-oli.png["Schema: Deaktivierung des opsi-client-agenten", width=332]
...
image::opsi-modules:opsi-wim-cap-sysprep.png["Schema: Depersonaliserung der Systempartition mit 'sysprep'", width=332]

Damit das Image, welches erstellt werden soll, sich wie ein Standard Windows Setup auf einem beliebigen Rechner ausrollen lässt, muss es depersonalisiert werden. Dies wird mit dem Winows Werzeug `sysprep` erledigt.

TIP: Installierte Software wird nicht depersonalisiert.
Es ist durchaus möglich, dass installierte Software sich in Ihrer Konfiguration merkt, auf welchem Rechner sie installiert wurde. Eine solche Konfiguration wird dann wahrscheinlich Probleme machen, wenn das Image auf einem anderen Rechner ausgerollt wird. Daher sollte man sich genau überlegen, welche software mit in das image soll.

.Schema: Aktivieren und bootbar machen der PE Partition
image::opsi-modules:opsi-wim-cap-activate-pe.png["Schema: Aktivieren und bootbar machen der PE Partition", width=332]
...
image::opsi-modules:opsi-wim-cap-work-cmd.png["Schema: Erstellen der work.cmd im PE", width=332]
...
image::opsi-modules:opsi-wim-cap-capture.png["Schema: Capture der Systempartition vom PE aus", width=332]
...
image::opsi-modules:opsi-wim-cap-restore.png["Schema: Restore mit opsi-clonezilla", width=332]
...
Bei der Wiederherstellung der opsi Metadaten zu installierten Produkten gibt es zwei Varianten: +
...
Daher werden in diesem Fall auch nur die Produkte installiert, welche vor der Installation des Betriebssystems auf `setup` standen. Produkte die auf 'installed' stehen werden *nicht* auf 'setup' gestellt.+
