...
* user roles: xref:configed:configed.adoc#opsi-manual-configed-hostproperties-userroles[Verwaltung von Benutzerrollen]
* opsi-directory-connector: xref:directory-connector#opsi-manual-dircon[Zusammenarbeit mit Verzeichnisdienst]
* opsi-wim-capture: xref:wim-capture#opsi-manual-wimcap[Windows Imaging Format]
* opsi-linux-client-agent: xref:linux#opsi-manual-linux[Linux-Support]
* opsi-mac-client-agent: xref:clients:macos-client/mac-client-manual.adoc[macOS-Support]
* UEFI-Unterstützung: xref:uefi#opsi-manual-uefi[opsi mit UEFI/GPT]
* Secureboot-Unterstützung: xref:secureboot#opsi-manual-secureboot[opsi mit Secureboot]
* opsi-local-image/opsi-vhd-reset: xref:local-image#opsi-manual-localimage[Lokale Images] und xref:vhd#opsi-manual-vhd[Virtual Hard Disk]
* Lizenzmanagement: xref:licensemanagement#opsi-manual-licensemanagement[Verwaltung von Software-Lizenzen]
* WAN/VPN-Erweiterung: xref:wan-support#opsi-manual-wansupport[Verwaltung von Clients hinter langsamen/instabilen Verbindungen]
* opsi-Nagios-Connector: xref:nagios-connector#opsi-Nagios-Connector[Schnittstelle für Nagios/Icinga]
* MySQL-Backend: xref:server:opsi-backends.adoc#opsi-manual-backends-mysql-configdata-overview[Datenbank-Backend für Konfigurationsdaten]
* Scalability: xref:scalability.adoc#opsi-manual-scalability[Mehrere Worker-Prozesse]
