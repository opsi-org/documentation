...
TIP: Eine Liste von Netboot-Produkten mit UEFI-Unterstützung und Secureboot-Unterstützung finden Sie am Ende dieses Kapitels.
...
* UEFI enthält einen eigenen Bootmanager, der nicht nur (siehe oben) in seiner Reihenfolge durch das Betriebssystem veränderbar ist, sondern auch die Starteinträge der Betriebssysteme selbst enthält. Das vereinfacht ein Nebeneinander unterschiedlicher Betriebssysteme auf dem Gerät, weil die Bootloader sich nicht mehr gegenseitig überschreiben.

* UEFI ist in 32 oder 64{nbsp}Bit implementiert. Damit ist dann auch die Bittigkeit des Betriebssystems vorgegeben, das heißt, dass ein 32-Bit-OS nicht auf einem 64-Bit-UEFI installierbar ist (und umgekehrt).

* xref:modules/secureboot#opsi-manual-secureboot[Secureboot] ist eine UEFI-Funktion, die dafür sorgt, dass ein Windows-Start nur dann funktioniert, wenn bestimmte Firmware-Elemente (z.{nbsp}B. der Bootloader) nicht durch Dritte verändert wurden. Secureboot ist seit Windows 8 standardmäßig aktiviert.
...
* Wegfall der 2-TByte-Grenze (jetzt 8{nbsp}Zebibyte bzw. 9,6{nbsp}Zetabytes, also 9.600 Millionen Terabyte)
...
CAUTION: Falls Ihr opsi-Server bereits frühere Paketversionen installiert hatte, müssen Sie eventuell die Datei `/etc/opsi/opsipxeconfd.conf` anpassen, da der UEFI-Bootloader sonst inkompatibel zur Named Pipe ist.
...
* Secureboot deaktivieren: Die Einstellung finden Sie oft im Bereich _Boot_ oder _Startup_, manchmal auch im Bereich _Security_. Weitere Informationen dazu finden Sie im Kapitel xref:modules/secureboot#opsi-manual-secureboot[Secureboot-Unterstützung].
...
NOTE: Diese Kriterien sagen nichts über die Qualität der Hardware aus -- vielmehr geht es darum, wie gut es per Netboot gewartet werden kann. Es geht hier also um die BIOS-Funktionen von UEFI Netboot.
...
Mit beiden Programmen, `efibootmgr` und `bcdedit`, können Sie neue Einträge erstellen, vorhandene Löschen, `nextboot` setzen und die Reihenfolge ändern. 
...
Um die Attribute zu setzen, verwenden Sie unter Linux `sgdisk -A` (`sgdisk --attributes`) und unter Windows das Programm `diskpart` und das Kommando `gpt attributes`.
...
Detaillierte Infos zu einer Partition erhalten Sie über `--info=`:
