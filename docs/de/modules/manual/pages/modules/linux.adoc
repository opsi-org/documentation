...
Unter Linux stehen die meisten Pakete über die Repositorys der Distribution zur Verfügung. Dadurch wird der Installationsanteil kleiner, der Konfigurationsanteil aber bleibt. Weiterhin gibt es auch Applikationen, welche nicht über die Standardrepositorys verfügbar sind. +
Hier müssen unter Umständen zunächst weitere Repositorys dem System hinzugefügt werden bzw. Installationsquellen dem Paket zugefügt werden. +
...
Beispiel: Hinzufügen eines Repositorys:
[source,winst]
----
[Actions]
requiredWinstVersion >= "4.11.4.1"
