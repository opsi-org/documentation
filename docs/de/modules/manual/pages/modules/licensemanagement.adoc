...
Das opsi-Lizenzmanagement-Modul soll die aufwändige und komplexe Verwaltung von Lizenzen nicht-freier Software erleichtern, die auf mit opsi verwalteten Clients installiert ist.
...
image::licensemanagement-licensepools.png["Lizenzmanagement:Tab Lizenzpools", pdfwidth=80%]
...
TIP: Bei lizenzpflichtiger Software packen Sie zwecks größerer Klarheit nur ein Software-Produkt in ein opsi-Produkt. Binden Sie dieses (und nur dieses) opsi-Produkt an den entsprechenden Lizenzpool. Technisch zwingend ist diese Beschränkung, wenn Sie das Lizenzmanagement zusammen mit der 'opsi-WAN-Erweiterung' verwenden; siehe Kapitel xref:modules/wan-support#opsi-manual-wansupport[opsi WAN/VPN-Erweiterung].
