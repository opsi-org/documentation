...
TIP: Bei lizenzpflichtiger Software packen Sie zwecks größerer Klarheit nur ein Software-Produkt in ein opsi-Produkt. Binden Sie dieses (und nur dieses) {opsi-product} an den entsprechenden Lizenzpool. Technisch zwingend ist diese Beschränkung, wenn Sie das Lizenzmanagement zusammen mit der 'opsi-WAN-Erweiterung' verwenden; siehe Kapitel xref:modules/wan-support#opsi-manual-wansupport[opsi WAN/VPN-Erweiterung].
