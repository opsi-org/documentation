...
=== Vorbedingungen für die opsi Erweiterung _opsi wim capture_
...
Als Target-Produkt wird in der Regel eines der zur Verfügung gestellten Capture-Produkte verwendet, zB.: 'win10-x64-capture' +
...
* Installieren Sie alle verfügbaren Windows-Updates oder aber verwenden Sie config-win10 in Kombination mit mshotfix zur Aktualisierung.
...
* Target-Product (z.B. 'win10-x64-captured') mit folgender Anpassung auf setup setzen: +
