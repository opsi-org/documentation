...
=== Vorbedingungen für die opsi Erweiterung _opsi local image_
...
Mit der Berechtigung opsi-local-image zu verwenden erwerben Sie gleichzeitig auch das Recht, die Erweiterung opsi-vhd-reset (siehe xref:modules/vhd#opsi-manual-vhd[opsi vhd reset]) zu verwenden.
...
CAUTION: Für das Produkt `opsi-local-image-capture` muss der share `opsi_depot_rw` für 'pcpatch' beschreibbar sein. Prüfen Sie Ihre Samba Konfiguration.
...
Der einfache Aufruf des Produktes `opsi-local-image-restore` stellt automatisch das letzte erstellte Image wieder her. Soll ein anderes Image wieder hergestellt werden, so muss dies im Property `imagefile` angegeben werden.
...
Die opsi-local-image Produkte ab Version 4.1 unterstützen auch Mehrplattensysteme. Bitte beachte Sie dazu auch den Abschnitt xref:products/netboot-products#opsi-manual-netboot-nt6[Hinweise zu den NT6 Netbootprodukten ]
...
Hier kann das Produkt zur Installation eines Betriebssystems ausgewählt werden, welches im Anschluss an die Partitionierung automatisch gestartet wird.
...
Die Properties der Windows NT6 Produkte ab Version 4.1 sind eine Teilmenge der Properties der NT6 Standardprodukte, wie sie im Abschnitt xref:products/netboot-products#opsi-manual-netboot-nt6[Hinweise zu den NT6 Netbootprodukten] beschrieben sind. Die fehlenden Properties zu Platten und Partitionen werden dem Produkt `opsi-local-image-prepare` entnommen.

ACHTUNG:: Ändern Sie die Propertyeinstellungen von opsi-local-image-prepare nicht mehr nachdem Sie die Maschine damit präpariert haben, da die nachfolgenden Produkte auf diese Werte zugreifen.
...
* 'setup_after_install' +
Hier können ein oder mehrere Produkte angegeben werden, welche nach Abschluss der Betriebssysteminstallation auf 'setup' gestellt werden. Dabei werden auch die Abhängigkeiten diese Produkte mit aufgelöst.
...
** `setup_after_install` +
Hier können ein oder mehrere Produkte angegeben werden, welche auf 'setup' gestellt werden und somit nach Abschluss der Betriebssysteminstallation installiert werden. Dabei werden auch die Abhängigkeiten dieser Produkte mit aufgelöst.
** `wget_and_execute`: +
...
** `setup_after_install` +
Hier können ein oder mehrere Produkte angegeben werden, welche auf 'setup' gestellt werden und somit nach Abschluss installiert werden. Dabei werden auch die Abhängigkeiten dieser Produkte mit aufgelöst.
...
** `setup_after_restore` +
Hier können ein oder mehrere opsi-Produkte angegeben werden, welche nach dem Abschluss des Restores auf 'setup' gesetzt werden und somit nach dem Reboot automatisch ausgeführt werden. Der Default ist das Produkt 'windomain' zur erneuten Aufnahme des restorten Clients in die Windows Domain.
