...
Im Normalfall ist zur Inbetriebnahme der Installation bei Shutdown, wie in <<opsi-on-shutdown-activate>> beschrieben, nichts weiter erforderlich als ein aktuelles `opsi-client-agent` Paket. Für geeignete Clients kann mit dem {opsi-config-object} `opsiclientd.event_on_shutdown.active` die Installation beim Shutdown aktiviert werden. 
...
Wenn für einen Install_on_shutdown-Client nun die Installation im Startup deaktiviert werden soll, wird der entsprechende '{opsi-config-object}' wie folgt angelegt:
