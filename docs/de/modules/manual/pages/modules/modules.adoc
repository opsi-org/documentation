...
* MySQL-Backend für Konfigurationsdaten (siehe <<opsi-manual-backends-mysql-configdata-overview,mysql-Backend für Konfigurationsdaten>>)
* UEFI Support (siehe <<opsi-manual-uefi,opsi mit UEFI>>)
* opsi-Lizenzmanagement Modul (siehe <<opsi-manual-licensemanagement,opsi-Lizenzmanagement>>)
* Unterstützung für Clients über WAN/VPN (siehe <<opsi-manual-wansupport,opsi WAN/VPN-Erweiterung>>)
* opsi WIM Capture (siehe <<opsi-manual-wimcap,opsi WIM Capture>>)
* opsi Local Image (siehe <<opsi-manual-localimage,opsi local image>>) und opsi-vhd-reset (siehe <<opsi-manual-vhd,opsi vhd reset>>)
* opsi Linux Agent (siehe <<opsi-manual-linux,opsi Linux Support>>)
* opsi Nagios Connector (siehe <<opsi-Nagios-Connector,opsi Nagios Connector>>)
* User-Rollen (siehe <<opsi-manual-configed-hostproperties-userroles,Verwaltung von User-Rechten und -Rollen>>)
