...
Eine Beispiel-Konfiguration wird unter `/usr/share/opsi-directoy-connector/opsi-directory-connector.example.conf.conf` bereitgestellt.
Diese Datei kann als eine Vorlage für eigene Konfigurationen verwendet werden.

[source,prompt]
----
cp /usr/share/opsi-directoy-connector/opsi-directory-connector.example.conf.conf /etc/opsi/opsidirectoryconnector-custom.conf
----
...
Werden die Parameter nicht angegeben, dann ist `start_tls` aktiviert, aber das Zertifikat wird nicht überprüft. 
