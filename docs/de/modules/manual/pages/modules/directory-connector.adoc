...
Der opsi Directory Connector überträgt Daten aus einem LDAP-fähigen Verzeichnisdienst in eine opsi-Installation. Damit ist es nicht mehr nötig, Daten auf mehreren Systemen zu pflegen -- die automatische Synchronisation verringert den Administrationsaufwand. Da der Directory Connector im Hintergrund auf LDAP setzt, funktioniert er sowohl mit Active Directory als auch mit Samba 4. 
...
* Der Rechner, auf dem der Directory Connector laufen soll, muss Netzwerzugriff auf den Rechner mit dem Verzeichnisdienst und auf den {opsi-server} haben.
...
Um den Directory Connector zu installieren, fügen Sie als Erstes das opsi-Repository hinzu (siehe Kapitel xref:../../../getting-started/pages/getting-started.adoc[Getting Started], Abschnitte xref:../../../getting-started/pages/server/base-installation.adoc#opsi-getting-started-installation-base-deb[Debian/Ubuntu], xref:../../../getting-started/pages/server/base-installation.adoc#opsi-getting-started-installation-base-deb[openSUSE/SLES] und xref:../../../getting-started/pages/server/base-installation.adoc#opsi-getting-started-installation-base-centos-rhel[CentOS/RHEL]). Installieren Sie danach über den Paketmanager Ihrer Distribution das Paket `opsi-directory-connector`. 
...
NOTE: Die Management-Oberfläche xref:../configed.adoc[opsi-configed] zeigt die Gruppe `clientdirectory` als `DIRECTORY` an. Wenn Clients oder Gruppen also direkt unterhalb von `DIRECTORY` erscheinen sollen, tragen Sie `clientdirectory` als Wert für `root_dir_in_opsi` ein.
...
NOTE: Sollte der aus dem Verzeichnis ausgelesene Wert für die Client-ID nicht als FQDN erkennbar sein, so wird ein enstprechender FQDN erstellt. Dabei wird der Domain-Teil aus den DC-Werten des Elements gebildet.
...
TIP: Alternativ können Sie einem Depot auch eine Liste von Netzwerkenbereichen zuordnen:
...
Alternativ zu den beiden systemd-Units können Sie den regelmäßigen Abgleich mit dem Directory Connector über einen Crobjob automatisieren.
