...
Eine Beispiel-Konfiguration wird unter `/usr/share/opsi-directoy-connector/opsi-directory-connector.example.conf` bereitgestellt.
Diese Datei kann als eine Vorlage für eigene Konfigurationen verwendet werden.

[source,prompt]
----
cp /usr/share/opsi-directoy-connector/opsi-directory-connector.example.conf /etc/opsi/opsidirectoryconnector-custom.conf
----
...
Wird die Verbingung über den unverschlüsselten Port 389 gestartet, dann kann mit `start_tls` gesteuert werden, ob nach Anmelden am Directory eine gesicherte Verbindung gestartet wird. Im Default ist beim 389 `start_tls` aktiviert, aber das Zertifikat wird nicht überprüft.
