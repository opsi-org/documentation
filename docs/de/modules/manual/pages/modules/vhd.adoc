...
Die Erweiterung `opsi-vhd-reset` ergänzt `opsi-local-image` (siehe xref:modules/local-image#opsi-manual-localimage[Lokale Images (`opsi-local-image`)]) und arbeitet mit VHD-Containern.
