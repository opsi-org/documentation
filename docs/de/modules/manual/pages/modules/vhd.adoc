...
Die Erweiterung `opsi-vhd-reset` ergänzt `opsi-local-image`(siehe xref:modules/local-image#opsi-manual-localimage[Lokale Images (`opsi-local-image`)]).
...
`opsi-vhd-reset` kommt im Bundle mit der Erweiterung `opsi-local-image` (siehe xref:modules/vhd#opsi-manual-vhd[Virtual Hard Disk (opsi-vhd-reset)]).
