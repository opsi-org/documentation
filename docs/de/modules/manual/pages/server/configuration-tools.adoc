...
[source,prompt]
----
opsi-cli --output-format table --attributes id,type,lastSeen jsonrpc execute host_getObjects [] '{"id":"testclient0*.uib.local"}'
----

image::opsi-cli-table-format.png["Tabellenformatierung"]
...
...
id;lastSeen;type
testclient000.uib.local;2018-05-03 11:12:19;OpsiClient
testclient01.uib.local;2015-07-14 16:10:42;OpsiClient
testclient02.uib.local;2015-07-14 16:10:47;OpsiClient
testclient03.uib.local;2015-07-14 16:10:51;OpsiClient
testclient04.uib.local;2015-07-14 16:10:57;OpsiClient
----
...
Über dieses Kommando lässt sich von die `opsi-cli`-Konfiguration anpassen:
...
CAUTION: Beim Aufruf ohne explizite Angabe von Clients und Produkten, wirkt sich der Befehl auf alle Clients und alle Produkte aus. Dabei wird jede Produkt, für jeden Client auf `setup` gesetzt.

Folgender Befehl setzt alle Pakete für alle Clients auf `setup`:
...
Um für einen Client alle Produkte die veraltet oder auf failed-Status stehen auf setup zu setzen:

[source,bash]
----
opsi-cli -l5 client-action --clients test-98.domain.tld set-action-request --where-outdated --where-failed

[5] [2022-10-28 12:58:16.968] [               ] Selected clients: ['test-98.domain.tld']   (client_action_worker.py:48)
[5] [2022-10-28 12:58:17.028] [               ] Handling products ['config-win10', 'hwaudit', 'jedit', 'l-debuntu-upgrade', 'l-desktop', 'l-opsi-client-kiosk', 'l-opsi-server', 'l-opsi-server-migrate', 'l-opsi-server-register-depot', 'l-ssh-root-login', 'l-system-update', 'l-virtualbox', 'networklocation', 'opsi-client-agent', 'opsi-client-kiosk', 'opsi-configed', 'opsi-linux-client-agent', 'opsi-logviewer', 'opsi-script-beautifier', 'opsi-setup-detector', 'opsi-template-with-admin', 'opsi-template-with-userlogin', 'opsi-wim-capture', 'opsi-wim-delete', 'opsi-wim-info', 'opsipackagebuilder_wlm', 'swaudit', 'windows10-enablement', 'windows11-upgrade']   (set_action_request_worker.py:105)
[5] [2022-10-28 12:58:17.039] [               ] Setting 'setup' ProductActionRequest: l-virtualbox -> test-98.domain.tld   (set_action_request_worker.py:134)
----
