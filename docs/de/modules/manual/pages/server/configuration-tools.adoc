...
Da die opsi-cli ein Binary ist, kann es portabel eingesetzt werden. Wenn man das Binary auf einem Client in den Pfad installieren möchte, kann man die opsi-cli selbst dafür nutzen:

[source,prompt]
----
opsi-cli self --help
                                                                                                                                            
 Usage: opsi-cli self [OPTIONS] COMMAND [ARGS]...                                                                                           
                                                                                                                                            
 opsi-cli self command. This command is used to manage opsi-cli.                                                                            
...
╰──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯
╭─ Commands ───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
│ install                                                         Install opsi-cli locally                                                 │
│ setup-shell-completion                                          Setup shell completion                                                   │
│ uninstall                                                       Uninstall opsi-cli locally                                               │
╰──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯
----
...
Die `opsi-cli` ist ein Python-Binary, welches über Subkommandos gesteuert wird. Ähnlich wie bei docker oder kubectl. Dabei hat jedes Subkammando jeweils ein eigenes Untermenü. Im folgenden werden erst die Grundfunktionalitäten und dann die schon verfügbaren `Commands`, speziellen Funktionen von opsi-cli behandelt. An jeder Stelle des Aufrufs gibt eine Möglichkeit sich eine Hilfeseite anzeigen zu lassen, hier ein Beispiel: `opsi-cli --help` oder `opsi-cli config --help`.
...
│ --log-level-file          LOG_LEVEL_FILE      The log level for the log file. Possible values are:                                       │
│                                               none/0, essential/1, critical/2, error/3, warning/4, notice/5, info/6, debug/7, trace/8,   │
│                                               secret/9.                                                                                  │
│                                               [default: 0]                                                                               │
│ --log-level-stderr    -l  LOG_LEVEL_STDERR    The log level for the console (stderr). Possible values are:                               │
│                                               none/0, essential/1, critical/2, error/3, warning/4, notice/5, info/6, debug/7, trace/8,   │
│                                               secret/9.                                                                                  │
│                                               [default: 0]                                                                               │
│ --color                   COLOR               Enable or disable colorized output. [default: True]                                        │
│ --dry-run                 DRY_RUN             Simulation only. Does not perform actions.                                                 │
│ --config-file-system      CONFIG_FILE_SYSTEM  System wide config file location [default: /etc/opsi/opsi-cli.yaml]                        │
│ --config-file-user        CONFIG_FILE_USER    User specific config file [default: /home/user/.config/opsi-cli/opsi-cli.yaml]    │
│ --help                                        Show this message and exit.                                                                │
│ --version                                     Show the version and exit.                                                                 │
╰──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯
----
...
Mit der opsi-cli lässt sich die Ausgabe aller Subkommandos vereinheitlichen und bietet diverse Formate und Filter Möglichkeiten.

[source,prompt]
----
╭─ IO options ─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
│ --output-format                    OUTPUT_FORMAT  Set output format. Possible values are: auto, json, pretty-json, msgpack, table, csv.  │
│                                                   [default: auto]                                                                        │
│ --output-file                      OUTPUT_FILE    Write data to the given file. [default: -]                                             │
│ --input-file                       INPUT_FILE     Read data from the given file. [default: -]                                            │
│ --interactive/--non-interactive    INTERACTIVE    Enable or disable interactive mode. [default: interactive]                             │
│ --metadata                         METADATA       Enable or disable output of metadata. [default: False]                                 │
│ --header                           HEADER         Enable or disable header for data input and output. [default: True]                    │
│ --attributes                       ATTRIBUTES     Select data attributes (all selects all available attributes).. Comma separated list.  │
╰──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╯

Im folgenden noch ein Beispiel, wie sich die Ausgabe steuern lässt:


[source,prompt]
----
opsi-cli jsonrpc execute host_getObjects [] '{"id":"testclient01.uib.local"}'
[
  {
    "description": "",
    "notes": "",
    "id": "testclient01.uib.local",
    "hardwareAddress": "",
    "ipAddress": null,
    "inventoryNumber": "",
    "opsiHostKey": null,
    "created": "2022-07-14 16:10:42",
    "lastSeen": "2022-07-14 16:10:42",
    "oneTimePassword": null,
    "type": "OpsiClient",
    "ident": "testclient01.uib.local"
  }
]
----

Der selbe Aufruf mit verschiedenen Formaten und auf zwei Attribute gefiltert:

[source,prompt]
----
opsi-cli --output-format table --attributes id,type jsonrpc execute host_getObjects [] '{"id":"testclient01.uib.local"}'
╭────────────────────────┬────────────╮
│ id                     │ type       │
├────────────────────────┼────────────┤
│ testclient01.uib.local │ OpsiClient │
╰────────────────────────┴────────────╯
...
opsi-cli --output-format csv --attributes id,type jsonrpc execute host_getObjects [] '{"id":"testclient01.uib.local"}'
id;type
testclient01.uib.local;OpsiClient
----
...
Im Folgenden werden die aktuell verfügbaren Subkommandos erläutert.


==== config

Über dieses Subkommando lässt sich die opsi-cli Konfiguration anpassen:

[source,prompt]
----
opsi-cli config --help
                                                                                                                                            
 Usage: opsi-cli config [OPTIONS] COMMAND [ARGS]...                                                                                         
                                                                                                                                            
 opsi-cli config command. This command is used to manage opsi-cli configuration.                                                            
...
│ show                         Show configuration item details                                                                             │
...
│ color              │ Bool                        │ true                                               │ true                                               │
│ config_file_system │ File                        │ /etc/opsi/opsi-cli.yaml                            │ /etc/opsi/opsi-cli.yaml                            │
│ config_file_user   │ File                        │ /home/user/.config/opsi-cli/opsi-cli.yaml          │ /home/user/.config/opsi-cli/opsi-cli.yaml          │
│ dry_run            │ Bool                        │ false                                              │ false                                              │
│ header             │ Bool                        │ true                                               │ true                                               │
│ input_file         │ File                        │ -                                                  │ -                                                  │
│ interactive        │ Bool                        │ true                                               │ true                                               │
│ log_file           │ File                        │                                                    │                                                    │
│ log_level_file     │ LogLevel                    │ 0                                                  │ 0                                                  │
│ log_level_stderr   │ LogLevel                    │ 0                                                  │ 0                                                  │
│ metadata           │ Bool                        │ false                                              │ false                                              │
│ output_file        │ File                        │ -                                                  │ -                                                  │
│ output_format      │ OutputFormat                │ auto                                               │ auto                                               │
│ password           │ Password                    │                                                    │                                                    │
│ plugin_bundle_dir  │ Directory                   │ /tmp/_MEIxXVaVV/plugins                            │ /tmp/_MEIxXVaVV/plugins                            │
│ plugin_system_dir  │ Directory                   │ /var/lib/opsi-cli/plugins                          │ /var/lib/opsi-cli/plugins                          │
│ plugin_user_dir    │ Directory                   │ /home/user/.local/lib/opsi-cli/plugins             │ /home/user/.local/lib/opsi-cli/plugins             │
│ python_lib_dir     │ Directory                   │ /home/user/.local/lib/opsi-cli/lib                 │ /home/user/.local/lib/opsi-cli/lib                 │
│ service            │ OPSIServiceUrlOrServiceName │ https://localhost:4447                             │ https://localhost:4447                             │
│ services           │ OPSIService                 │                                                    │                                                    │
│ user_lib_dir       │ Directory                   │ /home/user/.local/lib/opsi-cli                     │ /home/user/.local/lib/opsi-cli                     │
│ username           │ str                         │                                                    │                                                    │
╰────────────────────┴─────────────────────────────┴────────────────────────────────────────────────────┴────────────────────────────────────────────────────╯
----
...
Mit diesem Subkammando erhält man direkten Zugriff auf die Opsi-Webservice-API. Mit diesem Subkammando ist die opsi-cli ein komplettes Drop-In-Replacement für den opsi-admin. jsonrpc hat zwei Unterkommandos. Mit `opsi-cli jsonrpc methods` erhält man eine Liste von allen verfügbaren API-Methoden und Parameter. In dieser wird ebenfalls angezeigt, ob die Methode als "deprecated" markiert ist und in diesem Fall auch eine alternative Methode als empfohlene Ausweichmöglichkeit. Diese Methoden erscheinen auch im Log. Allerdings hat die Erfahrung gezeigt, dass diese Meldungen in der Praxis nicht wahrgenommen werden. Hier hat man direkt Zugriff auf den aktuellen Stand. Das zweite Subkammando `execute` führt die API-Methode aus. Die Syntax für den Auruf selbst ähnelt dabei stark der Nutzung mit opsi-admin.
...
Die Methodenparameter werden dabei absolut identisch übergeben. Hier noch ein etwas kompliziertes Beispiel zur Verdeutlichung:
...
//TODO: Ist das wirklich so?
Bei den Beispielen wurde bewusst auf die Option -d von opsi-admin verzichtet. Die Umgehung des opsi-Webservice hat in der Vergangenheit zu Problemen innerhalb des opsi-Systems geführt. Deshalb ist der Weg zur API nur noch über den opsi-Webservice möglich und opsi-cli Unterstützt diesen Modus erst gar nicht. Um auch die opsi-cli in Skripten oder Cronjobs außerhalb des opsi-Servers zu nutzen gibt es die Möglichkeit die Zugangsdaten zu nutzen.

Auch hier lassen sich die Ausgabeformate wie überall bei der opsi-cli beeinflussen:
...
Das Subkommando client-action gibt die Möglichkeit die opsi-Clients zu managen. client-action bietet selbst in erster Linie Optionen, um die Liste der Clients zu beeinflussen, auf die sich die Aktionen auswirken:

[source,prompt]
----
opsi-cli client-action --help
Usage: opsi-cli client-action [OPTIONS] COMMAND [ARGS]...

  This command can be used to manage opsi client actions.

Options:
  --version                     Show the version and exit.
  --clients TEXT                Comma-separated list of clients or 'all'
  --client-groups TEXT          Comma-separated list of host groups
  --exclude-clients TEXT        Do not perform actions for these clients
  --exclude-client-groups TEXT  Do not perform actions for these client groups
  --help                        Show this message and exit.
----

Aktuell unterstützt dieses Subkammando ein weiteres Unterkommando `set-action-request`, welches Produktaktionen für Clients setzt. Es ist vergleichbar mit den tasks von opsi-admin, arbeitet aber nach einem Auschlussprinzip. Unterhalb dieses Unterkommandos gibt es weitere Optionen, um die Produkte zu filtern, auf diese die Aktion eine Auswirkung hat:

[source,bash]
----
opsi-cli client-action set-action-request --help
Usage: opsi-cli client-action set-action-request [OPTIONS]

  opsi-cli client-action set-action-request command

Options:
  --where-failed                  Set this to add actionRequests for all
                                  selected failed products
  --where-outdated                Set this to add actionRequests for all
                                  selected outdated products
  --uninstall-where-only-uninstall
                                  If this is set, any installed package which
                                  only has an uninstall script will be set to
                                  uninstall
  --exclude-products TEXT         Do not set actionRequests for these products
  --products TEXT                 Set actionRequests for these products
  --product-groups TEXT           Set actionRequests for the products of these
                                  product groups
  --exclude-product-groups TEXT   Do not set actionRequests for these product
                                  groups
  --request-type TEXT             The type of action request to set  [default:
                                  setup]
  --setup-on-action TEXT          After actionRequest was set for a client,
                                  set these products to setup
  --help                          Show this message and exit.
----

Folgendes wurde gesetzt 
