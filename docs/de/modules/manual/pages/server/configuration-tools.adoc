...
`opsi-cli` ist ein Python-Binary, welches über Kommandos gesteuert wird. Ähnlich wie bei docker oder kubectl. Dabei kann jedes Kommando einen oder mehrere Unterkommandos im weiteren als Subkommando bezeichnet. Im folgenden werden erst die Grundfunktionalitäten und dann die schon verfügbaren Kommandos (`Commands`) vorgestellt. An jeder Stelle des Aufrufs gibt es die Möglichkeit sich eine Hilfeseite anzeigen zu lassen, hier ein Beispiel: `opsi-cli --help` oder `opsi-cli config --help`.
...
image::opsi-cli-table-format.png["Tabellenformatierung", pdfwidth=90%]
