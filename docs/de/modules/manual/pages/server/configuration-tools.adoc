...
Der Health Check kann auf verschiedene Arten aufgerufen werden. 
Alle Varianten beziehen ihre Daten vom API Call `service_healthCheck`. 
Die opsi API gibt die Daten des {health-check}s als JSON zurück. 
Dieses JSON Objekt kann bei Support-Anfragen nützlich sein. 

 
TIP: Sie können das RPC-Interface auf der Adminseite verwenden, um die {health-check} Methode aufzurufen.
...
options:
  ...
  --upgrade-check [UPGRADE_CHECK]
          ...
          (default: False)
----
//TODO Link to admininterface
TIP: Über die Adminseite oder {opsi-cli} können Sie ein Terminal auf den Server öffnen und den Befehel ausführen (siehe xref:opsi-manual-configuration-tools-opsi-cli-commands-terminal[opsi-cli terminal]). 

=== opsi-cli

Der {health-check} kann auch über das {opsi-cli} aufgerufen werden. 
Siehe hier zu den Abschnitt xref:opsi-manual-configuration-tools-opsi-cli-commands-support[] im {opsi-cli} Kapitel.


=== Health Checks

Hier wird beshreiben, was der {health-check} überprüft und wie der Zustand des Systems bewertet wird.

==== Opsiconfd config

Der Check schaut sich die Konfiguration des opsiconfd service an. Überprüft werden folgende Werte:

* _log-level-stderr_, _log-level-file_, _log-level_ +
  Wenn das Log-Level zu hoch für eine produktive Umgebung ist, dann kann es zu Performanceproblemen kommen. +
  Aus diesem Grund wird bei einem Log-Level von 7 eine Warnung und ab Log-Level 8 ein Fehler ausgegeben.
* _debug-options_ +
  Ist eine Debug Option aktiv, dann wird dies als Fehler gewertet, da es zu Performanceproblemen in produktiven Umgebungen kommen kann.
* _profiler_ +
  Auch der Profiler sollte aus Performancegründen deaktivirt sein. Ein aktiver Profiler führt auch zu einer Fehlerausgabe.
* _run-as-user_ +
  Wenn der opsiconfd vom Benutzer root ausgeführt wird, dann wird das als Fehler gewertet, da root zu viele Rechte auf dem System hat.


==== opsi config

Hier wir geprüft, ob gewisse Konfigurationen vom Standard abweischen. 
Ist das der Fall, dann eine Warung ausgegeben. 
Ein Fehler wird ausgegeben, wenn der Wert nicht existiert.

Aktuell wird nur in opsi 4.3 überprüft, ob `opsiclientd.global.verify_server_cert` aktiviert ist. 

==== Festplattenplatz (Disk usage)

Überprüft den freien Paltz für die folgenden Einhängepunkt:

* /
* /temp
* /var, /var/lib oder var/lib/opsi 

Sind weniger als 15 GiB frei, wird eine Warnung ausgegben. 
Sind es weniger als 7.5 GiB, wird das als Fehler gewertet.


==== Depotserver check

Seit opsi 4.3 muss das opsi Repository unter /var/lib/opsi/repository liegen. Ist das nicht der Fall wird ein Fehler ausgegben.

==== Check System Packages

Aktuell werden folgende Systempakete auf Aktuallität geprüft:

 * opsiconfd
 * opsi-utils
 * opsipxeconfd

Die Prüfung erfolgt gegen das stable Repository von uib (https://download.opensuse.org/repositories/home:/uibmz:/opsi:/4.2:/stable/Debian_11/). 
Dabei werden ältere Versionen als Warnung gewertet und wenn eins der Pakete nicht installiert ist wird ein Fehler ausgegeben.


==== Check Products on Depot

Es wird geprüft, ob folgende Produkte auf den Depots installiert und aktuell sind:

 * opsi-script
 * opsi-client-agent

Sind opsi-linux-client-agent und opsi-macos-client-agent werden auch diese Pakte überprüft.
Hier gilt ein veraltetes Paket wird als Warung und ein nicht installiertes Paket wird als Fehler gewertet.


==== Check Products on Client

Auf den Clients wird überprüft, ob der jeweilige Clientagent auf dem neuesten Stand ist. 
Ist eine älter Version installiert, dann gitb der Health Check eine Warnung aus.

==== Check Redis

Überprüft, ob der redis Server erreichbar ist und ob das Redis Timeseries Modul geladen ist.
Ist der Server nicht erreichbar oder das Modul nicht geladen wird das als Fehler gewertet.

==== Check {mysql}

Überprüft, ob die Datenbank erreichabr ist.
Zur Verbindung werden die Daten aus der Datei /etc/opsi/backends/mysql.conf verwendet.
Kann keine Verbinfung aufgebaut werden ist das ein Fehler.


==== Check Deprecated Calls

opsi speichert in redis alle Aufrufe von als veraltet markierten Methoden.
Dieser Check schaut, ob es solche Aufrufe gegeben hat und gibt dann eine Warnung aus. 
In der Meldung steht dann auch welcher client agent die API-Methode aufgerufen hat. 


==== Check Operating System End Of Life

Überprüft, ob das Server System noch Updates erhält. 
90 Tage vor dem End of Life einer Distro wird eine Warnung ausgegeben und ab dem End of Life Datum wird ein Fehler ausgegeben.
