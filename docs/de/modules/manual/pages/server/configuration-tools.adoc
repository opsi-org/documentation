...
opsi-Produkte aus einem oder mehreren Repositorys zu laden und auf dem Server zu installieren.
...
Repositorys sind die Quellen, von denen sich der {opsi-server} die Pakete holt.
...
- `--repo <Name eines Repositorys>` schränkt die Aktionen auf das angegebene Repository ein. Die Namen der verfügbaren Repositorys können über `list --active-repos` ermittelt werden.
...
Beispiel: Das Installieren aller in den Repositorys vorhandenen Pakete:
...
Mittels `list --active-repos` werden die aktivierten Repositorys angezeigt.
Die gezeigten Informationen sind der Name, die Adresse des Repositorys und sofern vorhanden die Beschreibung des Repositorys.
...
Falls ein gemeinsamer Proxy verwendet werden soll, kann dieser für alle Repositorys in `opsi-package-updater.conf` eingetragen werden.
Dies benötigt mindestens opsi-utils 4.1.1.33.
Alle Repositorys ohne eigene Proxy-Konfiguration werden diesen verwenden.
