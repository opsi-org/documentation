...
Eine solche JSON-Datei ist besonders bei Support-Anfragen nützlich.
...
Dieser Abschnitt beschreibt, was der {health check} überprüft und wie er den Zustand des Systems bewertet.

==== opsiconfd-Konfiguration

Der {health check} untersucht die Konfiguration des Dienstes opsiconfd und überprüft dazu die folgenden Werte:
...
* `run-as-user` +
  Das Ausführen des Dienstes opsiconfd als Benutzer root wird als Fehler gewertet, da root zu viele Rechte auf dem System hat.
...
Überprüft, ob der Redis-Serve erreichbar ist und ob das RedisTimeSeries-Modul geladen ist.
Ist der Server nicht erreichbar oder das Modul nicht geladen, dann wird das als Fehler gewertet.
...
Kann keine Verbinfung aufgebaut werden, dann ist das ein Fehler.


==== Check Deprecated Calls

opsi speichert in Redis unter anderem Aufrufe von als veraltet markierten Methoden.
...
Der Check gibt 90 Tage vor dem End of Life einer Distribution eine Warnung aus. Ab dem End-of-Life-Datum gibt er einen Fehler aus.
