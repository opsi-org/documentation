...
CAUTION: MySQL-Server verwendet seit Version 5.7 den vorher optionalen _strict mode_ nun standardmäßig. Dies führt zu einem Fehlschlag des Befehls `opsi-setup --configure-mysql`. Dementsprechend sollte vor dem Befehlsaufruf die Datei `/etc/mysql/mysql.conf.d/mysqld.cnf` editiert werden. +
...
CAUTION: Der Dienst _opsiconfd_ hat per Default keine feste Abhängigkeit zu MySQL,
