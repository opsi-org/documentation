...
CAUTION: Für die Produkte `opsi-wim-capture` und `opsi-local-image-capture` muß der share `opsi_depot_rw` für 'pcpatch' beschreibbar sein. Für das Produkt `opsi-clonezilla` muß der share `opsi_images` beschreibbar sein.
