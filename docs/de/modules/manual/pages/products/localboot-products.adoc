...
Boot des Rechners über den _{opsi-client-agent}_ installiert werden. Dies im Gegensatz
zu den weiter unten beschriebenen Netboot Produkten xref:products/netboot-products#opsi-manual-netboot-products[Netboot Produkte].
...
beschrieben: siehe Kapitel xref:client/opsi-client-agent#opsi-manual-clientagent[opsi-client-agent].
...
Das Produkt javavm stellt die für den _{opsi-configed}_ benötigte Java
...
Siehe auch Kapitel: xref:configed.adoc#opsi-manual-configed[opsi-Management GUI: opsi-configed]

[[opsi-manual-localboot-jedit]]
=== jedit

Java basierter Editor mit Syntax Highlighting für _{opsi-script}_ Scripte.
...
_{opsi-webservice}_ an den Server zurück gemeldet.
Bei der Software-Inventarisierung werden die Daten aus der Registry
(HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall)
erhoben und über den _{opsi-webservice}_ an den Server zurück gemeldet.
...
ifeval::["{mode}"!= "antora"]
link:https://docs.opsi.org/opsi-docs-de/4.2/opsi-script-manual/cook-book.html#opsi-script-cookbook-local-admin[Installationen im Kontext eines lokalen Benutzers]
endif::[]
...
Siehe auch Kapitel: xref:modules/wim-capture#opsi-manual-wimcap[opsi WIM Capture]
...
Siehe auch Kapitel: xref:modules/uefi#opsi-manual-uefi[opsi mit UEFI / GPT]
...
Siehe auch Kapitel: +
xref:modules:setup-detector.adoc#opsi-setup-detector[opsi-setup-detector]

////
ifeval::["{mode}" == "antora"]
xref:modules:setup-detector.adoc[Verwendung des opsi-setup-detector]
endif::[]
ifeval::["{mode}"!= "antora"]
link:https://docs.opsi.org/opsi-docs-de/4.2/manual/modules/setup-detector.html[Verwendung des opsi-setup-detector]
endif::[]
////
...
siehe auch: xref:modules/vhd#opsi-manual-vhd[opsi vhd reset] +
siehe auch: xref:modules/local-image#opsi-manual-localimage[opsi local image]
...
Zur Dokumentation siehe hier: xref:server/temporal-job-control#opsi-manual-temporal-job-control-cron-opsi-auto-update[opsi-auto-update]

[[opsi-manual-localboot-opsi-win-driver-update]]
=== opsi-win-driver-update

CAUTION: opsi-win-driver-update ist nicht geeignet für Clients mit der WAN-Erweiterung.
...
ifeval::["{mode}" == "antora"]
xref:windows-client-manual:os-installation.adoc#firststeps-osinstall-driverintegration-byaudit[Treiber die über die Felder <vendor>/<model> der Inventarisierung automatisch den Rechnern zu geordnet werden.]
endif::[]
ifeval::["{mode}"!= "antora"]
link:https://docs.opsi.org/opsi-docs-de/4.2/windows-client-manual/os-installation.html#firststeps-osinstall-driverintegration-byaudit[Treiber die über die Felder <vendor>/<model> der Inventarisierung automatisch den Rechnern zu geordnet werden.]
endif::[]

und im opsi-Handuch Kapitel: +
xref:configed.adoc#opsi-manual-configed-driverupload[Automatisierte Treiberintegration]
...
{opsi-product-packages}n. Typisches Beispiel ist die Abhängigkeit von Java Programmen von der Java Laufzeitumgebung (javavm).
