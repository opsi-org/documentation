...
Zur nachträglichen Installation oder zu Reparaturzwecken siehe Kapitel xref:client/opsi-client-agent#opsi-manual-clientagent-subsequent-installation[Nachträgliche Installation des opsi-client-agents].
...
proxy_url = <url>::
  ...
  Beispiel: `http://<proxy-address>:3128`
...
Ein Hilfsprogramm mit dem Sie schnell prüfen können ob eine gegeben Bitmap Grafik korrekt angezeigt werden wird, ist der opsi bitmap viewer.
