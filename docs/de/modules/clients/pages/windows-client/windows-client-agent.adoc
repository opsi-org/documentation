...
Den Client-Agent für Windows gibt es ebenfalls als MSI-Paket (Microsoft Installer). Wir stellen es in unserem öffentlichen http://download.uib.de/4.2/stable/misc/opsi-client-agent.msi[Repository] zur Verfügung. Es bietet dieselben Parameter wie das Programm `oca-installation-helper[.exe]` (siehe Abschnitt <<opsi-manual-clientagent-subsequent-installation-oca-installation-helper>>), die Sie über das Property `INSTALL_PARAMS` übergeben:
