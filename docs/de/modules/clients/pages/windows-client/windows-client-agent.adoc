...
Um zu verhindern, dass sich ein Anwender sich anmeldet, kann unter Windows der opsi-Loginblocker installiert werden. Er sorgt dafür, dass die Anmeldemase erst erscheint, wenn der Installations-Prozess beendet ist.

TIP: Ob der Loginblocker aktiv ist oder nicht, bestimmt das Produkt-Property `loginblockerstart`. In der Voreinstellung steht dieses auf `on`.
