...
== *opsi-client-agent*, *opsi-script* und *hwaudit* installieren

Um die Produkte `opsi-client-agent`, `opsi-script` und `hwaudit` (zur Hardware-Inventarisierung) auf dem OPSI-Server zu installieren, öffnen Sie auf dem Server ein Terminalfenster, entweder lokal oder per SSH.

TIP: Sie können auch aus der Oberfläche `opsi-configed` heraus ein Terminal öffnen. Dazu wählen Sie aus dem Menü _Server-Konsole_ den Eintrag  _Terminal_:

.*opsi-configed*: Terminal öffnen
image::first-steps:opsi-configed-terminal.png["*opsi-configed*: Terminal öffnen", width=800, pdfwidth=80%]

Geben Sie den folgenden Befehl ein, um die Pakete `opsi-client-agent`, `opsi-script` und `hwaudit` zu installieren:
