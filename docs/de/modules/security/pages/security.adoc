...
Sollten Sie ein Sicherheitsproblem entdecken, kontaktieren Sie uns bitte unter security@opsi.org, bevor Sie es öffentlich machen.
...
== Redis
Der OPSI-Server nutzt Redis als In-Memory-Datenbank für die Kommunikation zwischen seinen Komponenten.
Die Sicherheit von Redis ist daher entscheidend für die Gesamtsicherheit Ihres OPSI-Servers.
Weitere Informationen zu Redis finden Sie im Abschnitt xref:server:components/redis.adoc#server-components-redis-security[Sicherheit von Redis].
