...
Stellen Sie sicher, dass alle opsi-Komponenten die Server-Identität prüfen.
Details dazu finden Sie unter xref:server:components/tls.adoc[SSL/TLS & Zertifikate].
...
Hierfür ist der Konfigurations-Parameter `opsiclientd.control_server.interface` auf `[::1, 127.0.0.1]` zu setzen.
...
Hierfür ist der Konfigurations-Parameter `opsiclientd.control_server.kiosk_api_active` auf `false` zu setzen.
Mittels opsi-cli kann dies wie folgt gesetzt werden:
[source,shell]
----
...
Nutzen Sie die xref:opsi-modules:sso.adoc#opsi-module-sso[Single Sign-On (SSO)] um die Sicherheit und gleichzeitig den Komfort zu erhöhen.


[[opsi-manual-security-disabled-auth-methods]]
== Authentifizierungs-Methoden deaktivieren
Über den Konfigurations-Parameter `disabled-auth-methods` in der `/etc/opsi/opsiconfd.conf` können bestimmte Authentifizierungs-Methoden deaktiviert werden.
Wenn Sie beispielweise SSO verwenden und nur WebDAV als Depot-Protokoll verwenden, können Sie alle anderen Authentifizierungs-Methoden deaktivieren:
[source,configfile]
----
...
Das Passwort des Benutzers 'pcpatch' wird verschlüsselt abgelegt und auch nur verschlüsselt übertragen.
...
Als weitere Maßnahme sollten das Passwort des Benutzers 'pcpatch' regelmäßig geändert werden.
Da das Klartext-Passwort niemandem bekannt sein muss, kann es z.B. durch den regelmäßigen Aufruf (z.B. per Cronjob) des folgenden Skriptes auf ein zufälliges Passwort setzen.
...
WARNING: Dem Benutzer 'pcpatch' sollte keine Shell zugewiesen werden, damit dieser sich nicht direkt am Server anmelden kann.

NOTE: Wenn die opsi-Clients ausschließlich über WebDAV auf die Depots zugreifen (siehe Hostparameter `clientconfig.depot.protocol`), können sowohl der Benutzer 'pcpatch' als auch der Samba-Server komplett deaktiviert werden.
