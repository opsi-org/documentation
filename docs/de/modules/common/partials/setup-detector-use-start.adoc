...
image::common:opsi-setup-detector-context.png["Den *opsi-setup-detector* starten Sie auch aus dem Explorer heraus.", width=600, pdfwidth=60%]
...
image::opsi-modules:osd_config_dlg_de.png["Nach dem ersten Start konfigurieren Sie den *opsi-setup-detector*.", pdfwidth=40%, width=500]
...
TIP: Eine ausführliche Beschreibung aller Einstellungen finden Sie im Kapitel xref:opsi-modules:setup-detector.adoc#opsi-setup-detector-configuration[*opsi-setup-detector Konfiguration*].
...
image::opsi-modules:osd_help-circle22.png["Über dieses Icon zeigen Sie die Onlinehilfe an.", pdfwidth=10%]

[[osd_page_start]]
.Die *opsi-setup-detector*-Startseite
image::opsi-modules:osd_page_start_de.png["Die *opsi-setup-detector*-Startseite", pdfwidth=80%, width=800]
