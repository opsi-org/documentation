...
+ 
.Stellen Sie die beiden Produkte auf _setup_ und bestätigen Sie die Änderungen.
image::opsi-configed-hwaudit-setup.png["Stellen Sie die beiden Produkte auf _setup_ und bestätigen Sie die Änderungen.", pdfwidth=80%]
...
Nach dem Booten des Windows-Clients sehen Sie zunächst ein `opsi-script`-Fenster und den Installer. Das kleine Vorhängeschloss oben rechts weist darauf hin, dass der opsi-Loginblocker aktiv ist. Er verhindert, dass sich ein Anwender anmeldet, bevor die Installation abgeschlossen ist (siehe Abschnitt xref:clients:windows-client/windows-client-agent.adoc#opsi-manual-client-agent-opsi-login-blocker[opsi-Loginblocker]). 

.Die Installation von `swaudit` beginnt nach dem Reboot.
image::opsi-install-swaudit.png["Die Installation von `swaudit` beginnt nach dem Reboot.", pdfwidth=80%]

Die gesammelten Informationen werden unter den Tabs _Hardwareinformationen_ bzw. _Software-Inventarisierung_ angezeigt.//WO??
