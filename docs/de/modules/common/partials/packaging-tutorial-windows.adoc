...
image::newprod-new-property.png["Screenshot: Eine (weitere) {product-property} definieren", pdfwidth=90%]

Antworten Sie ja, so müssen Sie die {product-property} beschreiben:

Die {product-property} wird clientspezifisch gespeichert und besteht aus einem Namen (key) der verschiedene Werte (Values) zugeordnet bekommen kann und die dann vom opsi-script-Skript abgefragt werden können.
...
image::newprod-property-type.png["Screenshot: Datentyp der {product-property} wählen", pdfwidth=90%]
