...
Die folgenden Abschnitte beschreiben, wie Sie eine Setup-Datei analysieren und daraus ein OPSI-Produkt erstellen. Klicken Sie dazu auf der Startseite (<<osd_page_start>>) auf die Schaltfläche _Analysiere eine Datei und erzeuge ein OPSI Paket_. Danach navigieren Sie im Dateiauswahldialog zur gewünschten Installerdatei. Der `opsi-setup-detector` beginnt direkt mit der Analyse.
...
image::opsi-modules:osd_page_analyze_de.png["Der *opsi-setup-detector* hat eine Datei analysiert.", pdfwidth=80%, width=800]

War die Analyse nicht erfolgreich, sehen Sie stattdessen entweder diesen Dialog:

.Der Dialog *Sorry Unknown Installer*
image::opsi-modules:osd_unknown_select_installer.png["Der Dialog *Sorry Unknown Installer*", pdfwidth=30%]

Der Typ der Installerdatei konnte nicht ermittelt werden. +
Sie können jetzt den Vorgang per Klick auf _Cancel_ abbrechen oder aus dem Drop-down-Menü einen Installertyp auswählen und die Paketerstellung fortsetzen.

oder Sie sehen diesen Dialog:

.Der Dialog *Detektierter Installer ist nicht bekannt*
image::opsi-modules:osd_detected_not_known_installer_de.png["Der Dialog *Detektierter Installer ist nicht bekannt*", pdfwidth=30%]
...
image::opsi-modules:osd_installer_info_installshield_de.png["Additional Info: InstallShield", pdfwidth=90%]

image::opsi-modules:osd_installer_info_QT_de.png["Additional Info: Qt-Installer", pdfwidth=90%]

image::opsi-modules:osd_installer_info_anywhere_de.png["Additional Info: InstallAnywhere", pdfwidth=90%]
...
image::opsi-modules:osd_page_setup1_de.png["Auf diesem Reiter sehen Sie das Ergebnis einer erfolgreichen Analyse.", pdfwidth=80%, width=800]
...
* _MsiUpgrade_: Upgradecode bei MSI-Installern oder Installern, die MSI enthalten
...
Siehe auch: xref:opsi-modules:setup-detector.adoc#osd_config_preferMsiUninstall[Konfiguration: preferMsiUninstall]
...
image::opsi-modules:osd_page_product1_de.png["Konfigurieren Sie das OPSI-Produkt.", pdfwidth=80%, width=800]
...
* _Uninstall_before_install_: Erzeugt ein zusätzliches, boolesches Property (Voreinstellung `true`) zur Steuerung ob vor einer Installation eine Deinstallation durchgeführt werden soll. (siehe Abschnitt xref:clients:windows-client/softwareintegration.adoc#opsi-setup-detector-uninstall_before_install[Uninstall_before_install]).
...
image::opsi-modules:osd_page_product2_de.png["Konfigurieren Sie Prioritäten und Abhängigkeiten.", pdfwidth=80%, width=800]
...
image::opsi-modules:osd_password_dlg.png["Dialog zur Passworteingabe", pdfwidth=40%]
...
image::opsi-modules:osd_dependency_editor.png["In diesem Dialog konfigurieren Sie Abhängigkeiten.", pdfwidth=40%]
...
image::opsi-modules:osd_property-editor.png["In diesem Dialog konfigurieren Sie Produkt-Propertys.", pdfwidth=40%]
...
image::opsi-modules:osd_page_producticon_de.png["In diesem Dialog wählen Sie ein Symbol für Ihr Produkt aus.", pdfwidth=80%, width=800]
...
image::opsi-modules:osd_page_create_de.png["Auf dem letzten Reiter erzeugen Sie das OPSI-Produkt.", pdfwidth=80%, width=800]
...
image::opsi-modules:osd_overwrite_dlg.png["Backup-Dialog", pdfwidth=40%]
