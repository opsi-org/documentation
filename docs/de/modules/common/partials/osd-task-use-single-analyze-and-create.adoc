...
* *Bevorzuge Silent Installation:* Wird (wenn möglich) eine 'silent' Installation einer 'unattended' vorgezogen.
...
* *Importiere control Datei:* Mit diesem Button, können Die Daten aus einer bestehenden opsi control Datei (`control, control.toml`) in das laufende Projekt importiert werden.
