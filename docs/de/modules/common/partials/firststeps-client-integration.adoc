...
. Laden Sie den Installer von ihrem configserver herunter. Der Installer liegt unter https://<fqdn oder ip des configservers>:4447/public/opsi-client-agent/ und hat den Dateinamen +
...
. Client-Id soll der fqdn des Clients sein. Opsi Service url sollte das Format https://<fqdn oder ip des opsi servers>:4447 haben. Username und Passwort sollten im Fall einer Erstinstallation +
...
Beim Aufruf des Installers können Parameter mitgegeben werden, welche eine Voll-Automatisierung des Vorgangs ermöglichen. Details dazu finden Sie im Handbuch unter <<opsi-manual-clientagent-subsequent-installation>>.
