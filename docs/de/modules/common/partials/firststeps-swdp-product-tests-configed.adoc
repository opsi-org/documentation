...
. Klicken Sie neben dem Produkt _opsi-configed_ in die Spalte _Angefordert_ und wählen Sie aus dem Drop-down-Menü den Eintrag _setup_. Das zweite Symbol von links (die Diskette) wechselt seine Farbe auf Pink. Klicken Sie sie an, um die Einstellungen zum OPSI-Server zu übermitteln.
+
.Stellen Sie das Produkt auf *setup* und bestätigen Sie die Änderungen.
image::common:opsi-configed-setup.png["Stellen Sie das Produkt auf *setup* und bestätigen Sie die Änderungen.", width=800, pdfwidth=80%]
...
image::common:install-opsi-configed.png["Die Installation von *opsi-configed* beginnt nach dem Reboot (hier: Windows).", pdfwidth=80%]
