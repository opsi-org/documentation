...
* _opsi Server Version_: Aktivieren Sie die Checkbox _opsi 4.1 oder neuer_.

* _opsi Workbench_ : Wählen Sie die Workbench aus (`/var/lib/opsi/workbench`).

* _Kompatibilität der Befehlsausführung (SUDO)_ : Aktivieren Sie die Option _opsi 4.0.4 oder neuer_ und darunter die Checkbox _SUDO ohne Passwort_.
...
* _Bestehendes Netzlaufwerk verwenden_: Aktivieren Sie diese Option.

* _Entwicklungsordner_: Hier tragen Sie den vollständigen Pfad zum Verzeichnis ein, in dem die opsi-Pakete erstellt werden sollen. Idealerweise ist das der Pfad zur gemounteten Freigabe mit der opsi-Workbench.
...
* _Skripteditor_: Wählen Sie den Texteditor zur Bearbeitung der Skripte aus:
  ...
  - Linux: Pfad zum Editor, z.{nbsp}B. `/usr/local/bin/jedit`; Feld _Kommandozeilenoption_ bleibt leer
...
image::opb_conf_program.jpeg["oPB-Einstellungen: Reiter *Programm*", pdfwidth=80%]

Auf dem Reiter _opsi Verwaltungsbefehle_ nehmen Sie die folgende Einstellung (*abweichend von den Voreinstellungen*) vor:

* _Paketieren_: Tragen Sie in dieses Feld den Befehl zum Bauen des Paketes ein: `opsi-makepackage -v`

.oPB-Einstellungen: Reiter *opsi Verwaltungsbefehle*
image::opb_conf_opsi.png["oPB-Einstellungen: Reiter *opsi Verwaltungsbefehle*", pdfwidth=80%]
...
image::opb_start.jpg["opsi PackageBuilder: Start im Onlinemodus", pdfwidth=80%]
...
image::opb_tab_product.jpg["Öffnen Sie ein vorhandenes opsi-Paket.", pdfwidth=80%]
...
.oPB: *Edit*-Button
image::opb_btnSkriptEdit.png["oPB: *Edit*-Button",width=20]

Wenn Sie diesen Button anklicken, öffnet sich der in der Konfiguration definierte Editor zum Modifizieren des Skriptes. Unter Windows ist das der oPB-ScriptEditor:

.Der integrierte oPB-ScriptEditor unter Windows
image::opb_ScEdit.jpg["Der integrierte oPB-ScriptEditor unter Windows", pdfwidth=80%]
...
* anpassbare Lexerdefinition (dazu muss der Editor über das Startmenü aufgerufen werden)

* automatische Vervollständigung für Syntax-Elemente und Variablen
...
Auf dem Reiter _Abhängigkeit_ sehen Sie die definierten Abhängigkeiten zu anderen opsi-Produkten (siehe Abschnitt <<opsi-setup-detector-product-configuration-priority_dependecy>>):

.oPB: Reiter *Abhängigkeit*
image::opb_tab_dependencies.jpg[oPB: Reiter *Abhängigkeit*", pdfwidth=80%]

Auf dem Reiter _Produktvariable_ stehen links jeweils die Produkt-Propertys, inklusive Typ, Wert usw:

.oPB: Reiter *Produktvariable*
image::opb_tab_property.jpg["oPB: Reiter *Produktvariable*", pdfwidth=80%]
...
|image::opb_btnPacken.png[width=18] | Startet eine eine SSH-Verbindung zum opsi-Server und ruft dort das Kommando zum Paketieren auf (siehe Abschnitt <<opsi-softwintegration-create-opsi-package-makeproductfile>>).
|====
