...
* Debian-basierte Distributionen (Debian, Ubuntu): Diese Distributionen verwenden die Installations-Quellen aus dem Netz. In der Depot-Freigabe liegen lediglich die Netboot-Versionen der Kernel und der dazugehörigen `initrd`-Dateien. Da diese Dateien nicht besonders viel Platz benötigen, sind sie Teil des OPSI-Paketes. Ausnahmen sind:
  ...
  - Ubuntu 22.04: Das Paket `ubuntu22-04` erwartet ein Server ISO-Image im Produkt-Unterverzeichnis `iso`.
...
image::common:netboot-debian12-properties.png["Propertys der Linux-Netboot-Produkte (hier: *debian12*)", width=300, pdfwidth=30%]
...
NOTE: Der Linux-Client-Agent ist eine xref:opsi-modules:linux.adoc[kostenpflichtige Erweiterung]. Das heißt, dass Sie eine Lizenz-Datei benötigen. Sie erhalten diese, nachdem Sie die Erweiterung gekauft haben. Zu Evaluierungszwecken stellen wir Ihnen kostenlos eine zeitlich befristete Lizenz zur Verfügung. Bitte kontaktieren Sie uns dazu per mailto:info@uib.de[E-Mail].

[[firststeps-osinstall-netboot-distro-debian-ubuntu-mint]]
=== Debian, Ubuntu und Linux Mint

Für gewöhnlich stehen für OPSI alle aktuell vom Hersteller bzw. der Community unterstützten Version von Debian, Mint und Ubuntu zu Verfügung. (siehe xref:supportmatrix:supportmatrix.adoc#opsi-manual-supportmatrix-clients-linux[OPSI-Client: unterstützte Linux-Distributionen]).

Ubuntu ab `ubuntu22-04` und die Linux-Mint-Netboot-Produkte erwarten zusätzlich ein ISO-Image im Produkt-Unterverzeichnis `iso`. Die Debian Pakete beziehen die Installationspakete aus dem Netz bzw. dem angegebenen Repository.

NOTE: Wegen des ab Ubuntu 22.04 verwendeten Installers sind nicht alle oben aufgeführten Propertys im Netboot-Produkt enthalten.
...
Aktuell stehen die folgenden Netboot-Produkte für SUSE Linux Enterprise Server (SLES) und openSUSE Leap in den aktuellen Versionen zur Verfügung. (siehe xref:supportmatrix:supportmatrix.adoc#opsi-manual-supportmatrix-clients-linux[OPSI-Client: unterstützte Linux-Distributionen]).
...
Aktuell stehen die folgenden Netboot-Produkte für Red Hat Enterprise Linux (RHEL), AlmaLinux, Rocky Linux und Oracle Linux in den aktuellen Versionen zur Verfügung. (siehe xref:supportmatrix:supportmatrix.adoc#opsi-manual-supportmatrix-clients-linux[OPSI-Client: unterstützte Linux-Distributionen]).
...
Zusätzlich gibt es bei den Produkten `redhat8`, `redhat9` und `redhat10` das Property `cdn_credentials`, mit der Sie Ihre Red Hat CDN-Organisation-ID und den Aktivierungsschlüssel angeben können, um die Installation über das CDN durchzuführen. Diese Property ist optional.
