...
* Bei den Suse- und RedHat-artigen, Linux Mint Distributionen und Ubuntu 22-04 findet die Installation aus auf dem opsi-Server liegenden Installationsquellen statt und ist damit schneller und unempfindlicher gegen Störungen als beim Zugriff auf Repositorys aus dem Internet.
...
Die Basis-Installation bezieht ihre Pakete von den offiziellen UCS-Repositorys. Eine Installation mit lokalen Paketquellen ist ebenfalls möglich.
