...
NOTE: Der distributionseigene Installer ist kein eigentliches Programm, das opsi dann aufruft. Stattdessen handelt es sich um eine Kombination aus Kernel und `initramfs` (komprimierte Datei, die für den Systemstart benötigte Dateien enthält).

Die grundsätzliche OS-Installation, inklusive Partitionierung, LVM, Basissoftware usw., liegt in der Hand des Installers und findet nach dem Start des opsi-Linux-Bootimages statt. Wie Sie die Installations-Quellen bereitstellen, hängt unter anderem von der Linux-Distribution ab:

* Debian-basierte Distributionen (Debian, Ubuntu, Linux Mint usw.): Es werden die Installations-Quellen aus dem Netz verwendet. Auf dem Depot-Share liegen lediglich die Netboot-Versionen der Kernel und der dazugehörigen `initramfs`-Dateien. Da diese Dateien nicht besonders viel Platz benötigen, sind sie Teil des opsi-Paketes. Ausnahmen sind:
  - Das Paket `ubuntu22-04` erwartet ein ISO-Image im Produkt-Unterverzeichnis `iso`.
  - Linux Mint: opsi 4.2.0.1-6 und früher erwarten den Inhalt der offiziellen Installations-Medien über eine NFS-Freigabe. Ab opsi 4.2.0.2-1 speichern Sie ein ISO-Image im Produkt-Unterverzeichnis `iso`.

* SUSE- und Red-Hat-basierte Distributionen: Sie stellen die Installations-Quellen bereit und legen ein ISO-Image auf dem Depot-Share ab.
