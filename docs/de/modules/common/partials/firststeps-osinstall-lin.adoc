...
NOTE: Der distributionseigene Installer ist kein eigentliches Programm, das opsi dann aufruft. Stattdessen handelt es sich um eine Kombination aus Kernel und `initrd`.
...
* Debian-basierte Distributionen (Debian, Ubuntu, Linux Mint usw.): Es werden die Installations-Quellen aus dem Netz verwendet. Auf dem Depot-Share liegen lediglich die Netboot-Versionen der Kernel und der dazugehörigen `initrd`-Dateien. Da diese Dateien nicht besonders viel Platz benötigen, sind sie Teil des opsi-Paketes. Ausnahmen sind:
