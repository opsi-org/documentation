...
* `root password`: Setzt das Passwort für `root`. (Default=`linux123`)

* `user name`: Hier steht der Name des nicht-privilegierten Benutzers ohne `root`-Rechte. (Default=`user`)
...
* `setup after install`: Tragen Sie hier eine Liste von opsi-Produkten ein, die im Anschluss an die OS-Installation auf `setup` gestellt werden sollen.
