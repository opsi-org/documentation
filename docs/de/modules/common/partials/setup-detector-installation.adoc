...
Den _opsi-setup-detector_ gibt es derzeit für Windows, Linux und macOS. Sie können das Programm direkt über opsi installieren. Es gehört zu den Standard-Localboot-Produkten und sollte bereits auf dem opsi-Server vorhanden sein. Ist das nicht der Fall, stellen Sie es über den folgenden Befehl bereit:
...
* `.deb`- bzw. `.rpm`-Pakete analysieren die entsprechenden Standard Paketverwaltungstools unter Linux. Diese Detailanalyse ist daher nur unter einem passenden Linux möglich.
