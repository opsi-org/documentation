...
//gibt's das Paket noch? wenn ja, wo?
TIP: Einige für die Windows-Bereitstellung mit opsi nützliche Werkzeuge installiert das Paket `opsi-windows-support`.

[[firststeps-osinstall-fill-base-packages]]
== opsi-Produkte vervollständigen

Template, Hülle
Windows-Basispakete vervollständigen

zur Installation von Windows angebotene Netboot-Produkte enthalten (im Verzeichnis `installfiles`, siehe Abschnitt <<firststeps-osinstall-structure-dirs>>), das ist leer, 


Hüllen, die einen wesentlichen Teil nciht enthalten
es sind drin: alle Skripte usw. von opsi, aber den Inhalt der Installations-Medien muss der Anwender selbst beitragen


In den zum Download empfohlenen Paketen sind lediglich Basispakete () enthalten. Diese dienen zur Installation der Windows-Betriebssysteme, müssen jedoch noch vervollständigt werden, da sie nicht die Dateien zur Installation des Betriebssystems selbst enthalten.
...
== NT6-Familie (ab Windows 7/2008R2)
...
In jedem Fall benötigen Sie ein "Windows PE add-on für das Assessment and Deployment Kit" (ADK, Windows 10 bzw. 11), 
// bzw dessen Vorgänger "Windows Automated Installation Kit" (Windows AIK; bis Windows 7), 
//
welches Sie auf ein unterstütztes Windows Betriebssystem (bevorzugt 64 Bit) installieren.: +


* link:https://learn.microsoft.com/de-de/windows-hardware/get-started/adk-install[Windows ADK und Add-Ons]


Installieren Sie das Windows PE Addon für ADK (nach Möglichkeit auf einer 64Bit- Maschine) in den vorgeschlagenen Pfad unter `Program Files (x86)`. Wählen Sie nur die "Windows-Vorinstallationsumgebung (Windows PE)" aus; Abhängigkeiten werden automatisch ausgewählt. +
////
...
//im stable Zweig ist die Doku der winpe erstellung etwas neuer als in Deinem Zweig:
//https://docs.opsi.org/opsi-docs-de/4.2/windows-client-manual/os-installation.html#firststeps-osinstall-fill-base-packages-nt6

Die einfachste Methode setzt voraus, dass Sie bereits einen Windows-Computer haben, auf dem sowohl der opsi-client-agent installiert ist, als auch das Windows ADK (Win10, Win11).
...
==== WinPE automatisiert mit opsi erstellen
...
* Wenn Treiber für Massenspeicher (Festplatten) integriert werden sollen, gehen Sie bitte wie hier beschrieben vor: <<firststeps-osinstall-fill-base-packages-nt6-extendpe-opsi>>.
