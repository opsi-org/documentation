...
TIP: Sie können die Treiber über die Management-Oberfläche `opsi-configed`, Reiter _Hardware-Information_, bereitstellen. Lesen Sie dazu auch den Abschnitt xref:gui:configed.adoc#opsi-manual-configed-automat-treiberintegration[Automatisierte Treiberintegration]. 
...
Liegt zu einem Client eine Hardware-Inventarisierung vor, so können Sie über den folgenden Befehl ausgeben, welche Treiber das Bootimage via PCI-IDs, USB-IDs, HD-Audio-IDs und `additional_drivers` (bzw. `byAudit`) zur Installation auswählen würde und zu welcher Hardware noch kein Treiber bereitsteht:
