...
Für Dell-Hardware gibt es beispielsweise Netzwerk- und Speicher-Treiber, die der Hersteller speziell für den WinPE-Einsatz empfiehlt. Dieser Abschnitt beschreibt, wie Sie weitere Treiber von Hand in ein WinPE integrieren. Voraussetzung ist, dass Sie vorher alle Schritte zum Erstellen des WinPE ausgeführt haben (siehe Abschnitt <<firststeps-osinstall-fill-base-packages-nt6-pe-opsi>>).
