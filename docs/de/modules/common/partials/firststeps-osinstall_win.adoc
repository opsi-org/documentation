...
//In den zum Download empfohlenen Paketen sind lediglich Basispakete (Grundausstattung) enthalten. Diese Komponenten dienen zur Installation der Windows-Betriebssysteme, müssen jedoch noch vervollständigt werden, da sie nicht die Dateien zur Installation des Betriebssystems selbst enthalten.

//Verwendung des Wortes 'Framework'. Die netboot-Produkte für Windows sind komplett fertige Produkte, bei denen noch die Windows Installations DVD,  WinPE und Treiber hinein kopiert werden müssen.
//Also so etwas 'zu befüllendes'. Ein richtig gutes Wort dafür habe ich auch nicht. Ein 'Framework' oder 'Template' trifft es aber nicht richtig.
...
Für unbeaufsichtigte Installationen gibt es eine Steuerdatei namens `unattend.xml`. Sie finden diese im Verzeichnis `custom`, z.{nbsp}B. unter `/var/lib/opsi/depot/win10/custom` oder `/var/lib/opsi/depot/win10-x64/custom`. Die von uns mitgelieferte Datei `unattend.xml` enthält Verweise auf die Eigenschaften der Netboot-Produkte, darunter Platzhalter für den Namen des Administrator-Accounts und des dazugehörigen Passworts.
