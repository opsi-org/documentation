...
Der bevorzugte Weg, Treiber automatisch zuzuordnen, ist über die opsi-Hardware-Inventarisierung (siehe Abschnitt xref:clients:windows-client/rollout-products.adoc#firststeps-software-deployment-product-tests-inventory[Inventarisierung (*hwaudit*/*swaudit*)]).
...
//Client auswählen, Reiter Hardware-Informationen, ich kann das auch von Hand kopieren, wenn ich das einmal gemacht habe, werden die hochgel. Treiber automat. für jeden dieser vendor gefunden, von daher ist die Vorgehensweise sehr gut, insb. für Firmen, die eine überschaubare Anzahl von Rechnertypen haben, für jeden Rechnertyp enmal Treiberverzeichnis runterladen, nciht benötigte entfernen, dann hochladen ins Netboot-Produkt, das bringen wir heutzutage bei, nach additional/byAudit (im Rahmen des Hardware-Audits automatisch finden)
...
Im Verzeichnis des Netboot-Produktes gibt es Platz für manuell und automatisch hinzugefügte Treiber.

.So sieht der Verzeichnisbaum im Ordner `drivers`aus.
image::drivers-directory-tree.png["So sieht der Verzeichnisbaum im Ordner `drivers`aus.", pdfwidth=80%]
...
//in 90 % der Fälle reicht dieses automatisch Hochladen, 
