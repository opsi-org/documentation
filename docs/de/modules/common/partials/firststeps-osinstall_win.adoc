...
Für einen ersten Tests können Sie eine VMware-Appliance verwenden, die einen _leeren_ Rechner abbildet und in _VMware Workstation Player_ laufen kann.
...
Die Steuerdatei für die unattended Installation ist die `unattend.xml`, welche unter `/var/lib/opsi/depot/<productid>/custom` zu finden ist.
