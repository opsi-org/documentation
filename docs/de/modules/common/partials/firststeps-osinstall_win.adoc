...
NOTE: Überprüfen Sie die Einstellungen im BIOS. Netzboot muss aktiviert sein und an erster Stelle der Bootoptionen stehen.

Virtuelle Hardware wird in der Regel gut von den Windows-Standardtreibern unterstützt. Zur Installation von Windows auf neueren physischen Rechnern müssen Sie möglicherweise vorab zusätzliche Treiber integrieren.

Um die in diesem Kapitel vorgestellten Schritte nachvollziehen zu können, sollten Sie vorher einen Windows-Client in die opsi-Ugebung aufgenommen haben (siehe Kapitel xref:clients:windows-client/adding-clients.adoc#add-new-clients[Neue Clients hinzufügen]).
