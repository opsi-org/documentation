...
image::logdateien-os-install.png["Viele der bei der OS-Installation geschriebenen Logdateien finden Sie im Verzeichnis `c:\Windows\Panther\`.", pdfwidth=80%]
...
image::product-key.png["Den Windows-Produktschlüssel hinterlegen Sie in den Produkteigenschaften.", pdfwidth=80%]
...
. Klicken Sie rechts oben auf die Kachel _Depoteigenschaften_. 
. Öffnen Sie den Reiter _Produkt-Defaultproperties_.
. Wählen Sie dort das gewünschte Netboot-Produkt aus, z.{nbsp}B. _win10-x64_.
...
image::bootimage-parameter.png["Die Parameter für das Linux-Bootimage können Sie über den `opsi-configed` anpassen.", pdfwidth=80%]
...
image::product-directory-tree.png["So sieht der Verzeichnisbaum für das Netboot-Produkt `win10-x64` aus.", pdfwidth=80%]
...
enthält die Metadaten des Produktes, so wie sie vom Paketierer bereitgestellt wurden. Die Datei liegt hier nur zu Informationszwecken, d.h. Änderungen an dieser Datei haben keinerlei Auswirkungen auf das System.
...
//nachdem das Windows fertig installiert ist, wollen wir bestimmte Dinge tun: in das fertige Windows den Client-Agent rein installieren, der nach Abschluss der OS-Inst dafür sorgt, dass opsi anfangen kann, die Applikationssoftware zu installieren. Die Skripte werden der Reihe nach abgearbeitet, deswegen nummeriert, das entscheidene Skript 
// das Skript, was wir erwarten würden zur Client-Agent-Installation, ist hier gar nicht drin, weil das kommt mit dem opsi-Client-Agent

//wesentl. Unterschiede: Verzeichnis custom: Änderungen, die in einem solchen Verzeichnis gemacht werden, überschreibt keine Installations-/Update-Routine von opsi, sie werden also nicht mehr angefasst. Letztendlich sind aus unserer Sicht die Verzeichnisse installfiles/winpe auch custom-Verzeichnisse, alle drei Verzeichnisse werden, wenn ein neues Windows-Produkt kommt, zunächst gesichert, dann neues Produkt installiert (mit leeren Verzeichnissen) und am Ende wird der gesicherte Kram zurückkopiert 
