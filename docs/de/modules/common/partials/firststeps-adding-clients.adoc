...
Auf der Website link:https://tools.43.opsi.org/[opsi-Tools] finden Sie ein ISO-Abbild der Boot-CD. Laden Sie das Image herunter und brennen es auf eine CD oder flashen Sie ISO mit einem geeignetem Tool, zB. Balena Etcher auf einen USB-Stick. Danach starten Sie den Rechner von der CD oder USB-Stick. Der Installer begrüßt Sie mit dem folgenden Boot-Menü:

.Die Boot-CD präsentiert nach dem Start das folgende Menü.
image::common:opsi-client-boot-cd.png["Die Boot-CD präsentiert nach dem Start das folgende Menü.", pdfwidth=90%]
...
image::common:boot-cd-config.png["Die Netzwerkeinstellungen für den neuen Client.", pdfwidth=90%]

Springen Sie mit der [Tab]-Taste zu _OK_ und drücken dann [Eingabe].

.Wie soll sich der Client beim opsi-Server anmelden?
image::common:boot-cd-select.png["Wie soll sich der Client beim opsi-Server anmelden?", pdfwidth=70%]
...
image::common:boot-cd-authent.png["Authentifizieren Sie sich am opsi-Server.", pdfwidth=90%]
...
image::common:boot-cd-product.png["Netboot-Produkte zur Installation auswählen", pdfwidth=70%]
