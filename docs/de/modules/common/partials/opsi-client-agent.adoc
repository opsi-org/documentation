...
Das Programm `oca-installation-helper[.exe]` erfordert Root/Administrator-Rechte; es kennt einige Parameter und Optionen, die Sie mit `--help` anzeigen.

.Die Onlinehilfe von *oca-installation-helper*
image::common:oca_installer_help.png["Die Onlinehilfe von *oca-installation-helper*", pdfwidth=80%]
...
Unter windows können Parameter für den `oca-installation-helper` auch in der Registry spezifiziert werden.
Legen Sie dazu unter HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\opsi-client-agent einen Schlüssel "INSTALL_PARAMS" an.
Dort können Sie dann Aufrufparameter hinterlegen, wie z.B. `--service-password \{crypt\}qwertzuiop --client-id client1.test.lan`.
...
image::common:opsiclientd-shutdown-notifier_timepicker.png["*opsiclientd*: Shutdown Notifier mit Zeitauswahl (*timepicker*)", pdfwidth=30%]
...
image::common:eventflowchart.png["Der Ablauf eines Standard-Events im Überblick", pdfwidth=80%, width=1000]
...
image::common:opsiclientd-control-server-web-interface.png["Eine Möglichkeit zur Authentifizierung ist der lokale Administrator-Account des Clients.", pdfwidth=70%, width=800]
...
image::common:opsiclientd_infopage_event_on_demand.png["Die Infoseite stellt die Abläufe auf einer Zeitachse dar.", pdfwidth=70%, width=800]
...
Die in unserem link:https://github.com/opsi-org/opsiclientd/tree/v4.3/opsiclientd_data/[GitHub-Repository] veröffentlichten Dateien zeigen jeweils die Standardwerte für Windows, Linux und macOS.

.Unser GitHub-Repository enthält eine *opsiclientd.conf* mit Standardwerten.
image::common:github-client-agent.png["Unser GitHub-Repository enthält eine *opsiclientd.conf* mit Standardwerten.", pdfwidth=80%, width=800]
...
Zum Verändern von Host-Parametern stehen Ihnen drei Möglichkeiten zur Verfügung:
...
image::common:opsiclientd-configuration-via-configed-serverdefault.png["Die Host-Parameter für alle Clients konfigurieren Sie in der *Server-Konfiguration*.", pdfwidth=70%, width=800]
...
image::common:opsiclientd-configuration-via-configed.png["Die Host-Parameter für einzelne Clients konfigurieren Sie in der *Client-Konfiguration*.",pdfwidth=70%, width=800]

[[opsi-manual-clientagent-configuration-webservice-webgui]]
==== *OPSI-WebGUI*

Im `OPSI-WebGUI` bearbeiten Sie Host-Parameter entweder pro Client über xref:gui:webgui/userguide-clients.adoc#opsi-manual-opsiwebgui-opsiclients-attributes-parameter[_Clients_ / _Konfiguration_] oder serverweit über xref:gui:webgui/userguide-server.adoc#opsi-manual-opsiwebgui-opsiservers-attributes-parameter[_Server_ / _Konfiguration_].

Gerade bei der Arbeit mit mehreren Clients ist die direkte Gegenüberstellung von Tabelle und Konfigurationsbereich hilfreich.
Zusätzlich sammelt das WebGUI Änderungen zunächst lokal und zeigt sie gesammelt unter _Ungespeicherte Änderungen_ an, bevor Sie diese explizit auf dem Server speichern.
...
image::common:opsi-script-configuration-skin.png["In diesem Verzeichnis finden Sie das Hintergrundbild und die Konfigurationsdatei für *opsi-script*.",pdfwidth=70%, width=800]
...
image::common:opsi-systray-message.png["Das Systray-Programm des Client-Agent informiert über anstehende Installationen."]

.Über das Kontextmenü der rechten Maustaste steuern Sie verschiedene Aktionen.
image::common:opsi-systray-menue.png["Über das Kontextmenü der rechten Maustaste steuern Sie verschiedene Aktionen."]

.Auch unter macOS gibt es ein Systray-Programm, das Sie über die rechte Maustaste steuern.
image::common:opsi-systray-macos.png["Auch unter macOS gibt es ein Systray-Programm, das Sie über die rechte Maustaste steuern.", pdfwidth=70%, width=326]
...
image::common:opsi-systray-properties.png["Das Systray-Programm konfigurieren Sie über Produkt-Propertys.", width=800, pdfwidth=70%]
...
image::common:opsi-systray-linux.png["Das Systray-Programm unter Linux (GNOME Shell)", pdfwidth=70%]
