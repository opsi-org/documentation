...
Der Client-Agent bietet unter Windows ein Systray-Programm, das in der Taskleiste läuft. Es bietet schnellen Zugriff auf wichtige Funktionen und erledigt die folgenden Aufgaben:
...
* Start einer Installation aktiv anfordern (ebenfalls Kontextmenü)
...
Sie steuern das Systray-Programm über die Produkteigenschaften des Localboot-Produktes `opsi-client-agent`:
...
TIP: Die Logdateien zu `opsi-client-systray` finden Sie unter `c:\opsi.org\applog`. Weitere Informationen zu wichtigen Verzeichnissen lesen Sie in Abschnitt <<opsi-manual-client-agent-directories>>.
