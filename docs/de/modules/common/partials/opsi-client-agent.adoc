...
TIPP: Event-Konfigurationen können voneinander "erben". Ist beispielsweise über die Option `super` die ID einer anderen Event-Konfiguration gesetzt, erbt sie alle Optionen der Parent-Konfiguration (bis auf `active`). Geerbte Optionen können überschrieben werden. Das Deaktivieren von Events beeinflusst die Vererbung nicht.
