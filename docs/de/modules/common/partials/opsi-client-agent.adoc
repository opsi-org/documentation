...
Sie können die Anweisungen entweder über die Management-Oberfläche `opsi-configed` übermitteln oder das Kommandozeilentool `opsi-cli` verwenden (siehe Abschnitt xref:server:components/commandline.adoc#server-components-opsi-cli[*opsi-cli*]). Dazu führen Sie JSON-RPC-Methoden `hostControlSafe_*` aus (siehe Abschnitt xref:server:components/commandline.adoc#server-components-opsi-cli-commands-jsonrpc[jsonrpc]). Sie haben die folgende Form:
...
Über die Web-Schnittstelle können Sie Clients auch herunterfahren und neu starten (siehe Abschnitt <<>>)

Über den `opsiclientd`-Webservice können Sie Clients herunterfahren und neu starten. Über die Management-Oberfläche `opsi-configed` erreichen Sie die Funktionen über das Menü _OpsiClient_
