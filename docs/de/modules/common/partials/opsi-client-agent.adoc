...
image::eventflowchsrt.png["Abbildung: Ablauf eines Standard-Events", pdfwidth=80%]
...
NOTE: Ob es einen _Abbrechen_-Button gibt, hängt vom Paramter `action_user_cancelable` an. Ist die dahinter definierte Zahl > 0, kann der Benutzer die anstehende Aktion genau so viel mal verschieben. Nach Erreichen des Maximalwerts (oder bei `action_user_cancelable` = 0), kann der Anwender die Aktion nicht mehr verschieben. Ein Button, der die Wartezeit unterbricht und die Aktion ohne weitere Verzögerung startet, ist in jedem Fall sichtbar.
