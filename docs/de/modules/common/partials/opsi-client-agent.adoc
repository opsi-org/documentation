...
Der zentrale Client-Agent-Dienst heißt `opsiclientd`. Der Service läuft die ganze Zeit und übernimmt mehrere Aufgaben. Seine wesentlichen Funktionen und Features sind:

er kann zu bestimmten Situationen noch einmal besonders aktiv werden -- unabhängig davon, dass er eh die ganze Zeit läuft, er wartet, besondere Aktivitäten heißen Events. Wenn ein Event tatsächlich stattfindet, passiert das: `opsiclientd` nimmt Kontakt zum opsi-Server auf, authentifiziert sich, fragt, ob es Aufgaben für ihn gibt. Der opsi-Server schaut ins Backend und antwortet z.B: ja, Du brauchst einen neuen Firefox, den holst Du Dir aus dem folgenden Depot, hier ist das Passwort. `opsiclientd` mountet die entsprechende Freigabe (Share), startet opsi-script mit den richtigen Credentials für den Service, opsi-script schaut nach, welche Dinge installiert werden müssen, holt sich die Pakete und installiert. Wenn opsi-script fertig ist, sorgt `opsiclientd` dafür, dass die Freigabe wieder ausgehängt wird und bei Bedarf führt er einen Reboot aus. Das ist der eigentliche Kern des Event-Ablaufs.
...
3. **Remote Konfiguration:** Alle wichtigen Konfigurationsdaten des `opsiclientd` lassen sich zentral über die Host-Parameter global oder client-spezifisch bearbeiten.

4. **Kontakt mit dem opsi-Configserver bei konfigurierten Events:** Sobald die konfigurierten Events auftreten, nimmt der `opsiclientd` Kontakt zum *opsi-Configserver* auf. Dabei werden Konfigurationen und anstehende `action-requests` per JSON-RPC abgefragt. Ein Standard-Event ist beispielsweise `gui_startup`, das beim Start des Rechners (Start der GUI) und somit vor dem Login aktiv wird.

5. **Interaktion mit dem Anwender über den opsi-notifier:** Der `opsiclientd` startet den *opsi-notifier*, um mit dem Anwender zu interagieren und Informationen zu kommunizieren.

6. **Verbindung zum opsi-Depot und Verarbeitung von `action-requests`:** Bei Bedarf stellt der `opsiclientd` eine Verbindung zum *opsi-Depot* her, aktualisiert die lokale Installation des *opsi-script* und führt diesen zur Bearbeitung der anstehenden `action-requests` (Installationen und Deinstallationen) aus.

Durch diese Mechanismen bietet der `opsiclientd` eine umfassende Kontrolle und Konfiguration von Installationen und Wartungsarbeiten in einem zentralen oder verteilten Netzwerkumfeld.
