...
WARNING: Die Konfigurationsdatei `opsiclientd.conf` ist in UTF-8 kodiert. Wenn Sie einen Texteditor verwenden, der die Kodierung nicht unterstützt (z.{}B. `notepad.exe`), zerstören Sie ggf. Umlaute in der Datei.
...
Alternativ können Sie das Anlegen und Löschen von Host-Parametern mit dem Werkzeug `opsi-cli` durchführen (siehe Abschnitt xref:server:components/commandline.adoc#server-components-opsi-cli[*opsi-cli*]).
Beispiel:
