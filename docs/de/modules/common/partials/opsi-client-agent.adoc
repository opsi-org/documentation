...
Über den `opsiclientd`-Webservice können Sie Clients auch herunterfahren und neu starten. Sie erreichen die Funktion unter anderem über die Management-Oberfläche `opsi-configed`. Wählen Sie aus dem Menü _OpsiClient_ den Eintrag ##. Alternativ klicken Sie mit der rechten Maustaste auf einen Client und wählen ##.
...
Alle visuellen Elemente, darunter auch der `opsi-notifier` und `opsi-script`, verwenden die gleichen Darstellungskomponenten, um Grafiken anzuzeigen. Diese Elemente werden auf ähnliche Weise angepasst: 
