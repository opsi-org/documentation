...
Fügen Sie jetzt das Repository für opsi 4.2 hinzu. Dabei laden Sie auch (sofern benötigt) den GnuPG-Schlüssel des Repositorys herunter. Die Befehle führen Sie als Benutzer `root` aus.
...
IMPORTANT: Die Standard-ACLs (`/etc/opsi/backendManager/acl.conf`) haben wir für opsi 4.2 aus Sicherheitsgründen verändert. Wir empfehlen daher, die neue Version der Konfigurationsdatei zu verwenden.
...
RedHat Enterprise Linux (RHEL), Alma Linux und Rocky Linux aktualisieren Sie mit den folgenden Befehlen auf opsi 4.2:
