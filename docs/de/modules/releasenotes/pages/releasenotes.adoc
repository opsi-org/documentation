...
Nutzen Sie nach der Aktualisierung des {opsi-server}s auf opsi 4.2 das Werkzeug `opsi-package-manager`, um die opsi-Pakete neu zu installieren, die Python-Skripte für die Ausführung auf dem Server enthalten:
...
Ab opsi 4.2 können Sie die Vertrauenswürdigkeit des {opsi-server}s über TLS-Standardmethoden sicherstellen.
...
NOTE: Aktualisieren Sie vor dem Upgrade des {opsi-server}s auf allen Clients die Pakete `opsi-winst`, `opsi-client-agent` (bzw. `opsi-linux-client-agent` und `opsi-mac-client-agent`).
