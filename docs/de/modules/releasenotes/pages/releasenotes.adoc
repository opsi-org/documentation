...
TIP: Eine Liste mit aktuell unterstützten Linux-Distributionen für den opsi-Servern sowie für den opsi-Client (Windows, Linux und macOS) finden Sie in Kapitel xref:supportmatrix:supportmatrix.adoc[opsi-Support-Matrix].
...
Erst danach können Sie die Installation bzw. die Aktualisierung durchgeführen.
