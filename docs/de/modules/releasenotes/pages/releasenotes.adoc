...
TIP: Sie steuern dieses Verhalten über die Optionen `verify_server_cert` und `verify_server_cert_by_ca` in der Datei `opsiclientd.conf` (siehe Kapitel  xref:manual:security.adoc#opsi-manual-security-server2client[Security]).
