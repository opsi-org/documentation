...
Wenn es nur einen opsi-server gibt, so hat dieser beide Rollen.


* opsi-server Webservice: TCP 4447 +
Client zum (Config / Depot)-Server, Depot-Server zum Config-Server (bidirektional, Verbindungen via localhost).
...
* SSH (optional): TCP 22 +
Management Zugriff auf (Config / Depot)-Server
...
* HTTP: TCP 80 +
(Config / Depot)-Server ins Internet. +
