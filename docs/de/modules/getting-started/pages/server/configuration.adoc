...
In den nicht auskommentierten Zeilen steht vorne immer der Name der opsi-Methoden (mit Wildcard `.*`). Nach einem Doppelpunkt folgt die Liste der dafür zuständigen Backends.
...
Die Standardeinstellung nach der Installation ist: Das File-Backend steht an erster Stelle, außer beim Lizenzmanagement und bei der Hardware-/Software-Inventarisierung. Dort übernimmt das MySQL-Backend. Die Datei `dispatch.conf` enthält im oberen Bereich weitere Beispiele und Erläuterungen.
