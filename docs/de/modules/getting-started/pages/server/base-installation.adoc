...
NOTE: Falls Sie einen Proxy-Server zum Zugriff auf das Internet verwenden, konfigurieren Sie diesen über die Umgebungsvariablen `http_proxy` und `https_proxy` (siehe Kapitel xref:getting-started:server/requirements.adoc[Voraussetzungen]). Zusätzlich tragen Sie den Proxy in die Konfigurationsdatei `/etc/apt/apt.conf` ein. Sie bearbeiten diese Datei als Benutzer `root` in einem Texteditor Ihrer Wahl, z.{nbsp}B. über den Befehl `nano /etc/apt/apt.conf`.
...
image::oqi.png["Wählen Sie die Sprache und die Art der Installation aus.", pdfwidth=90%]
...
image::oqiInfo.png["Fahren Sie mit der Maus über das kleine Symbol, um weitere Informationen einzublenden.", pdfwidth=90%]
...
image::quickinstall_l-opsi-server.png["Die opsi-server-Installation läuft.", pdfwidth=90%]
...
image::oqiFailed.png["Falls die Installation fehlschlägt, finden Sie in den Logdateien mögliche Fehlerursachen.", pdfwidth=90%]
...
image::quickinstallNoGuiFailed.png["Das Setup-Programm gibt Hinweise auf Fehler und Logdateien.", pdfwidth=90%]

// ab hier freigeschalten für das 4.3-Release
////
