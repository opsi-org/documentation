...
<<opsi-getting-started-installation-base-quickinstall>>, die <<opsi-getting-started-installation-base-packages>> unter Debian, Ubuntu, openSUSE/Suse Linux Enterprise Server (SLES), CentOS, Red Hat Enterprise Linux (RHEL), Alma Linux, Rocky Linux und Univention Corporate Server (UCS).
