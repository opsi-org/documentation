...
TIP: Weitere Informationen dazu finden Sie im Kapitel xref:server/minimal-products#opsi-getting-started-installation-config-get-essential-products[Produkte einspielen].

[[opsi-getting-started-installation-base-vm-start-configed]]
=== Management-Oberfläche starten

Die Management-Oberfläche `opsi-configed` starten Sie über einen Doppelklick auf das Desktopsymbol _Opsi Configuration Editor_. Weitere Informationen zu dieser grafischen Anwendung finden Sie in xref:opsiconfiged.adoc[Management-Oberfläche `opsi-configed`].
 
Damit ist die grundlegende Server-Konfiguration beendet.

Sie können nun fortfahren mit:

* xref:adding-clients.adoc#firststeps-software-deployment-client-integration[Clients zu opsi hinzufügen]

* xref:os-installation.adoc#firststeps-osinstall[Betriebssystem-Installation]
...
Die Konfigurationsdatei `docker-compose.yml` definiert Netzwerke, Volumes und vier Dienste (Abschnitt `services`):
...
.Den Status und die Logfiles mit dem Skript `opsi-server.sh` überprüfen
image::opsi-server-script.png["Den Status und die Logfiles mit dem Skript `opsi-server.sh` überprüfen", pdfwidth=80%]
...
Wir haben zwei Helfer-Skripte beigelegt, die den Umgang mit dem opsi-Docker-Container erleichtern. Wenn Sie Linux oder macOS verwenden, machen Sie das Skript `opsi-server.sh` zunächst ausführbar:
...
Danach rufen Sie es als normaler Benutzer über diesen Befehl auf, um eine Onlinehilfe einzublenden:
...
TIP: Nutzen Sie zur Installation von Grafana die offiziellen Grafana-Repositorys. Im November 2022 sind die Repositorys von `<packages.grafana.com/deb/{product}>` nach `<apt.grafana.com>` umgezogen, was die nächsten Abschnitte berücksichtigen. Die alten Adressen funktionieren noch, aber es kann bei der Installation oder Aktualisierung zu Warnungen auf dem System kommen.
...
TIP: Falls bei der Installation eine Frage nach dem TFTP-Basisverzeichnis auftaucht, antworten Sie mit `/tftpboot`.
...
NOTE: Stellen Sie sicher, dass Ihre Firewall die folgenden Ports für TFTP (69/UDP) und opsi (4447/TCP und 4441/TCP) freigibt.
...
Es kann vorkommen, dass Sie beim Importieren des GPG-Schlüssel des Repositorys eine solche Meldung sehen:
...
NOTE: Stellen Sie sicher, dass Ihre Firewall und die SELinux-Konfiguration die folgenden Ports für TFTP (69/UDP) und opsi (4447/TCP und 4441/TCP) freigibt.
...
Benutzer müssen Mitglied der Gruppe `opsi-admin-group` sein, um den `opsiconfiged` verwenden zu können. Während der Installation, wird der Benutzer `Administrator` automatisch zu dieser Gruppe hinzugefügt. Andere Accounts fügen Sie über das UMC-Modul _Benutzerverwaltung_ , Reiter _Gruppen_ hinzu.
