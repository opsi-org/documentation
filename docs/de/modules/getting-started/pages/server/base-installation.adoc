...
NOTE: Wenn Sie Univention Corporate Server (UCS) einsetzen, empfehlen wir die Installation des Pakets `opsi4ucs` (siehe Abschnitt <<opsi-getting-started-installation-base-ucs>>).
...
Die Installation von opsi auf Red Hat Enterprise Linux (RHEL), Alma Linux oder Rocky Linux unterscheidet sich nur durch das verwendete Repository.
...
WARNING: Die verwendeten Unix-Befehle in den folgenden Kapiteln beziehen sich auf Debian-Systeme. Sie müssen sie durch die entsprechenden RHEL-/Alma-/Rocky-Befehle ersetzen.
