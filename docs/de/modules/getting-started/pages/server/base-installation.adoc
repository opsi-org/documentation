...
TIP: Als Alternative zum schnellen Einstieg empfehlen wir das Programm opsi-QuickInstall, das über ein grafisches Setup-Programm oder auf der Kommandozeile einen opsi-Server unter Linux einrichtet (siehe Kapitel xref:quickinstall[opsi-QuickInstall]).
...
Für ESXi, VMware und  VirtualBox haben wir bereits eine entsprechende Maschine eingerichtet.
