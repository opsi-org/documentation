...
== ShellScript-Sektion [W/L/M]

Die ShellScript-Sektion (früher ShellBatch/ShellInAnIcon/DosBatch/DosInAnIcon genannt) dient in erster Linie dazun, schon vorhandene Kommandozeilenroutinen und Shellskripte in opsi-script zu integrieren bzw. Betriebssystem-Kommandos aus opsi-script heraus nutzen zu können.  Dabei wird ein mögliches Fenster des aufgerufenen Kommandozeilen-Interpreters nicht angezeigt. 
...
Eine ShellScript-Sektion wird bei der Abarbeitung des opsi-Skriptes in eine temporäre Datei '_opsiscript_<random>.cmd' (Windows) oder '_opsiscript_<random>.sh' (Linux/MacOS) umgewandelt. Die Datei wird dann mit dem (Standard-)Kommando-Interpreter des Betriebssystems (cmd unter Windows, bash unter Linux/MacOs) ausgeführt. Dies erklärt warum in einer ShellScript-Sektion alle Shell-Kommandos verwendet werden können.

Die ShellScript-Sektion bietet gegenüber einer reinen Shellskript-Ausführung mit einem Kommando-Interpreter folgende Vorteile:
...
* Die Ausgaben des Aufrufs können in einem gesonderten Fenster ausgegeben werden (Parameter `/showoutput`). Ein Schliessen diese Fensters, z.B. durch einen Benutzer, hat keine Auswirkung auf die weitere Abarbeitung des Shellskriptes.

CAUTION: _{opsi-script}_ wartet auf die Beendigung der Shell-Sektion, bevor das Skript weiter abgearbeitet wird.
...
Parameter des Aufrufs der Shell-Sektion in der Actions-Sektion werden unmittelbar als Parameter der Shell-Datei interpretiert.
