...
[source,opsiscript]
----
ShellInAnIcon_say_hello WINST /timeoutseconds 20
...
== ShellBatch/ShellInAnIcon Sektionen [W/L/M]

ShellBatch/ShellInAnIcon-Sektionen (früher DosBatch/DosInAnIcon genannt) sollen in erster Linie dazu dienen, vorhandene Kommandozeilenroutinen für bestimmte Zwecke zu nutzen. Der Sektionstyp ShellInAnIcon ist identisch mit der betreffenden ShellBatch-Syntax und ausführenden Methoden. Allerdings wird das aufgerufenen Fenster minimiert dargestellt und die Ausgaben in die Logdatei übernommen.

Eine ShellBatch/ShellInAnIcon-Sektion wird bei der Abarbeitung des Skripts in eine temporäre Batch-Datei '_opsiscript_<random>.cmd' umgewandelt. Da die Datei in 'c:\opsi.org\tmp' angelegt wird, muss dieses Verzeichnis existieren und zugänglich sein. Die Datei wird dann in einem Kommando-Fenster mit cmd.exe als Kommando-Interpreter ausgeführt. Dies erklärt warum in einer ShellBatch-Sektion alle Shell-Kommandos verwendet werden können.

Gegenüber dem Aufruf einer cmd-Datei per Winbatch-Sektion bietet die ShellBatch/ShellInAnIcon-Sektionen mehrere Vorteile:
...
* Die Ausgaben eines ShellInAnIcon Aufrufs unter Windows werden in der Logdatei abgespeichert.

* Die Ausgaben eines ShellInAnIcon Aufrufs werden in einem gesonderten Fenster ausgegeben, wenn die Sektion mit dem Parameter `/showoutput` aufgerufen wurde.

* Die Ausgaben eines ShellInAnIcon bzw. ShellBatch Aufrufs unter Linux werden in der Logdatei abgespeichert.

* Die Ausgaben des Aufrufs können einer String-Liste übergeben und weiterverarbeitet werden.

CAUTION: _{opsi-script}_ wartet auf die Beendigung der ShellBatch/ShellInAnIcon, bevor die nächste Sektion des Skripts abgearbeitet wird.
...
[source,opsiscript]
----
ShellInAnIcon_do_encoding_example WINST /encoding "utf8"
----
Die erlaubten Strings für `<encoding>` finden sich unter: xref:configuration-options#opsi-script-encoding[_{opsi-script}_ encoding]
...
[source,opsiscript]
----
ShellInAnIcon_uninstall WINST /WaitForProcessEnding "uninstall.exe" /TimeOutSeconds 20
...
[Actions]
DefVar $para$
ShellBatch_1 today we say "Hello World"
set $para$ = "today"
ShellBatch_1 $para$ we say "Hello World"
...
----
[Actions]
ShellBatch_1 today we say winst /64bit
...
[source,opsiscript]
----
comment "Testing parameters for ShellInAnIcon"
...
if ($ConstTest$ = $CompValue$)
	comment "ShellInAnIcon parameter passed"
else
	set $TestResult$ = "not o.k."
	LogWarning "ShellInAnIcon parameter failed"
endif
