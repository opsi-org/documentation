...
für Aufrufe von `ShellScript` (`ShellInAnIcon`),`ExecWith` und `ExecPython`. Siehe auch xref:prim-section.adoc#getOutStreamFromSection[]

* `getreturnListFromSection(<section with params>)` +
für `XMLPatch`-Sektionen und `opsiServiceCall`-Sektionen. Siehe auch xref:prim-section.adoc#getReturnListFromSection[getReturnListFromSection]
...
Für alle anderen Aktionen steht eine Variable `%UserProfileDir%` oder seit  _opsi-script_ version 4.11.2 `%CurrentProfileDir%` zur Verfügung, mit der Verzeichnisnamen konstruiert werden können.
...
+
WARNING: Zielpfad wird in jedem Fall als Directory-Name interpretiert. Umbenennen beim Kopieren ist nicht möglich: Ziel ist immer ein Pfad, nicht ein (neuer) Dateinamen. Existiert der Pfad nicht, wird er (auch mit geschachtelten Directories) erzeugt.
+
...
**  `c` ->  continue [W] +
Wenn eine Systemdatei in Benutzung ist, kann sie erst nur nach einem Reboot überschrieben werden. Das _opsi-script_ default-Verhalten ist dabei, dass ein Datei in Benutzung zum Überschreiben beim nächsten Reboot markiert wird UND die _opsi-script_ Reboot Markierung gesetzt wird. Das Setzen der Option `-c` stellt den automatischen Reboot aus. Das Kopieren wird in diesem Fall erst dann vervollständigt, wenn ein Reboot auf eine andere Weise ausgelöst wird.
...
Wenn eine Datei in Benutzung ist, kann sie unter Windows nicht direkt gelöscht werden. Durch die Option `r` wird dafür gesorgt, das diese Datei in so einem Fall automatisch im Rahmen des nächsten Reboots gelöscht wird. Das _opsi-script_ default-Verhalten ist das dabei auch die _opsi-script_ Reboot Markierung gesetzt wird. D.h. das die Maschine nach Abschluß des Skriptes automatisch rebootet.
siehe auch die Option: `c`

**  `c` ->  continue with out reboot [W] +
Wenn eine Datei in Benutzung ist, kann sie über die Option `r` im Rahmen eines Reboots gelöscht werden. Das _opsi-script_ default-Verhalten ist das dabei auch die _opsi-script_ Reboot Markierung gesetzt wird. Das Setzen der Option `-c` stellt den automatischen Reboot aus. Das Löschen wird in diesem Fall erst dann vervollständigt, wenn ein Reboot auf eine andere Weise ausgelöst wird.
...
+
WARNING: Bitte beachten Sie die Reihenfolge sowohl von 'rwx' (read, write, execute) als auch von 'ugo' (user, group, others). +
...
WARNING: Falls *Umlaute in den Dateinamen* der entpackten Dateien nicht korrekt dargestellt werden, kann der optionale dritte encoding Parameter gesetzt werden, wie im Folgenden beschrieben. Falls die Codierung der einzelnen Dateinamen unterschiedlich ist, empfehlen wir, das Archiv neu zu packen mit 'utf8'-codierten Dateinamen (z.B. mit 7zip).

Seit der Version 4.12.13 verwendet `zipfile` per default das encoding 'utf8' für Umlaute in zu zippenden Dateinamen. Dies ist das für ZIP Archive empfohlene Encoding für Dateinamen.

Seit der Version 4.12.13 entpackt `unzipfile` auch Archive, welche Dateien mit Umlauten im Dateinamen enthalten, mit richtig codierten Dateinamen. Laut ZIP-Spezifikation ist für Dateinamen mit Umlauten als encoding nur 'cp437' oder 'utf8' erlaubt. Falls das Zip-Archiv der Spezifikation entspricht und für das encoding der Dateinamen 'cp437' oder 'utf8' verwendet, wird dies per default beim entpacken erkannt und richtig gehandhabt, so dass die Umlaute in den Dateinamen der entpackten Dateien korrekt gesetzt sind.
Aber nicht alle ZIP-Archive halten sich an die Spezifikation und verwenden eventuell ein anderes encoding für die Dateinamen. Falls `unzipfile` Dateinamen mit Umlauten nicht richtig setzt, gibt es mit Version 4.12.13 einen optionalen dritten Parameter, um das im Archiv verwendete encoding der Dateinamen anzugeben:
...
----
In diesem Beispiel wird das encoding der Dateinamen in testfile.zip als 'cp850' angegeben.
Dadurch wandelt `unzipfile` beim entpacken die Dateinamen um von 'cp850' nach system codepage.

WARNING: Das Dateinamen encoding als dritter Parameter sollte nur gesetzt werden wenn notwendig. Die encodings 'cp437' und 'utf8' werden per default gehandhabt und sollten nicht als dritter Parameter gesetzt werden.

Die erlaubten Strings für <encoding> finden sich unter xref:configuration-options.adoc#opsi-script-encoding[_opsi-script_ encoding]

In sehr seltenen Fällen können die Dateinamen sogar unterschiedliche encodings haben. In diesem Fall bleibt nur noch, das Archiv neu zu packen mit 'utf8' codierten Dateinamen. Hierzu kann zum Beispiel das Programm '7zip' verwendet werden, welches auch Archive mit gemischtem Dateinamen encoding entpacken kann und dann das Archiv neu zippen mit 'utf8'.
...
(Seitdem eine gepatchete INI-Datei auf die gleiche Weise wie die Sektionen vom _opsi-script_ Skript erstellt werden, muss man vorsichtig mit den Bezeichnungen umgehen, damit kein Durcheinander entsteht).
...
----
Die erlaubten Strings für `<encoding>` finden sich unter: xref:configuration-options#opsi-script-encoding[_opsi-script_ encoding]
...
IdapConfig-Sektionen werden vom aktuellen _opsi-script_ nicht mehr unterstützt.
...
Für den _opsi-script_ - ungewöhnlicherweise - sind alle Strings case sensitive.
...
Der 'Key' ist ein Buchstabe ('A' - 'Z') oder eine Zahl ('0' - '9'). Alle anderen Tasten müssen mit Ihrem 'Virtual Key Code' Bezeichner eingegeben werden. +
...
WARNING: Windows: Wenn das referenzierte target auf einem, zum Zeitpunkt der Befehlsausführung nicht erreichbaren, Share liegt, werden alle Bestandteile des Pfades auf das Längenschema 8.3 gekürzt. +
Workaround: +
...
Der _opsi-script_ bietet XMLPatch-Sektionen an, um XML-Dokumente zu bearbeiten.
...
Das bedeutet, die Aktionen, die _opsi-script_ ausführen kann, gliedern sich in:
...
* Zum Element können außer dem Hauptknoten noch Attribute, sogenannte Attributknoten, gehören. Es könnte z.B. Attribute „Farbe“ oder „Winkel“ geben, die den Knoten 1 in der Ebene 1 näher beschreiben.
+
[source,xml]
----
...
Im _opsi-script_ ist derzeit die Auswahl nach den Gesichtspunkten (1) bis (3) sowie (7) implementiert:
...
Der _opsi-script_ Befehl hierfür lautet
...
Das gleiche Nodeset beschreibt folgende Kurzsyntax (muss in einer Zeile des Skriptes untergebracht werden):
[source,opsiscript]
----
...
Durch die Parameter des WinBatch-Aufrufs wird festgelegt, wie sich _opsi-script_ gegenüber den in der WinBatch-Sektion gestarteten Programmen verhält.
...
Im 1. Fall wartet _opsi-script_ solange, bis ein Prozess, der sich durch ein mit [Fenstertitel] benanntes Fenster kenntlich macht, gestartet ist. Im 2. Fall wartet _opsi-script_ bis ein, mit [Fenstertitel] benanntes, Fenster auf dem Desktop erst einmal erscheint und dann auch wieder geschlossen wird. Auf diese Weise kann unter geeigneten Umständen geprüft werden, ob sekundäre, indirekt gestartete Prozesse sich beendet haben.
+
WARNING: Diese Befehle erkennen nur Fenster von 32 Bit-Programmen.
...
Gegenüber dem Aufruf einer cmd-Datei per Winbatch-Sektion bietet die ShellScript Sektionen drei Vorteile:

* In der Sektion vorhandene _opsi-script_ Variablen oder Konstanten werden vor der Ausführung durch Ihren Inhalt ersetzt und können so unkompliziert verwendet werden.

* Die Ausgaben eines ShellScript Aufrufs unter Windows werden in der Logdatei abgespeichert.

* Die Ausgaben eines ShellScript Aufrufs werden in einem gesonderten Fenster ausgegeben, enn die Sektion mit dem Parameter `/showoutput` aufgerufen wurde.

* Die Ausgaben eines ShellScript bzw. DosBatch/ShellBatch Aufrufs unter Linux werden in der Logdatei abgespeichert.

* Die Ausgaben des Aufrufs können einer String-Liste übergeben und weiterverarbeitet werden.



WARNING: Verwenden Sie keine Kommandos, die auf Eingaben warten.

[[opsi-script-dosbatch-params]]
=== Aufrufparameter

Zu unterscheiden ist zwischen Parametern die der aufgerufenen Batchdatei übergeben werden und denen die _opsi-script_-intern verwendet werden. Der Aufrufsyntax ist daher:
...
* `/LetThemGo` // seit 4.12.10.0
Verschiebt die aufgerufene Shell in den Hintergrund und wartet *nicht* auf deren Beendigung;
d.h. das sofort die nächste Zeile der WinBatch-Sektion bzw. die nächste Zeile des übergeordneten Programms abgearbeitet werden.
...
Sollen die Ausgaben, die von Befehlen einer DosBatch-Sektion kommen, aufgefangen werden, so geschieht dies mittels `getOutStreamFromSection ()` aus der Haupt-Sektion des opsi-script-Skriptes (siehe xref:prim-section#opsi-script-stringlist-getstring[Kapitel "(Wieder-) Gewinnen von Einzelstrings aus String-Listen"]).
...
*  `/AllUserProfiles` seit 4.12.10.0 (früher `/AllNTUserDats`)  +
Es gibt jedoch die Möglichkeit, dass die Patches einer Registry-Sektion automatisch für "alle User", entsprechend den verschiedenen User-Zweigen der Registry, vorgenommen werden. Das entsprechende Verfahren bei der Abarbeitung der Sektion wird mit dem Parameter `/AllUserProfiles` aufgerufen.
`/AllNTUserDats` ist veraltet funktioniert aber noch.
...
Diese nicht _opsi-script_ spezifischen syntaktischen Varianten sind im Handbuch nicht beschrieben, da sie normalerweise automatisch generiert werden.
...
Die Syntax der Defaultform einer Registry-Sektion ist an der Kommandosyntax anderer Patchoperationen des _opsi-script_ orientiert.
...
In der _opsi-script_ Syntax bei den Registry-Pfaden werden die weiteren folgenden Ebenen jeweils durch einen Backslash getrennt.
...
----
+
WARNING: Nach dem Patchen des Registry-Path enthält die Umgebungsvariable Path den veränderten Wert erst nach einem Reboot oder nach einem Aufruf von `UpdateEnvironment` siehe: xref:prim-section#UpdateEnvironment[UpdateEnvironment]
...
Seit _opsi-script_ version 4.11.2 darf man den root key 'HKEY_CURRENT_USER' beim `openkey` Kommando mitgeben. +
Beispiel:
[source,opsiscript]
----
[Registry_AllUsers]
...
Diese Anweisungen können aber nun nicht als Sektion innerhalb eine _opsi-script_ Skriptes untergebracht werden. Daher kann die Registry Sektion mit dem Parameter `/regedit` nur als ausgelagerte Sektion oder über die Funktion loadTextFile geladen werden:
[source,opsiscript]
----
...
Windows sieht hier zusätzliche Wertetypen vor. Gravierender ist, dass die Exportdatei ursprünglich in Unicode erzeugt wird. Um sie mit den 8 Bit-Mitteln der Standardumgebung des _opsi-script_ zu verarbeiten, muss der Zeichensatz konvertiert werden.
Die Konvertierung kann z.B. mit einem geeigneten Editor durchgeführt werden. Eine andere Möglichkeit besteht darin, die Konvertierung on the fly vom _opsi-script_ durchführen zu lassen. Dazu lässt sich die String-Listenfunktion `loadUnicodeTextFile` verwenden. Wenn z.B. printerconnections.reg ein Unicode-Export ist, wäre regedit in folgender Form aufzurufen:
[source,opsiscript]
----
...
WARNING: Es gibt eine Standard Webserviceverbindung. Diese wird beim Start des opsi-script über den opsi-client-agent auf die bestehende Verbindung zum opsi-server gesetzt. +
...
Siehe dazu auch: xref:server:interfaces/jsonrpc-api.adoc[JSON-RPC-API]
...
Die ExecPython Sektionen basieren auf Shell-Sektionen (ähnlich wie ShellScript). Während diese den Inhalt der Sektion dem Interpreter cmd.exe übergeben, wird der Inhalt einer ExecPython Sektion dem Python Interpreter übergeben (welcher auf dem System installiert sein muss).
...
=== Verflechten eines Python Skriptes mit einem _opsi-script_ Skript

Aktuell ist die execPython Sektion dem _opsi-script_ Skript über vier Kategorien von gemeinsam genutzten Daten integriert:

* Eine Parameterliste geht zum Python Skript über.

* Alles was vom Python Skript gedruckt wird, wird in die _opsi-script_ log-Datei geschrieben.

* Der _opsi-script_ Skript Mechanismus für die Einführung von Konstanten und Variablen in Sektionen arbeitet erwartungsgemäß für die execPython Sektion.

* Die Ausgabe einer execPython Sektion kann umgewandelt werden in eine String-Liste und dann vom laufenden _opsi-script_ Skript weiter verwendet werden.

Ein Beispiel für die ersten beiden Wege der Verflechtung des Python Skriptes mit dem _opsi-script_ Skript werden im Anschluss beschrieben. Es wurde erweitert, damit einige der Werte von _opsi-script_ Konstanten oder Variablen aufgerufen werden können.
[source,opsiscript]
----
[execpython_hello]
import sys
...
Es sind folgende _opsi-script_-Optionen verfügbar:
...
*  `/EscapeStrings` +
Diese Option legt fest, dass die Backslashes in _opsi-script_-Variablen und Konstanten dupliziert werden, so dass sie das ausführende Programm in der üblichen Form von Strings in 'C'-Syntax vorfindet.

*  `/LetThemGo` +
Diese Option hat den Effekt (wie bei `winBatch` Aufrufen), dass das aufgerufene Programm in einem neuen Thread startet, während der _opsi-script_ mit dem Auslesen des Skriptes fortfährt.
...
----
ruft Notepad auf und öffnet die Sektion als Datei (allerdings ohne die Zeilen, die mit einem Semikolon beginnen, da der _opsi-script_ solche Zeilen als Kommentarzeilen interpretiert und vor der weiteren Behandlung der Sektion entfernt).
...
Bevor wir zu den _opsi-script_ Kommandos übergehen, gibt es erst noch einige Erklärungen zum Syntax von LDAP selbst
...
image::opsi-script-manual:ldap-in-jxplorer.png["OPSI LDAP Baum", pdfwidth=90%]
...
Eine _opsi-script_ Sektion `ldapsearch_generalConfigs` ist wie folgt definiert:
[source,opsiscript]
----
...
Es gibt nun verschiedene _opsi-script_ Optionen, um die Komplexität der Auswertung der Ergebnisse solcher Anfragen zu reduzieren und zu handhaben.
...
(Aber die _opsi-script_ Funktion getValue (key, list) (vgl. xref:prim-section#opsi-script-stringlist-getstring[Kapitel "(Wieder-) Gewinnen von Einzelstrings aus String-Listen"]) hilft in diesem Fall: z.B. getValue ("secsuntilconnectiontimeout", list) würde die gewünschte Zahl ausgeben).
...
*  `filter:` +
Der Filter für eine LDAP Suche hat eine spezielle LDAP Syntax, die nicht vom _opsi-script_ überprüft wird. Voreingestellt ist "(objectclass=*)".
