...
include::section_modifiers/WaitForProcessEnding.adoc[]
...
Generell haben wir die Aufruf-Syntax:
...
* `/encoding <encoding>` // seit 4.12.4.17 [W/L/M] +
Der Inhalt der Sektion wird per default im System-Encoding in eine temporäre Datei gespeichert. Dies ist normalerweise eine gute Wahl. In Ausnahmefällen kann aber ein anderes Encoding gewünscht sein. +
...
Die Ausführung von Skripten ist in der Powershell per default ausgeschaltet. Um eine ExecWith-Sektion mit Powershell nutzen zu können, muss die Skript-Ausführung erst erlaubt werden.
