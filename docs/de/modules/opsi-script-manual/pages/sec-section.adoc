...
Seit opsi-script 4.12.5 unterstützt <mode> verschiedene Formate : +
- die numerische (oktale) Darstellung (zB: "755") (seit 4.11.4.1) +
- das Modusformat '-rwxrwxrwx' (z. B.: "-r--r--r--", "--wx-w-r-x") +
- das Modusformat 'ugo=+-rwx' (z. B.: "ugo=rwx", "ug+r", "go-wx") +

ACHTUNG: Bitte respektieren Sie die Reihenfolge sowohl von 'rwx' (read, write, execute) als auch von 'ugo' (user, group, others) +

ACHTUNG: Vergessen Sie nicht das '-' am Anfang des -rwxrwxrwx-Formats +

Seit opsi-script 4.12.5 gibt es einen optionalen Parameter:
