...
DOSBatch-Sektionen (auch ShellBatch genannt) sollen in erster Linie dazu dienen, vorhandene Kommandozeilenroutinen für bestimmte Zwecke zu nutzen. _opsi-script_ wartet auf die Beendigung des DOS-Batch, bevor die nächste Sektion des Skriptes abgearbeitet wird.

Eine DosBatch-Sektion wird bei der Abarbeitung des Skriptes in eine temporäre Batch-Datei '_winst_<random>.cmd' umgewandelt. Da die Datei in 'c:\opsi.org\tmp' angelegt wird, muss dieses Verzeichnis existieren und zugänglich sein. Die Batch-Datei wird dann in einem Kommando-Fenster mit cmd.exe als Kommando-Interpreter ausgeführt. Dies erklärt warum in einer DosBatch Sektion alle Windows Shell Kommandos verwendet werden können.
...
Anzumerken ist hierbei, dass der Log-Level auf '1' gesetzt werden muss, damit die Ausgabe wirklich den Weg in die Logdatei findet.
