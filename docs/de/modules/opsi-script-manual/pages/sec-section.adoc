...
Sollen die Ausgaben, die von Befehlen einer ShellScript-Sektion kommen, aufgefangen werden, so geschieht dies mittels `getOutStreamFromSection ()` aus der Haupt-Sektion des {opsi-script}-Skripts (siehe xref:prim-section#opsi-script-stringlist-getstring[Kapitel "(Wieder-) Gewinnen von Einzelstrings aus String-Listen"]).
