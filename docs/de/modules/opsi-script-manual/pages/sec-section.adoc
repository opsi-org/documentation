...
Es gibt jedoch die Möglichkeit, dass die Patches einer Registry-Sektion automatisch für "alle NT User", entsprechend den verschiedenen User-Zweigen der Registry, vorgenommen werden. Das entsprechende Verfahren bei der Abarbeitung der Sektion wird mit dem Parameter `/AllUserProfiles` aufgerufen.

NOTE: Die alte Bezeichnung `/AllNTUserDats` funktioniert noch, wir empfehlen aber stattdessen `/AllUserProfiles` zu verwenden.
