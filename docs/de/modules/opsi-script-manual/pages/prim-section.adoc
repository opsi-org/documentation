...
=== TOML-Dateien Funktionen [W/L/M]

NOTE: Ab opsi-script v. 4.12.5 unterstützt opsi-script die Handhabung von TOML-Dateien. +
Obwohl die Syntax von TOML hauptsächlich aus : Key = "Value"-Paaren, [Table] als Tabellen interpretiert und #Kommentaren besteht, behält opsi-script im Moment keine Kommentare. +

NOTE: Bitte beachten Sie die offizielle Dokumentation der TOML-Dateien, um die offiziellen Datenformate zu respektieren: https://toml.io/en/ ++
...
Ein vollständiges opsi-Skript mit allen Funktionen ist unten verfügbar. +

anchor:LoadTOMLFile[]

`LoadTOMLFile(`<TOMLfilePath: String>`) : StringList` //seit 4.12.5 +
Lädt die TOML-Datei <TOMLfilePath> und gibt eine Stringliste des Dateiinhalts zurück. +

anchor:ReadTOMLFile[]

`ReadTOMLFile(`<TOMLfilePath: String>`) : String`  //seit 4.12.5 +
Liest die TOML-Datei <TOML-Dateipfad> und gibt eine String des Dateiinhalts zurück. +
...
`GetTOMLAsStringList(`<TOMLcontents: String>`) : StringList`  //seit 4.12.5 +
Parst den TOML-String-Inhalt <TOMLcontents> und gibt eine String-Liste des geparsten TOML-Inhalts zurück. +


anchor:GetTOMLAsString[]

`GetTOMLAsString(`<TOMLcontents: String>`) : String`   //seit 4.12.5 +
Parst den TOML-String-Inhalt <TOMLcontents> und gibt einen String des geparsten TOML-Inhalts zurück. +

anchor:GetTOMLKeys[]

`GetTOMLKeys(`<TOMLcontents: String>`) : StringList`   //seit 4.12.5 +
...
`GetTOMLTableNames(`<TOMLcontents: String>`) : StringList`   //seit 4.12.5 +
...
`GetTOMLTable(`<TOMLcontents: String> , <table name : String>`) : StringList`   //seit 4.12.5 +
Sucht in den angegebenen <TOMLcontents>-String nach einer Tabelle mit dem angegebenen <table name> und gibt eine String-Liste des gefundenen Tabelleninhalts zurück. +

anchor:GetTOMLTableAsString[]

`GetTOMLTableAsString(`<TOMLcontents: String> , <table name : String>`) : String`   //seit 4.12.5 +
...
`GetValueFromTOML(`<TOMLcontents: String> , <keyPath: String> , <defaultValue: String>`) : String`   //seit 4.12.5 +
Sucht in der angegebenen <TOMLcontents>-String nach dem Key, der mit <keyPath> definiert ist, also: +
- Ein einfacher Key-name, wenn sich der Key in der Stammtabelle befindet. +
...
`ModifyTOML(`<TOMLcontents: String> , <command: String> , <keyPath: String> , <value: String>`) : String`   //seit 4.12.5 +
Diese Funktion ermöglicht es, den als Eingabe angegebenen <TOMLcontents>-String mit Hilfe von 4 Befehlen zu ändern: +
...
Aber wenn der <keyPath> existiert, nimmt diese Funktion keine Änderungen vor. Derselbe Eingabe-String <TOMLcontents> wird zurückgegeben. +
- 'SET' : Legt ein neues <keyPath>-<value>-Paar fest, wenn der Key nicht existiert, und selbst wenn er existiert. Mit der neuen Eingabe <Value> wird der entsprechende Value geändert. +
Die Funktion gibt den neuen modifizierten TOMLcontens-String zurück. +
- 'CHANGE ' : Ändert den entsprechenden Value der Eingabe <keyPath> auf den angegebenen <value>. Es gibt einen neuen modifizierten TOMLcontens-String zurück. +
Aber wenn der keyPath/Key nicht existiert, wird nichts getan. Derselbe Eingabe-String <TOMLcontents> wird zurückgegeben. +
- 'DEL' : Löscht den <keyPath> und seinen gefundenen Value. Es gibt einen neuen modifizierten TOMLcontens-String zurück. +
Aber wenn der keyPath nicht existiert, wird nichts getan. Derselbe Eingabe-String <TOMLcontents> wird zurückgegeben. +

Wenn eine Behandlungsausnahme auftritt, wird dieselbe Eingabe-<TOMLcontents>-String zurückgegeben. +
Wenn ein Fehler auftritt, wird ein leerer String zurückgegeben. +

NOTE: Beachten Sie, dass das Hinzufügen neuer Value die TOML-Notation innerhalb des Eingabe-String-Parameters gemäß der offiziellen Dokumentation berücksichtigen muss: https://toml.io/en/ +
Zum Beispiel: +
...
Alle ausführlichen Beispiele sind im folgenden opsi-script-Beispiel enthalten. +

anchor:DeleteTableFromTOML[]

`DeleteTableFromTOML(`<TOMLcontents: String> , <tablePath: String>`) : String`   //seit 4.12.5 +
...
`SaveToTOMLFile(`<TOMLcontents: String> , <TOML file Path: String>`) : boolean`   //seit 4.12.5 +
Diese Funktion gibt den eingegebenen <TOMLcontents>-String an einen formatierten TOML-Dateiinhalt zurück und speichert ihn im angegebenen <TOML file path>. +
...
`ConvertTOMLfileToJSONfile(`<TOMLfilePath: String> , <JSONfilePath: String>`) : boolean`  //seit 4.12.5 +
Diese Funktion konvertiert den Inhalt des ersten angegebenen <TOMLfilePath> in einen JSON-formatierten Inhalt und speichert ihn in der zweiten angegebenen <JSONfilePath>-Datei. +
Läuft das Speichern der Datei fehlerfrei, wird „True“ zurückgegeben. +
Wenn Fehler auftreten, wird "False" zurückgegeben. +
...
Also, die Datei $newTOMLFile$ ist jetzt:
