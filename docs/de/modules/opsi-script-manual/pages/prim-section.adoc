...
siehe auch : xref:cli-params.adoc#opsi-script-configs_debug_lib[opsi-script-configs_debug_lib]
...
* `forceLogInAppendMode =` <boolean value>   //seit 4.12.3.6
(default=false); +
...
* `opsi-wmi-test.exe`
Für den Test von WMI Abfragen gibt es eine kleine Hilfsanwendung (opsi-wmi-test.exe).
...
„fängt“ – derzeit bei `ShellScript`, `ExecWith` und `ExecPython` Aufrufen – die Ausgabe der Kommandozeilenprogramme in der Form einer String-Liste ein. Z.B. liefert der Ausdruck
...
* Winbatch/ShellScript/ExecWith-Sektionen mit `/RunAsLoggedOnUser` +
Der opsi-script läuft auch dann, wenn er über das Loginevent gestartet wird, im SYSTEM Kontext und nicht im Kontext des Users, der sich gerade eingeloggt hat. Um einen Prozess im Kontext dieses Users zu starten, kann eine Winbatch-, ShellScript- oder ExecWit-Sektion mit der Option `/RunAsLoggedOnUser` verwendet werden.
