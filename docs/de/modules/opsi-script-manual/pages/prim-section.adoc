...
* Winbatch/ShellBatch/ExecWith-Sektionen mit `/RunAsLoggedOnUser` +
Der opsi-script läuft auch dann, wenn er über das Loginevent gestartet wird, im SYSTEM Kontext und nicht im Kontext des Users, der sich gerade eingeloggt hat. Um einen Prozess im Kontext dieses Users zu starten, kann eine Winbatch-, ShellBatch- oder ExecWit-Sektion mit der Option `/RunAsLoggedOnUser` verwendet werden.
