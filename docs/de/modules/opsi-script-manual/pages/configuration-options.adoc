...
Setzt (überschreibt) den Standard-Log-Level von opsi-script. +
Dieser Config hat keinen Einfluss auf Skripte, bei denen der Log-Level per `setLogLevel` explizit gesetzt worden ist. +
Default :  '7' +
...
Erzwingt einen minimalen Log-Level. +
Dies dient dazu bei der Entwicklung und/oder Fehlersuche gezielt und temporär für einzelne Clients den Log-Level zu erhöhen ohne hierzu Anpassungen am Script vornehmen zu müssen. +
Default: '0' +
siehe auch xref:prim-section#SetLogLevel[SetLogLevel] +
