...
Wenn true wird auf dem Client im Unterverzeichnis `lastprodlogs` des {opsi-script} Logverzeichnisses ( z.B. `c:\opsi.org\log\lastprodlogs`) für jedes {opsi-product} eine Logdatei eine Logdatei geschrieben. Der Name der Logdatei ist `<productId>.log`. Es existiert dann für jedes Prokt der Log des letzten Skript laufs. Im Fall, das ein Skript Reboots innerhalb des Skriptes beinhaltet so enthält dieses Log nur den Teil nach dem letzten Reboot. +
Default: false +
