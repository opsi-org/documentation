...
In der Voreinstellung protokolliert `opsi-configed` im Level 4 (`warning`). Sie können den Log-Level auch niedriger oder höher setzen, beispielsweise auf Log-Level 6 (`info`) oder Level 7 (`debug`). Insgesamt unterscheidet opsi 10 verschiedene Log-Level:
...
Um den Log-Level beim Programmstart zu setzen, verwenden Sie den Parameter `--loglevel <zahl>`; es sind Werte zwischen `0`  und `9` erlaubt.
...
Läuft `opsi-configed` bereits, können Sie den Log-Level über das Menü _Hilfe_ einstellen.

.*opsi-configed*: Log-Level über das Menü einstellen
image::opsi-configed-log-level.png["*opsi-configed*: Log-Level über das Menü einstellen", pdfwidth=80%]
...
TIP: Den Pfad zur aktuell verwendeten Logdatei erfahren Sie, wenn Sie im Menü _Hilfe_ den Eintrag _Aktueller Logfile_ anklicken. Im sich öffnenden Dialogfenster können Sie den Pfad in die Zwischenablage kopieren oder im Standard-Texteditor öffnen.
...
Im laufenden Programm ändern Sie die Sprache über den Menüpunkt _Datei_ / _International languages_. Anschließend findet eine Reinitialisierung des Programms statt mit einem Neuaufbau (fast) aller Komponenten in der neuen Sprache statt.
