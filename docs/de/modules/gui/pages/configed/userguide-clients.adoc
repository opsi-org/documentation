...
image::gui:opsi-configed-clientselection.png["*opsi-configed*: Kontextmenü der rechten Maustaste", width=800, pdfwidth=80%]

Am oberen Rand gibt es ein Suchfeld, in das Sie eine Zeichenkette eingeben können. Direkt daneben bestimmen Sie über das Drop-down-Menü, wo `opsi-configed` suchen soll: in allen Spalten (bedeutet: in allen Spalten, die in der aktuellen Konfiguration eingeblendet sind), nach Client-Name, nach der Beschreibung, nach Status (ein-/ausgeschaltet), nach Datum (zuletzt gesehen) und nach allen anderen aktiven Spalten. Innerhalb des Suchfeldes gibt es noch zwei weitere Optionen, die die Suche beeinflusst:

* _Groß- und Kleinschreibung beachten_ (Cc): Wenn dieser Knopf aktiv ist, muss die Groß- und Kleinschreibung der Ergebnisse mit der Suche übereinstimmen. Ansonsten spielt sie keine Rolle.

* _Regulärer Ausdruck_ (.*): Ist dieser Knopf aktiv, wird der Suchbegriff als regulärer Ausdruck interpretiert. Ein Regulärer Ausdruck besteht aus Mustern, nach denen gesucht wird. Für Details zu diesen Mustern schauen Sie bitte in die Java-Dokumentation für `java.util.regex.Pattern`.

Wenn Sie nach Eingabe Ihres Suchausdruckes die Taste [Eingabe] drücken, springt `opsi-configed` jeweils zum nächsten Treffer (ohne Treffer keine Auswahl). Klicken Sie mit der rechten Maustaste ins Suchfeld, um weitere Optionen einzublenden.

.*opsi-configed*: Suchfunktion für Clientauswahl
image::gui:opsi-configed-clientselection-search.png["*opsi-configed*: Suchfunktion für Clientauswahl", width=800, pdfwidth=80%]
...
* _An_: Die Spalte zeigt an, welche Clients aktuell verbunden sind. Die Werte werden mittels des Messagebus automatisch aktualisiert.

.*opsi-configed*: Client erreichbar
image::gui:opsi-configed-client-connected.svg["Client verbunden", width=20]
...
image::gui:opsi-configed-config-host-displayfields.png["*opsi-configed*: Konfigurieren Sie, welche Spalten die Clientliste anzeigt.", width=800, pdfwidth=80%]

In der Spalte _Session-Informationen_ wird der Accountname der aktuell aktiven Benutzersitzung angezeigt. Über das Kontextmenü, oder über den Menüpunkt _Client_, lässt sich diese Information mit einem Klick auf _Session-Informationen anfordern_ für die ausgewählten Clients abrufen. Dabei versucht der `opsiconfd`, eine Verbindung zu diesen Clients aufzubauen und diese Informationen zu sammeln. Ist kein Client ausgewählt, wird die Information für alle Clients abgerufen. Durch gezielte Client-Auswahl vermeiden Sie unnötiges Warten auf Netzwerk-Timeouts, falls versucht wird, eine Verbindung zu ausgeschalteten Rechnern herzustellen.
...
Sie können in der Clientliste einen oder mehrere Clients markieren, um sie gemeinsam zu bearbeiten. Verwenden Sie das Trichter-Icon, um die Liste nur auf Ihre ausgewählten Clients zu beschränken.

.*opsi-configed*: Über dieses Icon beschränken Sie die Anzeige auf ausgewählte Clients.
image::gui:configed_icons_filter.png["*opsi-configed*: Über dieses Icon beschränken Sie die Anzeige auf ausgewählte Clients.", pdfwidth=80%]
...
image::gui:configed_clientselection.png["*opsi-configed*: Über die freie Clientsuche stellen Sie eigene Suchkriterien zusammen.", pdfwidth=80%]
...
image::gui:configed_clientselection2.png["*opsi-configed*: Gespeicherte Suchanfragen", pdfwidth=80%]
...
image::gui:opsi-configed-failedactions-today.png["*opsi-configed*: Suchen Sie nach fehlgeschlagenen Aktionen.", pdfwidth=80%]
...
NOTE: Ist ein Client ausgewählt, erscheinen alle Gruppen, zu denen er gehört, mit gefüllter Fläche.

.*opsi-configed*: Baumansicht
image::gui:opsi-configed-treeview.png["*opsi-configed*: Baumansicht", pdfwidth=80%]
...
image::gui:opsi-configed-treeview-generate-group.png["*opsi-configed*: Gruppe anlegen", pdfwidth=80%]
...
image::gui:opsi-configed-treeview-groupname.png["*opsi-configed*: Name und Beschreibung der Gruppe eintragen", pdfwidth=80%]
...
* Ziehen Sie Clients einer Gruppe im Client-Baum, z.{nbsp}B. aus dem Knoten _CLIENT-LISTE_, in eine andere Gruppe.
...
image::gui:opsi-configed-client-contextmenu.png["*opsi-configed*: Das Menü *Client*", pdfwidth=80%]
...
image::gui:opsi-configed-clientinfos.png["*opsi-configed*: Erweiterte Spaltensicht für OPSI-Clients", width=800, pdfwidth=80%]
...
Es ist außerdem möglich, auch andere in der Datei `opsiclientd.conf` konfigurierte Events auszulösen. Die verfügbaren Optionen werden über den Server-Host-Parameter `configed.opsiclientd_events` bestimmt.
...
image::gui:opsi-configed-client-message-editing.png["*opsi-configed*: Nachricht an Client(s) verschicken", pdfwidth=80%]

Klicken Sie auf _OK_, um die Nachricht abzusenden. Sie wird auf dem Client als Pop-up-Fenster angezeigt.

.*opsi-configed*: Nachrichtenfenster auf dem Client
image::gui:opsi-configed-client-message-reading.png["*opsi-configed*: Nachrichtenfenster auf dem Client", pdfwidth=80%, width=800]
...
image::gui:opsi-configed-client-remotecontrol-choice.png["*opsi-configed*: Remote-Control-Software starten", pdfwidth=80%]
...
Die Kommandos bearbeiten Sie im Bereich Server-Konfiguration in _configed_ / _remote_control_:

.*opsi-configed*: Remote-Control-Aufrufe konfigurieren
image::gui:opsi-configed-properties-remotecontrol.png["*opsi-configed*: Remote-Control-Aufrufe konfigurieren", pdfwidth=80%]

TIP: Sofern ein Kommando mit dem Zusatz `.editable` aktiv ist, können Sie in der angezeigten Kommandozeile Variationen des Befehls eingeben, darunter beispielsweise auch Optionen oder Passwörter.
...
image::gui:opsi-configed-client-new.png["*opsi-configed*: Neuen Client anlegen", pdfwidth=80%]
...
Beim Anlegen neuer Clients darüber hinaus diese Optionen direkt spezifizieren:
...
[source,console]
----
opsi-cli jsonrpc execute config_updateObjects '{"defaultValues": ["add client", "remove client"], "editable": false, "multiValue": true, "possibleValues": ["add client", "remove client"], "type": "UnicodeConfig", "id": "configed.host_actions_disabled"}'
----
...
image::gui:opsi-configed-client-change-depot.png["*opsi-configed*: Client(s) in anderes Depot bewegen", pdfwidth=80%]
...
image::gui:opsi-configed-localbootproducts.png["*opsi-configed*: Liste der Localboot-Produkte mit Detailansicht rechts", width=800, pdfwidth=80%]
...
image::gui:opsi-configed-editlist.png["*opsi-configed*: Property-Tabelle", pdfwidth=80%]

Sie können die Werte direkt in der Tabelle bearbeiten. In diesem Fall können Sie beispielsweise aus einer Liste vorkonfigurierter Werte auswählen. Je nach Eigenschaft (sofern sie bearbeitbar ist) können Sie auch neue Werte eingeben.

Nur wenn eine Liste mit mehreren Werten zulässig ist, öffnet sich ein zusätzliches Dialogfenster. Sie können ganz einfach mehrere Einträge auswählen, indem Sie die [Strg]-Taste gedrückt halten und darauf klicken.
...
image::gui:opsi-configed-netbootproducts.png["*opsi-configed*: Liste der Netboot-Produkte mit Detailansicht rechts", width=800, pdfwidth=80%]
...
NOTE: Einige Hersteller verwenden Bezeichnungen mit Sonderzeichen, beispielsweise `5000/6000/7000`. Folgende Sonderzeichen werden intern durch einen Unterstrich (`_`) ersetzt: `< > ? " : | \ / *`
...
image::gui:opsi-configed-softwareinventory.png["*opsi-configed*: Software-Informationen", width=800, pdfwidth=80%]
...
image::gui:opsi-configed-logfiles.png["*opsi-configed*: Logdateien betrachten", width=800, pdfwidth=80%]
...
image::gui:opsi-configed-hostparameter_setting.png["*opsi-configed*: Host-Parameter (Kontextmenü eines Client-Eintrags)", width=800, pdfwidth=80%]
...
- Es gibt für die Client-Propertys ein Kontextmenü mit den Optionen _Aktuellen Default als spezifischen Wert setzen_ und _Spezifischen Wert entfernen_ (Client-Wert ist immer der aktuelle Server-Standard).
...
image::gui:opsi-configed-hostparameters.png["*opsi-configed*: Host-Parameter in der Client-Konfiguration", pdfwidth=80%]
