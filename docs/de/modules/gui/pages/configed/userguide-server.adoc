...
Link zurück zu Client-Konfig und Host-Parameter (xref:gui:pages/configed/userguide-clients.adoc#opsi-manual-configed-hostproperties[Host-Parameter: Client- und Server-Konfiguration])
...
Die anderen Einträge, die an dieser Stelle des Property-Baums stehen, sind die Defaultwerte für die auch userspezifisch mögliche Konfiguration der Server-Konsole (siehe Kapitel xref:gui:pages/configed/userguide-serverconsole.adoc[Server-Konsole]).
