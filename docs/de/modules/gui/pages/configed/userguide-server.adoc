...
Grundsätzlich funktioniert die Anzeige und die Bearbeitung der Host-Parameter genauso wie dir für die Clients (xref:gui:configed/userguide-clients.adoc#opsi-manual-configed-hostproperties[Host-Parameter: Client- und Server-Konfiguration]).

.*opsi-configed*: Host-Parameter in der Server-Konfiguration
image::gui:opsi-configed-hostparameters-server.png["*opsi-configed*: Host-Parameter in der Server-Konfiguration", width=800, pdfwidth=80%]
