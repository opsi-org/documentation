...
image::gui:configed_serverkonsole.png["opsi-configed: Server-Konsole", width=800, pdfwidth=80%]
...
image::gui:configed_ssh.png["*opsi-configed*: SSH-Verbindungsdaten", pdfwidth=80%]
...
* _Lizenz-Datei hochladen_: Laden Sie eine Lizenz-Datei für opsi-Erweiterungen zum Server hoch. Diese wählen Sie entweder auf dem lokalen Rechner aus oder laden Sie mit `wget` aus dem Internet herunter. Optional können Sie Zugangsdaten für den `wget`-Befehl eingeben.
...
image::gui:configed_serverkonsole-defcommands.png["*opsi-configed*: Befehle definieren", width=800, pdfwidth=80%]
...
image::gui:configed_serverkonsole-exec.png["*opsi-configed*: Befehl ausführen, Parameterabfrage", width=800, pdfwidth=80%]
