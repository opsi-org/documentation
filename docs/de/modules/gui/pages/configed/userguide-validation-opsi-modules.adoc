...
Über das OPSI-Icon in der linken Toolbar öffnen Sie den _Validierungsstatus der OPSI-Module_ und können den aktuellen Lizenzstatus der OPSI-Erweiterungen einsehen.

.*opsi-configed*: Validierungsstatus der OPSI-Module
image::gui:configed_validierung-opsi-module.png["*opsi-configed*: Validierungsstatus der OPSI-Module", pdfwidth=90%]
...
Die Ansicht zeigt außerdem eine Legende zu den Warnungen und Warngrenzen an:
...
Im unteren Teil befinden sich Informationen zu den Clients und den Kundendaten, für welche die Lizenzen ausgestellt wurden.
