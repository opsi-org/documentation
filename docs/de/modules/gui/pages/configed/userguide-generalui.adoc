...
Nach der erfolgreichen Anmeldung sehen Sie das `opsi-configed`-Hauptfenster. Auf der linken Seite finden Sie eine Gruppe von Icons, über die Sie die verschiedenen Hauptdarstellungen auswählen können. Standardmäßig ist die Client-Konfiguration ausgewählt. Darunter befinden sich Depot-Konfiguration, Server-Konfiguration, Dashboard, Validierungsstatus der opsi-Module, Health Check und xref:opsi-modules:licensemanagement.adoc[Lizenzmanagement]..
