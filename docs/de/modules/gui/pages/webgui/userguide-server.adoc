...
Klappen Sie das Quick Panel aus, um einen oder mehrere Server auszuwählen. Auf dem ersten Reiter in der Schnellauswahl (Symbol image::gui:webgui/opsi-webgui_quickpanel_tab_selection.png[Auswahl]) sehen Sie eine Liste aller aktuell markierten Elemente. Über einen Klick auf das kleine Kreuz entfernen Sie einen Server aus der Liste.
...
Wechseln Sie in der Schnellauswahl auf den zweiten Reiter (Symbol image::gui:webgui/opsi-webgui_quickpanel_tab_depots.png[Server]). Die Zahl neben dem Icon zeigt die Anzahl aktuell ausgewählter Server an.
