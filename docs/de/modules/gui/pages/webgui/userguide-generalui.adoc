...
[[webgui-quickpanel]]
.WebGUI: Quick Panel
image::webgui/opsi-webgui_quickpanel.png["WebGUI: Quick Panel", width=250, pdfwidth=80%]
...
Die Schnellnavigation im Quick Panel (<<webgui-quickpanel>>) erleichtert Ihnen das Ansehen der gerade ausgewählten Elemente und das Umschalten zwischen Servern, Clients und Produkten, unabhängig davon, auf welcher Seite Sie sich gerade befinden. Sie ermöglicht außerdem das Ausführen bestimmter Aktionen für alle ausgewählten Clients. Zusätzlich bietet die Schnellnavigation Optionen, um die Sprache, das Theme und den Speicherungsmodus anzupassen.
