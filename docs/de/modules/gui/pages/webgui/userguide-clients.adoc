////
; Copyright (c) uib GmbH (www.uib.de)
; This documentation is owned by uib
; and published under the german creative commons by-sa license
; see:
; https://creativecommons.org/licenses/by-sa/3.0/de/
; https://creativecommons.org/licenses/by-sa/3.0/de/legalcode
; english:
; https://creativecommons.org/licenses/by-sa/3.0/
; https://creativecommons.org/licenses/by-sa/3.0/legalcode
;
; credits: https://www.opsi.org/credits/
////

:Author:    uib GmbH
:Email:     info@uib.de
:Date:      24.11.2023
:Revision:  4.3
:toclevels: 6
:doctype:   book
:icons:     font
:xrefstyle: full

include::common:partial$opsi_terms.adoc[]

[[opsi-manual-opsiwebgui-opsiclients]]
= Clients

Alle Clients der ausgewählten Server listet die Tabelle _Clients_ auf. Sie öffnen die Ansicht über die linke Navigationsleiste / _Clients_ / _Alle Clients_.

.WebGUI: Client-Ansicht
image::webgui/opsi-webgui_clients.png["WebGUI: Client-Ansicht", width=800, pdfwidth=80%]

[[opsi-manual-opsiwebgui-opsiclients-creation]]
== Client erstellen

Um mit opsi Rechner zu verwalten, müssen diese Clients dem System bekannt sein. Außerdem muss auf den Rechnern ein Client-Agent laufen, damit eine Kommunikation zwischen Server und Client möglich ist (siehe Kapitel xref:clients:opsi-clients.adoc[opsi-Clients]).

Um einen neuen opsi-Client über das WebGUI zu erstellen, öffnen Sie aus der linken Leiste _Clients_ / _Erstellung_. Füllen Sie das Formular aus und tragen Sie einen Namen (Feld _ID_), eine Beschreibung, die IP- und MAC-Adresse ein, wählen Sie einen Depotserver und optional eine Gruppe aus. Im unteren Bereich öffnet ein Klick ins Feld _Netboot Product_ eine Auswahlliste vorhandener xref:opsi-products:netboot-products.adoc[Netboot-Produkte]. Optional aktivieren Sie die Checkbox _UEFI_. Abschließend klicken Sie auf _Hinzufügen_.

.WebGUI: Client erstellen
image:webgui/opsi-webgui_clientcreation.png["WebGUI: Client erstellen", width=800, pdfwidth=80%]

NOTE: Um einen neuen opsi-Client zu erstellen, ist mindestens ein Eintrag im Feld _ID_ erforderlich. Neue Clients werden in der Voreinstellung dem {opsi-configserver} hinzugefügt.

[[opsi-manual-opsiwebgui-opsiclients-actions]]
== Aktionen ausführen

Es gibt eine Reihe von Aktionen, die Sie über das WebGUI für einen bestimmten Client ausführen können. Diese erreichen Sie über die Client-Tabelle (_Clients_ / _Alle Clients_). Wählen Sie einen Client aus und klicken Sie auf das Icon mit den drei Punkten (image:webgui/opsi-webgui_buttonclientactions.png[Aktionen]). Alternativ erreichen Sie das Menü _Aktionen_ auch über das Kontextmenü der rechten Maustaste.

.WebGUI: Das Menü *Aktionen*
image:webgui/opsi-webgui_clientactions.png["WebGUI: Das Menü *Aktionen*", width=200, pdfwidth=80%]

Die nächsten Abschnitte beschreiben die einzelnen Aktionen genauer.

[[opsi-manual-opsiwebgui-opsiclients-actions-clientagent]]
=== Deploy Client Agent

Aktion Deploy Client Agent image:webgui/opsi-webgui_buttondeployagent.png[Deploy Client Agent]
Um einen Client mit OPSI zu verbinden und als {opsi-client} zu registrieren, muss auf diesem der {opsi-client-agent} installiert sein.
Die Installation des {opsi-client-agent} kann auch bei der Clienterstellung angeordert werden.

NOTE: Wenn das Betriebssystem auf dem Client mit OPSI eingespielt wurde, dann wurde der {opsi-client-agent} automatisch installiert und muss nicht explizit angefordert werden.

image::webgui/opsi-webgui_clientdeployagent.png[Deploy Client Agent, width=300]

[[opsi-manual-opsiwebgui-opsiclients-actions-ondemand]]
=== On Demand

Aktion OnDemand image:webgui/opsi-webgui_buttonondemand.png[OnDemand]
Sie können Installationen/Updates/.. on Demand pushen und Änderungen somit sofort auf den Client anwenden, indem Sie das `on_demand` Ereignis auf ausgewählten Clients ausführen.

NOTE: Es ist möglich, `on_demand` auf mehreren Clients auszuführen.
Wählen Sie mindestens einen Client aus und klicken Sie in der Schnellnavigation auf image:webgui/opsi-webgui_iconondemand.png[Ondemand Icon].
Es zeigt ein Bestätigungs-Popup mit einer Liste ausgewählter Clients. Versehentlich ausgewählt Clients können somit vorher entfernt werden.

image::webgui/opsi-webgui_clientondemand.png[On Demand, width=300]

[[opsi-manual-opsiwebgui-opsiclients-actions-notify]]
=== Benachrichtigen

Aktion Benachrichtigen image:webgui/opsi-webgui_buttonnotify.png[Notify]
Die Benachrichtigung eines Clients ist in Situationen nützlich, die die Aufmerksamkeit des Benutzers erfordern.
Der Nutzer kann eine Popup-Nachricht oder Warnung an den ausgewählten Client senden, bevor er einen Neustart oder andere sofortige Aktionen auslöst. 

image::webgui/opsi-webgui_clientnotify.png[NotifyClient, width=300]

[[opsi-manual-opsiwebgui-opsiclients-actions-reboot]]
=== Neustarten 

image:webgui/opsi-webgui_buttonreboot.png[Reboot]
Bei diesem Ereignis wird versucht sofort die ausgewählten Client neuzustarten. 

image::webgui/opsi-webgui_clientreboot.png[ClientReboot, width=300]

[[opsi-manual-opsiwebgui-opsiclients-actions-rename]]
=== Umbenennen

[[opsi-manual-opsiwebgui-opsiclients-actions-delete]]
=== Löschen 

image:webgui/opsi-webgui_buttondeletion.png[Delete]
Durch das Löschen eines Clients wird dieser aus der Datenbank mit allen zugehörigen Daten entfernt. 

image::webgui/opsi-webgui_clientdeletion.png[Client Deletion, width=300]

[[opsi-manual-opsiwebgui-opsiclients-groups]]

== Client-Gruppen
Clients werden hauptsächlich in `groups`, `clientdirectory` und `clientlist` gruppiert.
Ein Client kann in der Gruppe `clientdirectory` nur einmal vorkommen, in der Gruppe `groups` allerdings mehrmals. 
In der `clientlist` werden alle clients der ausgewählten Depots aufgelistet.

In der Schnellnavigation werden die Gruppen und ihre Clients in einer Baumstruktur unter dem Reiter image:webgui/opsi-webgui_quickpanel_tab_clients.png[Clients tab in quickpanel] angezeigt.
Die Zahl neben dem Icon entspricht der Anzahl der ausgewählten Clients.
Der erste Tab image:webgui/opsi-webgui_quickpanel_tab_selection.png[Auswahl-Tab im Quickpanel] zeigt eine Schnellansicht der editierbaren Liste aller Auswahlen.

image::webgui/opsi-webgui_quickpanel_tab_clients_content.png[Clients tab content in quickpanel, width=300]

// image::webgui/opsi-webgui_clientgroups.png[Client Groups]

TIP: Sortieren Sie die Clienttabelle nach den ausgewählten Clients für eine Detailansicht der Clients.

[[opsi-manual-opsiwebgui-opsiclients-attributes-parameter]]
== Attribute und Parameter bearbeiten

Um die Client-Attribute und -Parameter zu bearbeiten gibt es zwei Möglichkeiten: Entweder über die Seite  *Clients > Konfiguration*, oder  *Clients > Alle Clients* und den Konfigurationsbuttonimage:webgui/opsi-webgui_buttonconfig.png[Konfiguration]. 

Letzteres hat den Vorteil, dass man schneller zwischen den Clients wechseln kann, da die Konfiguration neben den Clients angezeigt wird.

image::webgui/opsi-webgui_clientconfig.png[Client Configuration, width=600]

include::partial$warn-quicksave.adoc[]

[[opsi-manual-opsiwebgui-opsiclients-logs]]
== Logfiles betrachten

Um die Client Logs zu betrachten gibt es zwei Möglichkeiten: Entweder über die Seite  *Clients > Logs*, oder  *Clients > Alle Clients* und den Logbuttonimage:webgui/opsi-webgui_buttonlog.png[Logs Button]. 
Letzteres hat den Vorteil, dass man schneller zwischen den Servern wechseln kann, da die Logausgabe neben den Clients angezeigt wird.
Dabei kann das anzuzeigende Loglevel (default: `5`) und der Logtyp (`bootimage`, `clientconnect`, default: `instlog`, `opsiconfd`, `userlogin`) ausgewählt werden. 


image::webgui/opsi-webgui_clientlogs.png[Client Logs, width=800]
