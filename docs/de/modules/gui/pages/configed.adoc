...
Das Programm `opsi-configed` ist ein komfortables Interface, das per HTTPS mit dem opsi-Server kommuniziert und daher auf jedem Rechner läuft, der über dieses Protokoll Verbindung zum Server aufnehmen kann. 

[[opsi-manual-configed-preconditions]]
== Voraussetzungen

Die Management-Oberfläche `opsi-configed` benötigt `opsiconfd` 4.1 oder neuer (siehe Abschnitt xref:server:components/opsiconfd.adoc[Der Dienst *opsiconfd*]). Wenn Sie anstelle des opsi-Pakets die portable Java-Anwendung einsetzen möchten, ist eine aktuelle Java-Laufzeitumgebung erforderlich.
...
Alternativ gibt es für alle drei Betriebssysteme eine portable Anwendung, die ohne Installation läuft. Sie finden das Java-Programm auf unseren Downloadservern im Verzeichnis `stable/misc`; es heißt `opsi-configed-windows.exe` (Windows), `opsi-configed-linux.run` (Linux) bzw. `opsi-configed-macos` (macOS). Sie können die zum Betriebssystem passende Version auch über den Webbrowser herunterladen: Geben Sie die Adresse \`https://<opsidepotserver>:4447/` ein, wechseln Sie zum Reiter _Links_ und klicken auf den Download-Link.

== Start

Wenn Sie den `opsi-configed` als Localboot-Produkt installiert haben, starten Sie die Management-Oberfläche über die Verknüpfung in Ihrem Startmenü. Unter macOS finden Sie das Werkzeug im Ordner _Programme_ über den Finder.

Die portable Anwendung starten Sie unter Windows über einen Doppelklick auf `opsi-configed-windows.exe`. Unter Linux und macOS machen Sie die heruntergeladene Datei zunächst ausführbar. Dazu öffnen Sie ein Terminal, navigieren ins Downloadverzeichnis und geben das folgende Kommando ein:
...
Danach starten Sie das Programm aus dem Terminal heraus über dieses Kommando:
...
Ins Anmeldefenster geben Sie den Namen eines Benutzers ein, der Mitglied der Gruppe `opsiadmin` ist (siehe xref:server:components/authorization.adoc[Berechtigungen]).
