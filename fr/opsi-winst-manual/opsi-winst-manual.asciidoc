...
* Quand il est appelé avec l'option `/allloginscripts` '{opsi-winst}' fait les configurations pour l'utilisateur de la session courante (particulièrement dans un contexte de profils Itinérants). C'est une option en co-financement - vous devez l'acheter pour pouvoir l'utiliser. +
...
Pour d'autres exemples regarder le produit 'opsi-winst-test' particulièrement '$Flag_winst_opsiServiceCall$ = "on"'
...
Pour d'autres exemples regarder le produit 'opsi-winst-test' et là particulièrement 
...
Il y a deux options '{opsi-winst}' reconnue:
...
Pour d'autres exemples regarder le produit 'opsi-winst-test' particulièrement 
...
Pour les sections 'Registry' et 'Files' nous avons des nouvelles options d'appel:
...
Les installations MSI qui semblent avoir besoin d'un utilisateur local peuvent parfois être configurées avec l'option 'ALLUSERS=2' pour procéder sans un tel utilisateur:
[source,winst]
----
[Actions]
DefVar $LOG_LOCATION$
...
* Déplacez tous les fichiers d'installation dans ce répertoire.

* Renommez le script d'installation de '<productname>.ins' à 'local_<productname>.ins'

* Créez un nouveau '<produktname>.ins' dans 'install\productId' et écrivez dedans les déclarations comme ci-dessous documentées (avec les valeurs des variables adaptées à votre situation). 

* Assurez-vous que le script qui est maintenant nommé 'local_<produktname>.ins' se termine par un appel au redémarrage: La dernière commande exécutée dans la section Actions doit être la ligne +
`ExitWindows /Reboot`

* Insérez un appel au début du script 'local_<produktname>.ins' qui supprime le mot de passe de l'administrateur local temporaire:
[source,winst]
----
[Actions]
...
Il n'y a pas d'accès direct à partir du script local aux propriétés de produits (généralement par l'intermédiaire de la fonction chaîne `GetProductProperty`) . Si il ya des valeurs nécessaires le script maître doit les récupérer et par exemple les enregistrer temporairement dans la base de registre.
...
Le script local fonctionne avec un administrateur connecté. Vous devez verrouiller le clavier lorsque le test est fait. Autrement n'importe qui assis devant le client pourrait arrêter l'exécution du script et prendre le contrôle de la session.

Dans l'exemple suivant, le mot de passe de l'utilisateur temporaire opsiSetupAdmin est réglée via la fonction RandomStr, qui est fortement recommandé.
