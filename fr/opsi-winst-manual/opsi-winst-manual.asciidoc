...
* `Set` <nom de la variable> = <valeur> +
sets the registry variable <nom de la variable> to value <valeur>.
<nom de la variable> as well as <valeur> are strings and have to be enclosed in citations marks. A non-existing variable will be created. 
...
* `Set` <nom de la variable> = <registry type>:<valeur> + 
sets the registry variable <nom de la variable> to value <valeur> of type <registry type>. The following registry types are supported:
...
resp.
+
`Add` <nom de la variable> = <registry type> <valeur> +
...
exports the referred (String or MultiSZ) value as file <nom de fichier> lines (each String forming a line).

* `DeleteVar` <nom de la variable> +
removes the entry with variable <nom de la variable> from the opened key.
