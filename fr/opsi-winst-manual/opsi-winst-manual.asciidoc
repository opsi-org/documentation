...
* Lorsqu'il est appelé avec l'option `/usercontext` '{opsi-winst}' peut faire des configurations pour un utilisateur connecté précisé (en particulier dans un Windows Terminal Server ou d'un contexte de profil itinérant).
...
La constante `%ScriptPath%` est le nom prédéfini de l'endroit où '{opsi-winst}', trouve et de lit le script qu'il exécute. Cet emplacement peut être, par exemple,  `p:\product`. Puis nous devons écrire +
`"%ScriptPath%"` +
...
Exemple (Par Exemple voir <<opsi-winst-string>>):
...
`StayOnTop` il est `false` par défaut afin d'éviter que certains autres processus génère un message d'erreur qui peut éventuellement ne pas être vu si '{opsi-winst}' reste au-dessus.
...
permet à '{opsi-winst}'  d'afficher la valeur de l'expression de chaîne respectivement la séquence de caractères dans la fenêtre batch dans la ligne d'information en haut. Le texte est maintenu aussi longtemps qu'aucune nouveau `message` est fixé. +
Exemple:
[source,winst]
----
...
place l'image nommée <nom de l'image> (dans le format BMP, JPEG ou PNG, de taille 160x160 pixel) et montre l'inscription. + 
...
écrit la valeur de l'expression de chaîne respectivement la séquence de caractères dans le fichier journal.
...
* La cible la plus commune d'un sous-programme appel une autre section intérieure dans le fichier de script où la déclaration d'appel est placé (comme dans l'exemple).

* Nous pouvons mettre les instructions mentionnées dans un autre fichier qui sert une section externe.

* Toute liste de chaînes peuvent être utilisés comme liste d'instructions pour un appel de sous-programme.

Nous décrivons la syntaxe des appels sous-programme en détail:

[[opsi-winst-commands-subcall-components]]
==== Syntaxe de la procédure d'appel

Formellement, la syntaxe peut être donnée par +
'<proc. type>(<proc. name> | <External proc. file> | <String list function> )'

Cette expression peut être complétée par un ou plusieurs paramètres (Type de procédure dépendante). 

Cela signifie: Un appel de procédure se compose de trois parties principales.

La première partie est le spécificateur de type sous-programme. +
Exemples de noms de type sont 'Sub' (nous appelons une procédure de type 'Sub' qui est à nouveau une section primaire) ou 'Files' et 'WinBatch' (appels des sections spéciales du secondaire). L'aperçu complet des types existants de sous-programme est donné à <<opsi-winst-commands-subcall>>.

La deuxième partie détermine où et comment les lignes de sous-programme se retrouvent.

. Le sous-programme est une séquence de lignes situées dans le script exécutés '{opsi-winst}' comme une autre section interne. Ensuite, un nom (constitué à partir des lettres, des chiffres et des caractères spéciaux) doit être annexée au spécificateur de type (sans espace) afin de former un nom de section unique. +
`sub_install_winXP` +
ou +
`files_copy_winXP` +
Les noms de section sont indépendants comme n'importe quelle autre chaîne. 

. Si le spécificateur de type est unique une expression de la liste chaîne ou une expression chaîne est attendue. Si l'expression suivant le spécificateur de type ne peut être résolu comme une expression liste de chaînes (cf. cas (3)) il est supposé être une expression chaîne. La chaîne est alors interprétée comme un nom de fichier. '{opsi-winst}' essaie d'ouvrir le fichier comme un fichier texte et interprète ses lignes comme une section externe du type spécifié. +
Par exemple: +
`sub "p:\install\opsiutils\mainroutine.ins"`
essayé d'exécuter des lignes de programme principal .ins comme des déclarations d'un paragraphe. 

. Si l'expression suive un spécificateur de type pur la section est résoluble comme une expression de la liste de chaînes les lignes de la liste sont interprétés comme des états de la section. +
Ce mécanisme peut par exemple être utilisée pour charger un fichier qui a le format Unicode et ensuite la traiter par les mécanismes habituels
[source,winst]
----
...
* `registry`, l'énoncé de base, en précisant le type de section, +
* `loadUnicodeTextFile(...)`, une expression de la liste Chaîne spécifiant la façon d'obtenir les lignes d'un REEE section du Registre. son substitut. +
* `/regedit`, paramétrage de l'appel de registre. 

Dans cet exemple, le paramètre d'appel donne déjà un exemple pour la troisième partie d'un appel du paragraphe:

La troisième partie d'un appel de procédure comprend le type des options spécifiques à la police. 

Pour une référence des options d'achat voir les descriptions des appels dans la section <<opsi-winst-secsections>>.

[[opsi-winst-commands-reboot]]
=== Contrôle du redémarrage

La commande `ExitWindows` est utilisé pour contrôler les redémarrages , l'arrêt et des actions similaires qui devrait avoir lieu après que '{opsi-winst}' lui-même est terminée. Le nom de la commande et le fait qu'il n'y ait pas 'ExitWindows' sans modificateur a des raisons historiques: Travaillant sous Windows 3.1 vous pouvez quitter les fenêtres pour revenir au niveau de DOS.

* `ExitWindows /RebootWanted` +
OBSOLETE: une demande de redémarrage est enregistré qui devrait être exécuté lorsque toutes les demandes d'installations sont traitées, et le dernier script est terminé. +
En fait, cette commande est maintenant traitée comme un `ExitWindows /Reboot` (car, autrement, une installation pourrait échouer parce qu'un produit nécessaire n'est pas encore complètement installé).

* `ExitWindows /Reboot` +
déclenche le redémarrage après que '{opsi-winst}' a terminé le script actuellement traité.

* `ExitWindows /ImmediateReboot` +
rompt l'exécution normale d'un script n'importe où dedans le script. Lorsque cette commande est appelée '{opsi-winst}' fonctionne aussi directement que possible jusqu'au bout impliquant l'appel système ExitWindows. Dans le contexte d'un opsi-client-agent installé il est garanti que après le redémarrage '{opsi-winst}' fonctionne à nouveau dans le script qui a avorté. Par conséquent, le script doit prendre les dispositions que l'exécution se poursuit après le point où il a été laissé le tour avant (sinon on peut obtenir une boucle infinie ...) Voir l'exemple dans cette section.

* `ExitWindows /ImmediateLogout` +
L'exécution normale d'un script coupe au moment de l'appel, et '{opsi-winst}' s'arrête. Ce comportement est nécessaire si un utilisateur automatisé se connecte pour un autre utilisateur(voir <<opsi-winst-cookbook-local-admin>>).

* `ExitWindows /ShutdownWanted` +
définit un indicateur dans le Registre que le PC s'arrête lorsque toutes les demandes d'installations sont traitées, et le dernier script est terminé. 

La manière dont des drapeaux peuvent être établies pour s'assurer que le script ne fonctionne pas dans une boucle infinie quand `ExitWindows /ImmediateReboot` est appelé, on la démontre par le fragment de code suivant:
[source,winst]
----
DefVar $OS$
DefVar $Flag$
...
=== Suivi des installations qui ont échoué

Si une installation ne réussit pas ce produit doit être signalé au serveur.

Selon le fait qu'il n'y a pas d'automatisme pour détecter une installation qui a échoué cela doit être fait par un script de commandes. 

Pour indiquer dans un script '{opsi-winst}' que l'installation est échouénous devons appeler la déclaration: +
`isFatalError` +
Si cette déclaration est appelé '{opsi-winst}' arrête l'exécution normale du script et définit le résultat à 'failed' (sinon, il est à 'success'). +
Par exemple , un "erreur fatale" doit être déclenchée s'il n'y a pas beaucoup d'espace libre comme il est nécessaire pour l'installation: 
[source,winst]
----
DefVar $SpaceNeeded$"
...
Il est également possible de déclarer +
`isFatalError` +
selon le nombre d'erreurs qui sont survenues dans quelques parties critique d'un script d'installation. Pour ce faire, on initialise le compteur d'erreurs par la commande +
* `markErrorNumber` +
Le nombre d'erreurs d'exécution qui se produisent après avoir réglé le compteur peut être consultée par la fonction du nombre évalué +
* `errorsOccuredSinceMark` +
Nous pouvons évaluer le résultat dans une condition de comparaison numérique (que n'est encore mis pas en œuvre pour cette expression). Par exemple nous pouvons dire +
* `if errorsOccuredSinceMark > 0` +
et on peut, si cela semble logique, alors déclarer +
`isFatalError` +
...
Nous pouvons tester cette hypothèse par l'exemple du script suivant:
[source,winst]
----
markErrorNumber
...
[[opsi-winst-dosbatch]]
=== DOSBatch/DosInAnIcon (ShellBatch/ShellInAnIcon) Sections
...
<varname> as well as <value> are strings and have to be enclosed in citations marks. A non-existing variable will be created. 
...
'REG_DWORD'::: (integer values)

'REG_BINARY'::: (binary values usually given as two-digit hex numbers 00 01 02 .. 0F 10 .., notiert)

'REG_MULTI_SZ'::: (string value arrays, in '{opsi-winst}' we have to use "|" as separator) +
...
registry loadUnicodeTextFile("%scriptpath%/opsiorgkey.reg") /regedit
----
A registry patch using regedit format can as well be executed "for all NT users" similarly as the common '{opsi-winst}' registry section. That is, a path like 
