...
* `calculate(`<str>`)` +
cette fonction de chaîne, calcule l'expression arithmétique de la chaîne <str> et renvoie le résultat nombre entier comme une chaîne. +
...
Exemple: `c:\windows\system32\WindowsPowerShell\v1.0\powershell.exe` appels (aussi lorsqu'il est exécuté sur un système 64 bit) le 'powershell.exe' 32 bit.

*  `/64Bit` +
Le chemin de l'interpréteur est supposé être un chemin 64 bits. +
Exemple: `c:\windows\system32\WindowsPowerShell\v1.0\powershell.exe` appels (dans un système 64 bit) le 'powershell.exe' 64 bit.

*  `/SysNative` +
Le chemin de l'interpréteur est attribué en fonction de l'architecture OS. +
Exemple: `c:\windows\system32\WindowsPowerShell\v1.0\powershell.exe` appels (dans un système 64 bit) le 'powershell.exe' 64 bit et dans un système 32bit le 'powershell.exe' 32 bit.
