...
= Mise en route d'opsi (Version d'opsi 4.0.1)
...
Travailler avec une machine VMware demande un ordinateur hôte raisonnable. Pour les tests un client peut être lancé dans une autre machine VMware sur le même ordinateur hôte.
...
Choisissez l'option de menu 'OpsiClient/Créer un nouveau client opsi' et complétez la description du client:
...
[source,prompt]
----
./show_drivers.py <nom_du_client>
----
Lorsque cette commande est appelée, il vous montrera une sélection dont les pilotes seront choisis pour installation avec BootImage via PCI-IDs, USB-IDs, HD-Audio-IDs, et 'additional_drivers' (ou 'byAudit'),  et le matériel qui n'a toujours pas de pilote. 

Utilisez le résultat de `show_drivers.py` pour voir quels pilotes doivent être installés.

Il se pourrait que les fabricants incluent des pilotes différents pour différents systèmes d'exploitation (ex. Vista ou Win7) ou différentes configurations (ex. SATA ou SATA RAID).  `create_driver_links.py` ne peut pas faire cette distinction.  Si vous pensez que le mauvais pilote a été installé, déplacez le pilote dans le répertoire 'drivers/exclude' et ensuite appelez `create_driver_links.py` encore.  Les pilotes dans le répertoire 'drivers/exclude' ne sont pas utilisés au cours de l'intégration.
...
image::../images/opsi-winst-without-delsub.png["code double pour la désinstallation",width=300]

.éviter le code doubles en utilisant les sous-sections
image::../images/opsi-winst-with-delsub.png["éviter le code doubles en utilisant les sous-sections",width=300]
...
image::../images/winst-log-view.png["{opsi-winst} fenêtre de vue du fichier de logs",400]
...
image::../images/newprod-script-names.png["Screenshot: Entrée des noms de script {opsi-winst} pour les différentes actions",width=400]
