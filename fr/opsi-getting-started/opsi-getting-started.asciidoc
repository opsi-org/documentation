...
* `winpe` +
Ce répertoire contient une image WinPE bootableà pour Windows Vista et suivantes .
...
Donc vous devez utiliser `Au3info.exe` pour obtenir le 'ControlId' dont vous avez besoin dans cette commandes. S'il vous plaît utilisez le 'ControlId' numérique, les autres variantes semble ne fonctionner pas correctement:
[source,configfile]
----
...
| User login | _pas encore implémenté_ | _pas encore implémenté_
|=======================
