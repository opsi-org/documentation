...
Si vous prévoyez d'utiliser le backend mysql (par exemple pour les données d'inventaire), vous devez l'initialiser maintenant. Par conséquent, utilisez la commande `opsi-setup --configure-mysql`.
...
CAUTION: Ne pas utiliser le caractère '§' dans le cadre du mot de passe. Il rend pas possible de se connecter au service Web OPSI.
...
L'étape suivante consiste à définir un ou plusieurs dépendances du produit. S'il n'ya pas de dépendances des produits mettre en 'No'.
