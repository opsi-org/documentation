...
.Choix de la langue
image::../images/1stboot-language-selection.png["Screenshot: Sélection de la langue ",width=400]
...
image::../images/1st-startup-mask.png["Screenshot: 1stboot.py Masque de démarrage",width=400]

Remplissez les informations de configuration de votre réseau et répondez aux questions.

.1stboot Masque de saisie
image::../images/1stboot-input-mask.png["Screenshot: 1stboot Masque de saisie",width=400]
...
image::../images/opsiserver_start_gui.png["Screenshot: Vue de l'interface web de configuration de opsi-server",400]
...
Maintenant vous pouvez trouver un lien vers l'interface de gestion OPSI en accédant à l'URL `https://<nom_du_serveur>`.
...
Choisissez dans `opsi-configed`, le mode 'Configuration des clients' à droit en haut de la page.
...
Passez à l'onglet 'Configuration produit', cherchez la ligne du logiciel système souhaité ('hwaudit' et/ou 'swaudit'). Allez à la colonne 'Action demandée' et sélectionnez l'action 'setup'. Enfin enregistrer avec un click sur le bouton (ou avec le menu contextuel).

Maintenant redémarrez le client ou plus simplement choisissez 'Activer evénement à la demande' dans le menu qui apparaît au click droit de la souris, 'hwaudit' et/ou 'swaudit' devraient être lancé. Le client analyse l'inventaire matériel et/ou logiciels, et renvoie les résultats au serveur.

Vous pouvez voir ces données au niveau des onglets 'Information matériels' et 'Information logiciels'.
...
Passez à l'onglet 'Produits Netboot', cherchez la ligne du logiciel système souhaité ('hwinvent'). Allez à la colonne 'Action demandée' et sélectionnez l'action 'setup'. Enfin enregistrez avec un click sur le bouton (ou avec le menu contextuel).

Maintenant redémarrez le client (via PXE), l'image de démarrage avec 'hwinvent' devrait être lancé. Par conséquent, le client après le redémarrage, charge une image de démarrage linux qui analyse le matériel et renvoie les résultats au serveur.

Vous pouvez voir ces données par l'onglet 'Information matériels'.
...
Choisissez l'option de menu 'OpsiClient/Créer un nouveau client opsi' et completez la description du client:
...
Passez à l’onglet 'Produits Netboot', cherchez la ligne du logiciel système souhaité ('hwinvent'). Allez à la colonne 'Action demandée' et sélectionnez l’action 'setup'. Enfin enregistrer avec un click sur le bouton (ou avec le menu contextuel).
...
image::../images/opsi-client-boot-cd.png["Screenshot: Démarrer l'image opsi-client-boot-cd",width=400]
...
image::../images/boot-cd-config.png["Screenshot: bootimage/boot-cd écran de configuration",width=400]

Confirmez avec 'OK'.

.bootimage / boot-cd:  Choisissez comment créer le client
image::../images/boot-cd-select.png["Screenshot: bootimage / boot-cd:  Choisissez comment créer le client",width=200]
...
image::../images/boot-cd-authent.png["Screenshot: bootimage / boot-cd: Authentifier en tant que membre du groupe opsiadmin",width=400]
...
image::../images/boot-cd-product.png["Screenshot: bootimage / boot-cd:: Liste des produits netboot",width=200]
...
While driver integration at first all drivers which are assigned via the 'additional' property will be integrated and analyzed for which PCI device (or HD-Audio, USB) there will be used. Pour ces dispositifs, qui ont des pilotes assigné comme 'additional', aucune autre pilotes `drivers/prefered/` ou `drivers/` seront intégrés. En fonction de ce comportement 'Additional' a non seulement la fonction d'ajouter des pilotes qui ne sont pas détectés mais il a également la fonction de remplacer d'autres pilotes (donc 'Additional' vous pouvez également l'appeler 'super préféré').
...
La configuration standard peut être analysé et 'enregistré' pour faire les tâches d'installation directement par le programme 'opsi-winst'. Habituellement, ca veut dire créer le fichier d'installation du système de fichiers local et patcher le registre.
...
image::../images/winst-interactive.png["Screenshot: opsi-Winst démarré en mode interactif",400]
...
image::../images/jedit-with-winst-script.png["jEdit avec un script opsi",400]
...
Avant de commencer à intégrer un nouveau package, ce serait d'abord mieux jeter un oeil à internet si quelqu'un a déjà fait ce travail:
...
à la section 'Installer des interrupteurs spécifiques' plus de liens web vers des sites qui donnent des conseils sur la façon de détecter le fabricant du programme d'installation.
...
http://opsi-fr.opensides.be/opsi-doc/stable/pdf/fr/opsi-winst-reference-card/opsi-winst-reference-card.pdf
...
image::../images/newprod-localboot.png["Screenshot: Choisissez le type de produit: localboot",width=400]
...
image::../images/newprod-product-info.png["Screenshot: Entrée de l'information produit",width=400]
...
image::../images/newprod-product-new-dependency.png["Screenshot: Créer des dépendances au produit: No/Yes",width=400]
...
image::../images/newprod-new-property.png["Screenshot:  Une (autre) propriété de produit à créer?",width=400]
...
image::../images/newprod-property-type.png["Screenshot: Choisissez le type de données de la propriété",width=400]


Plus sur la description de l'interrupteur doit être spécifié, ce sera indiqué dans 'opsi-configeditor' comme un texte d'aide et aussi au déballage du paquet. Ensuite, vous pouvez définir l'ensemble des valeurs de l'interrupteur (séparés par des virgules). Si cela est laissé vide, aucune valeur n'est autorisé pour le commutateur.

.Description des propriétés du produit
image::../images/newprod-property-desc.png["Screenshot: Description des propriétés du produit",width=400]
...
image::../images/newprod-property-default.png["Screenshot: La valeur par défaut de la propriété du produit",width=400]
...
image::../images/newprod-property-boolean.png["Screenshot: Description de la propriété booléenne",width=400]
...
image::../images/newprod-maintainer.png["Screenshot: Entrée des données du mainteneur",width=400]
