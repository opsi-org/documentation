...
proxy::	Si l'accès a internet se fait via un proxy, rentrer ici les informations: ex. `http://myuser:mypass@<proxy-address>:8080`
...
Vous avez besoin d'un client (minimum 512 MB de RAM) qui est en mesure de démarrer par le réseau via PXE. Pour un premier essai, nous vous conseillons de télécharger une image vmware correspondant sur download.uib.de. L'avantage de vmware (matériel virtuel) est qu'il supporte les pilotes standards de Windows.
...
L'avantage des ce genre de manipulation des variables de chaîne est qu'il est possible d'utiliser ces variables dans les sections secondaires qui sont interprétés par d'autres programmes (ShellScript / Execwith).
...
Vous trouverez des scripts {opsi-winst} prêts à l'emploi, distribués par la communauté, à: link:https://wiki.opsi.org/[opsi wiki]

Vous trouverez un ensemble de liens vers des sites avec des collections de commutateur à: link:https://wiki.opsi.org/[opsi wiki: Software integration web-links]
...
Sur opsi.org, vous pouvez trouver un lien web link:https://wiki.opsi.org/[opsi wiki: Software integration web-links] +
...
Les répertoires contenant les fichiers avec l'attribut 'hidden' ou 'system' peuvent être supprimés en utilisant une section 'ShellScript':
