...
* Il ya un fichier de configuration dans /etc/opsi/opsi.conf, qui contrôle l'accès Samba 4.
...
La liste des clients qui sont affichés dans l'onglet Clients peut être limitée en utilisant le menu déroulant 'Sélection / Montrer seulement les clients sélectionnés'. 

Le regroupement de certains clients peut être fait avec 'enregistrer les clients sélectionnés en tant que groupe' ou utilisant le menu déroulant 'Sélection / enregistrer les clients sélectionnés en tant que groupe'.

Les clients peuvent être sélectionnés en fonction de leurs configurations, en utilisant des critères consultables dans le menu déroulant 'Sélection / Préciser la recherche'.

.'{opsi-configed}': dialogue de sélection
image::../images/configed_clientselection.png["{opsi-configed}: Définir la fenêtre de recherche des clients", width="73mm"]


À côté de chaque propriété du client on peut entrer les critères de recherche pour le matériel et les logiciels. La recherche ne fonctionne que pour les PC qui ont OPSI installé.  Il est également possible de fournir 
les opérateurs logiques "and" ou "or" pour chaque propriété qui sera recherché.  Il est également possible d'utiliser le caractère "*" pour désigner toutes les valeurs possibles.

On peut également utiliser les recherches enregistrées, qui peuvent être trouvées en sélectionnant dans le menu déroulant 'Sélection / Utilisez définition de recherche sauvegardée'.

.'{opsi-configed}': Recherches sauvegardées
image::../images/configed_clientselection2.png["{opsi-configed}: Recherches sauvegardées"]
...
* Les pilotes qui seront automatiquement attribués aux ordinateurs utilisant les champs <vendor> / <Modèle>
...
(le nom et la profondeur de la structure du répertoire n'est pas important).  Les pilotes qui se trouvent dans le répertoire `./drivers/drivers/preferred` seront intégrés dans l'installation de Windows, en supposant que opsi trouve une correspondance convenable pour le matériel basé sur le PCI IDs (i.e. USB or HD_Audo-ID) dans le fichier de description. +
...
==== Les pilotes qui seront automatiquement attribués à des ordinateurs à l'aide des champs <vendor> / <Modèle>
...
'opsi setup detector' fait partie du paquet 'opsi-adminutils'.  Les programme peut être téléchargé comme une application autonome à partir de:
