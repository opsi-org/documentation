...
image::../images/opsi_with_file_backend.png["Schema: opsi avec le backend de fichier", width=432]
...
image::../images/opsi_with_sql_ldap_backend.png["Schema: opsi avec le backend SQL / LDAP", width=430]
...
image::../images/opsibackend_configuration_layers.png["Schema: couches backend et contrôle d'accès", width=400]
...
image::../images/opsi-configed-sessioninfo.png["{opsi-configed}: Buton SessionInfo?", width="40"]
...
image::../images/configed_clientselection.png["{opsi-configed}:  mask: client selection", width="73mm"]


Avec la fonction 'Selection / Define Selection' (Définir la sélection) vous pouvez construire des groupes de clients selon certains critères (par exemple: tous les clients qui ont le produit 'firefox' avec le statut d'installation 'installed').
Il y a des critères sur l'hôte, les logiciels installés et le matériel. Dans les champs d'édition de texte, vous pouvez utiliser '*' comme joker. Les critères peuvent être connectés à l'aide de "and" et "or" logique.
La négation est également possible.

.'{opsi-configed}': recherches enregistrées
image::../images/configed_clientselection2.png["{opsi-configed}: recherches enregistrées"]

Si vous souhaitez réutiliser une recherche, vous pouvez l'enregistrer. Il y a deux façons d'exécuter des recherches enregistrées. Premièrement, on peut y accéder avec la fonction 'Selection / Saved Searches...'. Deuxièmement,
vous pouvez y accéder en ajoutant le paramètre "-qs" et le nom de la recherche à l'appel de l'application configed. Si vous omettez le nom de la recherche, vous obtiendrez une liste de recherches disponibles.
La seconde approche est particulièrement utile pour l'inclusion dans les scripts.
...
image::../images/opsi-configed-client-new.png["{opsi-configed}:  création d'un client", width="54mm"]
...
image::../images/opsi-configed-client-change-depot.png["{opsi-configed}: changer le dépôt d'un client", width="53mm"]
...
image::../images/opsi-configed-localbootproducts.png["{opsi-configed}: masque de configuration des produits", width=400]
...
image::../images/opsi-configed-editlist1.png["{opsi-configed}: Table de propriété avec info-bulle ", width="63mm"]
...
image::../images/opsi-configed-editlist2.png["{opsi-configed}:  éditeur de liste, liste de sélection", width="55mm"]
...
image::../images/opsi-configed-editlist3.png["{opsi-configed}: éditeur de liste, champ d'édition", width="55mm"]
...
image::../images/opsi-configed-netbootproducts.png["{opsi-configed}: masque pour lancer l'image de démarrage", width=400]
...
image::../images/opsi-configed-hardwareinformation.png["{opsi-configed}: Informations sur le matériel pour le client sélectionné", width=400]
...
image::../images/opsi-configed-softwareinventory.png["{opsi-configed}: Informations sur les logiciels pour le client sélectionné", width=400]
...
image::../images/opsi-configed-logfiles.png["{opsi-configed}: Affichage du journal système dans opsi-configed", width=400]
...
image::../images/opsi-configed-hostparameters.png["{opsi-configed}: Onglet Paramètres de l'hôte (Configuration du serveur et du client)", width=400]
...
image::../images/opsi-configed-depotconfiguration.png["{opsi-configed}: Onglet de configuration Dépôt", width=400]
...
image::../images/opsi-webinterface2.png["opsi config interface: L'accès au service web via un navigateur", width=400]
...
Comme exemple nous allons regarder l'objet 'product'. Un objet du type 'product' qui décrit le produit 'javavm' peut ressembler à ceci:
...
'<nom de l'objets>_<opération>'

Selon cette règle de nommage, es nouvelles méthodes sont facilement différenciés des anciens méthodes 'legacy' de opsi 3, que commencent par 'get', 'set' ou 'create'.
...
Par exemple, vous obtiendrez en appelant la méthode 'product_getObjects' avec 'attributes:["name"]' pour le produit 'javavm':
...
Ajoute des informations sur le matériel de l'ordinateur <id de l'hôte>. Le hachage de <info> est passé. Les informations existantes seront écrasées pour la correspondance des clés. Applicable uniquement pour les clés spéciales.
...
Assigne une clé de licence (supplémentaires) pour le produit <Id du produit>.
...
Supprime toutes les informations sur le matériel de l'ordinateur <id de l'hôte>.
...
Supprime une clé de licence pour le produit <Id du produit>.
...
Supprime une seule propriété ou toutes les propriétés du produit <Id du produit>.
...
Fournit les propriétés du produit du produit et du client spécifié.
...
Définir la 'pckey' pour un ordinateur.
...
Définissez les propriétés de produit pour le produit spécifié (et le client spécifié).
...
image::../images/opsiconfd-info-hour.png["opsiconfd info: Les valeurs de la dernière heure de opsiconfd", width="120mm"]

.opsiconfd info: les valeurs de la dernière journée de opsiconfd
image::../images/opsiconfd-info-day.png["opsiconfd info: les valeurs de la dernière journée de opsiconfd", width="120mm"]
