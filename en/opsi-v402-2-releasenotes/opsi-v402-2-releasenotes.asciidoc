...
* opsi 4.0.2 is works with the following Linux distributions:
...
The following instructions can be found in the 'opsi-getting-started' guide
...
Under 'Directory', a hierachichal tree can be created for client management.  In contrast to 'groups', ever client must be in exactly one point under 'directory'.
...
** 'Productname' is now an additional column in the tabs 'Product configuration' and 'Netboot products', and it is configurable.

** Pressing 'Localboot-Produkte Reset' in the context menu of the Client tab deletes all the information about installed products on this client boot.

** The values of 'Report'-Column can be reset in the tabs 'Product configuration' and 'Netboot products'. +
...
** The Hardware Inventory can be expanded using calling commands. See the descriptions below.
...
* There is a configurations file in /etc/opsi/opsi.conf
...
* opsi-winst: +
A variety of extensions, which are described below.

[[opsi-402-2-releasenotes-installation]]
== Installation Notes

The products that are published as part of this release are dependent upon each other.  You should not try to install a part without the other parts.
...
== Tool: The management inferface '{opsi-configed}'

[[opsi-manual-configuration-tools-opsi-configed-client-selection-selecting]]
=== Selection of Clients

The Client list can be one or more clients, which enables processing multiple clients together. 

The list of Clients that are displayed in the Clients tab can be limited using the 'Selection / Show only selected clients'. 

The grouping of selected Clients can be done with 'save the selected Clients as a group' or use the menu item 'Selection / save selected Clients as a group'.

The Clients can be selected, based on their configurations, using searchable criteria under 'Selection / Define search'.
...
Next to each Client Property one can enter the search criteria for hardware and software. The search only works for PCs that have OPSI installed.  It is also possible to 
provide logical "and" or "or" operators for each property that will be searched.  It is also possible to use the "*" character to designate all possible values.

One can also used saved searches by selecting 'Selection / Use saved search definition'.

.'{opsi-configed}': Gespeicherte Suchen
image::../images/configed_clientselection2.png["{opsi-configed}: Saved Searches"]

It is also possible to run a saved search from the command line when the opsi config editor is started.  By including the flag "-qs" and the name of the saved search, the configuration editor will start 
...
The chapter on driver integration in 'opsi-getting-started-manual' has been revised.  More specifically, the chapter describes the new features for 'Automatic assignment in Driver Inventory using the fields <vendor> / <model>'.
...
If a group of computers have drivers that are not part of the Windows default installation, it is best to put these computers into a pool and integrate their driver's during installation time.

Opsi supports the automatic integration of drivers into the installation, and therefore simplifies the deployment.  In this case, the drivers simply need to be place into the correct directory.  When the installation script is called it simply searches through these directories and creates a catalog. The boot image automatically uses this catalog to embed the correct drivers.  Opsi automatically includes standard drivers, USB drivers, HD audio drivers, and disk controller drivers (textmode drivers).

So that the driver will be immediately installed wih the Windows instalation, you must place the drivers on the Server in a specific format.  The drivers must be placed in the driver's directory, with the format '*.inf', where the name describes the driver for the Windows setup program. The drivers in the 'setup.exe' or '*.zip' are not used here. If you have a computer that already has the drivers installed, then you can extract the appropriate drivers using the progam 'double driver' (http://www.boozet.org/dd.htm).

There are offered several levels of drivers:  

* general driver packages

* Hardware drivers that are not explicitly listed

* Drivers that will be manually assigned to computers

* Drivers that will be automatically assined to the computers using the fields <vendor>/<model>.
...
----
This will unpack and store the driver in the directory `./drivers/drivers/`. +
A point of concern is that eventhough the drivers found here are used by opsi, they might not necessarily work with your hardware. +
For the drivers which are found in `./drivers/drivers/`, the driver will be matched to the coresponding hardware using the PCI IDs (i.e. USB- or HD_Audio-ID) in the description file, and then integrated into the Windows setup as needed.

[[opsi-getting-started-firststeps-osinstall-driverintegration-preferred]]
==== Necessary Drives that are Preferred

In the case that you have to support different or special hardware, and you can find the additional drivers from the manufactuers, then use the following procedure to include them in the installation. +
The additional drivers are placed in their own directory under (the naming and depth of the directory structure is not importatnt) +
`./drivers/drivers/preferred`. +
Drivers that are found in the directory `./drivers/drivers/preferred`, and that are a suitable match to the drive hardware based off of the PCI IDs (i.e. USB or HD_Audo-ID) in the description file will be integrated into the Windows setup. +
...
==== Manually Mapping Drivers to Computers

When installing additional drivers based on the PCAI or USB IDs, they should be installed under the directory `./drivers/drivers/additional` (where name and depth of the directory structure is not important).  You can map one or more drivers to a client using the Product-Property 'additional_drivers' and a list of driver directories under `./drivers/drivers/additional`.  The directories specified by 'additional_drivers' are search recursively until all drivers are found.  This method can be used to make a specific directory based on the client type (i.e. dell-optiplex-815).

When a driver is found under the drivers directory that is specified by 'additional_drivers' and the PCI identifier, then other drivers in `drivers/preferred` or `drivers/` will not be used.  Therefore the drivers under 'additional_drivers' add functionality that would not have been found with the normal drivers.  Also, the drivers that are manually bound to a cliend using 'additional_drivers' recieve priority over other drivers.

[[opsi-getting-started-firststeps-osinstall-driverintegration-byaudit]]
==== Automatically Mapping Drivers in the Fields <vendor>/<model> to a Computer.

The previously describted mechanisms that directly map drivers to devices is automated since the 4.0.2 Release 2 of opsi.  The directory `./drivers/drivers/additional/byAudit` will be searched, given the corresponding 'Vendor' field that was found in the Hardware Inventory. This 'Vendor' directory will be search for a 'Model' directory that corresponds to what is seen in Hardware Inventory.   If this directory is found, then it will be manually addigned to the product property 'additional_drivers'. +
The directory name 'byAudit' is case sensitive.  The directory names for 'Vendor' and 'Model' are not case sensititve ('Dell' and 'dELL' are treated the same way).
...
Since the top priority is binding drivers, which property 'additional_drivers' will be found from the Inventory data in `./drivers/drivers/additional/byAudit`.  Tests will be made during driver integration to determine which drivers are already installed on which hardware devices. When a driver is not found for a device, will the following method be used.

For devices that have drivers that were not listed in 'additional_drivers', opsi will search for, and integrated, an appropriate driver based off of the PCI ID (ie. USB-, HDAudio- ID).  
...
* The Windows Setup will read in the unattended file, under which sub-directory of `c:\drv\` to search for drivers.
...
Then call the script `./create_driver_links.py`.  The script searches through the directories under './drivers/drivers' and generates a list of links that enables hardware to know about drivers depending on (PCI-IDs, USB-IDs, HD-Audio-IDs).  The script will use the drivers in the preferred directories.   

The script `setup.py` examines the hardware of the installed computers and identifies the necessary drivers.  These will be copied to the disk and the file unattended.txt will be patched.  The script `create_driver_links.py` examines the NT5 products one at a time in the 'i386' tree and extracts the .inf files of the necessary drivers into 'windows_builtin'.  If you make a chage to the i386 directory tree (i.e. after installing a service pack), then delete that directory and run `create_driver_links.py` again.  With NT6 products the recognized drivers are found in WinPE at 'windows_builtin'. 
...
----
Background:
The classical installation with the user:  'pcpatch' with the primary group: 'pcpatch' does not function with Samba 4.  Samba 4 has fundamental restrictions on Active-Directory, such that groups with the same name as user (as is common in Unix/Linux) are no longer allowed. For this reason, a new configuration file has been introduced:  `/etc/opsi/opsi.conf`, which will control how the groups will access Samba-Access.  So for example, with UCS 3 will use this file to rename the group 'pcpatch' to  `opsifileadmins`.  That means that the user, show belonged to the group 'pcpatch' under UCS 3, must now belong to the group 'opsifileadmins'.  This special feature is only for UCS 3 and is not the same for other distributions (so long as they are using Samba 4, or have Samba 4 installed).

[[opsi-getting-started-softwintegration-tutorial-find-switches]]
== Finding Silent or Unattended Switches

In „Unattended" or „slient" mode, is provided by the original setup program provided that command line arguements are called for non-interactive mode. 

The problem is then to find the appropriate command line arguments for a „silent" installation.
...
Path were cold be installed the help prgram, libraries, and items needed for script execution. +
Since 4.11.3.2
...
Inclides the file <file name> as a log file, where the last <tail size> lines of the full log are written into this log file.  If you start another program that produces a log file, you could see that other program's log file in the opsi-winst script log using this command. + 
...
The name of the file to be patch is passed as a parameter.

There are optional modifiers:

* `/AllNTUserProfiles` +
If a patch section is called with this modifier then all directories under `%UserProfileDir%` will be patched, such that this patch is performed for all user profiles. +
When a 'Patches' which is called in a `[ProfileActions]` section will have 'Machine' mode the modifier `/AllNTUserProfiles` implicit. In logscript mode can be interpreted `%UserProfileDir%` as `%CurrentProfileDir%`. +
(Since Version 4.11.3.2)
