...
== Changes on new opsi-linux-bootimage

The '{opsi-linux-bootimage}' works in the actual version: 20120728-1 internal with `dhclient` instead of `pump` for the initial dhcp-request. 

The `pump` program was used since the new version, because `dhclient` don't processes the `next-server` flag. But this flag is used by opsi to define the opsiserver-address. It's not possible to configure the `pump` utility and it gives a "hardreset" on the networkcard to actualize the ip-address from the client. That causes many problems in different network-infrastructures. For this reason we decided to switch the application to `dhclient`.
...
== Changes on opsiconfd

The webservice of opsi will now build as "noarch". For some rpm-based distributions it was build for specifc architecture. But that isn't necessary, because there are no binaries in this package.

That could be produce some problems on some distribution when you try to update these package. If you get an error like this:  "`opsiconfd-4.0.2.3-1.1.i386.rpm` couldn't be found" you must reinstall the '{opsiconfd}'-package manually from repository.

The init-scripts was modified too. Now the runlevel 4 will not be used on rpm-bases distributions.

[[opsi-release-notes-402-opsi-pam-authentification]]
== opsi PAM authentification

opsi uses some `PAM`-modules to authenticate the user. Since tody opsi uses different modules for some distributions. The following list will give you a small overview about the used modules:
...
You can see in the above list, that some different `PAM`-modules are used, but depending on the local configuration it could be require another `PAM`-usage than the opsi does. These modifications must be changed in the sourcecode. To give more flexibility without changes in the sourcecode, it's possible since these release to create a file named: `opsi-auth` in the directory `/etc/pam.d/`. If this file exists, opsi will use this configuration automatically instead of modules listed above.

The following tiny example will show you the new feature: If you run a debian/ubuntu-System an you get a `PAM`-authentification-error even though you can connect with the same credentials over ssh on the server, you can create the file: `/etc/pam.d/opsi-auth` with following content:
...
After restart from opsiconfd opsi will use automatically the `sshd`-`PAM`-module for authentication of users.
