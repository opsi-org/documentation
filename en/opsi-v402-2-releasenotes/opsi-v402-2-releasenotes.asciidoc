...
** Pressing 'Localboot-Products Reset' in the context menu of the Client tab deletes all the information about installed products on this client.
...
* There is a configuration file in /etc/opsi/opsi.conf
...
The list of clients that are displayed in the clients tab can be limited using the drop-down menu 'Selection / Show only selected clients'. 

The grouping of selected clients can be done with 'save the selected Clients as a group' or use the drop-down menu item 'Selection / save selected Clients as a group'.

The clients can be selected, based on their configurations, using searchable criteria under the drop-down menu 'Selection / Define selection'.
...
Next to each Client Property one can enter the search criteria for hardware and software. The search only works for PCs that have OPSI installed on them.  It is also possible to 
...
One can also use saved searches, that can be found by selecting from the drop-down menu 'Selection / Use saved search definition'.
...
In the case that you have to support special hardware, and you can find the additional drivers from the manufactuers, then use the following procedure to include them in the installation. +
Place the additional drivers in their own directory under: +
`./drivers/drivers/preferred`. +
(the naming and depth of the directory structure is not importatnt).  Drivers that are found in the directory `./drivers/drivers/preferred` will be integrated into the Windows setup, assuming that opsi finds a suitable match to the drive hardware based off of the PCI IDs (i.e. USB or HD_Audo-ID) in the description file. +
...
When a driver is found under the drivers directory that is specified by 'additional_drivers' and also matches the PCI identifier, then other drivers in `drivers/preferred` or `drivers/` will not be used.  Therefore the drivers under 'additional_drivers' add functionality that would not have been found with the normal drivers.  Also, the drivers that are manually bound to a cliend using 'additional_drivers' recieve priority over other drivers ('additional_drivers' can be thought of as 'super-preferred').

[[opsi-getting-started-firststeps-osinstall-driverintegration-byaudit]]
==== Drivers that will be automatically assined to the computers using the fields <vendor>/<model>

The previously describted mechanisms that directly map drivers to devices is automated since the 4.0.2 Release 2 of opsi.  Opsi will search the directory `./drivers/drivers/additional/byAudit` for a director name that matches the field 'Vendor' that was given in the Hardware Inventory. This 'Vendor' directory will be search for a 'Model' directory that corresponds to what is seen in Hardware Inventory.   If this directory is found, then it will be manually assigned to the product property 'additional_drivers'. +
