...
Grundsätzlich gibt es die Möglichkeit die Installation des Agenten <<opsi-macclient-firststeps-software-deployment-client-integration-service-setup,auf dem Client auszuführen>> oder <<opsi-macclient-firststeps-software-deployment-client-integration-opsi-deploy,vom Server aus>> die Installation anzustoßen.

Das Ausführen der Installation direkt auf dem Client eignet sich für einzelne Rechner.
Für einen Massen-Rollout des Agenten eignet sich grundsätzlich der <<opsi-macclient-firststeps-software-deployment-client-integration-opsi-deploy,opsi-deploy-client-agent>>
...
Das Heimatverzeichnis dieses Users ist `/var/lib/opsisetupadmin`
