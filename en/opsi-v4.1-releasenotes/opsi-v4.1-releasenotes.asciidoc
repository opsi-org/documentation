...
* New opsi extension opsi-vhd-reset which is desingned for schools or other kinds of learning computers.
** This extension is for opsi 4.0 and opsi 4.1 as well.
** Like the well known opsi-local-image extension 'opsi-vhd-reset' also targets on the problem of fast recovery of computers between two training sessions.
...
** This extension is a non free extension.
** It is bundled with 'opsi-local-image' which means that in order to use it you need a activation for 'opsi-local-image'.
...
The <<opsi-releasenotes-4.1-changes-opsi-package-updater-configfile,migration of the configuration>> is described later in this document.
...
IMPORTANT: If you changed the configuration file of `opsi-product-updater`, i.e. when using additional repositories (Update subscriptions) or because you are synchronising your products from another opsi server, please proceed as described in chapter <<opsi-releasenotes-4.1-changes-opsi-package-updater-configfile, Migration with customized configuration>> before updating the packages.

[[opsi-4.1-releasenotes-installation-migration-opsi-packages-standard]]
===== Use of the standard configuration

If you have not made any changes compared to the standard delivery of `opsi-product-updater. conf`, you will recognize this after the integration on 4.1 because your `/etc/opsi/opsi-product-updater. conf` is no longer available and is completely removed by the repositories files under `/etc/opsi/package-updater. repos.d/` were replaced.
...
* On _UCS_ the file `/etc/pam.d/opsi-auth` is not generated from the existing templates. As a workaround create this file manually.
...
It now uses the following format:
...
There are a lot of changes at the Windows NT6 netboot products from 4.0.7.1 to 4.1.0.0.

=== Preconditions

All netboot products with the version  >= '4.1.0.0' requiring that a opsi-winst >= '4.12.0.13' are installed on the opsi server. +
The netboot products with the version   '4.1.0.0' are also running on opsi 4.0.7.

=== Multidiskmode

Windows OS installations on systems with more than one harddisk are now allowed by using the new multidiskmode property.
The multidiskmode property allows to select the target disk for the Windows installation by giving the disk number. It is also possible to select the first SSD by `prefer_ssd` or to select the first rotational ('classic') disk by `prefer_rotational`

IMPORTANT: Using the multidiskmode on a computer with *MBR BIOS* you have to make sure, that the by multidiskmode selected disk is also the first disk in the BIOS boot sequence. +
On *UEFI BIOS* systems you must do nothing, because here the boot sequence ist controled by the installation software.
...
The values "0","1","2","3" are the index of the harddisks ("0"= 1. harddisk) +
...
* `blockalignment` +
This property was used to decide if the partions have to be aligned to the disk block limits or not. Now this block alignment is allways on.


=== Actions while running inside the Windows PE
The preparation of a Windows installation start with the opsi-linux bootimage, witch select and prepares thes harddisk. It also copies a Windows PE to a partition of the harddisk. This Windows PE is booted in order to start the Windows setup. +
Starting with the 4.1.0.0 netboot products we use opsi-script inside the Windows PE. This has the following advantages:

* Easier and clearer scripts

* The creation of a log file of the actions inside the PE

* Sending this log file to the opsi server
