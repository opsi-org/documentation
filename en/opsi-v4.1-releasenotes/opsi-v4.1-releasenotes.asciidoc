...
*Key aspects of this release are:*
...
* Python 2.7 as minimum
* systemd as the sole supported init system

[[opsi-4.1-releasenotes-important]]
== Important Advices - Please pay attention

opsi 4.1 is an independet release that has it's own package repositories.
These must be inserted into the system you want to update before installation.

For an update from opsi 4.0 the packages need to be the latest version from the stable repositories.
...
=== Advice for Updating the Operatingsystem-Packages

Please make sure that at the time of the update you are using the latest packages from opsi 4.0 from the stable branch.

Because this updates brings changes to the data structure in the backend
it is required to first migrate the backends after the update.

[source,prompt]
----
# If using the MySQL backend for data storage
opsi-setup --update-mysql
# If using the file backend for data storage
...
We also plan on dropping support for those Windows version from the update subscriptions.
If you have any questions or problems regarding this please get in contact.
...
The tasks of the tools are the same but the commandline parameters changed and new options are now available.
...
The previously known commandline parameters can still be used.

`opsi-makepackage` will create files containing checksums automatically when creating a package.
The switches `--no-md5` and `--no-zsync` respectively allow to disable this behaviour.


[[opsi-releasenotes-4.1-changes-workbench]]
== New workbench path

The workbench directory will be placed at `/var/lib/opsi/workbench` by default during installation.
This brings an consistent directory structure across different systems and eases the backup of whole opsi installations.
...
* `opsi-makepackage` will automatically create .md5 and .zsync-files during the creation of a new package. This can be suppressed throught `--no-md5` and `--no-zsync` respectively.
