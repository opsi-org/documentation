...
With this service release the official support for Python 2.5 is discontinued. This regards the Python 2.5 distributions: Debian Lenny and UCS 2.x. The affected repositories are closed by the end of 2013. At that time the current packets will be moved to the archive of download.uib.de. For affected opsi systems we recommend upgrading to new distributuion versions that still are supported.
...
The regarded repositories are closed by the end of 2013. For corresponding installations, we urgently recommend upgrading the OS. Before doing so, please check that the selected version is supported by opsi.
