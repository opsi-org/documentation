...
The MySQL-Server which is used for inventory- and license management can be installed as follows:
...
After the described steps the opsi-Installation on a UCS-Server with the help of the Univention App-Center is finished. Please continue with following chapter:
...
----
+
returns a full qualified domain name containing two dots, e.g. 'opsidemo.domain.local'
...
----
+
to make sure it returns the IP-adress of the networkinterface the clients should connect to. If the command send the result '127.0.0.1' or '127.0.0.2' then file `/etc/hosts` has to be corrected.
...
You can use zypper to add the opsi-SLES-Repository:
...
----
+
returns the IP-adress of the network interface that the clients should connect to. If the command returns the results '127.0.0.1' or '127.0.0.2' then the file `/etc/hosts` has to be corrected.
+
...
If you want 'root' to use the opsi administration commands, then 'root' has to be a member of the group '{opsiadmin}'.
...
Opsi offers a user-friendly configuration management interface with the {opsi-configed} application.
...
A java version of at least 1.8 is required on the computer running the browser.
...
* The {opsi-configed} can be installed on your local, opsi-managed, machine using the provided opsi-product 'opsi-configed'. +
The required Java RE is already integrated. 

* A link to the executable for the installation of the 'opsi-configed' can be found following this link: +
`https://<opsidepotserver>:4447` +
The required Java RE is already integrated. 
...
A statically linked binary of version 1.0 is part of {opsi-client-agent}.

Winexe may be provided through the repositories of the used distribution.
For some distributions you may find a newer version at http://download.opensuse.org/repositories/home:/uibmz:/winexe/.
...
Now reboot the client and the product `opsi-configed` should automatically be installed. After the installation you should find the entry `opsi-configed` in the `Start Menue`
...
The following chapter describes how Windows can be deployed on a computer without an operating system with opsi.

You need a physical or virtual computer with a minimum of 1536MB RAM that is able to boot per PXE over the network.
Booting over the network perhaps has to be activated and/or moved to be the first boot entry in BIOS.

The advantage of virtual hardware is that it usually supports the standard drivers from windows.
For an initial test you can download a link:http://download.uib.de/vmware_pxeclient.zip[corresponding vmware-image] that acts as an _empty_ client and can be used through _VMWare Workstation Player_.

NOTE: Some utilities useful for deploying Windows with opsi are installed by the package 'opsi-windows-support'.
...
Once the client exists you have to add it in opsi.
This can be done using either the graphical user interface (GUI) or the command line.
...
You can find ISO images of the opsi-client-bootcd in https://download.uib.de/opsi4.1/boot-cd/. Just download the newest image and burn it to a cd-rom or dvd-rom. Boot your computer from this medium. You should see the following image:
...
Therefore you will get a login window, where you should authenticate yourself as a member of the {opsiadmin} group. If the authorization is successful, then the client transmits its data to the server, at which point the client will be created at the server. In the next step, the server provides a list of Netboot products to the client.
...
Regardless of how you want to create your PE, you'll need an installed "Assessment and Deployment Kit" (ADK, Win8.1 bzw 10), or its predecessor "Windows Automated Installation Kit" (Windows AIK; bis Windows 7): +
...
* link: http://www.microsoft.com/downloads/details.aspx?displaylang=en&FamilyID=696dd665-9f76-4177-a811-39c26d3b3b34[WAIK Windows 7]
...
===== Tweaking a WinPE (ADK / Windows 8 or 10)

Navigate to Start > Windows Kits > Windows ADK > Deployment and Imaging Toolkits Environment from the Start Menu. You will see a command prompt, but with some environment variables set.
...
When installing additional drivers not based on the PCI-IDs or USB-IDs, they should be installed under the directory `./drivers/drivers/additional` (where name and depth of the directory structure is not important).  You can map one or more drivers to a client using the Product-Property 'additional_drivers' by supplying one or multiple paths of driver directories under `./drivers/drivers/additional` as value.  The directories specified by 'additional_drivers' are searched recursively and all drivers which are foundwill be installed. The usage of symbolic links is also permitted, this can be used to make a specific directory based on the client type (i.e. dell-optiplex-815).

When a driver is found in one of the driver directories that is specified by 'additional_drivers' and matches the PCI identifier, then other drivers in `drivers/preferred` or `drivers/` will not be used ('additional_drivers' can be thought of as 'super-preferred').  Therefore the drivers under 'additional_drivers' have the functionality that they are installed even when the corresponding devices are not found.
