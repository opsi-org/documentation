...
The installation and commissioning of a {opsi-Server} is done in four steps:

. Basic installation of the server
. Server customization: network configuration, password assignment, server update
. Download and installation of the necessary opsi products for the clients
. Complete the operating system base packages for Windows from the original media
...
Please note as well the <<opsi-getting-started-supportmatrix,supported Operating Systems>>!
...
In other words: the full qualified domain name must contain at least one point.  Furthermore, the toplevel domain must consist of at least two characters.
...
IMPORTANT: Please check the <<opsi-getting-started-requirements,pre-requirements>>.
...
Clients will also be notified to the {opsi-Server} if <<opsi-getting-started-firststeps-software-deployment-client-integration,opsi-client-agent>> is installed on existing computers.
...
The network boot has to be activated in the Bios menu or moved to the first place of the boot options.
...
For an initial test, you can use a link:http://download.uib.de/vmware_pxeclient.zip[VMware Appliance] that maps an _empty_ machine and can run in _VMware Workstation Player_.
