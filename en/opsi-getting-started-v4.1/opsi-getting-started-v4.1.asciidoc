...
opsi requires set language settings ('locale') on the server. +
It is recommended to use an UTF-8 compatible locale. +
+
The following command performs an simplified check: +
[source,prompt]
----
...
If the output is _check locales:_ you should check if the followding list +
has settings for _LANG=_ or _LC_ALL_ that are according to your used language. +
+
...
In you want 'root' to use the opsi administration commands, then 'root' has to be a member of the group '{opsi-admin-group}'.
