...
The computer where the program is installed currently requires at least Java Runtime Enviromnent (JRE) version 8 (internal version 1.8).
...
You need a physical or virtual computer with a minimum of 2048MB RAM that is able to boot per PXE over the network.
