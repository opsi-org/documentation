...
The first opsi server in an environment will have it's backend configured to make use of the installed MySQL server.
...
With the App-Center opsi can only be installed as server roles 'master', 'backup' and 'slave'.
...
This is done via the join script. If your config server is run as the role 'master' or 'backup' the join script will be executed automatic. +
 +
