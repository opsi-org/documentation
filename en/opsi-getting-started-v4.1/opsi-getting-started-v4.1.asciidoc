...
By performing a double click the Icon 'opsi-package-updater FirstRun' the minimal opsi Products will be installed. To do this please introduce the current password for the adminuser.
...
----
apt update
apt install openjdk-7-jre icedtea-7-plugin
----
...
If the installation is made through the App Center the change to a newer UCS version (i.e. from UCS 4.0 to UCS 4.1) will be blocked until all installed apps are supported on the new version of the operating system.
...
Additional packages will be installed: 'opsi-tftpd-hpa', 'opsi-windows-support', 'mysql-server'.
...
Some utilities useful for deploying Windows with opsi are installed through the package 'opsi-windows-support'.
...
* (in case the opsi-product `opsi-winpe` is missing, install it onto your opsi server via CLI `opsi-package-updater -vv install opsi-winpe` )
