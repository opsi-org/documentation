...
Additional packages will be installed: 'opsi-tftpd-hpa', 'opsi-windows-support', 'univention-mysql'.
...
Make sure the conditions in <<opsi-getting-started-introduction-software-and-configuration-preconditions>> are fulfilled.
...
* `univention-mysql` has to be installed.
...
The packages can be found at http://download.uib.de/opsi4.0/products/ in the directories `netboot/`, `localboot/` and for Linux-clients also in `opsi-linux/`.
