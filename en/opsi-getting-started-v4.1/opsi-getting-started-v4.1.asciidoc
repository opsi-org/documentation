...
The procedure to perform different types of base installations are described in the chapter <<opsi-getting-started-installation>> of this manual.
The types of installations included offer the possibility to either use an existing virtual machine or to perform a direct installation of opsi to
the host machine.
...
Check needed network ports at: <<opsi-getting-started-installation-config-ports>>
...
<<opsi-getting-started-supportmatrix>> +
Please note the Configuration Requirements:  <<opsi-getting-started-introduction-software-and-configuration-preconditions>>.


In this chapter, we assume you are familiar with the debian-package system (you will find information about the debian-package in the appropriate Debian books, in the manual pages, or under https://www.debian.org/doc/).
...
If an existing config server is detected 'opsi-package-updater' will be configured to retrieve packages from this server.
...
This is done via the join script. If your config server is run on role 'master' or 'backup' the join script will be executed automatic. +
 +
The main reason for this drastic measure is that the manual rename of this group is not trivial, because it comes to be a primary group.
...
A solution for this is in <<opsi-getting-started-installation-base-ucs-listener>>.
...
In an opsi4ucs installation Windows-Clients have to be created in the UDM first and in a second step they have to be created in opsi.
...
* MariaDB-server hast to be installed.
...
[source,prompt]
----
   Importing GPG key 0xD8361F81 "home:uibmz OBS Project <home:uibmz@build.opensuse.org>" from http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40/CentOS_CentOS-5/repodata/repomd.xml.key
   Is this ok [y/N]: y
----
Please answer with 'y'.
...
The usage of the {file-backend} for inventory data is possible but noticeable slower and therefore not recommended.

NOTE: The use of the {mysql-Backend} for inventory data is free and does not required activation. +
More information about the activation of co-financed modules can be found <<opsi-getting-started-installation-config-modules, here>>.
...
This mode prevents the command _opsi-setup_ _--configure-mysql_ to finish properly, with the correct configurations. +
...
The computer where the program is installed currently requires a Java Runtime Enviromnent (JRE), at least JRE version 8 (internal version 1.8).

If you would like to run the opsi management interface '{opsi-configed}' directly on the server you need to install a graphical environment and the Java Runtime Environment on it.
If this is not needed or possible on the server this chapter can be skipped.

A graphical environment and Java Runtime Environment are already installed in the virtual machine provided by uib.

To check the Java version enter the following command in a terminal window:
...
TIP: Mind the resolution of the display of your virtual machine.
Usage of {opsi-configed} is clumsy with a resolution of less than 1024x768.
To improve graphics and mouse drivers in a virtual machine it is helpful to install 'VMware tools' for a VMware machine or the 'virtual-guest-additions' for a VirtualBox machine.
...
It is essential for opsi that the DHCP can be used for address lookups.
...
If you create a client at the {opsi-server} using the {opsi-configed}, it will also create a dhcp entry for this client.
...
Next you have to configure your external DHCP server, and provide the client information to the external DHCP server, such that clients know that our {opsi-server} is now the boot server.
...
Replace '<ip of {opsi-server}>' with the IP-number of your {opsi-server}.
...
They contain amongst other things the agent that is required for managing clients.

It is possible to install the packages in automated or manual fashion.
The automated way is recommended.

For this there is the tool `opsi-package-updater`.
It will automatically retrieve current packages and then install them on the server.
The configuration is made in the file `/etc/opsi/opsi-package-updater.conf`.
...
NOTE: Please notice that the OS-Installation products like win10 aren't ready for action after installation. The installation has to be supplemented by the installation files from the corresponding installation media (i.e. CD, see <<opsi-getting-started-firststeps-osinstall-fill-base-packages>>).
...
The packages can be found at http://download.uib.de/opsi4.1/stable/packages/windows in the directories `netboot/`, `localboot/` and for Linux-clients also in `https://download.uib.de/opsi4.1/stable/packages/linux`.

We recommend storing the .opsi-files at `/var/lib/opsi/repository`.
To make sure the access rights allow opsiconfd to access these files run `opsi-setup --set-rights /var/lib/opsi/repository`.

After the download you must to manually install the packages on your server with the command `opsi-package-manager -i <paketname>.opsi`.
...
The management interface hast to be installed as a local application on the administrative PCs.
Visit `https://<opsi-server>:4447/` in a browser.
There you will find links to installers for various operating systems.

Alternatively you will find these installers at link:https://download.uib.de/opsi4.1/misc/helper/[].

IMPORTANT: The installer for Windows needs to be explicitely started with elevated rights.
To do so open the context menu through a right click on the installer and then choose 'Run As Administator'.


If a PC has the management interface installed further PCs can have the interface deployed to by using the localboot product *opsi-configed*.

When {opsi-configed} is started a login window will appear.
Log in with an user who is member of the group {opsi-admin-group}.

The {opsi-configed} interface is mostly self-explanatory.
A detailed description can be found in the opsi manual.


NOTE: Any changes have to be saved in order to show any effect.
Saving changes can be done with the check-mark button.
To see changes that may have taken place, you have to reload the data, which can be done with the button at the top left hand corner that has the reload arrows.
Reloading can also be done with a right mouse click, and selecting reload.
...
Server to client. These ports can be configured.
...
===== service_setup.cmd on Windows NT6
...
The following chapter describes how an computer without an operating system will have Windows deployed on it through opsi.

You need a physical or virtual computer with a minimum of 1024MB RAM that is able to boot per PXE over the network.
Booting over the network may has to be activated and/or moved to be the first boot entry.

The advantage of virtual hardware is that it usually supports the standard drivers from windows.
For an initial test you can download a link:http://download.uib.de/vmware_pxeclient.zip[corresponding vmware-image] that acts as an _empty_ client and can be used through _VMWare Player_.

NOTE: Some utilities useful for deploying Windows with opsi are installed through the package 'opsi-windows-support'.
...
Once the clients exists you have to add it in opsi.
This can be done through a GUI or by using the command line.


.Through the graphical frontend:
...
If the client is configured as a PXE-client, then it will also be configured in the DHCP on the {opsi-server}.
...
the description `Testclient`, no notes, the MAC address of `00:0c:29:12:34:56` and the IP address `192.168.0.5`.
...
* Set the localboot-product `opsi-winpe` to `once` for the client you intend to use, if desired adjust the produkt properties to `x86` instead of `x64` at the lower right side, and save (right click > save)
...
Place your driver directories in `/var/lib/opsi/depot/<product-id>/drivers/drivers`.
