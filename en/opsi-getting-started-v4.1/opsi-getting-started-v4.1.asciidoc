...
This preparation includes the adding of required users and groups and adding shares.
...
* Required packages for the opsi-Installation will be installed: opsi4ucs, opsi-tftpd-hpa, p7zip-full, cabextract, mysql-server.

* 'opsi-package-updater' will be called to download and install the minimal opsi-Packages for the first work with the new installation. +
If an existing config server is detected 'opsi-package-updater' will be configured to do so from the config server.
