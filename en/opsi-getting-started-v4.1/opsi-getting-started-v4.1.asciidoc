...
Subsequently the management interface {opsi-configed} is used to distribute software to clients.
...
If a PC is equipped with the management interface, further PCs can be equipped with the interface later simply via the local boot product *opsi-configed*.
