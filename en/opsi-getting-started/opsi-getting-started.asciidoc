...
The {opsi-server}s and the connected clients are administrated
using the program {opsi-configed}. It communicates with the opsi server
via HTTPS. Therefore it can used on every computer that can build a HTTPS
connection the opsi server.

The program currently requires a Java Runtime Enviromnent (JRE) on the computer on which
it is run, at least JRE version 7 (internal version 1.7).

If the {opsi-server} offers a GUI the {opsi-configed} can be used directly on it;
this is prepared in the opsi virtual machine. Obviously, then Java must be installed on the server in the required version.
Otherwise Java is not needed on the server, and this chapter can be skipped.
...
In order to perform an OS Installation, a so-called WinPE is needed. It's easy to create one yourself. Follow the manual, because a tiny bit of tweaking is necessary.
...
First you need the Windows Automated Installation Kit (Windows AIK): +
...
For creating a Win10 - based PE, use 'ADK for Win10', as described in https://forum.opsi.org/viewtopic.php?&p=33997
...
Start a terminal as administrator (Start => Programs => Accessories => right click on â€žCommand Promptâ€œ => Run asâ€¦ => Administrator) and run the following command:
[source,prompt]
----
...
or use the more recent dism - command regardless
[source,prompt]
----
...
* enter the next command (again in one line; unmounts the wim again):
[source,prompt]
----
...
----
(watch out; the directory 'ISO' might have been renamed in newer AIKs to 'media'.)
