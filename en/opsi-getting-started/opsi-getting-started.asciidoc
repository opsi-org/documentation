...
The procedures for installing different types of base installations are described in Chapter <<opsi-getting-started-installation>> of this manual.
...
The performance requirements of the server are moderate in testing environments. For service in production environments increasing the performance of the server could be needed.
...
* Start the VMware player. Open "Open a Virtual Maschine" and search for the file `opsidemo.ovf` in the 'opsidemo' directory. You can import the server with an new name. The virtual machine will still boot.

*ESXi-Server*

* Download the opsi-ServerVM from http://uib.de/de/opsi/opsi-testen-download/
* Unzip the file and a directory 'opsidemo' will be generated.
* Start the vShere Client. +
Install a new client with 'File' / 'OVF-Vorlage bereitstellen....' and answer the following questions.
...
At the menue 'File' / 'Import Appliance' select your `opsidemo.ovf` file and import it.
...
domain::		DNS-Domain (not Windows-Domain) â€“ the name has to include a point e.g. +opsi.local+
...
The graphical user interface of the {opsi-server} should already be started (implemented as a sustainable window manager). A â€žFirefoxâ€œ browser window might appear at startup, and display further instructions and information.  This information can serve as a reference to the getting started document (the document you are currently reading).
...
The commands are input into a window called a â€œterminal windowâ€œ. Here are examples that explain how to access a terminal window:
...
* Open a terminal window in the {opsi-server} graphical interface with a right mouse click inside the interface, and choose  â€œTerminalâ€œ. +
...
Updating your opsi-Server to doubleclick the Icon "Update Manager" on the desktop.

Install the minimal opsi Products to doubelclick the Icon 'opsi-product-update FirstRun'. Please take the password for adminuser.
Through this the actual stock of opsi-products, incl. templates for Windows- and Linux-OS, will be downloaded from http://download.uib.de/opsi4.0/products/ and install them on the server. For more information you can see here <<opsi-getting-started-installation-config-get-essential-products>>.

[[opsi-getting-started-installation-base-vm-start-configed]]
===== Starting opsi-Server Interface
Description the Server Interface look <<opsi-getting-started-installation-config-configed>>.
...
For the following installation commands the command aptitude should be installed:
...
In the Univention App-Center are a automatic installation of opsi-Server available.
The installation-app for opsi can be found in the UCS-Management-Webinterface in the catogorie "System". Over the App-Center can opsi be installed on UCS4-Serverroles Master and Backup. If you want to update a existing opsi4ucs-Installation please check the next Chapter for further information. +
The following will describe what will be happen during a installation:

* The prerequires for a MySQL-Installation will be done.

* The Ignore-List of the Samba4-Connector will be modified: the user pcpatch will be removed if found as an entry in the ignorelist.
...
===== Not automated opsi-installation on a UCS 3 and UCS 4 System (without App-Center)
...
----
+
have to afford the IP-adress of the network interface which the client has to connect with. If the command shows the adress '127.0.0.1' or '127.0.0.2', you have to correct your name resolution in `/etc/hosts` file.

* Samba have to be configured.

* Mysql-server have to be installed.

* If the machine should also work as DHCP-server, then the daemon dhcpd has to be configured and should running.

The installation of opsi with the file- or MySQL-backend is possible on a the roll Master, Backup and Slave - not as Member-Server.
...
been introduced: `/etc/opsi/opsi.conf`, which control how the groups for the Samba-Access are realized. More specifical for UCS 3 the group name 'pcpatch' will be called 'opsifileadmins'. This means that users which need to clearance for opsi (opsi package builder) under UCS 3 can't be member of the group 'pcpatch', must now belong to the group 'opsifileadmins'.
...
'opsifileadmins' group will no longer be fix, but will be self-assigned in UDM which will also eliminate replication problems with the user and group). For more information about this new configuration file please refer the opsi-manual.

TIP: If you use UCS 4.0 or higher and you have no existing opsi installation, you can skip the following steps and continue with adding the repositories.
...
When any changes have been made to the server, then slapd at the server have to be restarted (Note: Starting slapd restarts LDAP on a productive system so the next commands only execute planned):
...
To use the opsi configurations editor the user have to be a member of the group {opsiadmin}. The group membership can be edited by using Univention-Admin. The user administrator will automatically be added to this group during the opsi installation.
...
Samba4 is a central service so restarting samba4 is not done automatical by the package but must be done manual. Please note that after restarting samba there may be a delay in access to any new releases.
...
* The configurations does not realized automatic during an opsi installation on an active UCS-Infrastruktur cause the configurations already exists.
* The opsi-atftpd is not configurate as ever using the directory `/tftpboot` as base directory but use `/var/lib/univention-client-boot`. All important files of {opsi-linux-bootimage} will be linked from '/tftpboot' in the base directory. The side effect is the DHCP-Option file name named `pxelinux.0` instead `linux/pxelinux.0`.

You have to set guide lines to realized the mentioned configurations at the UCS-System. These guide lines are depend on existent guide lines and has to realize appropriate. If opsi was installed on an UCS-test system without existing guide lines you need an installed DHCP-service at first. If the DHCP-service is installed already the easiest way to create guide lines in the UMC-web interface (Univention Management Console) is from UCS-server. Therefor choose the categorie "Dom�ne" and subjacent the modul DHCP-server. As next you have to choose the service (in a testing system you find only one entry usally). In the following detail views choose guide lines in the menue. The needed guide line is a DHCP-Boot guide line. During the guide line configuration dial as default entry `cn=default-settings` (sould be the only entry) and choose 'edit'. Under the normal settings DHCP-boot for the option Bootserver enter the IP from {opsi-server} and insert as boot-file name `pxelinux.0`.

WARNING: If the guide lines configured like outlined above this configurations be effected for every device which be operatetde with DHCP form this server. So one more time the hint that the description is only for evaluate persons which will testing in the course of opsi also UCS. In a productive UCS environment you should never configure the guide lines during the installation in this way.

Optional you can do the attitude with udm-commands at the console. More informations you will find in the UCS-documentation.
...
----
+
have to send the IP-adress of the network gateway the client connected with. If the command send the result '127.0.0.1' or '127.0.0.2' then file `/etc/hosts` have to be correctetd.


* Samba have to be installed and configured.
...
----
zypper refresh
  Wollen Sie den Schl�ssel (a)bweisen, ihm (t)empor�r oder (i)mmer vertrauen? [a/t/i/?] (a): i
...
Assuming all of the above steps completed successfull we can assume that the network is properly configured.
...
It may occur a question like the following that asks if you want to import the GPG key of the repository:
...
Please make sure that your iptables and SELinux configuration allows access to the tftp Port (69/UDP) and the opsi ports (4447/TCP and 4441/TCP).
...
Different kinds of data may be stored in different types of backends. For some actions (such as method calls) more than one backend has to be involved. Therefore, the different method calls can use more than one backend. These method-to-backend(s) calls are configured is in the file
...
Now at the initial start-up (even whitout changing the file) you should call:
...
A 'pcpatch' user is created on the system. This user is allowed to install software on a client PC. The 'pcpatch' user allows access to configuration data on the host shares. The user 'pcpatch' has to get a correct password - at once as system user, samba user and opsi user.
...
If the {opsi-server} offers a GUI the {opsi-configed} can be used directly on it; this is prepared in the opsi virtual machine.
...
to install VMware tools for VMware machine resp. the host extensions for a
VirtualBox machine.
...
CAUTION: Do not use the char 'Â§' as part of the passwords. It becomes impossible to login at the opsi web service.
...
* Enter the following address in a browser (anywhere in the net) https://<opsi-server>:4447/configed.jnlp, and then a web interface will appear with an embedded {opsi-configed}. A java version of at least 1.7 is required.

* Another option would be to right-mouse-click on the server desktop background, then open the context menu, and then choose opsi config editor. In this case a java runtime environment must be installed on the server.
...
. The script connects to the opsi-webservice in order to create the client on the server side and get the pckey. The first connection use the user/password combination registered in the config.ini. If the connection fails, a login window pops up, where the user can fill in a Service-URL ({opsi-configserver}), user and password. The provided user has to be a member of the group 'opsiadmin'.
...
Depending on the used distribution you can find a newer version at http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40-testing/.
For further information please look at the project page at +
...
Perhaps you have to make the script executable: +
...
The base package includes only the files that are used for automatic system software installation â€“ but not the system software itself.
...
==== NT 6 family: Vista / 2008 / Win7 / Win8 / Win10

In order to perform an OS Installation, a so-called WinPE is beeing used as a 'Live OS'. You can create it using an opsi package (ADK 8.1,10), or do it all yourself manually following the steps described.
Generally speaking, the Windows- Version of the PE does not matter with regard to the Windows OS version being installed. Still, drivers for at least disk and network devices should be present.
...
Its sufficient to install 'Windows Preinstallation Environment (Windows PE)' and the dependencies automatically selected. +
...
The easiest available method for tweaking your PE requires a computer that has opsi-client-agent installed, as well as a Windows ADK (Win8.1,Win10). Doing the steps manually is described below.
...
Here is the manual way now. The console commands for creating Windows PE in 32- or 64-bit versions are nearly the same, except for the *<ARCH>* entries below. These have to be set to either *x86* , *amd64* or *ia64*.
...
* Copy the contents of `C:\winpe\media` to `/var/lib/opsi/depot/win7/winpe` (or `/var/lib/opsi/depot/win2008/winpe`). +
Adjust the file access rights by entering(e.g.):
[source,prompt]
----
opsi-setup --set-rights /var/lib/opsi/depot/win7/winpe
...
CAUTION: This refers to clients with a hard drive larger than 2 terabyte. On a non UEFI-system the possible largest partition size is 2 terabyte. When you have a larger partiton scheme the installation will fail. This is a technical restriction. You have to configure the system partition with a maximal size of 2 terabyte and therefore configure two partitons. This can be done with the product-properties. On the other hand the UEFI-module bypasses this restriction by using another partition table.
...
PCI-Devices
   [(StandardsystemgerÃ¤te), PCI Standard-PCI-zu-PCI-BrÃ¼cke]
...
Aopsi-winst script contains *primary* and *secondary* sections. The section headers are in square brackets, similar to what you may have seen in ini-files. The primary section is noted by the identifier [Actions], and the secondary section is noted by the identifier [WinBatch_...].
...
Start the {opsi-winst} (`winst32.exe`) program via a double mouse click. (On Windows 7 Clients, you must right-click on the mouse button and select "run as Administrator"). If the opsi-client-agent is installed on your computer you will find the {opsi-winst} at the directory `C:\programÂ files\opsi.org\opsi-client-agent\opsi-winst. If the {opsi-client} agent is not installed you will find the {opsi-winst} at the share
...
For an â€žunattendedâ€œ or â€žsilentâ€� setup, the original setup will be switched to an unattended non-interactive mode using the proper command line arguments.
...
With Windows 2000, Microsoft launched its own installation concept based on the Microsoft Installer Service â€žMSIâ€œ. Since then, many setup programs have become MSI compliant.
...
The parameter 'f' means 'force' â€“ or to delete the files even if they are marked as 'read only' â€“ and the parameter 's' means including the 'subdirectories'. A file or directory can be deleted from all user profiles using the option '/AllNTUserProfiles' (see {opsi-winst} manual for details).
...
If the product â€“ or part of it â€“ runs as a service, you will have to stop the service before deleting the files. One way to do so, is to set the service state to â€œinactiveâ€œ in the registry and restart the computer. Or to stop the service by using the command 'net stop', which doesn't need a reboot:
[source,winst]
----
...
CAUTION: On SLES this directory is located at `/var/lib/opsi/workbench`.
...
If your are running into the problem that the creation of an package fails because of insufficent free space in `/tmp` you can use the option `-t` to specify a different temporary folder.
