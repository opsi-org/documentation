...
The installation of opsi with the default univention Backen is possible on the Rolling Master, Backup, and Slave.
It is not possible on the Memberserver.  When the opsi-config server should add another opsi-depotserver (in serveral places),
then the opsi-config server must be installed on a UCS-Master or -Backup.

The packet opsi4ucs-ldap-schema muse be imported to the Master.

The following documentation is used for an installation on a Master.
...
If you would like to install opsi on a Slave or on a Backup-Server, then the above commands are important for the Server.
(It is not imporant to enter the commands on the Server if you only want to import the opsi4ucs-ldap-schema.)
...
The opsi 'univention' is no longer available for UCS 3.
...
UCS 3 system that is the Master.  Especially when the UCS 3 System will be installed with Samba4 (deafult).
...
The classical installation, with the user: 'pcpatch' in the group: 'pcpatch', will not function with UCS 3. 
Samba4 has placed fundamental restrictions on the Active-Directory, such that groups with the same name  
as the user (usually in Unix/Linux) are no longer allowed.  For this reason, a new configuration file has 
been introduced:  `/etc/opsi/opsi.conf`, which will control how the groups will access Samba-Access. Especially for #
UCS 3 will the group name 'pcpatch' be called 'opsifileadmins'.  The means 
that the user who belonged to the group 'pcpatch' under UCS 3, must now belong to the group 'opsifileadmins'.  
This means that a user, who muss have access rights for opsi-packages under UCS3 could not be a member of 'pcpatch', 
but must be a member of the group 'opsifileadmins'.  This peculiarity initially apples only to UCS 3, and is different
...
'opsifileadmins' group will no longer befixed at 992, but will be self-assigned in UDM, which will also 
...
pcpatch account will be cleanly replicated to Samba4.  It must be checked that the s4-Connected from Univention has 
the pcpatch account in the Ignore-List:  
...
Should the output look like this:
...
This features is off for the default installation of UCS 2.4-x.  Therefore the opsi configserver has no 
possibility to know the group membership of each user.  Since no users (neither system nor domain users) 
can explictitly give reading rights, there remain two alternatives to provide reading rights from 
the Configserver:
...
(This will be done by automatically with an upgrade from UCS 2.4-x to UCS3.)
...
LDAP on a production system, it should be done on a schedule):
...
are important, and can be released to with the following command:
...
be added to this group during installation.
...
Samba Settings:  'follow symlinks'.  The same should be done for the 'opt_pcbin', so that the driver itegration will
...
To make sure that opsi is running with the proper settings, enter the following commands:
...
Since samba4 is a central service, this is not done automatically by the package, but must be done manually.  
Please note that after restarting samba, there may be a delay in acess to any new releases.
...
Deleting the LDAP clinets in Univention does not mean that the client was also deleted under opsi.

Since opsi has already been run on the server, it is assumed that the network configuration is correct. +
Continue with the installatin be skipping forward to <<opsi-getting-started-installation-config-backend>>.

WARNING:  The Unix commands in the following sections refer to Debian Systems.   You may need to replace 
them with UCS-specific commands.  
