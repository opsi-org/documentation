...
An '{opsi-server}' can be installed as a virtual machine, because of the low performance requirements. For VMware, a ready-to-use and pre-configured virtual machine is provided. The data files are available in the Internet. The free of charge VMware player is sufficient to run this machine. 
...
===== „1stboot“
The {opsi-server} should be connected to the internet to work properly. The script `1stboot.py` will start automatically at the first boot in order to configure the {opsi-server} network settings.
...
The graphical user interface of the {opsi-server} should already be started (implemented as a sustainable window manager). As a welcome a „Firefox“ browser window comes up with further instructions and information, so as a reference to the getting started document (the document you are currently reading).
...
If the network configuration information was correct, then you are able to remotely access the {opsi-server}, for example:

* command line access to the server via 'ssh' (already installed on linux systems, for Windows use putty: http://www.chiark.greenend.org.uk/~sgtatham/putty/) +
...
Helpfully: the graphical interface has many working applications that are reachable using the variety of buttons in the upper left corner of the display.
...
If everything is not working, then you have to open a terminal window (maybe remote access isn't possible, except using the server terminal window) and then perform the necessary network connection checks and fixes. 
...
If the network connection works, then you can install opsi packages or update them, and configure the environment for the first installation test. Therefore, skip to the Section <<opsi-getting-started-installation-config>>.
...
opsi needs samba to be installed:
...
During the tftpd-installation, you may be asked for the tftp directory. Answer with `/tftpboot`. Answer the question after the multicast support with a 'no'.
...
If you want opsi to manage the configuration of the DHCP server, then you need to correct this entry to point to the servers public IP adress.
...
If necessary, adapt the file `/etc/apt/apt.conf` to your network circumstances (enter the correct proxy, or comment/delete lines). You can edit your file with a program like „midnight commander“:
...
Different kinds of data may stored in different types of backends. For some actions (such as method calls) more than one backend has to be involved. Therefore the different method calls use different backends. This configuration is in the file `/etc/opsi/backendManager/dispatch.conf.`
...
You will find explanations and examples at the top of this file.  In uncommented lines, the first column is the name of the opsi method being called  (with wildcard \*), and the second column is the list of backends used by that opsi method.  If any method is called the first matching line will be used. The last line (.*) matches all opsi method calls.

The default is the file backend.

Now at the initial start-up (even if you haven't changed the file), and after any change in this file, you should call:
...
The {opsi-server}s and the connected clients are administered with the program '{opsi-configed}'. The program is written in Java, and requires a Java version that is at least 6 (or version 1.6 in the older nomenclature).
...
To use a version of Java that is at least verion 1.6.0, use the program `update-alternatives` :
[source,prompt]
----
