...
The second step would be to modify the file `/tmp/property_config.json` and change the entries in values. Finally, you must update the objects using this modified file (enter this command in one line):
[source,prompt]
----
...
Now the client should load the opsi linux bootimage via the network and start it. Before the windows installations starts, you have to confirm.
...
If some client hardware is not supported by the standard Windows drivers, then it could be useful, and sometimes even necessary, to integrate new drivers into the unattended installation. In regards to network devices this is very recommendable, because a client without network access is neither remote accessible, nor can the automated software distribution connect to any distributed file shares. 
...
If you just have special hardware, then you can take drivers that are provided by the hardware manufacturer and put them into the distribution file share.
