...
* Start the VMware player. Open "Open a Virtual Machine" and search for the file `opsidemo.ovf` in the 'opsidemo' directory. You can import the server with an new name. The virtual machine will still boot.
...
At the menu 'File' / 'Import Appliance' select your `opsidemo.ovf` file and import it.
...
Updating your opsi-Server to double click the Icon "Update Manager" on the desktop.

Install the minimal opsi Products to double click the Icon 'opsi-product-update FirstRun'. Please take the password for adminuser.
