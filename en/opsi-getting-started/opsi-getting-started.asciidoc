...
Since this situation represents a problem not only on the installation, but could also lead to potential problems with migrations (especially of Samba3 on Samba4) since the release of 4.0.5 the group 'pcpatch' will be created as 'opsifileadmins', no matter what combination of UCS 3.2 / 4.0 or Samba3 / Samba4
...
The installation of opsi is possible on the roles master, backup and slave - not as member-server.
