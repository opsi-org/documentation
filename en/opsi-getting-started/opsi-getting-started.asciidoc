...
----
NOTE:: At Ubuntu 10.04 (Lucid) you need the package `smbfs` instead of the package `cifs-utils`.
...
The actual tests indicates, that these issue will not prompt if the winbindd daemon is not running.

Disable winbindd start:
[source,prompt]
----
