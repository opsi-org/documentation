...
CAUTION: Caution on clients with a hard drive larger than 2TB. On a non UEFI-system the possible largest partition size is 2TB. When you have a larger partiton scheme the installation will fail. This is a technical restriction. You have to configure the system partition with a maximal size of 2TB and therefore configure twi partitons. This can be done with the product-properties. On the other hand the UEFI-module bypasses this restriction by using another partition table.
