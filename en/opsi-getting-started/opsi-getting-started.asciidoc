...
CAUTION: Using Debian 9 can lead to the following message:
[source,configfile]
----
W: GPG-Error: http://download.opensuse.org/repositories/home:/uibmz:/opsi:/opsi40-testing/Debian_9.0 ./ Release: The following signatures are invalid: 2371C96FC045365D00729A19520B97144DC87421
...
* Required packages for the opsi-Installation will be installed: opsi4ucs, opsi-tftpd, p7zip-full, cabextract, mysql-server.
...
Since opsi 4.0.5 the group 'openfileadmins' replaces the group 'pcpatch' in UCS.
...
* The opsi-tftpd is not configured as usual using the directory `/tftpboot` as base directory, instead the `/var/lib/univention-client-boot` is used. All important files of {opsi-linux-bootimage} will be linked from '/tftpboot' in the base directory. The side effect is that the DHCP-Option file named `pxelinux.0` will be replace instead with `linux/pxelinux.0`.
...
==== First Tests
////
[[opsi-getting-started-rollout-of-existing-products]]
=== Rollout from Existing Products

[[opsi-getting-started-rollout-of-existing-products-usage-of-standard-products]]
==== Usage of opsi standard products
