...
The {opsi-server}s and the connected clients are administered with the program '{opsi-configed}'. 
The program is written in Java, and requires a Java version of at least 7 (or version 1.7 as called traditionally).

Since Ubuntu has removed the Sun/Oracle JRE from its repositories, we recommend using OpenJDK. 
This will work fine if you start '{opsi-configed}' as an application or via webstart. 
