...
We recommend cuting and pasting commands from this handbook directly into the {opsi-server} terminal window (most applications support cut and paste).
...
Example snippets for commands that you have to execute are formated like this:
[source,prompt]
----
cd /tmp
ls -l
...
===== Check the network connection, and if necessary correct the settings
...
During the {opsi-server} installation, you have to allow the patching of the files `dhcpd.conf` and `smb.conf`. Answer the question with 'yes'. Also, you will be asked for a password for the user 'pcpatch'. Set a new password, and please remember this password when continuing with the following sections.

During the {opsi-server} installation, please ignore any warnings about a missing `/etc/opsi/modules` file.

If you would like to run the opsi management interface '{opsi-configed}' directly on the server, then you need to install the Java Runtime Environment from Sun/Oracle. In order to install these packages, enter these commands:
...
To alter the version of Java you are using, such that it is at least verion 1.6.0, use the program `update-alternatives` :
[source,prompt]
----
...
==== Using a DHCP server at the {opsi-server} VM
...
In the default method of opsi software deployment, only the client must konw how to contact the opsi-server.

However, if you would like to use one of the opsi 'push' feautures (like send messages to the client, or fire 'on_demand' events, or get session information, or start remote control software), then the server needs to know how to get the IP-Address of the client.
...
. The clients are not known by the DNS (only by netbios), and they get dynamically assigned freqently changing IP-Numbers by the DHCP.
...
* Enter the following address in a browser (anywhere in the net) https://<opsi-server>:4447/configed, and then a web interface will appear with an embedded {opsi-configed}. A java version of at least 1.6 is required.
...
To integrate existing Windows clients into opsi, the opsi-client-agent has to be installed on these systems. There are different ways to do this, which are described below. After you have done so, you should see the client in the {opsi-configed} after selecting the crystal tab 'clients'.
...
To see the changes at the '{opsi-configed}' management interface, select reload with the button at the top or with a right mouse click.  You may see the update after selecting the crystal tabs 'Hardware information' and 'Software inventory'.
...
* To integrate the extraced drivers into the mounted image, you need to execute this command:
[source,prompt]
----
...
If a particular client hardware is not supported by the standard Windows drivers, then it could be useful, and sometimes even necessary, to integrate new drivers into the unattended installation. We recommend installing new drivers for network devices, because a client without network access is neither remotely accessible, nor can the automated software distribution connect to any distributed file shares. 

opsi supports your work with an automated driver integration and detection. You only have to place the drivers in the correct directory. After calling the script (`create_driver_links.py`), a catalog is created which allows the boot image to find the correct drivers for the detected Hardware via PCI, USB or HD-Audio Identifier. Drivers for mass storage controllers (text mode drivers) may also integrated in the same way.

The server can provide additional drivers to be installed during the Windows setup. All of these drivers must come with an '.inf' file, which holds the driver's installation information. Any drivers which are packed as an executable cannot be used for this purpose (but often they can be unpacked to get the plain installation files).

If you have a running machine with the correct drivers installed, then you may use the program 'double driver' (http://www.boozet.org/dd.htm) to extract these drivers and save them to the {opsi-server}.

If you have special hardware, then you can take drivers that are provided by the hardware manufacturer and put them into the distribution file share.

However, if you have to support a lot of different hardware, it might be convenient to use readily made packages for a whole bunch of Windows drivers, which can be found at 
http://driverpacks.net/.

You may download the current driverpacks (!!! about 2,5 GB !!!) from http://driverpacks.net/DriverPacks/overview.php and save them to the server. Calling: 
[source,prompt]
----
...
Additional drivers can be added into its own sub directory at `winxppro/drivers/drivers/preferred`.
...
The `create_driver_links.py` script does a onetime extraction of the driver information from the `i386` or `installfiles` directory to the 'buildin' directory. If you make any changes to these directories (e.g by integrating a service pack) you should delete the build directory and call `create_dirver_links.py` again.

If there is any hardware inventory data for a client, then these data can be used to list the drivers that will be selected by the boot image for this client:
[source,prompt]
----
winxppro/show_drivers.py <clientname>
----

If there are additional drivers that should be used by the setup, even if they were not selected via PCI-IDs, then they should be placed beneath the directory `winxppro/drivers/drivers/additional`. Using the product property 'additional_drivers', it is possible to give a comma separated list of driver paths beneath the additional directory which should be used by the windows setup program. The directories given in the property 'additional_drivers' are recursively searched for drivers, and all detected driver directories are integrated into the installation. You may use symbolic links in the additional drivers directory.

During the first driver integration, all drivers which are assigned via the 'additional' property will be integrated and analyzed in order to determine which PCI device (or HD-Audio, USB) will be used. For the devices that have a driver assigned using 'additional_drivers', no other driver `drivers/prefered/` or `drivers/` will be integrated. The means that 'Additional' is not only the function that adds undeteced drivers. It has also the function of superseediong other drivers (so 'Additional' may also be called 'super-preferred').

You should examine the output of `show_drivers.py`, and check which drivers will be integrated.

It may be that driver directories from hardware vendors contain drivers for different operating systems versions (Vista/Win7) or different configurations (SATA / SATA-Raid). The `create_driver_links.py` script will take the first directory that matches the PCI-Id (or HD-Audio, USB). If suspect that the link goes to the wrong driver directory, then you can move the driver directory to `drivers/exclude` and run `create_driver_links.py` once again. Drivers in `drivers/exclude` are excluded from the driver integration.

Example output from show_drivers.py : 
...
This tutorial should help you to start with opsi. It is no replacement for professional training (which you may order through uib) or even studying the complete opsi manuals.
...
The answers provided by the user during the the manufacturer's setup program can be automatically saved using the free tool 'Autoit' or Autohotkey. That requires providing an autoIt script for unattended installation

. *Analyze and Repackaging* +
The standard setup can be analyzed and 'recorded' to do the installation tasks directly by the '{opsi-winst}' program. Usually that is something like file installation to the local file system, followed by patching the registry

NOTE: opsi supports all of these variants. +
Usually a combination of all different ways in one script provides the best result. For example, performaing the basic installation using the original setup if available, and then doing some customizing by patching the registry or by a file based configuration.
...
A winst script contains *primary* and *secondary* sections. The section headers in square brackets, as you may have seen in ini-files. The primary section is noted by the identifier [Actions], and the secondary section is noted by the identifier [WinBatch_...]. 

The core work like starting programs or copying files is done in the secondary sections, not in the primary sections. These secondary sections are topic specific, and have a specific syntax according to their specific topic.
...
Any part of the code that is called more then one time can be placed sub sections.
...
Start by creating a directory to build and test your script (e.g. `c:\test`), and then copy the template scripts from the opsi-template (`setup.ins`, `delsub.ins` und `uninstall.ins`) to this directory.
...
* Look at the log file and see how {opsi-winst} interpreted the script.

* Choose the setup.exe that you will use to install software, and copy setup.exe to the directory where the scripts are located (e.g. `c:\test`).
...
The next chapter contains some hints about how to view any problems that may arise while building a opsi-winst script. <<opsi-getting-started-softwintegration-create-opsi-package-newprod>> describes how to create an {opsi-product} from your scripts, and how to install them on the {opsi-server}.
...
The problem is to find the correct arguements

.Look on the internet:
Before you start to integrate a new package, you'd better first have a look online to see if somebody has already done that job for you:
...
Setup programs are not often written by the software manufacturers themselves, but use frame works like 'Inno', 'NSIS', 'Installshield' or 'Wise'. So the command line arguments for the resulting packet will be similar to the arguments used by the setup creation tool.

To determine which setup tool was used, you may use the following procedure: Use the `strings` command to extract the strings from your setup program, and than `grep` (or `findstr`) to filter for the names of the setup tools.
...
In Windows there is no native `strings` command, so you have to install it first. You can download a `strings.exe` program from here: http://technet.microsoft.com/en-us/sysinternals/bb897439
...
A short overview to the {opsi-winst} commands can be found in the reference card:
...
Here are some hints regarding important elements:
...
.Product Properties
For some products it is important to know the product properties that can modify the installation to make it client specific. Creating these properties is described below in <<opsi-getting-started-softwintegration-create-opsi-package,"Creating an opsi package">>.
...
Another possibility is that the installation starts a second process and stops before the second process is finished. So from the point of view of the {opsi-winst}, the task is finished while in fact the second process is still working (installing / uninstalling). +
In this case you may use the modifier +/WaitSeconds <seconds>+ , or
+/WaitForProcessEnding "program.exe" /TimeOutSeconds "<seconds>"+, in the WinBatch section so that the script waits for end of the second process.
...
If you use the opsi option of running the installation on a other desktop than the current desktop, or if the current desktop is locked, you will find that some autoit functions do not work properly under these conditions.
...
Because these commands are so widly used, we provide substitutes:
...
Below is a part of our jedit installation script as example. +
...
Products that are installed by MSI normally come with an uninstall option, which is usually the program `msiexec.exe` combined with the parameter `/x`. The parameter `/qb-!` is for the unattended mode (or without user interaction). So here is an example unattended deinstall command:
...
If none of these methods are available or sufficient, the deinstallation can be done using a {opsi-winst} script as described below:
...
Package Version:: is the version of the package for the product version. For example, this helps to distinquish between packages with the same product version but with modified '{opsi-winst}' scripts.
...
An *+Update-Script+* will be used for minor changes on existing big installations. If this product is switched to the requrired action 'setup', then the update script will be automatically executed after the setup script.
...
A *+userLoginScript+* is used to modify the user's profile after the user logs into the system. It only works with the opsi extension 'Roaming Profile Support', which is described at the 'Roaming Profile Support' chapter in the opsi-manual.
...
Next, a description for the switch needs to be specified, where the description will be shown in the 'opsi-configeditor' as a help text and also when the package is unpacked. Next, you can define the set of values for the switch (separated by comma). If this is left blank, then any value is allowed for the switch.
...
== More information
A lot more detailed information can be found in the opsi-manual.
