...
In order to perform an OS Installation, a so-called WinPE is needed. It's easy to create one yourself. Follow the manual, because a tiny bit of tweaking is necessary. We recommend using the corresponding WinPE for your OS (Win7/8/10)
...
First you need the Windows Automated Installation Kit (Windows AIK), or its successor "Windows Assessement and Deployment Toolkit" (ADK) for Windows 8 and 10 systems, respectively: +
