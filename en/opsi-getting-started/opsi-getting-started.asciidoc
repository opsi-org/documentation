...
Your DNS domain name should contain at least a domain name and a top level domain. So the full qualified domain name (FQDN) should contain one or more dots. +
Valid domain names are e.g.: 'uib.local' , 'uib.de', 'subdomain.domain.de'
...
* The command `hostname -f` must return a fully qualified domain name containing two dots, e.g. 'opsidemo.uib.local'
...
Sofern die Version des opsi-Servers sich nicht geändert hat, sollte die Aktualisierung unproblematisch sein. Wie opsi-Server von früheren Versionen nachgezogen werden können, ist im entsprechenden Upgrade Handbuch beschrieben.
...
If you are planning to use the mysql backend (e.g. inventory data), you have to initialize it now. Therefore use the command `opsi-setup --configure-mysql`.
...
| User login | _not implemented yet_ | _not implemented yet_
|=======================
