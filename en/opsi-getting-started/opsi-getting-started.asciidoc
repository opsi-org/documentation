...
The presented way here is the one for the manual. The console commands to create Windows PE in 32- or 64-bit versions are nearly the same, except for the *<ARCH>* entries below. These have to be set to either *x86* , *amd64* or *ia64*.
...
When the hardware configuration is very heterogeneous, then it may be reasonable sense to work with general driver packages. +
