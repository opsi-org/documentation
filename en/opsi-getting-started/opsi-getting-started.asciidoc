...
Through this the actual stock of opsi-products will be downloaded from http://download.uib.de/opsi4.0/products/ in the directories netboot/ and localboot/ and install them on the server.
...
* The CentOS packages for opsi 4.0 are build for CentOS 6 & 7.

* The support for CentOS 5 is canceled.

* We recommend to evaluate opsi with the opsi-VM.
...
* Enter the following address in a browser (anywhere in the net) https://<opsi-server>:4447/configed, and then a web interface will appear with an embedded {opsi-configed}. A java version of at least 1.7 is required.

* Another option would be to right-mouse-click on the server desktop background, then open the context menu, and then choose the â€žopsi config editorâ€œ. In this case a java runtime environment must be installed on the server.
