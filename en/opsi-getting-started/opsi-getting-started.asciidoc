...
===== Installation on a UCS 2 System

The installation of opsi with the default Univention backend is possible on the Rolling Master, Backup, and Slave.
...
The packet opsi4ucs-ldap-schema muse be imported to the Master system.

The following documentation is used for an installation on a Master system.
...
Only members of the group 'opsiadmin' are permitted to use the opsi management interface. Please use Univention-Admin to edit the group memberships of a user. The user Administrator will be added to the opsiadmin group automatically during the opsi installation process.
...
This means that a user, who must have access rights for opsi-packages under UCS3 could not be a member of 'pcpatch', 
...
pcpatch account will be cleanly replicated to Samba4.  It must be checked that the Univention s4-Connected connector has 
...
(This will be done by automatically with an upgrade from UCS 2.4-x to UCS 3.)
