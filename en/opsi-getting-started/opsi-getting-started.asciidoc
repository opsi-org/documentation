...
* The support for Debian Lenny and Ubuntu Maverick is deprecated.
...
* For the changes to be committed, the images muse be unmounted:
[source,prompt]
----
...
If a type of client hardware is not supported by the standard Windows drivers, then it could be useful, and sometimes even necessary, to integrate new drivers into the unattended installation. We recommend installing new drivers for network devices, because a client without network access is neither remotely accessible, nor can the automated software distribution connect to the distributed file shares. 

opsi supports your work with an automated driver integration and detection. You only have to place the drivers in the correct directory. After calling the script (`create_driver_links.py`), a catalog is created which allows the boot image to find the correct drivers for the detected Hardware via PCI, USB, or HD-Audio Identifier. Drivers for mass storage controllers (text mode drivers) may also integrated using this method.

The server can provide additional drivers to be installed during the Windows setup. All of these drivers must come with an '.inf' file, which holds the driver's installation information. Any drivers that are packed as an executable cannot be used for this purpose (but often they can be unpacked to get the plain installation files).
...
You may download the current driverpacks (!!! about 2,5 GB !!!) from http://driverpacks.net/DriverPacks/overview.php and save them to the server. To extract drivers, use extract_driver_pack.py as follows: 
[source,prompt]
----
...
Additional drivers can be added into their own sub directory at `winxppro/drivers/drivers/preferred`.

After adding new drivers (or any other change) adjust the rights with (e.g.):
[source,prompt]
----
opsi-setup --set-rights /opt/pcbin/install/winxppro/drivers
----

Then execute the script `create_driver_links.py` from the winxppro directory. 
[source,prompt]
----
/opt/pcbin/install/winxpro/create_driver_links.py
----

The script searches all the directories beneath `drivers`, and then creates links that assign drivers to their respective hardware (using the PCI-IDs, USB-IDs, HD-Audio-IDs). Drivers found beneath the directory `preferred` will indeed be preferred against other ones with the same IDs beneath other directories. During installation, the script `setup.py` (from the boot image) identifies the client hardware and determines the required drivers. The drivers will be copied to the hard disk, and the file `unattend.txt` (which is the control file for the Windows unattended installation) will be patched accordingly.
...
* After figuring out whic setup.exe that you will use to install software, copy setup.exe to the directory where the scripts are located (e.g. `c:\test`).
...
Setup programs are often not written by the software manufacturers themselves, but use frame works like 'Inno', 'NSIS', 'Installshield' or 'Wise'. So the command line arguments for the resulting packet will be similar to the arguments used by the setup creation tool.
