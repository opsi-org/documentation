...
It is assumed that MySQL is installed and configured. We then require the credentials for an database administrator. For specific information on installing and configuration of your database please refer to the manuals of your distribution.

The command will ask for information to database access and then use the provided credentials to create a database and an user with appropriate rights for accessing that database for opsi.
