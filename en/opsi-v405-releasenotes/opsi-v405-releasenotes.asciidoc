...
openSUSE 13.1, Fedora20, CentOS 70: +
...
* opsipxeconfd: +
On RHEL / CentOS the removal of the previous version will be marked as failed.
...
[[opsi-configed]]
== opsi-configed 4.0.5.
For all hosts on which a configed is expected to run Java Runtime of at least version 1.7.0 must be installed. 
...
=== Client reachability
The feature of testing the client reachability has been extended to run in the background and then show the results in the client table. 
It can be enabled from the logon screen mask or per command line parameter. Default is a test interval of 0 min (= deactivated). +
