...
=== WakeOnLan feature
The WakeOnLan feature has been expanded, so by default every ten 10 minutes the configed will test the availability of the clients. The result will be displayed in the client's table, and the interval can be changed in the login screen interface. +
 image:../images/opsi-configed-wakeonlan_status_true.png["Client reachable",width=20] : Client reachable +
 image:../images/opsi-configed-wakeonlan_status_false.png["Client unreachable",width=20] : Client unreachable
...
.'{opsi-configed}': Failures occurred
