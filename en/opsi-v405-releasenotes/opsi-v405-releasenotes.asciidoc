...
Choose the first setting to get a list of all products. If you select one product all clients will be shown, where the installation of this product failed. +
Choose e.g. _Failures occurred_ - _today_ to get all clients, where at least one product failed today. 

.'{opsi-configed}': Failures occurred
image::../../../../images/opsi-configed-failedactions-today_en_1.png["{opsi-configed}: Failures occurred - today", width=400] 
...
If you select a product, you can see on the right side the information about that specific product, that you already know from the _product configuration_,
a list of depots and a table with specific properties. This list of depots includes all depots, which own the selected product.

.'{opsi-configed}': Product default-properties
image::../../../../images/opsi-configed-produkt-defaultproperties-jedit-verschiedene-werte_en_1.png["{opsi-configed}: Product default-properties: jEdit", width=500]

Select one or more depots to change the default values of the product. By default, all available depots are selected. 
The usual keyboard shortcuts (_ctrl-a_, _ctrl-click_ or _shift-click_) can select several or all clients. +
If the property value is grayed out (see figure 9.2 - ``gui_language''), that means that there are several values set for the properties of this depots. +
To the right of the depots there are three buttons:

* (left): Mark all depots with identical values (``=+'') +
All depots, which own the same values will be selected.

* (middle): Mark all (``++'') +
All depots will be selected.

* (right): Set values from package defaults +
The values from the product-package will be use for the selected depot(s).
