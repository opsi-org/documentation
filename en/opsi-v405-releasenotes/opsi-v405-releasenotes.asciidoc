...
* opsi-clonezilla: +
You may see problems if the property `drbl_ocs_conf` has no value. +
The handling of linux system (ext filesystem) is broken right now.

* opsiconfd: +
On the info page in the section RPC statistics the average success rate will be either 100% if everything is okay or 0% in the case of one or more failures.
