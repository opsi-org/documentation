...
It should be observed that it's the '{opsi-configed}' which triggers the actions, therefore the program must not be shut down in the meantime.
...
Here you can find more than one group, that is why there is an additional entry called 'All groups' in the first place. This list can be also configured to display groups of products, or other products selections. By default, the first entry is selected, e.G. 'All groups' and in the case there is only one, then it will display the only available group. +
