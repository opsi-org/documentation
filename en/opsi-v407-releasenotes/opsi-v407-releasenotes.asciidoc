...
The main window displays on the left side a list of product groups (7) which are enabled via the Config `software-on-demand.product-group-ids` for this client. Here you could find more than one group, that is why there is an additional entry called 'All groups' in the first place. This list can be also configured to display groups of products, or other products selections. By default, the first entry is selected, e.G. 'All groups' and in the case there is only one, then it will display the only available group. +
