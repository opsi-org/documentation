...
** The sort algorithm 1 has been repaired.
** The sequence of products as seen by opsi-winst is kept even if some setup scripts were executed (and no more action request is set for them).  
...
image::opsi-configed-wake-on-lan-scheduler.png["{opsi-configed}: Scheduler for Wake On Lan", width="53mm"]
...
override priorities. By a combination of several mistakes, it was in 4.0.6 completely replaced by sort algorithm 2 which prefers priorities over dependencies. Now, everything is working (again) as the manual tells: +
...
CAUTION: Because this bug was active for a longer time, the fix may change the installation sequence in a way that is correct but unexpected because different from the past.
...
The following chapter shows an overview which Windows Releases are supported as opsi-clients:
