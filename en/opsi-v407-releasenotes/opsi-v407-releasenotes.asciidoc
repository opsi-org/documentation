...
** New Feature to control over productgroups which products will be processed  (include and exclude of products per Event)
** Support for Windows 10 in WAN-Mode for mshotfix-Abo
...
In this chapter we will explain which new configurationoptions for opsiclientd will be shipped with opsi 4.0.7.


[[opsi-407-releasenotes-clientagent-configuration-proxysupport]]
==== Proxysupport-Configuration

In the global section of opsiclientd.conf you have the option to define a proxyserver, that will be used from opsi-client-agent.
If a proxyserver is defined in config, all HTTP- and HTTPS-Connection of opsiclientd will be redirected to this proxyserver.
...
In that case you must define the credentails how is shown in configuration snippet.
...
The option `exclude_product_group_ids` allows to configure a comma seperated list of productGroups. The members of these groups will be excluded from the actual Event. Also if actionrequest is set for this products. These products will be ignored in this event, but the actionrequests will not be touched.

Whitlisting (including):

The option `include_product_group_ids` allows to also configure a comma seperated list of productsGroups. The members of these groups are the only products, that will be processed from the actual event if they have actionrequests set.

You can use these options globally from the default-Event. From that point this settings will be used in every event. You can also set these options in special event. If you use these option on event_on_demand, you can control which products will not be installed in push installations although they have actionrequests set. On normal restart of the client, this products will be installed from gui_startup (default event) at startup.

ATTENTION: For Clients that work in WAN/VPN-mode you must set this options in sync-event and also in the cacheservice-section, because the cacheservice have no access to the configuration of main sync-event. 

WARNING: Productdependencies will not be regarded from this feature. You have to observe to prevent dependency issues.
