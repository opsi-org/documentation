...
In this window and over the upper button `Install now` can an immediate installation now be triggered. In this case, it is advisable to close all applications (or at least save your data) since the path installations can interfere with running applications. +
The button below `Install at next standard occasion` will be terminated at this point and to that end all the stored actions will be executed later.
...
* Software that is already set to setup +
   - In these case, the Checkbox: _install_, will be already activated.
...
(/Var/lib/opsi/depot/opsi-client-agent/files/opsi/custom). The entire custom directory is saved when installing a new version of the opsi-client-agent and restored afterward, so the changes are not lost in case of an update.

* custom/opsiclientkioskskin/*.*
All the files in these directory will be copied, by the installation of an opsi-client-agent to C:\Program Files(x86)\opsi.org\opsi-client-agent\custom\opsiclientkioskskin. If available, this directory (opsiclientkioskskin) will have the preference.
...
== Wiederherstellung von opsi Metadaten aus Images (opsi-wim-capture, opsi-local-image-capture)
...
In this example, all the productOnClient objects will be fetched, which belong to the current computer (`%opsiserviceUser%` is in the service context of the FGDN client), Local boot products are where the action request is set to 'setup'.
[source,winst]
----
