...
* New opsi bootimage
* Extended Linux support with new opsi-linux netboot products, a new client-agent with 15 free starts and some more features.
...
* opsi-atftp
** compatible with the gcc5 compiler
...
=== Ubuntu 16.04 with mysql-backend
...
=== opsi-product-updater behaviour

By default the windows netboot products weren't updated when running the opsi-product-updater. The option *-i* ensured updating all products.
This behaviour has changed. The windows netboot products are updated when running the opsi-product-updater with default settings. This is the same behaviour as experienced by localboot-products.
If you want to keep exceptions please adjust your entry for *excludes* accordingly.

Similar to an update of localboot products. A remark during the update of the opsi-utils package informs about this changed configuration. You have to edit the configuration if you have user access to products.
...
.server packages:
* opsi-linux-support_4.0.7-4
* opsi-utils 4.0.7.5-1
* opsiconfd 4.0.7.4-1
* opsipxeconfd_4.0.7.1-1
* python-opsi 4.0.7.13-1
...
* l-opsi-server_4.0.7.1-3
* l-jedit 5.3-5


.opsi-Pakete (opsi-local-image):
* opsi-local-image-win* 4.0.7.1-1
* opsi-local-image-backup_4.0.7.1-1
* opsi-local-image-restore.0.7.1-1
...
For a long lasting maintenance of the installed Linux computer you need an activation after using this feature 15 times.
...
in an automated way. This can serve to quickly install a new opsi-depot-server
...
The debian8 and ubuntu 16-04 package can now be installed from a local Apache2 repository. +
...
#! /bin/bash
DOCUMENTROOT=/var/www/html
URL=http://download.uib.de/opsi4.0/experimental/opsi-v////
...
Our provided repositories include all needed packages for a succesfull installation. The Ubuntu-16.04 installation process downloads packages that are not part of an installation CD. During the installation process the official repositories are copied to `/etc/apt/sources.list.d/original.list`. The apache repository maintains as a repository. You can find the sources list in our provided repositories and modify it to your specific needs.

Elaboreate: +

Create a directory in your apache2 share. Usually `/var/www/html/`. On SLES12 this share resides in `/srv/www/htdocs/`.
In this directory create subdirectories for the distribution.
Each distribution usually installs on 32 or 64 bit architectures. Therefore you also need additional subdirectories for these.
Paste the content of an Ubuntu-16.04 CD or Debian8 DVD into this architecture created directory.
Ubuntu16.04 downloads additional packages during installation. For Debian8 the content of DVD-1 includes all needed packages for a successfull installation.
If you wish to add other packages into the package pool you have to add the chechsums (SHA1, SHA256 und SHA512) along with the file size into the Packages.gz file of this release. The package `dpkg-dev` can do this autimatically.
You also have to do this if you want to use the package pool of more than one installation CD/DVD.
...
opsi-linux-bootimage (20160706-1) testing; urgency=medium
...
opsi-atftp (0.7.dfsg-6) testing; urgency=medium
