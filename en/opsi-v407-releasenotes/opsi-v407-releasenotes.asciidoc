...
=== opsi-client-agent Option: verify_server_cert
...
In the case that you need to change the host parameter, for the opsiclientd please check:
[Source, prompt]
----
...
=== Configed connection problems

Due to the use of a new default protocol for the HTTPS connection TSLv1.2, different error images can come up in configed.
The following problems have been observed.

* Error message when logging in, login is thus not possible
* Crash when invoking larger log files
* Crash when updating the client list

As a temporary solution we recommend to use the previous HTTPS protocol TLSv1 for the execution. For this purpose, the following solutions are available:

* Set the property 'fallback_tlsv1 [true]' for the opsi package 'opsi-configed'.
* If the Configed can't be started, there is an executable version "setup.exe" under: +
http://download.uib.de/opsi4.0/helper/opsi-configed-setup.exe
* The local configed can be set to the previous protocol version by the Java call parameter '-Dhttps.protocols=TLSv1', the call is identified as (depending on the other configuration): +
[source, prompt]
----
...
* opsi-client-agent: +
The 'verify_server_cert = true' option does not work at this time. +
See also 'Important hints'

* configed +
Error message when starting or working with the Configed, which can be solved by using the previous HTTPS protocol TLSv1. +
See also 'Important Hints'
