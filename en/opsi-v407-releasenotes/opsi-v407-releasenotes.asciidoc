...
=== opsi-package-updater behavior

By default the windows netboot products are not updated when running the opsi-package-updater. The option -p installed a specific product. This behavior has changed. The windows netboot products are now updated when running the opsi-package-updater. Similar to an update of localboot products. A remark during the update of the opsi-utils package informs about this changed configuration. You have to edit the configuration if you have user access to products.
...
want to replace `/etc/opsi/opsi-package-updater.conf` with a newer version.
...
active = false
----
Then execute the opsi-package-updater.
...
We recommend to set in `/etc/opsi/opsi-package-updater.conf`
...
opsi-utils (4.0.7.4-1) stable; urgency=medium
  ...
  * opsi-package-updater.conf: Added additional repositories for Linux
...
opsi-utils (4.0.7.3-1) stable; urgency=medium
  ...
  * opsi-package-updater: Various small refactorings.
...
opsi-utils (4.0.7.1-1) testing; urgency=medium
  ...
  * opsi-package-updater: The notification mail will now have the DATE field
...
opsi-utils (4.0.6.13-1) testing; urgency=medium
  ...
  * opsi-package-updater: Attempts to close open connections now more reliable.
  * opsi-makeproductfile: use argparse instead of getopt.
  * opsi-package-updater: running with -p <productId> now implies -i.
