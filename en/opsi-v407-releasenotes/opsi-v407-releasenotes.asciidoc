...
CAUTION: Ubuntu Xenial (16.04) uses MySQL with Version 5.7. This version uses the 'strict mode' per default, previously this was optional. This leads to an error on the command _opsi-setup_ _--configure-mysql_. To that effect before you execute the command, the following should be edited:
[source,prompt]
----
