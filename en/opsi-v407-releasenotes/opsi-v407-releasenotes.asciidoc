...
* The sequence of products with action requests which the opsi-winst gets now keeps its order even if the scripts have partly been executed. Especially, 
if the execution of a script is stopped by an "ExitWindows /ImmediateReboot" command, then now it is guaranteed that the suspended product installation is directly continued 
after reboot.
