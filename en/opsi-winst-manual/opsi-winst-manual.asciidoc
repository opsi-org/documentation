...
opsi-winst / opsi-script Manual (4.11.5)
========================================
...
A software installation that is described by a '{opsi-winst}/opsi-script' script and performed by executing the script has several advantages compared with installations that are managed by a bunch of shell commands (e. g. copy etc.):

* '{opsi-winst}/opsi-script' offers to log very thoroughly all operations of the installation process. The support team can check the log files, and can easily detect when errors occurred or other problematic circumstances are evolving.
...
* There are different mode for writing to the Windows registry: 
...
Since version 4.11.4 there is a Linux port of 'opsi-winst' with the name 'opsi-script'.

According to the progess of porting and the differences between Linux and Windows not all functionalities are existing for both operating systems.

In the following section the availability is marked as:

* [W/L] may be used on Windows and on Linux as well
...
'opsi-script' is a GUI application which needs a running and accessable X Windows. 
...
By default log files are written into the directory `c:\opsi.org\log` which '{opsi-winst}/opsi-script' tries to create. If '{opsi-winst}/opsi-script' has no access to this directory it uses the user-TEMP directory. 

The default log file name is `opsi-script.log`. The log file name and location will be overwritten via the specific command line option. 
...
| %CommonProgramDir% | c:\Documents and Settings\All Users | C:\ProgramData\Microsoft\Windows\Start Menu\Programs
| %AllUsersProfileDir% | c:\Documents and Settings\All Users | C:\Users\Public
...
Typical entries of an Initial section set some the '{opsi-winst}/opsi-script' execution attributes. The following example shows how error responses may be configured:
...
* when an error occurs '{opsi-winst}/opsi-script' shall try to continue script execution
...
The above values are the default values, '{opsi-winst}/opsi-script' will assume them if these statements are missing.
...
I.e. the number can be given as an integer value or as a string expression (cf. <<opsi-winst-string>>). In the second case, '{opsi-winst}/opsi-script' tries to evaluate the string expression as a number.
...
`unquote2(<string>,<quote-string>)` [W/L] +
...
returns the content of <filename> as String.

* `GetShortWinPathName(<longpath string>)`  //since 4.11.5.2 [W] +
Returns the Shortpath (8.3) from <longpath string>. If ther is no short path for <longpath string>, so you will get an empty string. +
Exmple: `GetShortWinPathName("C:\Program Files (x86)")` returns `"C:\PROGRA~2"`
...
see also: `CompareDotSeparatedNumbers(`<str1>,<relation str>,<str2>`)`  //since 4.11.5.2: [W/L]
...
E. g., if $list1$ represents the list of the first five letters of the alphabet, using 
[source,winst]
----
takeString (2, $list1$)
----
...
* `getOutStreamFromSection (`section name`)`  [W/L] +
...
Since 4.11.4.2 there is a shortcut for simple calls to the shell. At Windows this command runs in the sysnative mode.
...
NOTE: The opsi 'User Profile Management' is under co-funding. This means that these features are not free yet (05.10.2011)
...
* `LineContaining_ExistsIn(` <string>, <file name> `)` //since 4.11.4.10[W/L] +
...
see also: `CompareDotSeparatedNumbers(`<string1>, <string2>`)`
...
If these parameters, at least the serviceurl, are given '{opsi-winst}/opsi-script' tries to open a connection to an opsi service which has the url.
