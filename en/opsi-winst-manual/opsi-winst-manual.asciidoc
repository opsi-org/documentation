...
* the `xml2` section as describied over here with the goal to make it easy to manipulate xml data
...
* `xml2path` +
is a notation to give a path throug the xml tree. +
...
So with the function `getXml2DocumentFromFile(`<path to xml file>`)` you get a stringlist that contains the `xmldocument`. This varibale may be used to analyze the `xmldocument` by other xml2 functions. +
<<opsi-winst-xml2-functions>>
