...
'opsi-script' is at the moment a GUI application which needs a running and accessable X Windows. A command line version 'opsi-script-nogui' which can run without any GUI is under development.
...
According to the Linux Filesystem Hierachie Standard the files that belong to 'opsi-script' are not at one place but distributed around the system. So here an overview where to find which part:
...
==== '{opsi-winst}/opsi-script' Path and Directory
