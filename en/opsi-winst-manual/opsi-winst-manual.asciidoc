...
Returns `true` if the Linux package system is not locked by an other process. If it is locked, it waits <seconds timeout string> to get the lock. If the timeout is reached and <bool should we kill> is `true` than the lockin process is killed but using this feature ist *not recommended*.
...
=== Patching a XML configuration file for a MsSql application: An example with misleading named attributes

The file which is to be patched has e.g. the following form; the values of DataSource and InitialCatalog are to be patched using the  
