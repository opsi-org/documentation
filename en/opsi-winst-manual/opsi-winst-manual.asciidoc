...
Return url components for the given url string.
...
* the `xml2` section as desribied over here with the goal to make it easy to manipulate xml data

* the `xml2` functions with the target to analyze give xml data +
see also : <<opsi-winst-rc-xml2-functions>> +
...
expecially the sector with '$Flag_winst_xml2$ = "on"'
