...
if $distro_type$ = "redhat"
	ShellInAnIcon_install_ssl_98_redhat
else
	if $distro_type$ = "suse"
		ShellInAnIcon_install_ssl_98_suse
	else
		if $distro_type$ = "debian"
			ShellInAnIcon_install_ssl_98_debian
		else
...
[ShellInAnIcon_install_ssl_98_debian]
apt-get --yes install libssl0.9.8

[ShellInAnIcon_install_ssl_98_redhat]
yum install openssl098e

[ShellInAnIcon_install_ssl_98_suse]
zypper --non-interactive install libopenssl0_9_8
----
...
By default log files are written into the directory `c:\tmp` which '{opsi-winst}/opsi-script' tries to create. If '{opsi-winst}/opsi-script' has no access to this directory it uses the user-TEMP directory. 
...
`%ScriptDrive%` : he drive where the just executed '{opsi-winst}/opsi-script' script is located (including the colon).
...
get OS informations and writes them to hash map string list. +
There are the folowinging keys: (example):
[source,ini]
----  
...
==== Simple String Values generated from String Lists [W/L] 
