...
There is a small helper application to test your wmi query, that works exactly like this function: +
...
Searches in the given <xml2stringlist> the last occurence of a node with the name <node name str> and returns this node (with all child nodes) as a xml2stringlist. +
