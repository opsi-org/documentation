...
* The parameter string, which is marked by the option `/parameter`, is accessible for every called '{opsi-winst}' script (via the string function `ParamStr`).
...
If the '{opsi-winst}' running in opsi web service mode, it sends the log file via opsi web service to the opsi server.
