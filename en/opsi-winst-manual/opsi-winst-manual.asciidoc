...
`<requestedExecutionLevel level="highestAvailable" />`. This means that if '{opsi-winst}' is called on NT6 by an Administrator, then it will run as a 'elevated' process. If '{opsi-winst}' is called with normal user priviliges, then it will run with the priviliges of this user.
...
4) Read the PC configuration from the opsi service and act accordingly, since '{opsi-winst}' 4.11.2
...
* When called with option `/allloginscripts` '{opsi-winst}' can do configurations for the logged in user (particularly in a Roaming Profile context). This is a co-funding feature - you need to buy it in order to use it. +
...
////
* If remoteerrorlogging has value 1, '{opsi-winst}' tries to open a $pcname$.err in the configshare, subdirectory pcpatch\pclog, and write the data to it. 
////
...
=== Primary and Secondary Subprograms of a opsi-winst script
...
=== String Expressions in a opsi-winst Script
...
CAUTION: The old function `LogLevel=` is deprecated since '{opsi-winst}' version 4.10.3. For backward compatibility reasons Loglevels ste by this old function will be increased by 4 before they are used.

There are two syntactical variants for specifying the logging level:

SetLogLevel = <number> 
SetLogLevel = <String expression> 
I.e. the number can be given as an integer value or as a string expression (cf. section 6.3). In the second case, '{opsi-winst}' tries to evaluate the string expression as a number.
...
I.e. the number can be given as an integer value or as a string expression (cf. <<opsi-winst-string>>). In the second case, '{opsi-winst}' tries to evaluate the string expression as a number.
...
. illegal statements which cannot be interpreted by '{opsi-winst}' (syntactical errors),
...
** 'true' = (default) If a syntax error occurs the script execution will be stopped and script result is set to 'failed'. In addition the message 'Syntax Error' will be passed to the opsi-server.
** 'false' = If a syntax error occurs the script execution will +not+ be stopped and script result is set to 'success'.

The syntax error will be logged as 'Critical' in any case. +
The error counter will be increased by 1 in any case. +
Since 4.11.3.2 +
In older Versions for syntax errors there is no logging, no increase of error counter and the result is always succes.
...
* `GetUsercontext` +
returns the string which was given to the '{opsi-winst}' by the optional parameter `/usercontext`. IF this parameter was not userd the returned string is empty.
...
* `GetRegistryStringValue32(`<string>`)` ->	see <<opsi-winst-64bit,Chapter 64 Bit>>

* `GetRegistryStringValue64(`<string>`)` ->	see <<opsi-winst-64bit,Chapter 64 Bit>>

* `GetRegistryStringValueSysNative(`<string>`)` ->	see <<opsi-winst-64bit,Chapter 64 Bit>>
...
If '{opsi-winst}' is connected to the opsi configuration service the product property is retrieved from the service. 
If the '{opsi-winst}' is not connected to the service or for other reasons the the call fails, the given +<DefaultValue>+ will be returned.
...
returns a string which is based on <string> but all ocurences of <oldPattern> are replaced by <newPattern>. This function is not case sensitive. +
Available since 4.11.3 +
Example:
[source,winst]
----
...
returns the part of <string> starting with <start pos> and includes the next <number of chars> chars. If there are fewer than <number of chars> after <start pos>, then the returned string will be the rest of the chars after <start pos>. +
Available since 4.11.3 +
Example:
[source,winst]
----
...
returns a string with the localized name of the group with the <well known sid>. For example,  For example, if <well known sid> is equal to 'S-1-5-32-544' then `SidToName` returns 'Administrators'.  +
Available since 4.11.3.1
...
==== Producing String Lists from opsi-winst Sections
...
32 Bit mode (with redirection). Avaiable since 4.11.3

* `getRegistryKeyList64(`<regkey>`)` +
returns a stringlist with the names of all keys within <regkey>. +
64 Bit mode (without redirection). Avaiable since 4.11.3

* `getRegistryKeyListSysnative(`<regkey>`)` +
returns a stringlist with the names of all keys within <regkey>. +
Mode (redirection) depends on the architecture of the OS. Avaiable since 4.11.3
...
Example: +
At first, we create entries in the registry with the following code: +
[source,winst]
----
...
----
Given these registry entries, and the following code:
[source,winst]
----
...
Set the given directory as working directory of the '{opsi-winst}'. Affects all subsequent actions (e.g. winbatch sections) and will be reset at the end of a script.  
Beispiel :
[source,winst]
----
...
=== Commands for userLoginScripts / Roaming Profile Support
NOTE: The opsi 'Roaming Profile Support' is under co-funding. This means that these features are not free yet (05.10.2011)
...
Avaiable since 4.11.1.1

* `isLoginScript` +
returns 'true' if the currently running script is called as 'userLoginScrip' using the opsi extension 'User Profile Management'. +
Avaiable since 4.11.2.1
...
The `include_*` commands may be used to add external script files to the currently-running script at runtime. The `include_*` command can includes external script files either as an insert (after the current line) or as an append (after the last line). The include commands may be used anywhere in a primary section. The external script files files may contain more include commands. +
...
The tests for the location of the <file name> are done in the order above. The first file found is the one that is used.
...
The file `include-test1.opsiinc` is run first.  The content of the included file `include-test1.opsiinc` is:
[source,winst]
----
DefVar $inctestvar$
...
. The subprogram is a sequence of lines situated in the executed '{opsi-winst}' script as another internal section. Then a name (constituted from letters, digits, and some special characters) has to be appended to the type specifier (without space) in order to form an unique section name. +
`sub_install_winXP` +
or +
`files_copy_winXP` +
...
The normal execution of a script breaks at the point of the call, and the '{opsi-winst}' stops running. This behaviour is needed if an automated user log in for some other user shall take place (cf. <<opsi-winst-cookbook-local-admin>>).
...
If a product installation fails this should be sinaled to the server.
...
Since 4.11.3.2 ther is a new variant of this command:

* `isFatalError` <string> +
in this case is string a short error message which is passed as 'actionProgress' to the opsi-server and displayed in the opsi-configed.
...
There are some external programs which start another process and then end without waiting for their child process to end. From '{opsi-winst}'s point of view, the process is ended and the next command could be started.
...
If run an uninstall program and a setup program in sequence and the uninstall program works with such a child process, you can have conflicting processes running because uninstallation and installation are running at the same time.
...
If you call an ExecWith section with the interpreter 'powershell', then the content of the section will be stored in a temporary file with the extension `.ps1`. (Since Version 4.11.3.1)
...
For 'Registry', 'Files' and `DosBatch` sections we have now the additional calling options:
...
For `DosBatch` you have to keep in mind that any modifiers has to separated by the keyword `winst`. +
Example:
[source,winst]
----
