...
If this command is used wit an empty or invalid path, the default skin dir is used.
...
Abort the script without notice to the server. The action requests remain at setup.
...
* `/RunAsLoggedOnUser` // since 4.11.3.5 ; works only inside 'userLoginScripts'

* `/32Bit` or `/64Bit` or `/SysNative`	 //since 4.11.3.5 +
