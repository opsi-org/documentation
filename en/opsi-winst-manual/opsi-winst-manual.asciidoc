...
* [W/L/M] may be used on Windows and Linux as well
...
* `EnvVar (`<environment variable>`) : string` [W/L/M] [W/L/M]  +
...
* `ExtractFileExtension (`<path>`) : string` [W/L/M] +
interprets the passed String value as file or path name and returns the path part (the string after the last dot ('.'), including it).
...
* `ExtractFileName (`<path>`) : string` [W/L/M] +
...
* `DirectoryExists (`<path> [,<access str>]`) : bool` [W/L/M] +
...
if ($INST_SystemType$ = "64 Bit System")
	set $ConstTest$ = "true"
	Set $tmp$ = "C:\Windows\system32\SecureBootUpdates"
	set $tmp1$ = "64bit"
...
Returns a info map of <wmi class str> that is limited to the properties in <property list> and also limited by the <condition str>. +
...
[source,winst]
----
set $plist$ = createStringList ('Model','Vendor')
set $resultlist$ = getListFromWMI('Win32_ComputerSystem',$plist$,'where model like 'Esprimo%')
----
...
* `node` +
The xml node starts with a open element `<` followed by a identifier and `>` and ends with the clode element `<\` followed by the same identifier and `>` Example: +
`<mynode><\mynode>` +
...
but on a stringlist representation of this file. +
So with the function `getXml2DocumentFromFile(`<path to xml file>`)` you get a stringlist that contains the `xml2document`. This variable may be used to analyze the `xml2document` by other xml2 functions. +
see : <<opsi-winst-xml2-functions>> +
In fact the the `xml2document` is a string list that contains the content of the xml file in a special format and without the header. But do not try to constuct it without using `getXml2DocumentFromFile` or `getXml2Document`.
...
* `setNodeText` <string> +
Set <string> as the `nodetet` of the actual node.
