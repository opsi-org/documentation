...
Over the call from a `opsiServiceCall` section with parameter call `/preloginservice` the standard connection will be restore to previously value. Alternatively
the free section call: +
...
* `/interactive` +
Means that the user must confirm the connection data and enter the corresponding password. This option can not be used in scripts that are to be run fully automatically.

* `/preloginservice` +
Resets the default connection to the value set at startup.

* `/opsiclientd` //since 4.11.2.1 +
calls the webservice from the local opsiclientd and changes the standard connection
