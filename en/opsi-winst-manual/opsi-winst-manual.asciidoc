...
* `getLinuxDistroType`  // 'debian' or 'redhat' or 'suse' [L] <<getLinuxDistroType>>
...
References to some mor Linux specific library function you will find at: <<opsi-script-rc-linux-specific, Linux specific functions>>
...
References to more macOS specific library functions you will find at:  <<opsi-script-rc-macos-specific, macOS specific functions>>
...
I.e. the number can be given as an integer value or as a string expression (cf.  <<opsi-script-string, String Expressions, String Values, and String Functions>>). In the second case, '{opsi-script}' tries to evaluate the string expression as a number.
...
see also: <<opsi-script-skins, Skinnable '{opsi-script}'>>
...
These 'XML2' section and functions replaces the older and Windows only 'XMLPatch' section <<opsi-script-xmlpatch, XMLPatch Sections>>.
