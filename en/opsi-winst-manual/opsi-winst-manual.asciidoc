...
The aim of this expansion is to implement the following concepts:
...
* Freely definable execute parameters:
...
The call parameters can be passed 'CallByValue' or 'callByReference'. 'CallByValue' is the default. That means, if no call method is specified explicitly, then 'CallByValue' will be applied. In the case that 'CallByValue' needs to be explicitly specified, then the keyword `val` should be used. 'CallByValue' means that the content of a variable used during the call is copied to the call variable. +
'CallByReference' must be specified explicitly using the keyword `ref`. 'callByReference' means that a call is created between the variables and local call parameter. Changing the local variable of the call parameter has a direct effect on the variable used during such call. +
The transfer parameters are available as local variables within the function.

* Local Variables: +
A function contains local variables. Implicitly, the call parameters are available as local variables and the variable `$result$` which is from the type of the the returned value. Further variables can be defined within the function. +
...
* Primary and secondary functions within functions: +
The functional body can contain its own sections of it. These are visible to this function only within the section.
...
* 'calltype' is the call type [`val` | `ref`]. If no call type is specified, then `val` will be set.

* 'parameter' is the selected name, freely, of the call parameter which is available as a local variable within the function under the aforementioned name.
...
* `endfunc` displays the end of a function definition as the keyword.
