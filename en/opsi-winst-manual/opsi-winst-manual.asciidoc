...
* When called with option `/allloginscripts` or `/loginscripts` '{opsi-winst}' can do configurations for the logged in user (particularly in a Roaming Profile context). This is a co-funding feature - you need to buy it in order to use it. +
...
'{opsi-winst}' has an adaptable skin. Its elements are located in a subdirectory winstskin of the directory of the executed '{opsi-winst}'. The definition file which you may edit is skin.ini. 

It will look for skins (more exactly: skin.ini) in the following sequence: +
    1) by parameter +
...
* `/AllNTUserProfiles` +
If a PatchTextFile section is called with this modifier, then all directories under `%UserProfileDir%` will be patched, which means that this PatchTextFile is performed for all user profiles. +
When a 'PatchTextFile' is called within a `[ProfileActions]` section, then the modifier `/AllNTUserProfiles` is implicit. In logscript mode, `%UserProfileDir%` will be interpreted as `%CurrentProfileDir%`. +
(Since Version 4.11.3.4)
...
There are some '{opsi-winst}' options recognized:
...
* `/32Bit` or `/64Bit` or `/SysNative`	 //since 4.11.3.5 +
These modifiers control if the path to a called program is interpreted as 32 or 64 Bit Path. So if want for example call a `"%System%\WindowsPowerShell\v1.0\powershell.exe"` you call a 32 bit program by default. If you use the /64bit modifier you will get with the same call the 64 bit version.
