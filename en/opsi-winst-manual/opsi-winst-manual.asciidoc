...
The open source program '{opsi-script}' serves in the context of opsi – open pc server integration (cf. www.opsi.org) – as the central function for initiating and performing the automatic software installation. It may also be used stand alone as a tool for setup programs for any piece of software.
...
needs a running and accessible X Windows.

'opsi-script' is at Linux and MacOS command line version which can be started without any access to a graphical display. Nevertheless this program starts a test if a access to a graphical display is possible and (if it is) starts the GUI version 'opsi-script-gui'. This featur may be supressed by calling `opsi-script` with the parameter `-silent`.
...
Since version 4.11.3, the '{opsi-script}' program contains a manifest with the statement: +
...
4) Read the PC configuration from the opsi service and act accordingly, since '{opsi-script}' 4.11.2
...
In addition, '{opsi-script}' uses the logging directory for saving certain temporary files.
...
Since version 4.11.3.5 the 'opsi-script' searches the skin directory in the following order (directory with the first skin.ini to be found wins):

. `%OpsiScriptDir%\..\custom\winstskin`
