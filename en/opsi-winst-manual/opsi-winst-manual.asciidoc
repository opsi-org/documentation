...
* When called with option `/usercontext` '{opsi-winst}' can do configurations for a specified logged in user (particularly in a Windows Terminal Server or Roaming Profile context).
...
Parameters of a DosBatch section are directly passed as quasi command line parameters to the Windows shell script. +
...
the execution of the Dos-Batch command echo with parameters "Hello" and "World".
