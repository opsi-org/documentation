...
* All config option are documented in the opsiconfd help text ('opsiconfd --help').
* All option starting with '--' can be used in the config file (witout '--').
* opsiconfd now uses its own log rotation. The log rotation can be configured by using the opstions 'max-log-size' and 'keep-rotated-logs'.
* The log file 'package.log' is now part of the 'opsiconfd.log'.
* The logs include a contex by whitch you can filter. For example you can use 'grep' or start opsiconfd with the option '--log-filter LOG_FILTER':
