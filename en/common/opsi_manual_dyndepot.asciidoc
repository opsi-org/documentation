...
The `opsi-package-updater` on the depot servers can be configured as a cronjob.

In the repository configuration files of the `opsi-package-updater` (files with fileending `.repo` under `/etc/opsi/package-updater.repos./`) set the value for `active` to `false` in the repository sections '[repository_...]'.
After that you can configure the section '[repository_master]' in `/etc/opsi/package-updater.repos.d/master-depot.repo`.
Set the value for 'active' to 'true'.
In the same section, set 'opsiDepotId' to the depot ID (FQDN) from which to synchronize.

The `opsi-package-updater` then synchronizes with the specified depot all the packages in the directory `/var/lib/opsi/repository`.
