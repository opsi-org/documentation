...
Therefore in the configuration file of the `opsi-package-updater` (`/etc/opsi/opsi-package-updater.conf`) set in the section '[repository_uib]' `active = false`, and in the section '[repository_master]' `active = true`.
