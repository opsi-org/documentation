...
context after restart, in particular a selected client group. Since version 4.0.7 the session data are used to produce a selection list 
...
gets the top most place, and can be directly used again.
...
Clients can be grouped in a convenient way by using the treeview control placed on the left side of '{opsi-configed}' frame.
