...
It works with the data of a running `opsiconfd` at least of version 4.0.6.
...
With  `java -jar configed.jar --help` you get a listing of the command line option.s
...
The call from the menu entry in the first address should, if the webstart mechanism is installed, be preferred against the call as an applet. 
The reason is that the main windows of an applet is bound into the browser window (this is the only difference between the application running as an applet and 
running by webstart).

The '{opsi-configed}' can be as well installed as a local application which is easier to administrate. The opsi-configed opsi package 
can be found at download.uib.de. Its installation produces an entry in the windows start menue.

At the server the '{opsi-configed}' is installed as part of the opsi-server installation. 

If the server has a GUI  the '{opsi-configed}' may be started using a menue entry or with the command `/usr/bin/opsi-configed`. On non-GUI servers it can be started 
in a console mode which at the moment has as the only useful mode the call with _querysavedsearch_ option. 

If the configed libraries are available the program can just be started by  `java -jar configed.jar`.
...
When starting through Java Web Start you can pass default values.
For this you need to call the address of the JNLP file directly with `javaws`
and then pass the desired parameters.

`javaws 'https://<servername>:4447/configed.jnlp?parameter1=value1&parameter2=value2'`

The possible parameters are the same that '{opsi-configed}' accepts as
commandline arguments - without the dashes.
The set the username to 'Roger' the following call can be used:
...
Accordingly, if you work with different opsi servers, it produces the server/port of your last session as top most entry in the selection list. 
