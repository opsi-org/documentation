...
In the interface, in the overview of the server host parameters, the category _user_ shows the availability of the function (not necessarily active).
In the top level of the properties tree, there is a boolean entry for _user_
...
Furthermore, the default values for user-specific configuration on the server console can also be found here.
...
that means, that this user has no restriction whatsoever. E.g., for a user named _admindepot1_ the entries
...
- depot restrictions do not exist or are unaccounted;
- consequently, the list of depots available to the user can stay empty
(and if a selection from the available depots is entered, this has no effect);
...
- the value "depot1" is to be put into _host.depotaccess.depots_.
...
However, _admindepot1_ him/herself can change that as long as the privilege over _host.opsiserver.write_ is not removed.
