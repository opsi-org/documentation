...
by means of the Linux distribution methods. This version of the program is identical in code with the corresponding local version, but is primarily used
...
If the server has a graphical desktop, the  '{opsi-configed}' can be started as well directly in the server via a desktop menu entry or
...
context after restart, in particular a selected client group. Since version 4.0.7 the session data is used to produce a selection list
...
In practice, little difference is noticed on LAN connections, but relevant differences are noticed on WAN connections, so the Gzip option is enabled by default.
...
(delimited by blanks) then a table line will be a match if any of the input parts is fully contained in some column text.

* Full-text (complete string): the search string is used like googling the search string embraced by citation marks;
i. e. a table line will match if the complete input string is part of some column content.
...
matches, if anything (any number of any characters) come before _iller_ and anything after _iller_.
...
With the version 4.0.7.5 and forth the '{opsi-configed}' includes the user roles function.

In the interface, in the overview of the server host parameters, the category _user_ shows the availavility of the function (not necessarily active).
In the properties tree, there is a boolean entry for _user_
...
Furthermore, the default values for user-specific configuration on the server console can be also found here.
...
The default settings used for the administration of rights are in conformity with the "classic" requirements for an administrator,
that means, that this user has no restriction whatsoever. E.g. For a user named admindepot1 are the entries
...
- _admindepot1_ has _not_ merely read only access to the server (a pure read only access might be appropriate for a help desk staff member);
- Depot restrictions do not exist or are unaccounted for;
- the list of depots available to the user can stay empty
(Also if a selection from the available is entered, this has as long as _depotaccess.configured false_, no effect);
- the user is allowed to edit config server settings of all kinds.

In the case that _admindepot1_ needs to have access to the computers in the depot server _depot1_, the following should be set:

- _host.depotaccess.configured_ is to be set to _true_;
- on the list _host.depotaccess.depots_ the value "depot1" should be set.

After the reload of data, clients from other depots are not visible to the _admindepot1_ user
(And also only the depot settings for _depot1_ are accessible).
However, _admindepot1_ can change that itself, as long as it has the privilege over _host.opsiserver.write_ available.
In order to complete the blocking, is required therefore to set

- _host.opsiserver.write_ to _false_.


CAUTION: If privileges are set this way, they restrict the functionality of the '{opsi-configed}'.
Other access methods to the json-RPC of the opsi-server, will not be effective.
...
=== Server-Console
With version 4.0.7.5 the configed will be extended with a new Menu entry, the "Server-Console". With it, it will be possible from Configed to send commands to the opsi-Server via a SSH-Connection. A Terminal will then be started, and there are also Menu points for certain defaults commands for the opsi-Server.

.'{opsi-configed}': Menü: Server-Konsole
image::configed_serverkonsole.png["{opsi-configed}: Server-Konsole", width=350]


==== Connection data and permisions

Unless otherwise stated, the user connected to the Configed and the associated Configserver will be used.
Should this not be the case, the connection can be also started via an SSH key (possibly with a password) when the Configed starts.
...
This settings can be changed or adjust under the Menu entry "Connection Information". +
...
Activate or Deactivate the Connection settings.

* user.{}.ssh.menu_control.active: +
Activate/Deactivate the command processing.

* user.{}.ssh.menu_main.active: +
Activate Deactivate the whole menu "Server-Console".

* user.{}.ssh.menu_terminal.active: +
Activate/Deactivate the Terminal.

* user.{}.ssh.menus_commands.active: +
Activate/Deactivate all Menu items displaying stored commands.
...
<<opsi-manual-configed-serverkonsole-commandcontrol>> - Item: Datasources)
...
//As predefined commands in the menu group "opsi", there are four commands which require different parameters during the runtime (marked by "...").
...
* Download from data ... +
Any data file can be downloaded from the Internet can be download with the "wget"-command and stored in a certain path on the Server. This could be used for example to download opsi-packages from download.uib.de

* Creation from an opsi product file ... +
Prerequisite for this command is an opsi-utils package with version >= 4.0.8. Using this menu item, an opsi package can be created, specifying the directory upon which must be stored. In addition, can the found versions (package and product version) be displayed and overwritten with a button. And also, a md5sum and/or a zsync file can be created.
...
If you want to add existing computers to opsi, the opsi-client-agent must be installed on the target computer. If you select the clients in the configed and call this command, the client names are copied into the corresponding field. If the command is to be executed on several clients in a single call, the login data must be the same on all the participating computers. The button to the right of the password window shows the password in plain text or also replaced with asterisks (*). +
Detailed information can be found in the 'opsi-getting-started' manual in the 'First steps' chapter.
...
The prerequisite for this is that the python-opsi package is installed and is at least in version 4.0.8, since appropriate methods are only provided after this version (see <<opsi-manual-configuration-tools-opsi-admin>>).  +
...
* Menu-Text: +
When creating a new command you _must_ make sure that the menu text has not been used already for another command. If a menu text is to be changed, the command must be first deleted with the minus button, and then the new command can be created.
