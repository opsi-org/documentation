...
It is possible also to only install specific packages, i.e. the packages for the update of Windows 7:
[source,prompt]
----
...
Prior to opsi 4.0.5 one could download the opsi-packages and install
them via `opsi-package-manager -p ask -i <packagename>.opsi` and
set the default properties for the '{opsi-configserver}'.
(Without the option `-p ask` the package defaults would be the depot defaults).
...
CAUTION: Windows 10 1511 is not supported anymore since 201712.
...
- Microsoft .NET Framework 4 and above for OS Windows 7 and 8.1
...
----
adobe.reader.dc.classic	20151500630394or20171701130070-1          Adobe Acrobat Reader dc
----
