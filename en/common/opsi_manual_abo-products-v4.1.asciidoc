...
It is then advisable to install only the packages from the new repositories:
[source,prompt]
----
# Install packages for mshotfix
opsi-package-updater --repo uib_abo_mshotfix install
# Install packages for msoffice
opsi-package-updater --repo uib_abo_msoffice install
# Install packaes for standard products
opsi-package-updater --repo uib_abo_standard install
----

It is possible also to download specific packages:
...
An equivalent approach ist the usage of for example an `/etc/opsi/opsi-package-updater.conf.abo.standard` which can  encalled via `opsi-package-updater -c /etc/opsi/opsi-product-updater.conf.abo.standard`
...
On new Windows installations (> Windows 7) you can also install older versions of Dotnet that are not included in the operating system installation.
...
values: ["3.5", "4.0", "4.5", "4.5.1", "4.5.2", "4.6", "4.6.1", "4.6.2", "4.7.-3","all"]
default: ["all"]
