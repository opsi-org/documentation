...
This opsi extension is currently in the co-funding process and is not free.
...
image::../images/opsi-configed-mode-selection.png["top right of opsi-configed interface: Menu bar with button licenses (rightmost)", width="50mm"]
...
The module will be available to the community when all the development costs have been recuperated.
...
Therefore, the first tab of the '{opsi-configed}' license management window is the management '{license-pool}s' tab.
...
The table at the top of the window contains the available '{license-pool}s'. 
...
A more complicated situation might occur while installing software that requires licenses from several '{license-pool}s' (i.e. "Designer tools" which installs 'Adobe Photoshop' as well as 'Acrobat Writer'). 
