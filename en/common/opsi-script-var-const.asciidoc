...
`%opsiserviceUser%` : The user ID for which there is a connection to the opsi service. If running in opsi service context this is usally the client FQDN used by opsi.

`%opsiserviceClientId%` : At start of opsi-script in opsi service context it is set to the command line option `/clientid`. So it will be the FQDN of the client in opsi service context and it will be empty without opsi service context.
