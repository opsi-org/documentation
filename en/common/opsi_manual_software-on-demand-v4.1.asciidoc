...
image::opsi-client-kiosk_hauptfenster.png["Main window with product tiles", width=500]
...
image::opsi-client-kiosk_suchfeld.png["Search field of the kiosk", height=60]
...
image::opsi-client-kiosk_produktansicht.png["Detailed product view", width=500]
...
image::opsi-client-kiosk_dialog_bestaetigung.png["Dialog for confirmation of the requested action"]

[[ock_image_dialog_timechoice]]
.Kiosk - Dialog to select the starting point of the requested action.
image::opsi-client-kiosk_dialog_zeitpunkt.png["Dialog to select the starting point of the requested action"]
...
[[ock_image_help]]
.Kiosk - Help window. (1)&#160;Info block. (2)&#160;Opsi manual. (3)&#160;Checkbox for expert mode
image::opsi-client-kiosk_hilfe.png["Help window"]
...
.Kiosk (expert mode) List view. (1)&#160;Check box to toggle view. (2)&#160;List view of the products. (3)&#160;Button for installing (as from now) products. (4)&#160;Button for synchronisation with the server. (5)&#160;Detailed informationen of the product.
image::opsi-client-kiosk_expertenmodus.png["Kiosk - Experte mode (List view)", width=500]
...
image::opsi-client-kiosk_aktionsfenster.png["Window for the execution of the actions set"]
...
A click on the button&#160;(4) opens a dialog to save the icons/screenshots on an opsi depot which means that these icons/screenshots are than also available for other kiosk installations.
...
CAUTION: After setting icons and screenshot within the kiosk the icons and screenshots are saved on the client only locally. To make these images available for kiosk installations on other clients they had to be copied to the ock-custom folder (e.g with the kiosk tool as described above).
