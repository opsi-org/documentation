...
The Linux related Localboot and Netboot products can be loaded over the opsi-package-updater. +
To that end, you have to make sure that in the `/etc/opsi/opsi-package-updater.conf` the corresponding directories are listed. The easiest way is, when you find the following section in your configurations file: +
[source,ini]
----
