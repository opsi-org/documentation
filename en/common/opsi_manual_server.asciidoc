...
* +[global] max log size+: +
This option limits the size of logfiles created through the 'opsiconfd'. Due to an historic reasons this is limited to 5MB per logfile. Since opsi 4.0.6 the size can be configured. To disable any size limits the value can be set to 0.
