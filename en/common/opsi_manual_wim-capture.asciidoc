...
For reading the OS partition and writing into the WIM file, we have to use a Windows OS, which cannot be the Windows OS we're reading (for obvious reasons). Therefore, we use the Windows PE we created and preserved at the initial installation.
