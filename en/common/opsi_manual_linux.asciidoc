...
CAUTION: An own repository created from an UCS 4.2-0 DVD leads to an uninstalable state. The package `depootstrap` on this DVD is not able to install the system without interaction. However our provided repository is not affected by this.
...
Download the installation DVD here, e.g. from link:http://isoredirect.centos.org/centos/7/isos/x86_64/[here].
