...
For a long lasting maintenance of the installed Linux computers you need after the 15 free starts the activation of the feature, in order to continue the usage.
...
[source,ini]
----
[repository_uib-linux]
baseUrl = http://download.uib.local
...
* Currently there is no UEFI support available. The informationen about UEFI gets lost during kexec boot. We hope to fix this with a future bootimage.
...
The installation file must have extension `.iso`, the name of the file does not matter. If there are several `.iso` files in the same directory, it is not specified which one to use.
...
To donwload the installation DVD you need an account on RedHat.
...
The script can work with IP addresses, host names and FQDNs. It will automatically try to recognize what kind of Adress was passed.

The script can be found at '/var/lib/opsi/depot/opsi-linux-client-agent' +
Run the script with root privileges. +
It could happend that you must first make the script executable with: +
