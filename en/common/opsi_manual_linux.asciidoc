...
* `online_repository`: +
distribution repository for the installation. (Not for SLES) (Default=distribution dependent)

* `opsi_online_repository`: +
repository of opsi-server packages. (Default=distribution dependent)

* `proxy`: +
Proxystring (if required) as: `http://<ip>:<port>`. (Default=pass:[''])

* `install_opsi-client-agent`: +
Install the Linux opsi-client-agent (co-funded project: a module file is required for activation. (Default='false')
