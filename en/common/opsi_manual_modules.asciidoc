...
When you got an activation file, copy it with 'root' privileges into the directory `/etc/opsi` (and rename it to `modules` if necessary). When this is done, execute:
