...
The content and structure of these files is explained in more detail in the <<opsi-manual-important-files-filebackend>> chapter.
...
image::mysql-schema-inventory-software.png["database schema: software inventory",width=400]
...
image::mysql-schema-configdata.png["database schema: configuration data",width=400]
...
Then a password must be set for the root access of mysql:
[source,prompt]
----
...
Manual configuration can be done via the backend configuration file.
...
* +broadcastAddresses+: +
Assignment of broadcast addresses and ports to be used for sending Wake-on-LAN broadcasts.
...
The `opsi-convert` command is used to convert the opsi configuration data between different backends. The destination or source can be assigned in different ways:
...
By specifying a configuration directory for the corresponding backend manager configuration, the source or target can be described in great detail.
+
[source,prompt]
----
