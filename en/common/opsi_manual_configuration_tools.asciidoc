...
- `--verbose` increases the amount of shown messages. This can be given multiple times to increase verbosity.
- `--repo <name of a repository>` limits the actions to the given repository. Available repositories can be shown with `repo --list-active`.

The different modes result in different behaviour.
The modes `install`, `update` and `download` load packages from a repository, whereas `repo` is mostly used to retrieve information about the packages in the repositories.

The mode `install` installs new packages.
The mode `update` updates already installed products.
...
Through `repo --list-active` the active repositories are shown.
The information consists of name, address and if applicable the description of the repository.

Through `repo --list-products` the available products per repository are shown.

To show the possible updates use `repo --list-updatable`.
...
This is in most cases on a '{opsi-depotserver}' the central '{opsi-configserver}'. So on any call of the 'opsi-package-updater' the '{opsi-product-packages}' will be fetched from the '{opsi-configserver}'. This can be done for example by a cronjob.
