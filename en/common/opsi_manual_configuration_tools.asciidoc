...
Grafana renders 'opsiconfd' actual tasks, sessions, load and cpu/memory utilization in a graphic time chart, besides other information.
The grafana dashboard can be reached at the following URL https://<opsi-server>:4447/admin.

.opsiconfd info: opsiconfd values from the last 3 hours
image::opsiconfd-grafana-hour.png["opsiconfd info: opsiconfd values from the last 3 hours", width="120mm"]

.opsiconfd info: opsiconfd values from the last day
image::opsiconfd-grafana-day.png["opsiconfd info: opsiconfd values from the last day", width="120mm"]
