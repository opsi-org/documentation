...
The command line utility `opsi-package-updater` is designed to download and install comfortable opsi packages from a repository or a other opsi server. Using the opsi-package-updater make it easy to keep the opsi server up to date. It may be also used in a cronjob to keep depot server in sync with the config server.

[source, prompt]
----
# opsi-package-updater --help

Usage: opsi-package-updater [options]
Options:
