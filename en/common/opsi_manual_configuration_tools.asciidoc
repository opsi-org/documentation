...
.'{opsi-configed}':  mask: client selection 
image::../images/configed_clientselection.png["{opsi-configed}:  mask: client selection", width="73mm"]


With the function 'Selection / Define Selection' you can build client groups by certain criteria (e.g.: all clients which have the product 'firefox' with the installation status 'installed').
There are critertia about the host, installed software and hardware. In text edit fields, you may use '*' as wildcard. Criteria can be connected using logical "and" and "or".
Negation is possible, too.

.'{opsi-configed}': saved searches
image::../images/configed_clientselection2.png["{opsi-configed}: saved searches"]

If you want to reuse a search, you can save it. There are two ways to run saved searches. First, they can be accessed with the function 'Selection / Saved Searches...'. Second,
you can access them by adding the parameter "-qs" and the name of the search to the call of the configed application. If you omit the name of the search, you will get a list of available searches.
The second approach is especially useful for the inclusion in scripts.
