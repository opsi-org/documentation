...
In version 4.2 _interface_ and _info page_ are merged into the new _admin page_.
To open this page you can call 'https://<opsi-server>:4447/admin' in your browser.
You have to login as a member of the group 'opsiadmin'.

The first tab _Blocked Clients_ shows a list of all blocked clients. Clicking the Button _unblock_ releases this client. You can unblock clients by IP or you can unblock all clients.

With _Delete client sessions_ you can delete all sessions of a client.

As response to the user a text and the JSON-Response from the server are printed. 


.'opsiconfd': Blocked Clients
image::opsi-webinterface-blocked-clients.png["opsiconfd: Blocked Clients", width=400]


The tab *RPC-Info* shows a table with the last RPC calls. Sorting is posible by clicking the table headers.

.'opsiconfd': RPC list
image::opsi-webinterface-rpc-list.png["opsiconfd: RPC list", width=400]


The *RPC-Interface* tab contains the former _interface page_. With the interface you can make API calls. The request and the response are printed as JSON.

.'opsiconfd': RPC interface
image::opsi-webinterface-rpc-interface.png["opsiconfd: RPC interface", width=400]

The *Redis interface* can be used to make redis calls. The response from the server is printed in JSON.

.'opsiconfd': Redis-Interface
image::opsi-webinterface-redis.png["opsiconfd: Redis-Interface", width=400]
