...
* 'IP address' shows the IP-Number.
* 'Inventory No' shows the (optional) given Inventory Number.
...
* 'opsi mac address' shows the MAC of the client

You may activate these columns using the context menu. The configuration which columns are activated may be changed using the 'host parameter' 'configed.host_displayfields'.


.'{opsi-configed}': Button 'Check wich clients are connected'
image::../images/opsi-configed-connected.png["{opsi-configed}: Button: Check wich clients are connected", width="48"]
...
The option 'Remote Control Software call' in the client context menu as well as the client main menu is very powerful. It can be used
to use any command that the operating system offers, parametrized e.g. by the client name. The list of commands that can be called 
is editable as server configuration entries (cf. 
...
image::../images/opsi-configed-client-remotecontrol-choice.png["{opsi-configed}: Choice of Remote Control call", width="100mm"]
...
==== 3.3.13.  Logfiles: Logs from client and server
...
[[opsi-manual-configuration-tools-opsi-configed-hostproperties]]
==== Host parameter at the client and the server configuration
