...
* *config_updates* had a change with the 4.0.7 release of the 'config-win10' package. The setting 'disable' moves into an own property, called 'disable_updates'. *config_updates* gives the possibility to influence the source of updates. One can selest the official Microsoft servers, a local peer-to-peer, or peer-to-peer connection over the internet. +
...
* *disable_wifi_sense*: 'WiFi Sense' is a service to share known wifi configurations with contacts. 'true' disables this service, whereby 'false' enables it. +
