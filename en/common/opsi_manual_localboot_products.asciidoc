...
However the behaviour of the config_updates property changed. Initially this property featured a disable option. This options was a hack to not search for updates. With the 4.0.7 release this option disappapeared and a seperate disaple_updates property is now available. setting this property to true disables all connections to Micrsoft Updates Services. Therefore not only updates shall be disabled but also upgrades.
