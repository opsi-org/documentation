...
The +sync completed+ is running until the next reboot, or until it is canceled by another event (e.g. on_demand). In the latter case the config cache is marked invalid (which implies that the config has to be resynced - in case of changes) and the other event is processed.
