...
Which protocol is to be used, can be configured client specific by the '{opsi-config-object}' +clientconfig.depot.sync_protocol+.
Available values to be set as +clientconfig.depot.sync_protocol+ are +cifs+ and +webdav+.
If this '{opsi-config-object}' does not exist, the value of +clientconfig.depot.protocol+ is used.
