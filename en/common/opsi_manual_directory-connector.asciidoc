...
With `ski_adding_clients` the creation of clients in opsi can be skipped completely. This option can be used in conjunction with `prefer_location_from_directory`, for example, so that only existing clients are moved.
