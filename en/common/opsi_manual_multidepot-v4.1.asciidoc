...
If the new depot should only update its packages from the main server, only the repository in `/etc/opsi/package-updater.repos.d/opsi-server.conf` should remain active.
