...
In order that the distribution of software does not become unmanagable, a client PC has to notice independently that new software packages or updates are available and install them.
...
This directory contains all programs of the '{opsi-client-agent}' such as '{opsiclientd}', '{opsiclientd-notifier}', '{opsi-script}' and some libraries. The configuration files and graphical templates of the mentioned programs can also be found here. +
...
Furthermore, there is the directory `c:\opsi.org`.

This directory is currently used for caching installation files and data (see WAN extension). This directory will have additional functions in the future. +
The directory `c:\opsi.org` can only be accessed with administrative rights.

Log files of the '{opsi-client-agent}' are located in `c:\opsi.org\log\`.
...
* '{opsiclientd-notifier}': Window for information / communication with the user
...
* If needed, it connects to the '{opsi-depot}' to update the local installation of the '{opsi-script}' and then starts it to process the '{action-requests}' (software packet installations).
...
For an examplary "working_window=20:00-07:00" the opsiclientd log would look like this:
