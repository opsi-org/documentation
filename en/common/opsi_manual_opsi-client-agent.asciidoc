...
The '{opsi-login-blocker}' for NT6 (Vista/Win7/Win10) is implemented as a 'credential provider filter' ('OpsiLoginBlocker.dll').
...
NOTE: The 'working_window' feature requires '{opsi-client-agent}' >= Version 4.1.0.0-39.
...
The files to be configured for opsi-script are to be found in the directory `/var/lib/opsi/depot/opsi-client-agent/files/opsi/opsi-winst/winstskin`:
...
* `custom/winstskin/*.*` +
All the files from this directory will be copied to the clients `C:\Program Files (x86)\opsi.org\opsi-client-agent\custom\winstskin` directory during installation of the opsi-client-agent on the client. This `winstskin` directory, if it exists, since opsi-script Version 4.11.3.4. is the preferred one. It must contain all required winstskin files and configurations, for the content of the default directory is ignored.
...
===== opsi loginblocker at NT 6 (Vista/Win7/Win10)
