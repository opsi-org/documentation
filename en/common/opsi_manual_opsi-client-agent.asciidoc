...
If software is to be installed, the script-controlled installation program '{opsi-winst}' is started.
...
In order for software packages to be installed without interaction with the program '{opsi-winst}', they have to be prepared for this.
...
This directory contains all programs of the '{opsi-client-agent}' such as '{opsiclientd}', '{opsiclientd-notifier}', '{opsi-winst}' and some libraries. The configuration files and graphical templates of the mentioned programs can also be found here. +
...
Just like with '{opsi-winst}', it is now also possible for reboots triggered by events of {opsiclientd} to suppress the password input when booting.
...
[source,prompt]
----
opsi-admin -d method config_createBool opsiclientd.global.w10BitlockerSuspendOnReboot "Suspending Bitlocker at Reboot" false
----
...
[source,prompt]
----
opsi-admin -d method configState_create opsiclientd.global.w10BitlockerSuspendOnReboot myclient.domain.de true
----
...
There are three files which are very relevant:

* `header.png`
* `logo.png`
* `opsiclientkiosk.ini`

The `header.png` file contains the image which will be loaded into this particular area. The `logo.png` contains the logo that will be displayed at the right side of the `header.png`. +

The `opsiclientkiosk.ini` defines the text and its presentation in this particular area.
...
You can have the design of the `header.png`, `logo.png` and the `opsiclientkiosk.ini` placed in the ock_custom directory (which can be found either under `/var/lib/opsi/depot/opsi-client-agent/files/opsi/opsiclientkiosk/ock_custom` or under `/var/lib/opsi/depot/opsi-client-agent/files/opsi/opsiclientkiosk/app/ock_custom`), afterwards they will stay there even after the installation of a new version.

The files placed on the ock_custom directory overwrite the default appearence. If you want to have the default appearence once again, then you'll have to delete the corresponding files in the ock_custom directory.

The `custom` directory (`/var/lib/opsi/depot/opsi-client-agent/files/opsi/custom`) doesn't play a role in this since version Version 4.1.1.1
