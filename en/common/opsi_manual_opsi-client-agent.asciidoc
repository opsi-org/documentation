...
To limit the functionality of an event to a configurable time frame a so called 'working_window' can be configured for all events.

NOTE: The 'working_window' feature requires '{opsi-client-agent}' >= Version 4.1.0.0-37.
...
NOTE: Starttime and endtime must be entered in the format hh:mm and are separated by a hyphen. Whitespaces between starttime and endtime are not allowed!
...
If the starttime is higher than the endtime the 'working_window' will be valid over the nightly day change. +
...
There is a helper program set allows you simply to check if a given bitmap file will be displayed correct or not: link:http://download.uib.de/opsi4.0/helper/opsi_bitmap_viewer.exe[opsi bitmap viewer].
