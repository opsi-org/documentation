...
Dabei spielen drei Dateien eine Rolle:

* `header.png`
* `logo.png`
* `opsiclientkiosk.ini`

Die `header.png` enthält das Bild welches in diesen Bereich geladen wird. Die `logo.png` enthäht das Logo welches rechts von der `header.png` angezeigt wird. +

Die `opsiclientkiosk.ini` definiert den Text und dessen Darstellung in diesem Bereich.
...
Sie können nach ihren Vorstellung gestaltete `header.png` ihr `logo.png` und die `opsiclientkiosk.ini` in das ock_custom Verzeichnis (befindet sich entweder unter `/var/lib/opsi/depot/opsi-client-agent/files/opsi/opsiclientkiosk/ock_custom` oder unter `/var/lib/opsi/depot/opsi-client-agent/files/opsi/opsiclientkiosk/app/ock_custom` ) hinterlegen, dann sollten diese auch nach Einspielen einer neuen Version dort wieder zu finden sein. 

Im ock_custom Verzeichnis hinterlegte Dateien überschreiben das Default-Aussehen. Wollen sie wieder das vorgegebene Default-Aussehen haben müssen sie die entsprechenden Dateien im ock_custom Verzeichnis löschen.

Das `custom` Verzeichnis (`/var/lib/opsi/depot/opsi-client-agent/files/opsi/custom`) hat für den Kiosk seit Version 4.1.1.1 keine Bedeutung mehr.
