...
. Integration of capturing installations to WIM
...
The restore must be completed within reliable short time (about 15 minutes) and should also be able to switch the workstations to a differnet base installation (like Win XP / Win 7 / Linux). Also the continuous system maintenance by installing security updates must be included.

The standard approaches for the system maintainment have different advantages and disadvantages:

.Advantages and disadvantages of the Unattend and Image based solutions
[options="header"]
|==========================
...
The concept of opsi-local-image is trying to combine the advantages of the different approaches and to avoid the disadvantages. 
...
| Feature | Unattend 
| Performance | (+) fast
| Sensitivity to heterogenous Hardware | (+) low
| Network load | (+) low
|==========================
...
** `ask_before_inst`: +
Should we ask at the client before start. (Default='true')
...
. Inital prepare with `opsi-local-image-prepare`
...
. `opsi-local-image-sysprep` starts (according to it's product properties) an `opsi-local-image-backup` run
. `opsi-local-image-sysprep` runs sysprep in order to depersonalize the OS installation
. `opsi-local-image-sysprep` starts (according to it's product properties) `opsi-local-image-capture`
. `opsi-local-image-capture` configures the winpe partition for boot and capture and reboots
. The PC boots from winpe and will do the capture process. The log is appended to the `opsi-local-image-capture` bootimag log
...
[NOTE]
===============================
If `always_backup_before_sysprep`='true'  the product `opsi-local-image-backup`
will be set to 'setup' and a reboot is done. +
