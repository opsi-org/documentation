...
To install those products please set the attribute *active* of the repository *uib_local_image* in the file `/etc/opsi/opsi-package-updater.conf` to 'True'.
Executing `opsi-package-updater -i -vv` will then install the new products.
