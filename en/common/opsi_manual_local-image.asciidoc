...
IMPORTANT: until further notice this extension module can only be used by public authorities (like schools / colleges / universities).
...
The netboot products for the operating system installation use the first two partitions (XP the first one only) and do not touch partiton three. So the backup images on partiton three are still available after installating a new operating system.
...
image::../images/oli-prepare.png["schema: static partitioning with opsi-local-image-prepare", width=332]
...
image::../images/oli-os-install.png["schema: OS installation with opsi-local-image-win*", width=332]

Per default after the installation they will be backuped as image.

.schema: image backup with opsi-local-image-backup
image::../images/oli-backup-1.png["schema: image backup with opsi-local-image-backup", width=332]
...
image::../images/oli-restore-image.png["schema: image restore with opsi-local-image-restore", width=332]
...
So with the product 'opsi-local-image-sysprep' by special properties can be configured, whether there has to be crated a backup before depersonalisation.: +
`always_backup_before_sysprep` (true/false) default=true: always make a backup before starting the sysprep. +
`abort_on_no_backup` (true/false) default=true: check whether the backup partition has a backup for this product and abort if not. +
After the sysprep usually the actual capture will be started, which is performed by the netboot product 'opsi-local-image-capture'. Whether it starts automatically is set by the property: +
`startcapture` (true/false) default=true: set the product `opsi-local-image-capture` to 'setup' and reboot the client
...
Within this multi-stage process the existing client will be readout and integrated as WIM into an existing opsi Windows installation product. SO there is an product property to define, in which product the image readout is to be integrated: +
`target_product` default='''' : name of target product. +
*Attention*: this property isn't smart, so it does not check whether the image matches the specified target. So you could by mistake integrate an win7-32Bit into a Win81-64Bit product without getting an error or warning message. So you must take care not to do so! We recommend using special capture target products (like `opsi-local-image-win81-x64-capture`). +
...
*Attention*: In case the `install.wim` contains an image with the same name it will be *deleted without notice*. With `always_create` a new `install.wim` will be created. +
...
The property `start_after_capture` is a list of products, which have to be set to 'setup' after the capture. This could be for instance 'opsi-local-image-restore', whioch restores the client from the backup that had been generated before the sysprep.
...
image::../images/oli-capture-sysprep1-backup.jpg["capture images: opsi-local-image-sysprep 2 : backup before sysprepp", width=350]

.capture images: opsi-local-image-sysprep 3 : deactivating the opsi-client-agent
image::../images/oli-capture-sysprep2a.jpg["capture images: opsi-local-image-sysprep 3 : deactivating the opsi-client-agent", width=350]

.capture images: opsi-local-image-sysprep 4 : actual sysprep
image::../images/oli-capture-sysprep2b.jpg["capture images: opsi-local-image-sysprep 4 : actual sysprep", width=350]
...
image::../images/oli-capture-scheme-capture1.png["capture images: sSchema: capture 1", width=500]
...
image::../images/oli-capture-del-existing-image.jpg["capture images: capture: delete an existing image", width=350]

.capture images: capture: capture and append the new image
image::../images/oli-capture-append-image-dism.jpg["capture images: capture: capture and append the new image", width=350]
