...
*Attention*: In case the `install.wim` contains an image with the same name it will be *deleted without notice*. With `always_create` a new `install.wim` will be created. The `install.wim` file is a container that can hold several images. The images have a name and a description. The name and the description of the new created images is set by these properties:
