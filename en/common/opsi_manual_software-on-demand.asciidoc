...
.opsi-client-kiosk: Main window with tiles (Expert Modus)
...
.opsi-client-kiosk: Main window with lists (Expert Modus)
image::opsi-client-kiosk_list_expert_en.png["opsi-client-kiosk: Main window with lists (Experten Modus)", width=400]
...
. Reload the data (e.g., after actions have been perfomed)
. Search maks (filter input field)
. Product display
