...
Since tne Kiosk version 4.1.1.11 and opsi-client-agent version 4.2.0.11-1 it's now possible to configure which event (`auto`, `timer` or `on_demand`) should be triggered via the Kiosk for installations. 
...
`auto` is the default value. Other possible values: `on_demand` and `timer`. If `auto` is set then `timer` is used for WAN/VPN clients and `on_demand` for other clients.
