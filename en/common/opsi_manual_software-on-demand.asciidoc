...
The installation may be controled by the following 'opsi-client-agent' product properties:
...
The used icon for the start menue / Desktop may be modified by storing a `kiosk.ico` file at
...
image::opsi-client-kiosk_tile_simple_en.png["opsi-client-kiosk: Hauptfenster mit Kacheln (Default Modus)", width=400]

Elements:

. Headerlist (Customizable to the specification of the customer)
. Button to start the requested installations
. Button to end the program
. Checkbox to activate the expert modus (per Default not active)
. Search mask (Filter input field)
. Tiling from the opsi packages
...
Using the checkbox 'Expert mode' (4) additional control elements are displayed.


.opsi-client-kiosk: Hauptfenster mit Kacheln (Experten Modus)
image::opsi-client-kiosk_tile_expert_en.png["opsi-client-kiosk: Hauptfenster mit Kacheln (Experten Modus)", width=400]
...
. Search maks (filter input field)
...
Via the checkbox 'Expert mode' (7) additional control elements will be displayed.


.opsi-client-kiosk: Hauptfenster mit Listen (Experten Modus)
image::opsi-client-kiosk_list_expert_en.png["opsi-client-kiosk: Hauptfenster mit Listen (Experten Modus)", width=400]
...
. Headerslist (Customizable to the specification of the Customer)
. Info Button: Version and loaded language
. Data load display
. Button to display the set actions
. Button to save and display the set actions
...
This windows appears only in the 'expert mode' after using the 'Store Actions' button. +
