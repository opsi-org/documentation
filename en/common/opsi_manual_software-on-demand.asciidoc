...
NOTE: Since Version >= 4.1.1.11 the kiosk uses the caching and synchronization methodes of the WAN/VPN-client if running on a WAN/VPN-client. Hence the kiosk can be used on an WAN/VPN-client, too. Therefore the config-variable `software-on-demand.installation-now-button` had to set to `false`. Otherwise display and messages of the kiosk are not in sync with the current client status.   
...
|software-on-demand.admin-mode|Enables specific functionality for admistrative purpose/setting icons/screenshots. Should be enabled client-specific if needed|true/false
...
CAUTION: Using the kiosk (Version>= 4.1.1.11) on an WAN/VPN-client the config-variable `software-on-demand.installation-now-button` had to set to `false`. Otherwise display and messages of the kiosk are not in sync with the current client status.
