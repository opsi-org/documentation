...
NOTE: Starting from Version >= 4.1.1.11 the kiosk uses the caching and synchronization methods of the WAN/VPN-client if running on a WAN/VPN-client. Thus the kiosk can be used on a WAN/VPN-client too. Therefore the config-variable `software-on-demand.installation-now-button` has to be set to `false`. Otherwise display and messages of the kiosk are not synchronize with the current client status.  
