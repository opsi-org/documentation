...
* `log_write` or `writelog` +
Writes a opsi log file to the server. +
Parameters: `logType data *objectId *append` +
Logtypes and objectId see above, `append` (true/false) (Default = false) should the log be appended to an existing log. There is an internal maximum size of 5 GB for theses log files.
