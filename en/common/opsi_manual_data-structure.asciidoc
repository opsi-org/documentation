...
Describes the detected hardware types (including the client specific values). The idea is that you will see here the client specific data and in `auditHardware` only one entry for a network card which is used in all your computers. +
...
This is in fact also a storage object, but it is a little aside of the standard.
It tells us to which depot a client is assigned.
...
'hostControl_reboot'. So with opsi 4.0.3 we breaked the backward compatibilty and now a empty `hostIds` is not any more allowed for the 'hostControl_reboot' and 'hostControl_shutdown' methods.
...
Connect the opsiclientd of the given `hostIds` but do not a login. +
Parameters: `hostIds`
...
Shows a popup message on the clients. +
...
This is the only 'hostControlSafe' method that not use the opsiclientd. +
Parameters: `hostIds`
...
Opsi does distinguish between groups of the type 'ProductGroup' and 'HostGroup'. The first is used for product groupings and the latter is used for grouping hosts.
...
IMPORTANT: In opsi 4.0 groups are identified by their ID. This ID must be unique throughout opsi's groups.



To create a first group from the commandline we can now issue the following comand:
[source,prompt]
----
...
Opsi has a default group that behaves like a directory service - i.e. OpenLDAP - in terms that a client can only be member of one group. There is a root group with the ID *clientdirectory* that assumes that exact behaviour for any group / client inside. Any client not in a subgroup of *clientdirectory* will be moved to another special group with the ID *NOT_ASSIGNED*.
...
To check if the creation was a succes we can execute the following command:
[source,prompt]
----
opsi-admin -d method objectToGroup_getObjects '' '{"groupType": "HostGroup", "groupId": "rechner_wenselowski2"}'
----

If for some reason we want to remove a client we can do this aswell. Just execute the following:
[source,prompt]
----
...
These methods are still available and will be still maintained.
...
The API uses http://json-rpc.org/wiki/specification[JSON-RPC 1.0] for
communication.

To use this interface *POST* your calls to the path `rpc` of your opsi
server, i.e. `https://opsiserver.domain.local:4447/rpc`.


NOTE: Communicating with an opsi 4.0 webservice leads to a HTTP header
"content-type" that does not match the real content encoding.
Since release of opsi 4.0.6 it is possible to activate a RFC 2616
compatible behaviour by creating the file
`/etc/opsi/opsi.header.fix.enable` and restarting opsiconfd.
This makes it easier to communicate with the server through third party
applications.
Because this changed behaviour may lead to communication failures with
clients expecting the old behaviour this is disabled by default.
The client components provided with opsi 4.0.6 have been updated so
