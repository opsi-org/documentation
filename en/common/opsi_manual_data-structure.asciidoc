...
[
          ...
          "description" : "Should the jedit server started at every startup ?",
          "editable" : false,
          "defaultValues" :
                    [
                    false
...
Unfortunately in reality this idea doesn't work as you might think.
...
Describes the detected hardware types (independent from client specific values). The idea is that you will see here only one entry for a network card which is used in all your computers. +
Unfortunately in reality this idea doesn't work as you might think.
...
compatible behavior by creating the file
...
applications.
Because this changed behavior may lead to communication failures with
clients expecting the old behavior this is disabled by default.
