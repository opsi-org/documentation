...
WARNING: For the products `opsi-wim-capture` and `opsi-local-image-capture` the share `opsi_depot_rw` must be have write permission for 'pcpatch'. For the product `opsi-clonezilla` the share `opsi_images` must have write permission for 'pcpatch'
