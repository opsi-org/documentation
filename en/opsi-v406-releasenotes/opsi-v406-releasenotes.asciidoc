...
** PLEASE NOTE: The new pxelinux.0 does not draw points while loading the booimage anymore. So you have to wait for the start of the bootimage without visual feedback.
...
It does enable a configuration of the lockscreen and hibernation boot behavior. +
Most important is the possibility to enable or disable personal data gathering. +
This package configures the Microsoft Update behavior. +
Additionally it gives the ability to enable or disable the following features: +
telemetry, cortana assistant, malware removal tool and the customer experience program.
