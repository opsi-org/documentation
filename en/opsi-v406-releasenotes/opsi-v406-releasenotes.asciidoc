...
** opsi-configed provides export of data tables as PDF files (e.g. for PDF viewer).

** The opsi-configed hardware inventory page provides automated PC specific driver integration. +
Optional the ByAudit driver path will be created according to the requirements of the bootimage. +
Afterwards driver files can be copied to the config server per opsi-configed. +
Suitable file access permissions of the driver files are set automatically.

** opsi-configed has improved data access performance for the MySQL backend.

** On the opsi-configed logging pages the search function has been improved.
The search starts now from the current cursor position +
(and not every time from the start of the file). +
Search strings are saved in a combo box.

** The opsi-configed software inventory page now provides a search function. +
Per default updates for Microsoft Windows are excluded from the view. +
They can be included through a switch.

** A bug of the opsi-configed license statistics has been fixed, +
which under certain conditions had resulted in a license count too low.
