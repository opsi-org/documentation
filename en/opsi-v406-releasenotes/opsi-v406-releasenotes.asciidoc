...
Optional the ByAudit driver path will be created according to the requirements of the bootimage. Afterwards driver files can be copied to the config server  per opsi-configed. Suitable file access permissions of the driver files are set automatically.

** opsi-configed has improved data access performance for the MySQL backend.

** On the opsi-configed logging pages the search function has been improved. The search starts now from the current cursor position (and not every time from the start of the file). Search strings are saved in a combo box.

** The opsi-configed inventory page now provides a search function for the data table. Per default MS updates are excluded from the view. Optionally they can be included.

** A bug of the opsi-configed license statistics has been fixed, which under certain conditions had resulted in a license count too low.
...
** New feature to exclude or activate specific packages for selected events only. This feature is managed by opsi-configed product groups.

** New property `depends_on_service`: +
This is a list of services that the opsiclientd depends upron. They must be running before the opsiclientd can start. The default is: `Dhcp, Dnscache` +
...
are not maintained anymore. Also in the near future the Update Abos will not support these Windows versions anymore. Please contact us in case of any questions or concerns.
...
This package can configure Windows 10 mostly in security related topics. It does enable a configuration of the lockscreen and hibernation boot behavior. Most important is the possibility to enable or disable personal data gathering. This package configures the Microsoft Update behavior. Additionally it gives the ability to enable or disable the following features: telemetry, cortana assistant, malware removal tool and the customer experience program.
