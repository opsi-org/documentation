...
Debian 8 Jessie currently has a problem with Syslinux 6.0.3 in combination with the opsi-atftpd. There is an issue with the t-size transfer. It can either be fixed by adding the --no-tsize option in /etc/inetd.conf or /etc/default/atftpd.conf, depending on the usage. Another way is to downgrade Syslinux. a downgrade package is available at the standard uib download server. We are aware of this issue and work on a fix.
* (Not documented yet)
