...
When installing the {opsi-Server} you will be asked whether the `smb.conf` may be patched. Answer the questions with 'Yes'.
You will also be asked for a password for the pcpatch user.
Assign a password (and note the following section on changing the passwords).
...
IMPORTANT: SLES support for opsi 4.2 is still under development and is not yet available.
