...
In the following chapters special opsi MacOS commands to install software are presented, which come from the opsi-script library `uib_lin_install`. This documentation was automatically generated directly from the source code.
...
include::../../en/common/firststeps_opsi-template-lin.asciidoc[]
...
but they look similar when using MacOS installer files like *.deb, *.rpm.
