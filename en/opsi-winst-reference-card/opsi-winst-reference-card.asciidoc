...
[[opsi-winst-rc]]
= opsi-winst / opsi-script reference card (4.11.5.1)
...
`encoding=`<encoding> //since 4.11.4.2
...
`removeFromListByContains(`<search string>`,` <target list>`)` //since 4.11.5.1 [W/L]

`removeFromListByContains(`<search list>`,` <target list>`)` //since 4.11.5.1 [W/L
...
Subsequent calls of `winbatch` with the parameter `/RunElevated` will see the changed Environment.
