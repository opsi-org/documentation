...
[[opsi-winst-rc]]
= opsi-winst reference card (4.11.3.5)
...
`GetSystemType`    //OS Architecture ("64 Bit System" or "x86 System")
...
`calculate(`<arithmetic string expression>`)` // since 4.11.3.5 : knows: `+-*/()`
...
`GetMyIpByTarget(`<target ip addr>`)` //since 4.11.3.2

`GetIpByName(`<ip addr / ip name>`)` //since 4.11.3.2
...
`getProfilesDirList` //since 4.11.3.2
...
`getRegistryVarMap32(`<regkey>`)`  //since 4.11.3

`getRegistryVarMap64(`<regkey>`)` //since 4.11.3

`getRegistryVarMapSysnative(`<regkey>`)` //since 4.11.3
...
`isFatalError` <string>  //since 4.11.3.2
...
`/AllNTUserProfiles`  //since 4.11.3
...
`/AllNTUserProfiles`  //since 4.11.3.4
