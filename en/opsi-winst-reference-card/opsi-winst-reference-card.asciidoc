...
[[opsi-winst-rc]]
= opsi-winst / opsi-script reference card (4.12.1.x)
...
`LineContaining_ExistsIn(` <string>, <file name> `) : bool` //since 4.11.4.10: true: if a in <file name> contains <string> [W/L/M]
ifdef::manual[see also : <<LineContaining_ExistsIn>>]

`saveTextFile(`<list>, < filename>`) : bool` //since 4.11.4.4: true: if list is succesfully written to file [W/L/M]
ifdef::manual[see also : <<saveTextFile>>]
...
`calculate(`<arithmetic string expression>`) : string (number)` // since 4.11.3.5 : knows: `+-*/()` [W/L/M]
ifdef::manual[see also : <<calculate>>]
...
ifdef::manual[see also : <<getListFromWMI>>]
...
`runningOnUefi` //since 4.11.4.3: true: if the running OS was booted in UEFI mode [W/L/M]
ifdef::manual[see also : <<runningOnUefi>>]

`runningInPE` //since 4.12.0.13: true: if the running OS is a Windows PE [W]
ifdef::manual[see also : <<runningInPE>>]
...
=== XML(2) related functions [W/L/M]:

ifdef::manual[see also : <<opsi-winst-xml2-functions>>]

`getXml2DocumentFromFile(`<path to xml file>`) : xmlstringlist`  //since 4.12.1
ifdef::manual[see also : <<getXml2DocumentFromFile>>]

`getXml2Document(`<stringlist wit xml>`) : xmlstringlist`  //since 4.12.1
ifdef::manual[see also : <<getXml2DocumentFromFile>>]

`xml2GetFirstChildNodeByName(<xmlstringlist>, <node name>) : xmlstringlist`   //since 4.12.1.
ifdef::manual[see also : <<xml2GetFirstChildNodeByName>>]

`getXml2UniqueChildnodeByName(<xmlstringlist>, <node name>) : xmlstringlist`   //since 4.12.1.
ifdef::manual[see also : <<getXml2UniqueChildnodeByName>>]

`getXml2AttributeValueByKey(<xmlstringlist>, <attr name>) : string`   //since 4.12.1.
ifdef::manual[see also : <<getXml2AttributeValueByKey>>]

`getXml2Text(<xmlstringlist>) : string`   //since 4.12.1.
